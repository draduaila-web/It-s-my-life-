...
(() => {
  ...
  function renderHomeTimeline(){
    ...
    const rows=[[316,455,'05:16–07:35','Manhã protegida',true],[480,840,'08:00–14:00','Trabalho oficial',false]];
    const timed=sourcesToday().filter(x=>x.kind==='commitment'&&x.time).sort((a,b)=>a.time.localeCompare(b.time));
    timed.forEach(x=>{const [h,mi]=(x.time||'00:00').split(':').map(Number),a=h*60+mi,b=a+Math.max(1,+x.minutes||30);rows.push([a,b,`${x.time}–${hhmm(new Date(new Date().setHours(h,mi+(+x.minutes||30),0,0)))}`,x.title,false,true]);});

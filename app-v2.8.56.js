// BERTH.A v2.8.56 — correção visual cirúrgica do raio externo em Criação & Ideias
...
function openIdeaModal(id=null){
  ...
  if(!document.querySelector('#ideaModalUXFix')){const s=document.createElement('style');s.id='ideaModalUXFix';s.textContent=`#ideaDialog input,#ideaDialog select,#ideaDialog textarea{font-size:16px!important}#ideaDialog .form-grid{display:grid;grid-template-columns:minmax(0,1fr) minmax(0,1fr);gap:10px}@media(max-width:480px){#ideaDialog .form-grid{grid-template-columns:1fr 1fr}}`;document.head.appendChild(s)}

...
// BERTH.A app-v2.8.126 · Casa modais blush/sálvia + menu de áreas alinhado
...
/* BERTH.A Casa v2.8.126 — menu ancorado + modais blush/sálvia */
...
function openCasaManual(id){
 ...
 const productRows=products.map(x=>`<li><span>${escapeHtml(x)}</span><button type="button" class="casa-buy-mini" data-casa-buy="${escapeHtml(x)}">＋ compras</button></li>`).join("");
 ...
 <div class="casa-how-top-actions"><button type="button" class="secondary casa-edit-procedure" id="editManualCasa">${casaLineIcon('edit')}<span>Editar este procedimento</span></button></div>
 <div class="casa-how-meta"><span>${casaLineIcon('clock')} ${escapeHtml(h.time)}</span></div>
 <div class="casa-how-section"><h3>${casaLineIcon('spray')}<span>Produtos</span></h3><ul>${productRows}</ul></div>
 <div class="casa-how-section"><h3>${casaLineIcon('toolbox')}<span>Utensílios / materiais</span></h3><ul>${materialRows||'<li><span>Nenhum material específico.</span></li>'}</ul></div>
 <div class="casa-how-section"><h3>${casaLineIcon('routines')}<span>Passo a passo</span></h3><ol>${(h.steps||[]).map(x=>`<li>${escapeHtml(x)}</li>`).join("")}</ol></div>
 ${h.tip?`<div class="casa-how-tip"><strong>${casaLineIcon('info')}<span>Dica</span></strong><p>${escapeHtml(h.tip)}</p></div>`:''}</div>`;
 document.body.appendChild(o);const close=()=>o.remove();o.querySelector('.casa-modal-x').onclick=close;o.addEventListener('click',e=>{if(e.target===o)close()});o.querySelector('#editManualCasa').onclick=()=>{close();openCasaManualEditor(id)};o.querySelectorAll('[data-casa-buy]').forEach(b=>b.onclick=()=>{addCasaShopping(b.dataset.casaBuy,h.title);b.textContent='✓ na lista';b.disabled=true;});
}
...
function renderCasaInventory(){return `<div class="card casa-inventory"><div class="panel-head"><div><h3>Produtos que você tem</h3><p class="note">Os produtos atuais permanecem em uso até acabarem. Quando precisar repor, envie direto para a Lista de Compras universal.</p></div><span class="pill">${CASA_PRODUCT_CATALOG.length}</span></div><div class="casa-product-list">${CASA_PRODUCT_CATALOG.map(x=>`<div class="casa-product-row"><div><strong>${escapeHtml(x.name)}</strong><small>${escapeHtml(x.use)}</small><small>Substituição: ${escapeHtml(x.substitute)}</small></div><div><span class="pill">${escapeHtml(x.status)}</span><button type="button" class="casa-buy-mini" data-inventory-buy="${escapeHtml(x.name)}">＋ compras</button></div></div>`).join("")}</div><div class="panel-head inventory-tools-head"><h3>Utensílios e equipamentos</h3><span class="pill">${CASA_INVENTORY_TOOLS.length}</span></div><div class="chip-list">${CASA_INVENTORY_TOOLS.map(x=>`<span class="pill">${escapeHtml(x)}</span>`).join("")}</div></div>`;}
...
function openCasaHow(id){
 ...
 <div class="casa-how-footer"><button type="button" class="secondary" id="editCasaHow">Editar procedimento</button></div></div>`;
...
function renderCasaCore(){
 ...
 const routinesHtml=`<div class="casa-routine-browser"><div class="casa-routine-picker"><button type="button" class="casa-routine-picker-btn" id="casaRoutineAreaButton" aria-expanded="false" aria-haspopup="listbox"><span class="casa-routine-picker-label"><span>${casaLineIcon('routines')}</span><strong>Escolher uma área…</strong></span><span class="casa-routine-picker-chevron" aria-hidden="true">⌄</span></button><div class="casa-routine-picker-menu" id="casaRoutineAreaMenu" role="listbox" hidden>${d.areas.map(a=>`<button type="button" role="option" data-routine-pick="${escapeHtml(String(a.id))}"><span>${casaAreaIcon(a.title)}<strong>${escapeHtml(a.title)}</strong></span><small>${a.tasks.length}</small></button>`).join("")}</div></div><div class="casa-routine-groups">${d.areas.map(a=>`<details class="casa-routine-group" data-routine-id="${escapeHtml(String(a.id))}"><summary><span>${casaAreaIcon(a.title)}<strong>${escapeHtml(a.title)}</strong></span><span class="casa-routine-count">${a.tasks.length}</span></summary><div class="casa-routine-group-body">${a.tasks.length?a.tasks.map(t=>routineTaskHtml(a,t)).join(""):`<p class="note">Sem rotina cadastrada. Mantemos espaço para incluir apenas o que realmente for necessário.</p>`}</div></details>`).join("")}</div></div>`
 ...
 const routineAreaButton=document.querySelector('#casaRoutineAreaButton'),routineAreaMenu=document.querySelector('#casaRoutineAreaMenu');
 if(routineAreaButton&&routineAreaMenu){
   const closePicker=()=>{routineAreaMenu.hidden=true;routineAreaButton.setAttribute('aria-expanded','false');};
   routineAreaButton.addEventListener('click',e=>{e.stopPropagation();const willOpen=routineAreaMenu.hidden;routineAreaMenu.hidden=!willOpen;routineAreaButton.setAttribute('aria-expanded',String(willOpen));});
   routineAreaMenu.querySelectorAll('[data-routine-pick]').forEach(opt=>opt.addEventListener('click',()=>{const id=opt.dataset.routinePick;const groups=[...document.querySelectorAll('.casa-routine-group')];groups.forEach(group=>group.open=false);const target=groups.find(group=>String(group.dataset.routineId)===String(id));const label=opt.querySelector('strong')?.textContent||'Escolher uma área…';routineAreaButton.querySelector('.casa-routine-picker-label strong').textContent=label;closePicker();if(target){target.open=true;requestAnimationFrame(()=>target.scrollIntoView({behavior:'smooth',block:'start'}));}}));
   document.addEventListener('click',closePicker,{once:true,capture:false});
 }
 ...
 document.querySelectorAll("[data-inventory-buy]").forEach(b=>b.onclick=()=>{addCasaShopping(b.dataset.inventoryBuy,"Inventário da Casa");b.textContent="✓ na lista";b.disabled=true;});

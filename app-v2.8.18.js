...
function exCategory(v){const x=String(v||'Outro').toLowerCase();if(['cardio','caminhada','dança','danca','corrida','esteira','bicicleta'].some(k=>x.includes(k)))return 'Cardio';if(['força','forca','muscul','resist'].some(k=>x.includes(k)))return 'Força';if(['mobil','along','yoga','pilates'].some(k=>x.includes(k)))return 'Mobilidade';return ['Cardio','Força','Mobilidade','Outro'].includes(v)?v:'Outro'}
function normalizeExPlan(p={}){const target=String(p.target||'');const n=parseInt(target.match(/\d+/)?.[0]||30);return {id:p.id||uid(),name:String(p.name||'Treino'),type:exCategory(p.type||p.name),durationValue:Math.max(1,+p.durationValue||n||30),durationUnit:p.durationUnit||'minutes',frequency:p.frequency||(Array.isArray(p.days)&&p.days.length?'Dias específicos':'Conforme necessário'),days:Array.isArray(p.days)?p.days:[],period:p.period||((p.name||'').toLowerCase().includes('esteira')?'morning':'flex'),time:p.time||'',untilMode:p.untilMode||'none',untilDate:p.untilDate||'',notify:!!p.notify,notifyWhen:p.notifyWhen||'No início do período',note:p.note||'',plan:p.plan||'',link:p.link||'',active:p.active!==false};}
...
function exFocusPlan(d){const doneNames=new Set(d.sessions.filter(s=>s.date===new Date().toISOString().slice(0,10)).map(s=>s.plannedName||s.name));const due=d.plans.filter(exDueToday).filter(p=>!doneNames.has(p.name));return due.sort((a,b)=>{const rank=x=>x.frequency==='Dias específicos'?0:x.frequency==='X vezes por semana'?1:2;return rank(a)-rank(b)})[0]||null;}
function exActivePlan(d){const a=window.BerthaTimeEngine?.active?.();if(!a||!String(a.id||'').startsWith('exercise:'))return null;return d.plans.find(p=>`exercise:${p.id}`===String(a.id))||null;}
function renderExercicios(){
 const d=loadEx(),today=new Date().toISOString().slice(0,10),sessions=d.sessions.slice().reverse(),activePlan=exActivePlan(d),focus=activePlan||exFocusPlan(d),active=window.BerthaTimeEngine?.active?.();
 ...
 ${focus?`<div class="exercise-focus card"><span class="eyebrow">${activePlan?'EM ANDAMENTO':'SUGESTÃO DE HOJE'}</span><strong>${escapeHtml(focus.name)} · ${exMinutes(focus)} min</strong><p>${escapeHtml(focus.type)} · ${escapeHtml(exPeriodText(focus))}${focus.time?` · ${escapeHtml(focus.time)}`:''}. ${escapeHtml(focus.note||'Escolha o movimento que combina com o seu dia.')}</p><div class="study-v10-actions">${activePlan?`<button class="primary" id="finishExercise">Concluir treino</button>`:`<button class="primary" id="startExercise">Começar sugestão</button>`}<button class="secondary" id="doneExercise">Já fiz</button></div></div>`:''}
 <div class="section-title">ROTINA DE MOVIMENTO</div>
 <div class="list">${d.plans.map(p=>`<div class="card exercise-plan" data-plan-actions="${p.id}"><div><strong>${escapeHtml(p.name)}</strong><span>${escapeHtml(p.type)} · ${exMinutes(p)} min · ${escapeHtml(exPeriodText(p))}</span>${p.days.length?`<small>${p.days.join(' · ')}</small>`:`<small>${escapeHtml(p.frequency)}</small>`}</div><span class="pill">${p.active?'Ativo':'Pausado'}</span></div>`).join('')}</div>
 <button class="secondary add-full" id="addPlan">＋ Adicionar treino</button>
 <div class="section-title">REGISTRO</div>
 <div class="list">${sessions.slice(0,12).map(s=>`<div class="card exercise-session"><div><strong>${escapeHtml(s.name)}</strong><span>${formatDate(s.date)} · ${escapeHtml(s.category||'Outro')} · ${escapeHtml(s.duration||'')}${s.plannedName&&s.plannedName!==s.name?`<small>Planejado: ${escapeHtml(s.plannedName)} · substituído por ${escapeHtml(s.category)}</small>`:''}</span></div><button class="more" data-ex="${s.id}">×</button></div>`).join('')||`<div class="empty compact"><strong>Nenhum treino registrado ainda.</strong><span>Quando você concluir um movimento, ele aparece aqui.</span></div>`}</div>
 <div class="card exercise-note"><span class="eyebrow">MOVIMENTO</span><p>O tempo reservado é para se mover. Cardio, Força, Mobilidade ou Outro: escolha o que cabe no seu corpo e no seu dia.</p></div>`;
 document.querySelector('#addPlan').onclick=()=>openExercisePlan();
 document.querySelectorAll('[data-plan-actions]').forEach(el=>el.onclick=()=>openExerciseActions(el.dataset.planActions));
 document.querySelector('#startExercise')?.addEventListener('click',()=>startExercisePlan(focus));
 document.querySelector('#finishExercise')?.addEventListener('click',()=>finishExercisePlan(focus));
 document.querySelector('#doneExercise')?.addEventListener('click',()=>recordExercisePlan(focus));
...
function startExercisePlan(p){if(!p?.active){alert('Este treino está pausado. Ative-o para começar.');return;}const a=window.BerthaTimeEngine?.active?.();if(a&&String(a.id)!==`exercise:${p.id}`){alert(`Já existe uma atividade em andamento: ${a.title||'atividade'}. Conclua ou encerre antes de iniciar outra.`);return;}startViaBertha({id:`exercise:${p.id}`,learningKey:`exercise-plan-v3:${p.id}`,source:'Exercícios',title:p.name,category:p.type,minutes:exMinutes(p),configuredMinutes:exMinutes(p),period:p.period,kind:'exercise'});setTimeout(renderExercicios,80)}
function plannedSameCategory(d,p,date){return d.plans.filter(x=>String(x.id)!==String(p.id)&&x.type===p.type&&exDueToday(x)&&x.frequency==='Dias específicos').find(x=>!d.sessions.some(s=>s.date===date&&(s.plannedName===x.name||s.name===x.name)))||null}
function addExerciseSessionForPlan(p,mins,date,note=''){const d=loadEx();const planned=plannedSameCategory(d,p,date);d.sessions.push({id:uid(),planId:p.id,name:p.name,category:p.type,duration:`${mins} min`,minutes:mins,date,note,plannedName:planned?.name||p.name,plannedMinutes:planned?exMinutes(planned):exMinutes(p),substituted:!!planned,createdAt:Date.now()});saveEx(d);renderExercicios();}
function finishExercisePlan(p){const a=window.BerthaTimeEngine?.active?.();let mins=exMinutes(p);if(a&&String(a.id)===`exercise:${p.id}`&&a.startedAt)mins=Math.max(1,Math.round((Date.now()-a.startedAt)/60000));window.BerthaTimeEngine?.finish?.();addExerciseSessionForPlan(p,mins,new Date().toISOString().slice(0,10),'Tempo real')}
function recordExercisePlan(p){addExerciseSessionForPlan(p,exMinutes(p),new Date().toISOString().slice(0,10),'Registrado manualmente')}
function openExerciseActions(id){const d=loadEx(),p=d.plans.find(x=>String(x.id)===String(id));if(!p)return;const a=window.BerthaTimeEngine?.active?.(),isActive=a&&String(a.id)===`exercise:${p.id}`;const dlg=document.createElement('dialog');dlg.className='bertha-dialog';dlg.innerHTML=`<div class="bertha-modal study-v10-modal"><div class="bertha-modal-head"><div><div class="eyebrow">${escapeHtml(p.type)}</div><h2>${escapeHtml(p.name)}</h2></div><button type="button" data-close>×</button></div><p>${exMinutes(p)} min · ${escapeHtml(exPeriodText(p))}</p>${p.plan?`<div class="card"><strong>Como fazer</strong><p>${escapeHtml(p.plan)}</p></div>`:''}<div class="study-v10-actions">${isActive?`<button class="primary" id="exActionFinish">Concluir</button>`:`<button class="primary" id="exActionStart">Começar agora</button>`}<button class="secondary" id="exActionDone">Já fiz</button><button class="secondary" id="exActionEdit">Editar</button></div></div>`;document.body.appendChild(dlg);const close=()=>dlg.close();dlg.querySelectorAll('[data-close]').forEach(b=>b.onclick=close);dlg.addEventListener('click',e=>{if(e.target===dlg)close()});dlg.onclose=()=>dlg.remove();dlg.querySelector('#exActionStart')?.addEventListener('click',()=>{close();startExercisePlan(p)});dlg.querySelector('#exActionFinish')?.addEventListener('click',()=>{close();finishExercisePlan(p)});dlg.querySelector('#exActionDone').onclick=()=>{close();recordExercisePlan(p)};dlg.querySelector('#exActionEdit').onclick=()=>{close();setTimeout(()=>openExercisePlan(p.id),30)};dlg.showModal()}
function openExercisePlan(id=null){
 ...
 const types=['Cardio','Força','Mobilidade','Outro'],days=['Seg','Ter','Qua','Qui','Sex','Sáb','Dom'];
 ...
 ${field('Nome',`<input id="eName" required maxlength="70" value="${escapeHtml(current?p.name:'')}" placeholder="Ex.: Dança">`)}
 ${field('Categoria',`<select id="eType">${types.map(x=>`<option ${x===p.type?'selected':''}>${x}</option>`).join('')}</select>`)}

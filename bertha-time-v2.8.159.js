...
(() => {
  ...
  function renderHome(){ ensureHomeUtilityStyles(); const d=new Date(), greet=d.getHours()<12?'Bom dia':d.getHours()<18?'Boa tarde':'Boa noite'; const today=new Intl.DateTimeFormat('pt-BR',{weekday:'long',day:'numeric',month:'long'}).format(d); const raw=sourcesToday().filter(x=>!doneToday(x.id)),smartCasa=candidates().filter(x=>x.kind==='home-routine'||x.kind==='home-maintenance'),nonCasa=raw.filter(x=>x.kind!=='home-routine'&&x.kind!=='home-maintenance'); const tasks=(smartCasa.length?[...nonCasa.slice(0,3),smartCasa[0]]:nonCasa.slice(0,4)); const free=currentSuggestion().free;
    const financeReminders = renderFinanceRemindersHome();
    const shopCount=window.BerthaShopping?.pendingCount?.()||0; const health=window.BerthaShopping?.renderHealthMini?.()||'';
  ...
  const BERTHA_RITUAL_PHRASES=['Space to think. Space to live.','Your life, with somewhere to land.','Everything handled. Nothing carried.','Create headroom for life.'];
  ...
  function renderRituals(){
    ...
    return `<section class="bertha-ritual-page">
      <section class="bertha-ritual-hero"><div class="bertha-task-kicker">RITUAIS</div><h1>Space to think. Space to live.</h1><p>Você cuida de você. A BERTH.A sustenta o ritual.</p><span class="bertha-ritual-hero-mark" aria-hidden="true"><svg viewBox="0 0 48 48"><path d="M10 29c6-8 13-12 22-12 3 0 5.8.5 8 1.5"/><path d="M8 35c7-6 14-9 22-9 4 0 7.5.8 10 2.1"/><path d="M15 21c4-4 9-6 15-6"/></svg></span></section>
  ...
  function ensureStyles(){
    ...
    .bertha-ritual-hero-mark{position:absolute;right:20px;top:19px;width:48px;height:48px;color:#a29ba2;opacity:.72;pointer-events:none}
    .bertha-ritual-hero-mark svg{width:100%;height:100%;fill:none;stroke:currentColor;stroke-width:1.35;stroke-linecap:round;stroke-linejoin:round}
    ...
    /* modal estável: fundo e bottom bar não participam do movimento */
    .bertha-ritual-dialog[open]{position:fixed!important;inset:0!important;width:100vw!important;height:100svh!important;max-width:none!important;max-height:none!important;margin:0!important;padding:12px!important;box-sizing:border-box!important;border:0!important;background:transparent!important;overflow:hidden!important;place-items:center!important}
    .bertha-ritual-dialog[open] .bertha-task-modal{width:min(720px,calc(100vw - 24px))!important;max-height:calc(100svh - 24px)!important;margin:auto!important;transform:none!important}


    /* v2.8.69 — mobile modal + product action */
    .bertha-ritual-dialog .bertha-task-modal{height:min(90dvh,760px);max-height:min(90dvh,760px)}
    .bertha-ritual-dialog .bertha-task-modal-body{flex:1 1 auto;min-height:0;overflow-y:auto;-webkit-overflow-scrolling:touch;padding-bottom:24px}
    .bertha-ritual-dialog .bertha-task-modal-actions{position:relative!important;bottom:auto!important;flex:0 0 auto}

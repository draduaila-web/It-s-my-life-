...
function exPlanWeeklyTarget(p){if(p.frequency==='X vezes por semana')return Math.max(1,+p.weeklyTarget||1);if(p.frequency==='Dias específicos')return Math.max(1,(p.days||[]).length);return 1;}
function exercisePlanMatchesBlock(p,block){const t=String(p.type||'').toLowerCase(),n=String(p.name||'').toLowerCase();if(block==='Cardio')return /cardio|caminh/.test(t)||/esteira|box|escada|bicic|bike|corrida|caminh/.test(n);if(block==='Força')return /força|forca/.test(t)||/bumbum|coxa|braç|brac|costas|abd|perna|glúte|glute/.test(n);if(block==='Mobilidade')return /mobil|yoga|pilates/.test(t)||/along|mobil|yoga|pilates|medita/.test(n);return false;}
function exerciseCycleSessionSuggestion(c,d=loadEx()){
 const blocks=Object.entries(c.blockMinutes||{}).filter(([,m])=>+m>0);if(!blocks.length)return null;
 const plans=d.plans.filter(p=>p.active!==false&&String(p.cycleId||'')===String(c.id));const used=new Set(),picked=[];
 for(const [block,minutes] of blocks){
  const cand=plans.filter(p=>!used.has(String(p.id))&&exercisePlanMatchesBlock(p,block)).sort((a,b)=>{const ar=exWeeklyDone(a,d)/exPlanWeeklyTarget(a),br=exWeeklyDone(b,d)/exPlanWeeklyTarget(b);return ar-br});
  const p=cand.find(x=>exWeeklyDone(x,d)<exPlanWeeklyTarget(x))||cand[0];if(p){used.add(String(p.id));picked.push({block,minutes:+minutes,plan:p});}
 }
 if(!picked.length)return null;return {cycle:c,total:Math.max(1,+c.sessionMinutes||picked.reduce((n,x)=>n+x.minutes,0)),blocks:picked};
}
function exerciseSuggestionForMeuDia(){
 const d=loadEx(), today=new Date().toISOString().slice(0,10),now=new Date();
 const activeCycles=d.cycles.filter(c=>c.active!==false).filter(c=>{const start=new Date(c.startDate+'T12:00:00'),end=new Date(start);end.setDate(end.getDate()+Math.max(1,+c.weeks||4)*7);return now>=start&&now<end;});
 for(const c of activeCycles){const sess=exerciseCycleSessionSuggestion(c,d);if(!sess)continue;const meta=exerciseCycleProgress(c,d),parts=sess.blocks.map(x=>`${x.block} ${x.minutes} min`).join(' + '),names=sess.blocks.map(x=>x.plan.name).join(' · ');return `<section class="day-section home-exercise-suggestion"><div class="section-head"><h2>Movimento de hoje</h2><a href="#exercicios">ver exercícios</a></div><a class="home-exercise-card" href="#exercicios"><span class="home-exercise-icon">${exerciseCycleIcon(c.icon)}</span><span class="home-exercise-copy"><small>CICLO · ${escapeHtml(c.name)} · semana ${Math.min(c.weeks,meta?.week||1)}/${c.weeks}</small><strong>${sess.total} min · ${escapeHtml(parts)}</strong><em>${escapeHtml(names)}</em></span><b>›</b></a></section>`;}
 const doneToday=new Set(d.sessions.filter(s=>s.date===today).map(s=>String(s.planId||''))),due=d.plans.filter(p=>exDueToday(p,d)&&!doneToday.has(String(p.id)));if(!due.length)return '';
 const p=due[0],why=p.frequency==='X vezes por semana'?`${exWeeklyDone(p,d)}/${Math.max(1,+p.weeklyTarget||1)} feitos nesta semana`:p.frequency==='Dias específicos'?'Programado para hoje':'Cabe no seu dia quando fizer sentido';
 return `<section class="day-section home-exercise-suggestion"><div class="section-head"><h2>Movimento de hoje</h2><a href="#exercicios">ver exercícios</a></div><a class="home-exercise-card" href="#exercicios"><span class="home-exercise-icon">${exercisePlanIcon(p)}</span><span class="home-exercise-copy"><small>MOVIMENTO SUGERIDO</small><strong>${escapeHtml(p.name)} · ${exMinutes(p)} min</strong><em>${escapeHtml(why)} · ${escapeHtml(exPeriodText(p))}</em></span><b>›</b></a></section>`;
}
...
/* v2.8.223 — iPhone: trava a página atrás do modal de Exercícios e ancora o diálogo ao viewport visual */
...
function openExercisePlanDialogStable(dlg){
  lockExerciseModalPage();
  let released=false;
  const release=()=>{if(released)return;released=true;unlockExerciseModalPage()};
  dlg.addEventListener('close',release,{once:true});
  dlg.classList.add('exercise-plan-dialog-fixed');
  try{
    const stableH=Math.max(520,window.innerHeight||document.documentElement.clientHeight||700);
    dlg.style.setProperty('--exercise-plan-fixed-height',`${stableH}px`);
    dlg.showModal();
    const panel=dlg.querySelector('.bertha-modal');if(panel)panel.scrollTop=0;
...
function normalizeExCycle(c={}){const bm=c.blockMinutes&&typeof c.blockMinutes==='object'?c.blockMinutes:{};return {id:c.id||uid(),name:String(c.name||'Novo ciclo'),weeks:Math.max(1,+c.weeks||4),startDate:c.startDate||new Date().toISOString().slice(0,10),icon:c.icon||'cycle',goal:c.goal||'Fortalecimento',sessionMinutes:Math.max(1,+c.sessionMinutes||30),blockMinutes:{Cardio:Math.max(0,+bm.Cardio||0),Força:Math.max(0,+bm['Força']||0),Mobilidade:Math.max(0,+bm.Mobilidade||0)},progression:c.progression||'',note:c.note||'',active:c.active!==false};}
function loadEx(){try{const raw=JSON.parse(window.berthaHmlStorage.getItem(EX_KEY));if(raw){return {...EX_BASE,...raw,plans:(Array.isArray(raw.plans)?raw.plans:EX_BASE.plans).map(normalizeExPlan),cycles:(Array.isArray(raw.cycles)?raw.cycles:[]).map(normalizeExCycle),sessions:Array.isArray(raw.sessions)?raw.sessions:[]};}}catch{}return JSON.parse(JSON.stringify(EX_BASE));}
...
function exerciseIconSvg(key='cycle'){
 const k=String(key||'cycle').toLowerCase();
 const svg=body=>`<svg viewBox="0 0 32 32" aria-hidden="true">${body}</svg>`;
 if(k==='glutes')return svg('<path d="M9 9c3 1 4.8 3.7 5 7-.2 4.2-2.2 7-5 8M23 9c-3 1-4.8 3.7-5 7 .2 4.2 2.2 7 5 8"/><path d="M12 8c1.4 2 2.7 3.2 4 3.2S18.6 10 20 8"/><path d="M11 20c1.6 1.3 3.2 2 5 2s3.4-.7 5-2"/>');
 if(k==='upper')return svg('<circle cx="16" cy="6.5" r="2.2"/><path d="M10 12c2-2 4-3 6-3s4 1 6 3M10 12l-4 6m16-6 4 6M12 12l-1 8m9-8 1 8M11 20h10"/>');
 if(k==='abs')return svg('<path d="M11 5c2 1 3.5 1.5 5 1.5S19 6 21 5M11 27c2-1 3.5-1.5 5-1.5S19 26 21 27"/><path d="M11 7c-1 5-1 13 0 18m10-18c1 5 1 13 0 18M16 7v18M12 12h8m-8 5h8m-8 5h8"/>');
 if(k==='treadmill')return svg('<path d="M7 24h18M9 20h14l3 4H7z"/><circle cx="17" cy="7" r="2"/><path d="M16 10l-3 5 4 2 3 3m-7-5-3 3m7-1 3-5"/>');
 if(k==='stairs')return svg('<path d="M5 25h6v-5h6v-5h6v-5h4"/><circle cx="12" cy="6" r="2"/><path d="M12 9l3 4-2 4m2-4 4 1m-6 3-3 5"/>');
 if(k==='bike')return svg('<circle cx="9" cy="22" r="5"/><circle cx="23" cy="22" r="5"/><path d="M9 22l5-9 4 9M14 13h6m-6 0-2-3m6 12 5-8"/>');
 if(k==='boxing')return svg('<path d="M8 18c-1-4 0-8 3-9 2-1 4 1 4 3 1-3 4-4 6-2 2 2 1 5 0 7l-2 8H10z"/><path d="M10 18h10"/>');
 if(k==='walk')return svg('<circle cx="17" cy="6.5" r="2.4"/><path d="M15.5 10.5l-2 6 4.5 3 2 7m-6.5-10-4 4m8.5-1 5-3m-7.5 2.5-4 7"/>');
 if(k==='meditation')return svg('<circle cx="16" cy="7" r="2.2"/><path d="M16 10v7M9 14c2.5 1 4.8 2 7 3 2.2-1 4.5-2 7-3M7 24c3-3 6-4.5 9-4.5S22 21 25 24M11 25h10"/>');
 if(k==='stretch')return svg('<circle cx="16" cy="6" r="2"/><path d="M16 9v7m0-4-7-3m7 3 7-5m-7 9-6 8m6-8 5 9"/>');
 if(k==='swim')return svg('<circle cx="18" cy="9" r="2"/><path d="M7 17l7-5 5 4 6-1M5 21c2 2 4 2 6 0 2 2 4 2 6 0 2 2 4 2 6 0 2 2 4 2 6 0"/>');
 if(k==='dance')return svg('<circle cx="17" cy="6" r="2"/><path d="M16 9l-2 7 5 4m-5-4-6 2m11 2 5 5m-10-9-3 10m5-17 6 4"/>');
 if(k==='sport')return svg('<circle cx="16" cy="16" r="10"/><path d="M10 8l4 4-2 5-5 1m15-10-4 4 2 5 5 1M12 17l4 3 4-3"/>');
 if(k==='cardio')return svg('<path d="M5 17h5l2.3-5.2 3.8 10.4 3-6.2 2 1H27"/><path d="M8.2 9.2c2.2-2.8 6.2-2.5 7.8.5 1.7-3 5.8-3.3 8-.7 3.1 4.2-.4 9.2-8 14.2-7.7-5-11.1-9.7-7.8-14z"/>');
 if(k==='strength')return svg('<path d="M3.5 13v6M6.5 11.5v9M25.5 13v6M22.5 11.5v9M6.5 16h16"/>');
 if(k==='mobility')return svg('<circle cx="16" cy="6.5" r="2.4"/><path d="M16 10v8m0-4-6 4m6-4 6 4m-6 0-5 7m5-7 5 7"/>');
 if(k==='wellness')return svg('<path d="M16 26S6 20.5 6 12.5C6 8.8 8.8 6 12.3 6c2 0 3.3 1 3.7 2 .5-1 1.8-2 3.8-2C23.2 6 26 8.8 26 12.5 26 20.5 16 26 16 26z"/>');
 return svg('<path d="M25 11a10 10 0 0 0-17-2l-2 3"/><path d="M6 7v5h5"/><path d="M7 21a10 10 0 0 0 17 2l2-3"/><path d="M26 25v-5h-5"/>');
}
...
function exerciseCycleProgress(c,d){const start=new Date(c.startDate+'T12:00:00'),end=new Date(start);end.setDate(end.getDate()+c.weeks*7);const now=new Date(),elapsed=Math.max(0,Math.min(c.weeks,Math.floor((now-start)/(7*86400000))+1));const plans=d.plans.filter(p=>String(p.cycleId)===String(c.id));const target=plans.reduce((n,p)=>n+(p.frequency==='X vezes por semana'?Math.max(1,+p.weeklyTarget||1):p.frequency==='Dias específicos'?Math.max(1,p.days.length):1),0)*Math.max(1,elapsed);const done=d.sessions.filter(s=>s.date>=c.startDate&&new Date(s.date+'T12:00:00')<end&&(String(s.cycleId||'')===String(c.id)||plans.some(p=>String(s.planId||'')===String(p.id)))).length;return {week:Math.max(1,elapsed),done,target,pct:target?Math.min(100,Math.round(done/target*100)):0};}
function renderExercicios(){
 ...
 <div class="exercise-cycle-list">${d.cycles.filter(c=>c.active!==false).map(c=>{const pg=exerciseCycleProgress(c,d),count=d.plans.filter(p=>String(p.cycleId)===String(c.id)).length;return `<div class="card exercise-cycle" data-cycle="${c.id}"><div class="exercise-cycle-head"><span class="exercise-icon">${exerciseCycleIcon(c.icon)}</span><div class="exercise-cycle-copy"><strong>${escapeHtml(c.name)}</strong><span>${escapeHtml(c.goal||'Ciclo de treino')} · Semana ${pg.week} de ${c.weeks}</span><small>${count} treino${count===1?'':'s'} · ${c.sessionMinutes||30} min/sessão · ${Object.entries(c.blockMinutes||{}).filter(([,m])=>+m>0).map(([k,m])=>`${k} ${m}`).join(' + ')||'composição livre'}</small></div></div><div class="exercise-cycle-progress"><i style="width:${pg.pct}%"></i></div></div>`}).join('')||`<div class="empty compact"><strong>Nenhum ciclo criado.</strong><span>Agrupe treinos em ciclos de 4, 6, 8 semanas ou no período que fizer sentido.</span></div>`}</div>
 ...
 document.querySelector('#addPlan').onclick=()=>openExercisePlan();document.querySelector('#addCycle').onclick=()=>openExerciseCycle();document.querySelectorAll('[data-cycle]').forEach(el=>el.onclick=()=>openExerciseCycle(el.dataset.cycle));
 ...
 const startPlan=p=>startViaBertha({id:`exercise:${p.id}`,selectedPlanId:p.id,learningKey:`exercise-plan-v2:${p.id}`,source:'Exercícios',title:p.name,minutes:exMinutes(p),configuredMinutes:exMinutes(p),period:p.period,kind:'exercise'});
...
function openExerciseSession(id){const d=loadEx(),s=d.sessions.find(x=>String(x.id)===String(id));if(!s)return;const dlg=document.createElement('dialog');dlg.className='bertha-dialog exercise-dialog';dlg.innerHTML=`<form class="bertha-modal study-v10-modal" id="editExSession"><div class="bertha-modal-head"><div><div class="eyebrow">EXERCÍCIOS · REGISTRO</div><h2>Editar realizado</h2></div><button type="button" data-close>×</button></div>${field('Nome',`<input id="sName" required maxlength="70" value="${escapeHtml(s.name)}">`)}<div class="form-grid">${field('Data',`<input id="sDate" type="date" value="${escapeHtml(s.date)}">`)}${field('Duração real (min)',`<input id="sDur" type="number" min="1" value="${Math.max(1,+s.minutes||parseInt(s.duration)||20)}">`)}</div>${field('Observação',`<textarea id="sNote" rows="3">${escapeHtml(s.note||'')}</textarea>`)}<div class="study-v10-actions"><button type="button" class="secondary" data-close>Cancelar</button><button class="primary" type="submit">Salvar</button></div></form>`;document.body.appendChild(dlg);const close=()=>dlg.close();dlg.querySelectorAll('[data-close]').forEach(b=>b.onclick=close);dlg.onclose=()=>dlg.remove();dlg.querySelector('#editExSession').onsubmit=e=>{e.preventDefault();const name=dlg.querySelector('#sName').value.trim(),date=dlg.querySelector('#sDate').value,minutes=Math.max(1,+dlg.querySelector('#sDur').value||20),note=dlg.querySelector('#sNote').value.trim();let pid=s.progressItemId||'';if(!pid){try{const en=JSON.parse(window.berthaHmlStorage.getItem('bertha.time-engine.v1')||'{}');const h=(en.history||[]).find(h=>h.source==='Exercícios'&&h.day===s.date&&h.title===s.name);pid=h?.itemId||'';}catch{}}if(pid)exerciseRecordProgress({name,minutes,date,plannedName:s.plannedName||name,substituted:!!s.substituted,note,progressItemId:pid});const row={...s,name,date,minutes,duration:`${minutes} min`,note,progressItemId:pid};d.sessions=d.sessions.map(x=>String(x.id)===String(id)?row:x);saveEx(d);close();renderExercicios();};openExerciseDialogStable(dlg);}
function openOtherExercise(focus){const d=loadEx(),dlg=document.createElement('dialog');dlg.className='bertha-dialog exercise-dialog';dlg.innerHTML=`<div class="bertha-modal study-v10-modal exercise-other-modal"><div class="bertha-modal-head"><div><div class="eyebrow">FIZ OUTRO</div><h2>O que você fez?</h2><p>Registre o movimento real sem perder o planejado.</p></div><button type="button" data-close>×</button></div><div class="exercise-other-list">${d.plans.filter(p=>String(p.id)!==String(focus?.id)).map(p=>`<button type="button" data-other-plan="${p.id}"><strong>${escapeHtml(p.name)}</strong><span>${escapeHtml(p.type)} · ${exMinutes(p)} min</span></button>`).join('')}<button type="button" data-other-new><strong>Registrar outro treino</strong><span>Nome e duração rápidos.</span></button></div></div>`;document.body.appendChild(dlg);const close=()=>dlg.close();dlg.querySelectorAll('[data-close]').forEach(b=>b.onclick=close);dlg.addEventListener('click',e=>{if(e.target===dlg)close()});dlg.onclose=()=>dlg.remove();dlg.querySelectorAll('[data-other-plan]').forEach(b=>b.onclick=()=>{const p=d.plans.find(x=>String(x.id)===String(b.dataset.otherPlan));if(!p)return;addExerciseSession(p.name,exMinutes(p),new Date().toISOString().slice(0,10),'Substituiu o treino planejado',{plannedName:focus?.name||'',planId:p.id,cycleId:p.cycleId,substituted:true,recordProgress:true});close()});dlg.querySelector('[data-other-new]').onclick=()=>{close();setTimeout(()=>openQuickExercise(focus),60)};openExerciseDialogStable(dlg);}
function openQuickExercise(focus){const dlg=document.createElement('dialog');dlg.className='bertha-dialog exercise-dialog';dlg.innerHTML=`<form class="bertha-modal study-v10-modal" id="quickExForm"><div class="bertha-modal-head"><div><div class="eyebrow">EXERCÍCIOS</div><h2>Registrar treino</h2></div><button type="button" data-close>×</button></div>${field('Nome',`<input id="qExName" required maxlength="70" placeholder="Ex.: Caminhada">`)}${field('Duração real (min)',`<input id="qExDur" type="number" min="1" value="20">`)}<div class="study-v10-actions"><button type="button" class="secondary" data-close>Cancelar</button><button class="primary" type="submit">Registrar</button></div></form>`;document.body.appendChild(dlg);const close=()=>dlg.close();dlg.querySelectorAll('[data-close]').forEach(b=>b.onclick=close);dlg.onclose=()=>dlg.remove();dlg.querySelector('#quickExForm').onsubmit=e=>{e.preventDefault();addExerciseSession(dlg.querySelector('#qExName').value.trim(),+dlg.querySelector('#qExDur').value||20,new Date().toISOString().slice(0,10),'Substituiu o treino planejado',{plannedName:focus?.name||'',substituted:true,recordProgress:true});close()};openExerciseDialogStable(dlg);}
function openExerciseCycle(id=null){
 const d=loadEx(),current=id?d.cycles.find(x=>String(x.id)===String(id)):null,c=normalizeExCycle(current||{}),dlg=document.createElement('dialog');dlg.className='bertha-dialog exercise-dialog';
 const icons=[['cycle','Ciclo'],['strength','Força'],['cardio','Cardio'],['mobility','Mobilidade'],['walk','Caminhada'],['wellness','Bem-estar']];
 const goals=['Fortalecimento','Condicionamento','Mobilidade','Retomada','Bem-estar','Outro'];
 const selectedPlans=d.plans.filter(p=>String(p.cycleId||'')===String(c.id)).map(p=>String(p.id));
 const endDate=()=>{const s=dlg.querySelector('#cStart')?.value,w=+dlg.querySelector('#cWeeks')?.value||4;if(!s)return '';const dt=new Date(s+'T12:00:00');dt.setDate(dt.getDate()+w*7-1);return dt.toLocaleDateString('pt-BR')};
 dlg.innerHTML=`<form class="bertha-modal study-v10-modal exercise-cycle-modal" id="exCycleForm"><div class="bertha-modal-head"><div><div class="eyebrow">EXERCÍCIOS · CICLO</div><h2>${current?'Editar ciclo':'Novo ciclo'}</h2><p>Crie uma direção para algumas semanas. A BERTH.A distribui os treinos sem transformar sua rotina em uma agenda rígida.</p></div><button type="button" data-close>×</button></div>
 ${field('Nome do ciclo',`<input id="cName" required maxlength="70" value="${escapeHtml(current?c.name:'')}" placeholder="Ex.: Fortalecimento · 8 semanas">`)}
 <div class="exercise-cycle-section"><span class="exercise-cycle-label">Ícone do ciclo</span><div class="exercise-icon-picker">${icons.map(([k,l])=>`<button type="button" class="exercise-icon-choice ${c.icon===k?'selected':''}" data-cycle-icon="${k}" aria-label="${l}"><span>${exerciseCycleIcon(k)}</span><small>${l}</small></button>`).join('')}</div><input type="hidden" id="cIcon" value="${escapeHtml(c.icon)}"></div>
 ${field('Objetivo principal',`<select id="cGoal">${goals.map(g=>`<option ${g===c.goal?'selected':''}>${g}</option>`).join('')}</select>`)}
 <div class="form-grid cycle-duration-grid">${field('Duração do ciclo',`<div class="exercise-unit-input"><input id="cWeeks" type="number" min="1" max="52" value="${c.weeks}"><span>semanas</span></div>`)}${field('Início',`<input id="cStart" type="date" value="${escapeHtml(c.startDate)}"><small class="exercise-field-help" id="cEndHelp"></small>`)}</div>
 <div class="exercise-cycle-section"><span class="exercise-cycle-label">Sessão do ciclo</span><p class="exercise-cycle-help">Defina quanto tempo a sessão deve ocupar e como esse tempo é dividido. A BERTH.A escolhe entre os treinos cadastrados de cada tipo, priorizando o que ainda falta na semana.</p>${field('Duração total da sessão',`<div class="exercise-unit-input"><input id="cSessionMinutes" type="number" min="1" max="240" value="${c.sessionMinutes}"><span>min</span></div>`)}<div class="form-grid">${field('Cardio',`<div class="exercise-unit-input"><input id="cBlockCardio" type="number" min="0" max="240" value="${c.blockMinutes.Cardio}"><span>min</span></div>`)}${field('Força',`<div class="exercise-unit-input"><input id="cBlockStrength" type="number" min="0" max="240" value="${c.blockMinutes['Força']}"><span>min</span></div>`)}</div>${field('Mobilidade / alongamento',`<div class="exercise-unit-input"><input id="cBlockMobility" type="number" min="0" max="240" value="${c.blockMinutes.Mobilidade}"><span>min</span></div><small class="exercise-field-help" id="cCompositionHelp"></small>`)}</div>
 <div class="exercise-cycle-section"><span class="exercise-cycle-label">Treinos deste ciclo</span><p class="exercise-cycle-help">Você pode associar agora ou depois. A meta semanal do ciclo é calculada a partir da frequência de cada treino.</p><div class="exercise-plan-picker">${d.plans.map(p=>`<label><input type="checkbox" value="${p.id}" ${selectedPlans.includes(String(p.id))?'checked':''}><span class="exercise-icon mini">${exercisePlanIcon(p)}</span><span><strong>${escapeHtml(p.name)}</strong><small>${escapeHtml(p.type)} · ${exMinutes(p)} min</small></span></label>`).join('')||'<p class="note">Cadastre um treino para associá-lo ao ciclo.</p>'}</div></div>
 ${field('Progressão / estratégia',`<textarea id="cProgression" rows="3" placeholder="Ex.: semanas 1–2 adaptação; 3–4 aumentar carga; 5–6 consolidar">${escapeHtml(c.progression||'')}</textarea>`)}
 ${field('Observações',`<textarea id="cNote" rows="3" placeholder="Objetivo, restrições, lembretes ou orientação geral">${escapeHtml(c.note||'')}</textarea>`)}
 ...
 const syncEnd=()=>{const el=dlg.querySelector('#cEndHelp');if(el)el.textContent='Término previsto: '+endDate()};dlg.querySelector('#cWeeks').addEventListener('input',syncEnd);dlg.querySelector('#cStart').addEventListener('change',syncEnd);syncEnd();
 const syncComposition=()=>{const total=Math.max(1,+dlg.querySelector('#cSessionMinutes').value||30),sum=['#cBlockCardio','#cBlockStrength','#cBlockMobility'].reduce((n,q)=>n+(+dlg.querySelector(q).value||0),0),el=dlg.querySelector('#cCompositionHelp');if(el)el.textContent=`Composição: ${sum}/${total} min${sum===total?' · pronta':' · ajuste para fechar a duração total'}`};['#cSessionMinutes','#cBlockCardio','#cBlockStrength','#cBlockMobility'].forEach(q=>dlg.querySelector(q).addEventListener('input',syncComposition));syncComposition();
 dlg.querySelectorAll('[data-cycle-icon]').forEach(b=>b.addEventListener('click',()=>{dlg.querySelectorAll('[data-cycle-icon]').forEach(x=>x.classList.remove('selected'));b.classList.add('selected');dlg.querySelector('#cIcon').value=b.dataset.cycleIcon}));
 dlg.querySelector('#deleteExCycle')?.addEventListener('click',()=>{if(!confirm(`Excluir o ciclo “${c.name}”? Os treinos serão mantidos fora do ciclo.`))return;d.cycles=d.cycles.filter(x=>String(x.id)!==String(c.id));d.plans=d.plans.map(p=>String(p.cycleId)===String(c.id)?{...p,cycleId:''}:p);saveEx(d);close();renderExercicios()});
 dlg.querySelector('#exCycleForm').onsubmit=e=>{e.preventDefault();const total=Math.max(1,+dlg.querySelector('#cSessionMinutes').value||30),blockMinutes={Cardio:Math.max(0,+dlg.querySelector('#cBlockCardio').value||0),Força:Math.max(0,+dlg.querySelector('#cBlockStrength').value||0),Mobilidade:Math.max(0,+dlg.querySelector('#cBlockMobility').value||0)},sum=Object.values(blockMinutes).reduce((n,x)=>n+x,0);if(sum!==total){alert(`A composição precisa totalizar ${total} min. Agora ela soma ${sum} min.`);return;}const cycleId=current?c.id:uid(),obj={...c,id:cycleId,name:dlg.querySelector('#cName').value.trim(),weeks:Math.max(1,+dlg.querySelector('#cWeeks').value||4),startDate:dlg.querySelector('#cStart').value,icon:dlg.querySelector('#cIcon').value,goal:dlg.querySelector('#cGoal').value,sessionMinutes:total,blockMinutes,progression:dlg.querySelector('#cProgression').value.trim(),note:dlg.querySelector('#cNote').value.trim(),active:dlg.querySelector('#cActive').value==='1'};const picked=new Set([...dlg.querySelectorAll('.exercise-plan-picker input:checked')].map(x=>String(x.value)));d.cycles=current?d.cycles.map(x=>String(x.id)===String(c.id)?obj:x):[...d.cycles,obj];d.plans=d.plans.map(p=>{if(picked.has(String(p.id)))return {...p,cycleId};if(String(p.cycleId||'')===String(cycleId))return {...p,cycleId:''};return p});saveEx(d);close();renderExercicios();};openExerciseDialogStable(dlg);
}
function openExercisePlan(id=null){
 const d=loadEx(),current=id?d.plans.find(x=>String(x.id)===String(id)):null,p=normalizeExPlan(current||{}),dlg=document.createElement('dialog');dlg.className='bertha-dialog exercise-dialog';
 const types=['Cardio','Força','Mobilidade','Caminhada','Pilates','Yoga','Outro'],days=['Seg','Ter','Qua','Qui','Sex','Sáb','Dom'];
 if(!p.icon){const n=String(p.name||'').toLowerCase();p.icon=n.includes('esteira')?'treadmill':n.includes('bumbum')||n.includes('coxa')?'glutes':n.includes('braço')||n.includes('braco')||n.includes('costas')?'upper':n.includes('abd')?'abs':n.includes('escad')?'stairs':n.includes('bike')||n.includes('bicic')?'bike':n.includes('box')?'boxing':n.includes('caminh')?'walk':n.includes('medita')?'meditation':n.includes('along')?'stretch':'sport';}
 ...
 <div class="exercise-cycle-section exercise-plan-icon-section"><span class="exercise-cycle-label">Ícone do treino</span><p class="exercise-cycle-help">O tipo organiza o treino (cardio, força, mobilidade). O ícone identifica a atividade que você realmente faz.</p><div class="exercise-icon-picker">${[['glutes','Bumbum e coxas'],['upper','Braços e costas'],['abs','Abdômen'],['treadmill','Esteira'],['stairs','Escada'],['bike','Bicicleta'],['boxing','Boxe'],['walk','Caminhada'],['meditation','Meditação'],['stretch','Alongamento'],['swim','Natação'],['dance','Dança'],['sport','Outros']].map(([k,l])=>`<button type="button" class="exercise-icon-choice ${(p.icon||'')===k?'selected':''}" data-plan-icon="${k}" aria-label="${l}"><span>${k?exerciseIconSvg(k):exerciseTypeIcon(p.type)}</span><small>${l}</small></button>`).join('')}</div><input type="hidden" id="eIcon" value="${escapeHtml(p.icon||'')}"></div>
 ${field('Ciclo (opcional)',`<select id="eCycle"><option value="">Sem ciclo</option>${d.cycles.map(c=>`<option value="${c.id}" ${String(c.id)===String(p.cycleId)?'selected':''}>${escapeHtml(c.name)}</option>`).join('')}</select>`)}
 ...
 dlg.querySelectorAll('[data-plan-icon]').forEach(b=>b.addEventListener('click',()=>{dlg.querySelectorAll('[data-plan-icon]').forEach(x=>x.classList.remove('selected'));b.classList.add('selected');dlg.querySelector('#eIcon').value=b.dataset.planIcon||'sport'}));
 ...
 dlg.querySelector('#exForm').addEventListener('submit',e=>{e.preventDefault();const obj={...p,name:dlg.querySelector('#eName').value.trim(),type:dlg.querySelector('#eType').value,icon:dlg.querySelector('#eIcon').value||'sport',cycleId:dlg.querySelector('#eCycle').value,durationValue:Math.max(1,+dlg.querySelector('#eDur').value||30),durationUnit:dlg.querySelector('#eDurUnit').value,frequency:dlg.querySelector('#eFreq').value,weeklyTarget:Math.max(1,+dlg.querySelector('#eWeeklyTarget').value||1),days:[...dlg.querySelectorAll('#eDays input:checked')].map(x=>x.value),period:dlg.querySelector('#ePeriod').value,time:dlg.querySelector('#eTime').value,untilMode:dlg.querySelector('#eUntil').value,untilDate:dlg.querySelector('#eUntilDate').value,notify:dlg.querySelector('#eNotify').checked,notifyWhen:dlg.querySelector('#eNotifyWhen').value,plan:dlg.querySelector('#ePlan').value.trim(),link:dlg.querySelector('#eLink').value.trim(),note:dlg.querySelector('#eNote').value.trim(),active:dlg.querySelector('#eActive').value==='1'};d.plans=current?d.plans.map(x=>String(x.id)===String(p.id)?obj:x):[...d.plans,{...obj,id:uid()}];saveEx(d);close();renderExercicios()});openExercisePlanDialogStable(dlg);
}

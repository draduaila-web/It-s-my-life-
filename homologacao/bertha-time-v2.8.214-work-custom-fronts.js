...
(() => {
  ...
  function openMore(){
    ...
    d.innerHTML=`<div class="bertha-modal bertha-more-modal"><div class="bertha-modal-head bertha-more-head"><div><div class="bertha-more-eyebrow">MAIS</div><strong>Everything in place.<br>More room for life.</strong></div><button data-close aria-label="Fechar">×</button></div><div class="module-links bertha-module-cards">${modules.map(([r,i,l])=>`<a href="#${r}"><span class="bertha-menu-icon">${icon(i)}</span><span>${l}</span></a>`).join('')}</div><section class="bertha-more-data" aria-label="Seus dados"><div class="bertha-more-data-copy"><span>SEUS DADOS</span><small>Leve a sua BERTH.A com você.</small></div><div class="bertha-more-data-actions"><button type="button" data-backup-export>Exportar backup</button><button type="button" data-backup-import>Restaurar backup</button><button type="button" data-hml-reset>Limpar homologação</button><input type="file" data-backup-file accept="application/json,.json" hidden></div></section></div>`;
    document.body.appendChild(d);

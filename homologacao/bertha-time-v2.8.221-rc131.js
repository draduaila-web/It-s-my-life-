/* BERTH.A — Meu Dia v2 / Motor de Tempo v2.8.133 — Home preservada + referências por área específicas
...
(() => {
  ...
  (function(){
    ...
    .bertha-repeat-dialog::backdrop{background:rgba(54,45,58,.30);backdrop-filter:blur(5px)}
    .bertha-repeat-card{position:relative;box-sizing:border-box;width:min(91vw,460px);background:#fbf7ef;border:1px solid rgba(116,103,121,.10);border-radius:28px;padding:24px;box-shadow:0 22px 64px rgba(60,48,66,.16);color:#40394a}
    .bertha-repeat-card .eyebrow{font-size:12px;letter-spacing:.22em;color:#8b7e8d;font-weight:750}
    .bertha-repeat-card h2{margin:7px 46px 10px 0;font-size:24px;line-height:1.12;font-weight:650;color:#383240}
    .bertha-repeat-card p{color:#756d77;margin:18px 0 0}
    .bertha-repeat-card small{display:block;color:#938995;line-height:1.45;margin-top:14px}
    .bertha-repeat-x{position:absolute;right:18px;top:18px;border:0!important;outline:0!important;background:transparent!important;color:#8c858e!important;width:38px;height:38px;border-radius:50%;font-size:28px;line-height:1;box-shadow:none!important;-webkit-appearance:none}
    .bertha-repeat-x:focus,.bertha-repeat-x:focus-visible{outline:0!important;box-shadow:none!important}
    .bertha-repeat-actions{display:flex;justify-content:flex-end;gap:10px;margin-top:20px}
    .bertha-repeat-actions button{min-height:48px;border-radius:16px!important;padding:0 20px!important;font-size:16px!important;font-weight:700!important}
    .bertha-repeat-actions .secondary{background:rgba(255,249,242,.96)!important;color:#6b5972!important;border:1px solid rgba(108,88,113,.075)!important}
    .bertha-repeat-actions .primary{background:linear-gradient(135deg,#f6aa84 0%,#ef9577 55%,#e4866b 100%)!important;color:#fff!important;border:0!important;box-shadow:0 6px 16px rgba(224,136,104,.14)!important}
    .bertha-study-repeat-dialog .bertha-repeat-actions .primary{background:linear-gradient(135deg,#79c7ed 0%,#f0b39f 100%)!important;color:#fff!important}
    .bertha-study-repeat-dialog .bertha-repeat-actions .secondary{background:linear-gradient(135deg,rgba(214,235,248,.92),rgba(250,226,215,.92))!important;color:#5f5965!important}`;
  ...
  function enqueueRepeat(item={}){
    const q=repeatQueue(),base=repeatBaseId(item),today=iso(),now=Date.now();
    const fresh={...item,repeatBaseId:base,
      learningKey:item.learningKey||durationLearningKey(item),
      configuredMinutes:+item.configuredMinutes||+item.minutes||30,
      minutes:effectiveDurationMinutes(item),eligibleDate:today,date:today,createdAt:now,repeatRequestedAt:now,
      repeatedFromTime:item.time||'',repeatedFromPeriod:item.period||'flex',
      repeatedFromWindowStart:item.windowStart||'',repeatedFromWindowEnd:item.windowEnd||'',
      // Repetir é um novo pedido para o dia atual: não herda a janela que já terminou.
      time:'',period:'flex',windowStart:'',windowEnd:'',
      status:'eligible',repeated:true};
    const existing=q.findIndex(x=>x.repeatBaseId===base&&x.status==='eligible');
    if(existing>=0){
      q[existing]={...q[existing],...fresh,id:q[existing].id||`repeat:${base}:${now}`};
    }else{
      q.push({...fresh,id:`repeat:${base}:${now}`});
    }
    saveRepeatQueue(q.slice(-200));
  }
  function repeatCandidatesToday(){const t=iso();return repeatQueue().filter(x=>x && x.status==='eligible'&&(!x.eligibleDate||x.eligibleDate<=t)&&x.kind!=='ideal'&&String(x.source||'').trim().toLowerCase()!=='meu dia ideal');}
  ...
  function commitmentOccursOn(x,date){
    if(!x||x.active===false)return false;
    if(!x.repeat||x.repeat==='none')return x.date===date;
    if(!x.date||x.date>date)return false;
    if(x.repeatUntil&&date>x.repeatUntil)return false;
    const start=new Date(x.date+'T12:00:00'),d=new Date(date+'T12:00:00');
    const days=Math.floor((d-start)/86400000);
    if(days<0)return false;
    if(x.repeat==='daily')return true;
    if(x.repeat==='weekly')return days%7===0;
    if(x.repeat==='weekdays')return (x.weekdays||[]).includes(d.getDay());
    if(x.repeat==='monthly')return d.getDate()===start.getDate();
    return false;
  }
  function commitmentMinutes(x={}){
    const n=Math.max(1,+x.durationValue||60),u=x.durationUnit||'minutes';
    return u==='days'?n*1440:u==='hours'?n*60:n;
  }

  function sourcesToday(){
    const today=iso(), out=[];
    const push=(x)=>{ if(x && x.title) out.push(applyLearnedDuration(x)); };
    // Compromissos — ocupações reais do dia. Entram como blocos fixos para proteger janelas.
    read(COMMITMENTS_KEY,[]).forEach(x=>{
      if(!commitmentOccursOn(x,today))return;
      const mins=commitmentMinutes(x);
      push({id:`commitment:${x.id}:${today}`,learningKey:`commitment:${x.id}`,commitmentId:x.id,source:'Compromissos',title:x.title||'Compromisso',date:today,minutes:mins,configuredMinutes:mins,time:x.time||'',period:'flex',priority:'Alta',kind:'commitment',location:x.location||'',notify:!!x.notify,notifyValue:+x.notifyValue||0,notifyUnit:x.notifyUnit||'minutes'});
    });
    ...
    read('minha-vida.pendencias.v1',[]).forEach(x=>{
      ...
      push({id:`task:${x.id}`,source:'Tarefas',title:x.title||x.name||'Tarefa',category:x.category||'',date,minutes:parseMinutes(x.minutes||x.duration||x.estimate||30),time:x.time||'',period:x.period||'flex',priority:x.priority||'Normal',notify:!!x.notify,notifyOffset:x.notifyOffset||'at-time',kind:'task'});
    });

    // Rituais — eventos previstos para hoje.
    ritualSourcesForToday().forEach(push);

    // Trabalho — CREFITO-11, BEC e TikTok entram na mesma inteligência transversal.
    // Itens vencidos continuam elegíveis; aguardando/pausado não viram sugestão ativa.
    [['minha-vida.trabalho.bec.v1','BEC','bec'],['minha-vida.trabalho.tiktok.v1','TikTok','tiktok']].forEach(([key,label,workKind])=>{
      read(key,[]).forEach(x=>{
        if(x.status==='Concluído'||x.status==='Aguardando'||x.status==='Pausado'||(x.date&&x.date>today)) return;
        push({id:`${key}:${x.id}`,learningKey:`${key}:${x.id}`,workKind,workTaskId:x.id,source:`Trabalho · ${label}`,title:x.title||'Tarefa',date:x.date||today,minutes:+x.minutes||parseMinutes(x.duration),configuredMinutes:+x.minutes||parseMinutes(x.duration),time:x.time||'',period:({manha:'morning',tarde:'afternoon',noite:'night',flexivel:'flex'})[String(x.period||'flex').toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'')]||'flex',priority:x.priority||'Normal',kind:'work',repeated:!!x.repeated,repeatRequestedAt:+x.repeatRequestedAt||0});
      });
    });
    read('minha-vida.trabalho.crefito.v1',[]).forEach(x=>{
      if(x.status==='Concluído'||x.status==='Aguardando'||x.status==='Pausado'||(x.date&&x.date>today)) return;
      push({id:`crefito:${x.id}`,learningKey:`crefito:${x.id}`,workKind:'crefito',workTaskId:x.id,source:'Trabalho · CREFITO-11',title:x.title||'Trabalho',date:x.date||today,minutes:+x.minutes||parseMinutes(x.duration),configuredMinutes:+x.minutes||parseMinutes(x.duration),time:x.time||'',period:({manha:'morning',tarde:'afternoon',noite:'night',flexivel:'flex'})[String(x.period||'flex').toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'')]||'flex',priority:x.priority||'Normal',kind:'work',repeated:!!x.repeated,repeatRequestedAt:+x.repeatRequestedAt||0});
    });
    // Frentes de trabalho cadastradas pelo usuário.
    read('bertha.work.fronts.v1',[]).forEach(f=>{
      if(!f?.id)return;const key=`minha-vida.trabalho.custom.${f.id}.v1`;
      read(key,[]).forEach(x=>{
        if(x.status==='Concluído'||x.status==='Aguardando'||x.status==='Pausado'||(x.date&&x.date>today)) return;
        push({id:`${key}:${x.id}`,learningKey:`${key}:${x.id}`,workKind:f.id,workTaskId:x.id,workStorageKey:key,source:`Trabalho · ${f.name||'Trabalho'}`,title:x.title||'Tarefa',date:x.date||today,minutes:+x.minutes||parseMinutes(x.duration),configuredMinutes:+x.minutes||parseMinutes(x.duration),time:x.time||'',period:({manha:'morning',tarde:'afternoon',noite:'night',flexivel:'flex'})[String(x.period||'flex').toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'')]||'flex',priority:x.priority||'Normal',kind:'work',repeated:!!x.repeated,repeatRequestedAt:+x.repeatRequestedAt||0});
      });
    });
    // Criação & Ideias — projetos acompanhados podem entrar como sugestões flexíveis.
    // A BERTA sugere; a escolha continua sendo da usuária.
    read('minha-vida.ideias.v1',[]).forEach(x=>{
      if(x.type!=='projeto'||x.status==='concluido'||x.status==='Concluído'||x.trackProject===false||!x.projectSuggestWindow) return;
      const total=Math.max(0,+x.totalMinutes||0),done=Math.max(0,+x.progressMinutes||0),remaining=total?Math.max(0,total-done):0;
      if(total && remaining<=0) return;
      const blocks=(Array.isArray(x.sessionMinutes)?x.sessionMinutes:[]).map(Number).filter(n=>n>0).sort((a,b)=>a-b);
      const preferred=Math.max(1,+x.preferredSession||blocks[0]||60);
      push({id:`project:${x.id}`,learningKey:`project:${x.id}`,projectId:x.id,source:'Projeto',title:x.nextAction?`${x.title} · ${x.nextAction}`:(x.title||'Projeto'),minutes:Math.min(preferred,remaining||preferred),configuredMinutes:preferred,projectSessionOptions:blocks.length?blocks:[preferred],projectRemainingMinutes:remaining||null,priority:x.priority||'Normal',dueDate:x.dueDate||'',continuity:!!x.lastWorkedAt&&x.projectContinuePrompt!==false,lastWorkedAt:x.lastWorkedAt||null,kind:'project'});
    });

    // Casa — a BERTA lê frequência, próxima ocorrência, duração e janela.
    // A Home recebe apenas rotinas realmente devidas/compatíveis, não as 26 de uma vez.
    const casa=read('minha-vida.casa.v1',null);
    const casaDur=read('minha-vida.casa.duration.v1',{}),casaTimes=read('minha-vida.casa.time.v2',{});
    const casaDay=(ts)=>{if(!ts)return'';const d=new Date(ts);return Number.isNaN(d.getTime())?'':`${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}-${String(d.getDate()).padStart(2,'0')}`};
    const normCasa=s=>String(s||'').toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'');
    const casaDue=t=>{
      if(!t)return false;
      const f=normCasa(t.freq),last=casaDay(t.lastCompletedAt||t.completedAt),next=String(t.nextDue||'');
      if(/eventual|conforme necessario|unica/.test(f))return false;
      if(last===today)return false;
      if(next && next>today)return false;
      if(t.done){
        if(next && next<=today)return true;
        if(f.includes('diario')&&last&&last<today)return true;
        return false;
      }
      // Sem histórico: a rotina é elegível para o primeiro ciclo. Depois, nextDue governa o retorno.
      return true;
    };
    if(casa && Array.isArray(casa.areas)){
      const routines=[];
      casa.areas.forEach(a=>(a.tasks||[]).forEach(t=>{
        if(!casaDue(t))return;
        const freq=normCasa(t.freq),mins=Math.max(5,+casaDur[t.id]||15),hint=String(casaTimes[t.id]||t.when||'').trim();
        const daily=freq.includes('diario'),micro=mins<=20;
        const weekend=/fim de semana/.test(normCasa(hint));
        if(weekend && ![0,6].includes(new Date().getDay()))return;
        routines.push({a,t,mins,daily,micro,hint});
      }));
      routines.sort((x,y)=>(y.daily-x.daily)||(y.micro-x.micro)||(x.mins-y.mins)).slice(0,8).forEach(({a,t,mins,hint})=>push({id:`casa:routine:${t.id}`,learningKey:`casa:routine:${t.id}`,source:`Casa · ${a.title||'Rotina'}`,title:t.name||'Rotina da casa',date:today,minutes:mins,configuredMinutes:mins,time:'',period:'flex',priority:'normal',kind:'home-routine',homeWindowHint:hint}));
    }
    ...
    // Estudos — conteúdos reais e próximo mapa podem entrar em uma janela livre compatível.
    // A BERTA não despeja a biblioteca: oferece apenas uma continuação plausível por vez.
    const studyContents=read('bertha.estudos.conteudos.v1',[]);
    const mapStatus=read('minha-vida.estudos.mapas.v1',{});
    const mapCrud=read('bertha.estudos.mapas.crud.v1',{custom:[],overrides:{},deleted:[]});
    const deletedMaps=new Set(Array.isArray(mapCrud.deleted)?mapCrud.deleted.map(Number):[]);
    const baseMapIds=Array.from({length:260},(_,i)=>i+1).filter(id=>!deletedMaps.has(id));
    const customMaps=Array.isArray(mapCrud.custom)?mapCrud.custom.filter(x=>!deletedMaps.has(+x.id)):[];
    const mapPool=[...baseMapIds.map(id=>{const o=mapCrud.overrides?.[String(id)]||{};return{id,minutes:+o.minutes||30,title:o.topico||'',materia:o.materia||''}}),...customMaps.map(x=>({id:+x.id,minutes:+x.minutes||30,title:x.topico||'',materia:x.materia||''}))].sort((a,b)=>a.id-b.id);
    const nextMap=mapPool.find(x=>!mapStatus[String(x.id)]?.domino && !mapStatus[x.id]?.domino) || null;
    if(nextMap){
      push({id:`study:tcdf:${nextMap.id}`,learningKey:`study:tcdf:${nextMap.id}`,source:'Estudos',title:nextMap.title?`TCDF · Mapa ${String(nextMap.id).padStart(3,'0')} · ${nextMap.title}`:`TCDF · Mapa ${String(nextMap.id).padStart(3,'0')}`,category:nextMap.materia||'TCDF 2026',date:today,minutes:Math.max(5,+nextMap.minutes||30),configuredMinutes:Math.max(5,+nextMap.minutes||30),period:'flex',priority:'normal',kind:'study'});
    }
    if(Array.isArray(studyContents)) studyContents.slice(0,3).forEach(x=>{
      const mins=Math.max(10,+x.minutes||(+x.durationValue||60)*(x.durationUnit==='hours'?60:1));
      push({id:`study:content:${x.id}`,learningKey:`study:content:${x.id}`,source:'Estudos',title:x.title||'Estudo',date:today,minutes:mins,configuredMinutes:mins,period:'flex',priority:'normal',kind:'study',studyUrl:x.url||''});
    });
    // Exercícios — fichas reais de Movimento podem ocupar uma janela compatível.
    // Meu Dia Ideal é apenas referência/meta: ele NÃO cria cards próprios no Meu Dia.
    const idealItems=read(IDEAL_KEY,[]);
    ...
    if(ex && Array.isArray(ex.plans)){
      const sessions=Array.isArray(ex.sessions)?ex.sessions:[];const nowD=new Date(),monday=new Date(nowD);monday.setDate(nowD.getDate()+(nowD.getDay()===0?-6:1-nowD.getDay()));const weekStart=monday.toISOString().slice(0,10);
      const due=p=>{if(p.active===false)return false;if(p.untilMode==='date'&&p.untilDate&&p.untilDate<today)return false;if(p.frequency==='Dias específicos')return !p.days?.length||p.days.includes(dow);if(p.frequency==='X vezes por semana'){const target=Math.max(1,+p.weeklyTarget||1),done=sessions.filter(s=>s.date>=weekStart&&(String(s.planId||'')===String(p.id)||(!s.planId&&s.name===p.name))).length;return done<target;}return true;};
      ex.plans.filter(due).forEach(p=>{ const dv=Math.max(1,+p.durationValue||parseMinutes(p.target)), mins=p.durationUnit==='hours'?dv*60:dv; const periods={morning:['05:16','07:35'],afternoon:['12:00','18:00'],evening:['18:00','22:00'],flex:['','']}; const w=periods[p.period||'flex']||periods.flex; push({id:`exercise:${p.id}`,selectedPlanId:p.id,learningKey:`exercise-plan-v2:${p.id}`,source:'Exercícios',title:p.name||'Movimento',date:today,minutes:mins,configuredMinutes:mins,time:p.time||'',period:p.period||'flex',windowStart:p.time?'':w[0],windowEnd:p.time?'':w[1],notify:!!p.notify,notifyOffset:p.notifyWhen||'',kind:'exercise'}); });
    }
    // idealItems permanece disponível para o motor de metas/progresso, sem virar tarefa.
  ...
  function homeRoutineEligible(x){
    if(x.kind!=='home-routine'&&x.kind!=='home-maintenance')return true;
    const m=minsNow(),w=new Date().getDay(),hint=String(x.homeWindowHint||x.time||'').toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'');
    // Casa não invade trabalho oficial, home office nem a noite protegida.
    if(m>=480&&m<840)return false;
    if((w===1||w===3)&&m>=880&&m<970)return false;
    const ho={1:[1000,1120],2:[940,1060],3:[1000,1120],4:[940,1060],5:[940,1060]}[w];
    if(ho&&m>=ho[0]&&m<ho[1])return false;
    if(m>=1140)return false;
    if(/fim de semana/.test(hint)&&![0,6].includes(w))return false;
    // Horários explícitos: aparece perto do horário; se passou, permanece disponível até o próximo bloco protegido.
    const times=[...hint.matchAll(/(\d{1,2}):(\d{2})/g)].map(a=>(+a[1])*60+(+a[2]));
    if(times.length){return times.some(t=>m>=t-20&&m<=t+120);}
    if(/apos as refeicoes|apos o jantar/.test(hint))return (m>=690&&m<840)||(m>=1050&&m<1140);
    if(/ao chegar/.test(hint))return m>=840&&m<1000;
    if(/manha/.test(hint)&&!/noite/.test(hint))return m>=316&&m<480;
    if(/noite/.test(hint))return m>=1050&&m<1140;
    // Janelas domésticas e microtarefas entram apenas quando existe espaço real.
    return true;
  }
  function studyEligible(x){
    if(x.kind!=='study')return true;
    const m=minsNow(),w=new Date().getDay();
    // Estudos usa janelas livres; não invade manhã protegida, trabalho oficial,
    // home office obrigatório nem noite protegida.
    if(m>=316&&m<455)return false;
    if(m>=480&&m<840)return false;
    const ho={1:[1000,1120],2:[940,1060],3:[1000,1120],4:[940,1060],5:[940,1060]}[w];
    if(ho&&m>=ho[0]&&m<ho[1])return false;
    if(m>=1140)return false;
  ...
  function isEligible(x){
    ...
    if(!homeRoutineEligible(x))return false;
    if(!studyEligible(x))return false;
    const m=minsNow();
  ...
  function doneToday(id){ return engine().history.some(h=>(h.itemId===id||h.plannedItemId===id) && h.day===iso() && h.status==='done'); }
  ...
  function fitProjectBlock(x,available){
    if(x.kind!=='project') return x;
    const opts=(Array.isArray(x.projectSessionOptions)?x.projectSessionOptions:[]).map(Number).filter(n=>n>0).sort((a,b)=>a-b);
    const rem=+x.projectRemainingMinutes||Infinity;
    const valid=opts.filter(n=>n<=rem);
    if(!valid.length) return x;
    let chosen=+x.configuredMinutes||valid[0];
    if(available!==null){ const fits=valid.filter(n=>n<=available); chosen=fits.length?fits[fits.length-1]:valid[0]; }
    else if(!valid.includes(chosen)){ chosen=valid.reduce((best,n)=>Math.abs(n-chosen)<Math.abs(best-chosen)?n:best,valid[0]); }
    return {...x,minutes:Math.min(chosen,rem),configuredMinutes:chosen};
  }
  function candidates(){
    const raw=sourcesToday();
    const available=nextFixedMinutes(raw);
    const list=raw.map(x=>fitProjectBlock(x,available)).filter(x=>!doneToday(x.id)).filter(isEligible);
  ...
  // Meu Dia Ideal não cria tarefas. Ele pesa atividades reais dos módulos.
  function normText(s=''){return String(s||'').toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'').replace(/[^a-z0-9 ]+/g,' ').replace(/\s+/g,' ').trim();}
  function candidateArea(x={}){
    const s=normText(`${x.source||''} ${x.category||''} ${x.kind||''} ${x.title||''}`);
    if(x.kind==='window'||/movimento|exercicio|treino|academia|caminhada/.test(s))return 'movimento';
    if(x.kind==='study'||/estudo|tcdf|mba|tarot|aula/.test(s))return 'estudos';
    if(x.kind==='project'||/projeto/.test(s))return 'projetos';
    if(x.kind==='work'||/trabalho|crefito|bec|tiktok/.test(s))return 'trabalho';
    if(x.kind==='home-routine'||x.kind==='home-maintenance'||/casa|limpeza|lavanderia|manutencao/.test(s))return 'casa';
    if(x.kind==='ritual'){if(/autocuidado/.test(s))return 'autocuidado';return 'rituais';}
    if(/alimentacao|refeicao|receita|cozinha/.test(s))return 'alimentacao';
    return '';
  }
  function weekStartTs(){const d=new Date();const day=(d.getDay()+6)%7;d.setHours(0,0,0,0);d.setDate(d.getDate()-day);return d.getTime();}
  function historyArea(h={}){return candidateArea({source:h.source,title:h.title,category:h.category,kind:h.kind||''});}
  function goalDeficitScore(area){
    if(!area)return 0; const goals=read(PROGRESS_GOALS_KEY,{}),g=goals[area]; if(!g)return 0;
    const hist=(engine().history||[]).filter(h=>h.status==='done'&&(+h.endedAt||+h.startedAt||0)>=weekStartTs()&&historyArea(h)===area);
    let target=0,actual=0;
    if(area==='movimento'){target=+g.minutesPerWeek||((+g.sessionsPerWeek||0)*30);actual=g.minutesPerWeek?hist.reduce((s,h)=>s+(+h.realMinutes||0),0):hist.length;}
    else if(area==='estudos'){target=(+g.hoursPerWeek||0)*60;actual=hist.reduce((s,h)=>s+(+h.realMinutes||0),0);}
    else if(area==='autocuidado'||area==='rituais'){target=+g.sessionsPerWeek||0;actual=hist.length;}
    else if(area==='trabalho'){target=+g.prioritiesPerWeek||0;actual=hist.length;}
    else if(area==='casa'){target=+g.routinesPerWeek||0;actual=hist.length;}
    else return 0;
    if(!target)return 0; const ratio=Math.min(1,actual/target); return Math.round((1-ratio)*24);
  }
  function idealTextScore(x){
    const items=read(IDEAL_KEY,[]).filter(i=>i&&i.active!==false&&(!(i.untilMode==='date'&&i.untilDate&&i.untilDate<iso())));
    if(!items.length)return 0; const title=normText(`${x.title||''} ${x.source||''} ${x.category||''}`);
    const tokens=new Set(title.split(' ').filter(w=>w.length>=4));
    let best=0;
    for(const i of items){
      let s=0; const ip=i.period||'flex'; if(ip==='flex'||ip===x.period)s+=6;
      const itokens=normText(i.title||'').split(' ').filter(w=>w.length>=4);
      const overlap=itokens.filter(w=>tokens.has(w)).length; s+=Math.min(16,overlap*8);
      const idur=+i.minutes||idealDurationMinutes(i.durationValue||30,i.durationUnit||'minutes'); const cdur=+x.minutes||30;
      if(idur&&Math.abs(idur-cdur)<=15)s+=5;
      best=Math.max(best,s);
    }
    return best;
  }
  function suggestionScore(x,available){
    let score=0; const mins=+x.minutes||30;
    if(available!==null)score+=mins<=available?20:-35;
    const pr=normText(x.priority); if(pr==='alta'||pr==='high')score+=16; else if(pr==='baixa'||pr==='low')score-=4; else score+=4;
    if(x.continuity)score+=10;
    // Pedido explícito de repetir deve voltar às sugestões do dia sem virar início automático.
    if(x.repeated)score+=36;
    const due=x.dueDate||x.date||''; if(due&&due<=iso())score+=10;
    score+=goalDeficitScore(candidateArea(x));
    score+=idealTextScore(x);
    // Pequeno desempate operacional, sem dominar o Ideal.
    score+=({commitment:18,task:5,work:5,'home-maintenance':3,'home-routine':1,ritual:2,window:2,study:2,project:2}[x.kind]||0);
    return score;
  }
  ...
  function syncCasaCompletion(active,end){
    ...
    if(id.startsWith('casa:routine:')){
      ...
      if(found){found.done=true;found.completedAt=end;found.lastCompletedAt=end;found.nextDue=casaRoutineNext(found.freq,end);executions.push({id:`ce-${end}`,kind:'routine',refId:ref,title:found.name||active.title,area:areaName,completedAt:end,nextDue:found.nextDue||null,frequency:found.freq||'cíclica'});}
  ...
  function syncExerciseCompletion(active,real,end){
    ...
    data.sessions.push({
      id:`exs-${end}`,
      name:active.title||'Movimento',
      category:active.category||'Outro',
      minutes:real,
      duration:`${real} min`,
      date:day,
      note:'Tempo real',
    ...
    write('minha-vida.exercicios.v1',data);
  }

  function syncStudyCompletion(active,real,end){
    const id=String(active?.id||'');
    if(!id.startsWith('study:'))return;
    const data=read('minha-vida.estudos.v1',{subjects:[],sessions:[],reviews:[],questions:[]})||{subjects:[],sessions:[],reviews:[],questions:[]};
    data.sessions=Array.isArray(data.sessions)?data.sessions:[];
    const day=iso();
    // evita duplicar a mesma conclusão caso o rerender aconteça mais de uma vez
    if(!data.sessions.some(s=>String(s.engineItemId||'')===id && String(s.date||'')===day && Math.abs((+s.createdAt||0)-end)<5000)){
      data.sessions.push({id:`s-${end}`,subject:active.title||'Estudo',date:day,minutes:Math.max(1,+real||1),engineItemId:id,createdAt:end});
      write('minha-vida.estudos.v1',data);
    }
  }

  function syncWorkCompletion(active,real,end){
    const id=String(active?.id||''); let key='',ref='';
    if(active?.workStorageKey&&active?.workTaskId){key=String(active.workStorageKey);ref=String(active.workTaskId);}
    else if(id.startsWith('crefito:')){key='minha-vida.trabalho.crefito.v1';ref=id.slice('crefito:'.length);}
    else if(id.startsWith('minha-vida.trabalho.bec.v1:')){key='minha-vida.trabalho.bec.v1';ref=id.slice('minha-vida.trabalho.bec.v1:'.length);}
    else if(id.startsWith('minha-vida.trabalho.tiktok.v1:')){key='minha-vida.trabalho.tiktok.v1';ref=id.slice('minha-vida.trabalho.tiktok.v1:'.length);}
    else if(id.startsWith('minha-vida.trabalho.custom.')){const m=id.match(/^(minha-vida\.trabalho\.custom\.[^:]+\.v1):(.*)$/);if(m){key=m[1];ref=m[2];}}
    else if(active?.workTaskId&&active?.workKind){key=`minha-vida.trabalho.${active.workKind}.v1`;ref=String(active.workTaskId);}
    if(!key||!ref)return;
    const arr=read(key,[]),i=arr.findIndex(x=>String(x.id)===ref);if(i<0)return;
    arr[i]={...arr[i],status:'Concluído',completedAt:end,actualMinutes:Math.max(1,+real||1),startedAt:arr[i].startedAt||active.startedAt||null,updatedAt:end};write(key,arr);
  }

  function syncProjectCompletion(active,real,end){
    if(!active?.projectId && !String(active?.id||'').startsWith('project:')) return;
    const ref=active.projectId||String(active.id).slice('project:'.length);
    const items=read('minha-vida.ideias.v1',[]),i=items.findIndex(x=>String(x.id)===String(ref));
    if(i<0)return;
    const x=items[i],total=Math.max(0,+x.totalMinutes||0),next=Math.max(0,+x.progressMinutes||0)+Math.max(1,+real||0);
    x.progressMinutes=next;x.lastWorkedAt=end;x.updatedAt=end;
    if(total&&next>=total){x.status='concluido';x.completedAt=end;}
    items[i]=x;write('minha-vida.ideias.v1',items);
  }


  function syncCommitmentCompletion(active,real,end){
    const ref=active?.commitmentId||String(active?.id||'').match(/^commitment:([^:]+):/)?.[1];
    if(!ref)return;
    const arr=read(COMMITMENTS_KEY,[]),i=arr.findIndex(x=>String(x.id)===String(ref));
    if(i<0)return;
    const x=arr[i];
    x.lastCompletedAt=end;
    x.lastActualMinutes=Math.max(1,+real||1);
    x.updatedAt=end;
    if(!x.repeat||x.repeat==='none'){x.completed=true;x.active=false;}
  ...
  function finishActive(activeId=null,realOverride=null){
    ...
    if(active.resumeHistoryEndedAt){
      const h=(e.history||[]).find(x=>String(x.endedAt)===String(active.resumeHistoryEndedAt));
      const total=Math.max(1,(+h?.realMinutes||0)+real);
      if(h){h.realMinutes=total;h.endedAt=end;h.resumed=true;h.updatedAt=end;}
      recordDurationLearning(active,total);
      e.actives.splice(idx,1);e.active=e.actives[0]||null;saveEngine(e);rerender();
      setTimeout(()=>repeatAfterFinishDialog({...active,id:h?.itemId||active.id,title:h?.title||active.title,source:h?.source||active.source},total),80);
      return;
    }
  ...
  function editCompletedTimeDialog(item,realMinutes){
    ...
    d.querySelector('[data-save-real]').onclick=()=>{const real=Math.max(1,+d.querySelector('[data-real]').value||realMinutes);const e=engine();const h=(e.history||[]).find(x=>x.status==='done'&&((item.historyEndedAt&&String(x.endedAt)===String(item.historyEndedAt))||(!item.historyEndedAt&&String(x.itemId)===String(item.id))));if(h){h.realMinutes=real;h.editedRealMinutes=true;h.updatedAt=Date.now();saveEngine(e)}const key=durationLearningKey(item),store=durationLearningStore(),rec=store.items?.[key];if(rec&&Array.isArray(rec.samples)&&rec.samples.length){rec.samples[rec.samples.length-1].realMinutes=real;const calc=computeDurationLearning(rec.samples,rec.configuredMinutes||item.configuredMinutes||item.plannedMinutes||30);Object.assign(rec,{count:calc.count,learned:calc.learned,learnedMinutes:calc.learnedMinutes,suggestedMinutes:calc.suggestedMinutes,confidence:calc.confidence,lastMinutes:calc.lastMinutes,updatedAt:new Date().toISOString()});store.items[key]=rec;saveDurationLearningStore(store)}d.close();d.remove();rerender();};
  }
  function recentCompleted(){
    const h=(engine().history||[]).find(x=>x.status==='done'&&x.day===iso()&&Date.now()-(+x.endedAt||0)<6*60*60*1000);
    return h||null;
  }
  function resumeCompleted(endedAt){
    const e=engine(),h=(e.history||[]).find(x=>String(x.endedAt)===String(endedAt));if(!h)return;
    e.actives=Array.isArray(e.actives)?e.actives:[];
    if(e.actives.some(a=>String(a.resumeHistoryEndedAt)===String(endedAt)))return;
    const active={id:`resume:${h.itemId}:${Date.now()}`,title:h.title||'Atividade',source:h.source||'Meu Dia',kind:h.kind||null,startedAt:Date.now(),plannedMinutes:10,configuredMinutes:+h.configuredMinutes||+h.plannedMinutes||30,learningKey:h.learningKey||durationLearningKey(h),resumeHistoryEndedAt:h.endedAt,overrunOpenEnded:false};
    e.actives.push(active);e.active=e.actives[0]||active;saveEngine(e);rerender();
  }
  function repeatAfterFinishDialog(item,realMinutes){
    ...
    if(route==='#meu-dia'||route==='#home'||route===''||route==='#') contextClass='bertha-meudia-repeat-dialog';
    else if(route.startsWith('#trabalho')) contextClass='bertha-work-repeat-dialog';
    else if(route.startsWith('#estudos')) contextClass='bertha-study-repeat-dialog';
    else if(route.startsWith('#exercicios')) contextClass='bertha-exercise-repeat-dialog';
    else if(item?.kind==='exercise'||item?.source==='Exercícios'||item?.source==='Movimento') contextClass='bertha-exercise-repeat-dialog';
    else if(item?.kind==='study'||item?.source==='Estudos') contextClass='bertha-study-repeat-dialog';
    else if(String(item?.source||'').toLowerCase().includes('trabalho')||String(item?.id||'').startsWith('crefito:')||String(item?.id||'').includes('trabalho.')) contextClass='bertha-work-repeat-dialog';
    const dlg=document.createElement('dialog');dlg.className='bertha-repeat-dialog '+contextClass;
  ...
  function nowCard(){ const s=currentSuggestion(), actives=s.actives||[];
    ...
    const recentHtml=recent&&!actives.length?`<section class="bertha-recent-done"><div class="bertha-recent-copy"><div class="card-kicker">CONCLUÍDO AGORA</div><strong>${esc(recent.title||'Atividade')}</strong><small>${esc(recent.source||'Meu Dia')} · ${durationText(+recent.realMinutes||1)} reais</small></div><div class="bertha-recent-actions"><button type="button" data-edit-completed="${esc(String(recent.endedAt))}">Editar tempo</button><button type="button" class="primary" data-resume-completed="${esc(String(recent.endedAt))}">Retomar</button></div></section>`:'';
    const running=actives.map(a=>{const elapsed=Math.max(0,Math.floor((Date.now()-a.startedAt)/60000)),end=new Date(a.startedAt+a.plannedMinutes*60000);return `<section class="now-card bertha-now active bertha-now-compact"><div class="bertha-now-main"><div><div class="card-kicker">AGORA · EM ANDAMENTO</div><div class="now-title">${esc(a.title)}</div><div class="now-time">${esc(a.source)} · ${elapsed} min · previsto ${durationText(a.plannedMinutes)} · até ${hhmm(end)}</div></div><button class="bertha-primary bertha-finish-side" data-finish-active="${esc(a.id)}">Concluir</button></div></section>`}).join('');
    if(s.item){const x=s.item;return `${running}<section class="now-card bertha-now bertha-now-suggestion"><div class="bertha-now-main"><div><div class="card-kicker">AGORA · ${esc(x.source).toUpperCase()}</div><div class="now-title">${esc(x.title)}</div><div class="now-time">${x.time?`previsto ${esc(x.time)} · `:''}${durationText(+x.minutes||30)}</div></div><button class="bertha-primary" data-start="${esc(x.id)}">Começar</button></div><div class="bertha-now-foot"><span>${x.windowStart?`Entre ${x.windowStart} e ${x.windowEnd}.`:x.kind==='ideal'?'Sugestão do seu Dia Ideal.':'Disponível para o seu dia.'}</span><button class="bertha-link-button" data-postpone="${esc(x.id)}">Adiar</button></div></section>`;}
    if(running)return running;
    return `${recentHtml}<section class="now-card bertha-now free bertha-free-compact"><div><div class="card-kicker">AGORA</div><div class="now-title">Espaço livre</div><small>Sem urgências neste momento. A BERTH.A preserva esta janela.</small></div></section>`;
  }
  ...
  function renderFinanceRemindersHome(){
    try{
      const raw=JSON.parse(window.berthaHmlStorage.getItem('minha-vida.financeiro.v2')||'null');
      const bills=Array.isArray(raw?.plannedBills)?raw.plannedBills:[];
      if(!bills.length)return '';
      const now=new Date(), today=new Date(now.getFullYear(),now.getMonth(),now.getDate());
      const monthKey=(offset=0)=>{const d=new Date(today.getFullYear(),today.getMonth()+offset,1);return `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}`};
      const months=[monthKey(-1),monthKey(0),monthKey(1)], rows=[], seen=new Set();
      const isBec=b=>/(^|[^a-z])bec([^a-z]|$)|bem[- ]estar consciente/.test([b?.name,b?.category,b?.source,b?.scope,b?.note,b?.payer].filter(Boolean).join(' ').toLowerCase());
      const hhmm=`${String(now.getHours()).padStart(2,'0')}:${String(now.getMinutes()).padStart(2,'0')}`;
      bills.forEach(b=>{
        if(!b||b.active===false||isBec(b)||b.reminderEnabled===false)return;
        months.forEach(month=>{
          const [y,m]=month.split('-').map(Number),lastDay=new Date(y,m,0).getDate();
          const rec=b.recurrence||'once'; let due='';
          if(rec==='monthly'){
            const start=b.startMonth||(b.dueDate||'').slice(0,7)||month;if(month<start)return;
            const day=Math.min(Math.max(1,Number(b.dueDay||String(b.dueDate||'').slice(8,10))||1),lastDay);
            due=`${month}-${String(day).padStart(2,'0')}`;
          }else{ if(!(b.dueDate||'').startsWith(month))return; due=b.dueDate; }
          if((b.payments||{})[month]?.paid)return;
          const [dy,dm,dd]=due.split('-').map(Number),dueDate=new Date(dy,dm-1,dd),days=Math.round((dueDate-today)/86400000),lead=Number(b.reminderDays??0);
          if(days>lead)return;
          const key=`${b.id}|${month}`;if(seen.has(key))return;seen.add(key);
          const first=b.reminderTime||'09:00',again=b.repeatReminderTime||'21:30',repeat=b.repeatIfPending!==false;
          let label=days<0?`Atrasada · ${Math.abs(days)}d`:days===0?'Vence hoje':days===1?'Vence amanhã':`Vence em ${days}d`;
          let detail='';
          if(days<0){
            if(hhmm<first)detail=`${label} · aviso ${first}`;
            else if(repeat&&hhmm<again)detail=`${label} · próximo aviso ${again}`;
            else detail=`${label} · ${String(dd).padStart(2,'0')}/${String(dm).padStart(2,'0')}`;
          }else if(days<=lead){
            if(hhmm<first)detail=`${label} · aviso ${first}`;
            else if(repeat&&hhmm<again)detail=`${label} · próximo aviso ${again}`;
            else detail=`${label} · ${String(dd).padStart(2,'0')}/${String(dm).padStart(2,'0')}`;
          }
          rows.push({name:b.name||'Conta prevista',due,days,detail});
        });
      });
      ...
      const html=rows.slice(0,4).map(o=>`<a class="home-fin-reminder-row" href="#financeiro"><span class="home-fin-reminder-icon ${o.days<0?'is-late':'is-clock'}" aria-hidden="true">${icon(o)}</span><span><strong>${esc(o.name)}</strong><small>${esc(o.detail)}</small></span><b>›</b></a>`).join('');
      return `<section class="day-section home-fin-reminders"><div class="section-head"><h2>Contas para lembrar</h2><a href="#financeiro">ver financeiro</a></div>${html}</section>`;
    }catch(e){ return ''; }
  }

  function ensureHomeUtilityStyles(){
    if(document.getElementById('bertha-home-utility-styles'))return;
    const st=document.createElement('style');st.id='bertha-home-utility-styles';st.textContent=`
      .home-utility-grid{display:grid;grid-template-columns:1fr 1fr;gap:10px;margin:14px 0 16px}
      .home-utility-card{display:grid;grid-template-columns:44px minmax(0,1fr) auto;gap:10px;align-items:center;text-decoration:none;color:#4f4553;background:linear-gradient(180deg,#fffdfb,#fbf5f1);border:1px solid rgba(91,72,96,.11);border-radius:19px;padding:12px;box-shadow:0 5px 14px rgba(73,56,78,.045)}
      .home-utility-card svg{width:23px;height:23px;display:block}
      .home-utility-icon{width:44px;height:44px;border-radius:14px;display:grid;place-items:center;background:#f2e9f3;color:#7c5d82}
      .home-utility-card:nth-child(2) .home-utility-icon{background:#eaf3f0;color:#55897e}
      .home-utility-copy{min-width:0}.home-utility-copy strong{display:block;font-size:14px}.home-utility-copy small{display:block;margin-top:2px;color:#817783;font-size:11.5px;line-height:1.25}.home-utility-count{min-width:28px;height:28px;border-radius:999px;background:#efe5f0;color:#745579;display:grid;place-items:center;font-size:12px;font-weight:800;padding:0 5px;box-sizing:border-box}
      .home-health-mini .section-head button{border:0;border-radius:999px;padding:8px 12px;background:#f1eaf2;color:#715978;font-weight:750}
      .home-food-shortcut-wrap{display:flex;justify-content:flex-end;margin:10px 2px 12px}
      .home-food-shortcut{display:inline-flex;align-items:center;gap:7px;min-height:36px;padding:7px 12px;border:1px solid rgba(173,104,128,.15);border-radius:999px;background:linear-gradient(120deg,rgba(255,248,244,.82),rgba(248,229,233,.56));box-shadow:0 4px 14px rgba(93,68,77,.03);text-decoration:none;color:#985f74;font-size:12px;font-weight:700;letter-spacing:.01em}
      .home-food-shortcut svg{width:15px;height:15px;display:block;fill:none;stroke:currentColor;stroke-width:1.65;stroke-linecap:round;stroke-linejoin:round}
      .home-food-shortcut .home-food-shortcut-arrow{font-size:15px;line-height:1;font-weight:400;color:#a8667d}

      #app .bertha-time-row.bertha-day-open{
        grid-template-columns:78px 56px minmax(0,1fr)!important;
      }

      @media(max-width:390px){.home-utility-grid{grid-template-columns:1fr}.home-utility-card{grid-template-columns:42px minmax(0,1fr) auto}}
    `;document.head.appendChild(st);
  }
  function homeUtilityIcon(name){const p={bag:'<path d="M6 8h12l1 13H5L6 8z"/><path d="M9 9V6a3 3 0 0 1 6 0v3"/>',trend:'<path d="M4 17l5-5 4 4 7-8"/><path d="M15 8h5v5"/>'};return `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">${p[name]||p.bag}</svg>`}
  function renderFoodHomeMini(){
    try{
      if(typeof getTodayMeals!=='function')return '';
      const meals=getTodayMeals(); if(!meals?.length)return '';
      const icon='<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M7 6h10M6 11h12M8 16h8"/><path d="M5 20h14"/></svg>';
      return `<div class="home-food-shortcut-wrap"><a class="home-food-shortcut" href="#receitas" onclick="setTimeout(()=>document.querySelector('.recipe-today')?.scrollIntoView({behavior:'smooth',block:'start'}),100)">${icon}<span>Receitas do dia</span><span class="home-food-shortcut-arrow">→</span></a></div>`;
    }catch(_){return ''}
  }
  function renderHomeScheduled(items){
    const now=minsNow();
    const timed=items.filter(x=>{
      if(!x.time)return false;
      const [h,m]=(x.time||'00:00').split(':').map(Number);
      const start=h*60+m,end=start+Math.max(1,+x.minutes||30);
      return end>=now;
  ...
  function editDayBlocksDialog(){
    ...
    const d=dialogBase('Horários-base',`<div class="bertha-dayhours-intro"><div class="eyebrow">MEU DIA · HORÁRIOS</div><p>Escolha em quais dias esta estrutura vale e cadastre apenas os horários que realmente protegem ou organizam seu dia.</p></div><div class="bertha-scope-title">Aplicar esta estrutura em</div><div class="bertha-dayhours-scope" data-scope-group><button type="button" data-scope="all">Todos os dias</button><button type="button" data-scope="weekdays">Seg–Sex</button><button type="button" data-scope="weekend">Fim de semana</button><button type="button" class="active" data-scope="custom">Dias específicos</button></div><div class="bertha-weekday-picks" data-weekdays>${names.map((n,i)=>`<button type="button" data-day="${i}" class="${i===current?'active':''}">${n}</button>`).join('')}</div><div class="bertha-dayhours-divider"><span>Referências do dia</span><small>A BERTH.A encaixa sugestões apenas fora destes horários e dos seus compromissos.</small></div><div data-block-list></div><div class="bertha-dayhours-quick"><button type="button" data-template="morning">+ Manhã</button><button type="button" data-template="afternoon">+ Tarde</button><button type="button" data-template="evening">+ Noite</button><button type="button" data-add-block>+ Horário</button></div><div class="bertha-actions"><button class="bertha-secondary" data-reset>Restaurar padrão</button><button class="bertha-primary" data-save>Salvar horários</button></div>`);
    d.classList.add('bertha-meudia-dialog','bertha-dayhours-dialog','bertha-dayhours-v2');
    const list=d.querySelector('[data-block-list]'),weekdayBox=d.querySelector('[data-weekdays]');
    let scope='custom',selectedDays=new Set([current]),working=[];
    const clone=x=>JSON.parse(JSON.stringify(x||[]));
    const load=()=>{working=clone(dayBlocks([...selectedDays][0]??current));render();};
    const targetDays=()=>scope==='all'?[0,1,2,3,4,5,6]:scope==='weekdays'?[1,2,3,4,5]:scope==='weekend'?[0,6]:[...selectedDays];
    const render=()=>{list.innerHTML=working.length?working.map((b,i)=>`<div class="bertha-fixed-edit" data-i="${i}"><div class="bertha-fixed-edit-top"><label class="bertha-block-active"><input type="checkbox" data-enabled ${b.enabled===false?'':'checked'}><span></span></label><input class="bertha-fixed-title" type="text" data-title value="${esc(b.title||'Período')}" aria-label="Nome do horário"><button type="button" data-remove aria-label="Remover horário">×</button></div><div class="bertha-fixed-times"><label><span>Início</span><input type="time" data-start value="${esc(b.start||'08:00')}"></label><span class="bertha-time-sep">–</span><label><span>Fim</span><input type="time" data-end value="${esc(b.end||'09:00')}"></label></div></div>`).join(''):`<div class="bertha-empty">Nenhum horário-base. Só os compromissos vão bloquear o dia.</div>`;
      [...list.querySelectorAll('.bertha-fixed-edit')].forEach((r,i)=>{r.querySelector('[data-remove]').onclick=()=>{working.splice(i,1);render();};});
    };
    d.querySelectorAll('[data-scope]').forEach(b=>b.onclick=()=>{scope=b.dataset.scope;d.querySelectorAll('[data-scope]').forEach(x=>x.classList.toggle('active',x===b));weekdayBox.hidden=scope!=='custom';if(scope!=='custom'){selectedDays=new Set(targetDays());working=clone(dayBlocks(targetDays()[0]));render();}});
    d.querySelectorAll('[data-day]').forEach(b=>b.onclick=()=>{const day=+b.dataset.day;if(selectedDays.has(day)&&selectedDays.size>1)selectedDays.delete(day);else selectedDays.add(day);b.classList.toggle('active',selectedDays.has(day));working=clone(dayBlocks([...selectedDays][0]??current));render();});
    const addPreset=(name,start,end)=>{working.push({id:'custom-'+Date.now()+'-'+working.length,title:name,start,end,enabled:true});render();};
    d.querySelectorAll('[data-template]').forEach(b=>b.onclick=()=>{const p={morning:['Manhã','06:00','12:00'],afternoon:['Tarde','12:00','18:00'],evening:['Noite','18:00','23:00']}[b.dataset.template];addPreset(...p);});
    d.querySelector('[data-add-block]').onclick=()=>addPreset('Novo horário','09:00','10:00');
    d.querySelector('[data-reset]').onclick=()=>{working=clone(defaultDayBlocks([...selectedDays][0]??current));render();};
    d.querySelector('[data-save]').onclick=()=>{const rows=[...list.querySelectorAll('.bertha-fixed-edit')],next=rows.map((r,i)=>({...working[i],title:r.querySelector('[data-title]').value.trim()||'Período',enabled:r.querySelector('[data-enabled]').checked,start:r.querySelector('[data-start]').value||working[i].start,end:r.querySelector('[data-end]').value||working[i].end}));const days=targetDays();if(!days.length)return;days.forEach(day=>saveDayBlocks(day,clone(next)));d.close();d.remove();rerender();};
    load();
  }
  ...
  function renderHomeTimeline(){
    ...
    const state=(a,b,prot=false,commit=false)=>{
      if(prot)return 'PROTEGIDO';
      if(commit)return m>=a&&m<b?'AGORA':'COMPROMISSO';
      return m>=b?'FEITO':m>=a?'AGORA':'PRÓXIMO';
    };
    const rows=[];
  ...
  function ensureMeuDiaV168Styles(){
    ...
    const st=document.createElement('style');st.id='bertha-meu-dia-v168';st.textContent=`
      /* MEU DIA v168 — fluxo cromático contínuo da marca */
      #app .day-hero{position:relative!important;padding:23px 72px 22px 22px!important;min-height:0!important;background:linear-gradient(135deg,rgba(255,224,203,.96) 0%,rgba(248,205,220,.92) 48%,rgba(222,207,247,.92) 100%)!important;border:1px solid rgba(193,145,166,.14)!important;box-shadow:0 10px 28px rgba(91,69,98,.05)!important;overflow:hidden!important}
      #app .day-hero:after{content:"B"!important;white-space:normal!important;position:absolute!important;right:18px!important;top:13px!important;width:auto!important;color:rgba(91,82,104,.12)!important;font-family:Georgia,"Times New Roman",serif!important;font-size:74px!important;line-height:1!important;letter-spacing:-.08em!important;text-align:right!important}
      #app .day-hero .eyebrow{color:#d27f73!important}
      #app .day-hero h1{font-family:Montserrat,Inter,-apple-system,BlinkMacSystemFont,"SF Pro Display",sans-serif!important;font-size:clamp(28px,7.4vw,32px)!important;line-height:1.08!important;font-weight:400!important;letter-spacing:-.035em!important;color:#3f3545!important;margin:8px 0 6px!important}
      #app .day-hero .day-date{color:#786d78!important}
      .home-utility-grid{margin:14px 0 15px!important}
      .home-utility-card{background:linear-gradient(135deg,rgba(250,220,232,.72) 0%,rgba(232,219,247,.68) 48%,rgba(218,237,250,.76) 100%)!important;border-color:rgba(115,96,130,.09)!important;box-shadow:0 6px 18px rgba(73,56,78,.035)!important}
      .home-utility-icon{width:34px!important;height:34px!important;border-radius:0!important;background:transparent!important;color:#a15f80!important}
      .home-utility-card:nth-child(2) .home-utility-icon{background:transparent!important;color:#5c879a!important}
      .home-utility-count{background:rgba(255,255,255,.42)!important;color:#a05f78!important}
      .home-scheduled{margin-top:2px!important}
      .home-scheduled-row{display:grid;grid-template-columns:62px minmax(0,1fr) 18px;gap:10px;align-items:center;padding:11px 12px;margin:7px 0;border:1px solid rgba(115,96,130,.075);border-radius:16px;background:linear-gradient(135deg,rgba(243,222,239,.54),rgba(225,226,248,.54),rgba(217,239,247,.48));box-shadow:0 4px 12px rgba(73,56,78,.025)}
      .home-scheduled-time{font-size:12px;font-weight:800;color:#9c6d82}.home-scheduled-row strong{display:block;font-size:13px;color:#413948}.home-scheduled-row small{display:block;margin-top:2px;font-size:10.5px;color:#857987}.home-scheduled-arrow{color:#8f8391;font-size:20px}
      #app .bertha-now{padding-right:22px!important;min-height:0!important;background:linear-gradient(135deg,rgba(248,220,231,.78) 0%,rgba(232,219,248,.72) 52%,rgba(215,235,249,.72) 100%)!important;border-color:rgba(121,96,137,.10)!important;box-shadow:0 8px 22px rgba(83,62,89,.045)!important}
      #app .bertha-now:after{display:none!important}
      #app .bertha-now .card-kicker{color:#d17f74!important}.bertha-primary{background:linear-gradient(135deg,#f1a07f 0%,#e88f75 55%,#db816f 100%)!important;color:#fff!important}.bertha-secondary{background:rgba(244,235,244,.82)!important;color:#6b5972!important}
      .day-section .section-head h2{color:#3f3845!important}
      .day-section .soft-count{background:rgba(241,204,218,.54)!important;color:#9d657a!important}
      .bertha-focus{background:linear-gradient(135deg,rgba(245,224,236,.56),rgba(226,226,247,.50))!important;border:1px solid rgba(112,91,123,.065)!important;box-shadow:none!important}
      .bertha-focus:nth-of-type(3){background:linear-gradient(135deg,rgba(235,225,246,.56),rgba(218,235,249,.52))!important}
      .bertha-focus:nth-of-type(4){background:linear-gradient(135deg,rgba(224,233,248,.58),rgba(216,242,235,.52))!important}
      .bertha-focus:nth-of-type(5){background:linear-gradient(135deg,rgba(229,239,243,.58),rgba(224,244,232,.56))!important}
      .focus-dot{background:transparent!important;border:0!important;color:#a87991!important;width:auto!important;height:auto!important;border-radius:0!important}
      .bertha-focus button{background:rgba(255,255,255,.40)!important;color:#745f79!important}
      .home-health-mini:has(.health-mini-list:empty),.home-health-mini:has(.bertha-empty){display:none!important}
      .bertha-timeline{gap:7px!important}.bertha-time-row{background:linear-gradient(135deg,rgba(227,230,248,.50),rgba(220,239,244,.48))!important;border-color:rgba(102,90,120,.065)!important;box-shadow:none!important}.bertha-time-row::before{background:#86aec9!important}.bertha-time-row:nth-last-child(2){background:linear-gradient(135deg,rgba(222,240,239,.54),rgba(230,244,226,.52))!important}.bertha-time-row:last-child{background:linear-gradient(135deg,rgba(248,225,205,.58),rgba(221,244,229,.58))!important}.bertha-time-row:last-child::before{background:#d7a76c!important}.bertha-time-row.is-commitment::before{background:#df8c78!important}
      .free-space{background:linear-gradient(135deg,rgba(247,226,208,.48),rgba(220,243,231,.52))!important;border-color:rgba(112,105,94,.07)!important}.free-space>div{display:none!important}
      @media(max-width:430px){#app .day-hero{padding-right:68px!important}.home-utility-card{grid-template-columns:34px minmax(0,1fr) auto!important}}

      /* v2.8.165 — MEU DIA: uma única narrativa cromática, sem bolinhas */
      #app .day-hero{
        background:
          radial-gradient(circle at 23% 18%,rgba(255,255,255,.38),transparent 42%),
          linear-gradient(135deg,rgba(250,218,198,.88) 0%,rgba(244,202,219,.84) 50%,rgba(220,207,244,.86) 100%)!important;
        box-shadow:0 8px 24px rgba(91,69,98,.04)!important;
      }
      #app .day-hero:after{
        color:rgba(91,82,104,.085)!important;
        font-size:68px!important;
      }

      .home-utility-card{
        background:
          radial-gradient(circle at 16% 12%,rgba(255,255,255,.30),transparent 38%),
          linear-gradient(135deg,rgba(244,205,223,.78) 0%,rgba(226,214,246,.78) 52%,rgba(207,232,248,.80) 100%)!important;
      }
      .home-utility-icon{
      ...
      .home-scheduled-row{
        background:
          linear-gradient(135deg,rgba(232,214,242,.74) 0%,rgba(214,226,247,.76) 72%,rgba(207,235,246,.76) 100%)!important;
      }

      #app .bertha-now{
        background:
          radial-gradient(circle at 18% 10%,rgba(255,255,255,.28),transparent 38%),
          linear-gradient(135deg,rgba(243,207,223,.78) 0%,rgba(225,214,245,.78) 52%,rgba(207,230,248,.80) 100%)!important;
      }

      /* fila flexível: um mesmo degradê descendo pela lista, sem cores independentes */
      .bertha-focus{
        ...
        background-image:linear-gradient(180deg,
          rgba(239,211,232,.78) 0%,
          rgba(225,215,244,.79) 42%,
          rgba(207,230,247,.80) 100%)!important;
        background-size:100% 420%!important;
        border-color:rgba(104,91,119,.07)!important;
      }
      .bertha-focus:nth-of-type(2){background-position:center 0%!important}
      .bertha-focus:nth-of-type(3){background-position:center 32%!important}
      .bertha-focus:nth-of-type(4){background-position:center 66%!important}
      .bertha-focus:nth-of-type(5){background-position:center 100%!important}
      .focus-dot{display:none!important}
      .bertha-focus button{
        background:rgba(255,255,255,.34)!important;
        border:1px solid rgba(255,255,255,.25)!important;
        color:#705b77!important;
      }

      /* timeline: leitura explícita, sem bolinhas e sem barras coloridas aleatórias */
      .bertha-timeline{gap:8px!important}
      .bertha-time-row{
        display:grid!important;
        grid-template-columns:82px 92px minmax(0,1fr)!important;
        gap:10px!important;
        align-items:center!important;
        padding:12px 13px!important;
        background-image:linear-gradient(180deg,
          rgba(211,229,246,.78) 0%,
          rgba(211,237,241,.79) 52%,
          rgba(217,240,226,.82) 100%)!important;
        background-size:100% 360%!important;
        border-color:rgba(99,105,116,.07)!important;
      }
      .bertha-time-row:nth-child(1){background-position:center 0%!important}
      .bertha-time-row:nth-child(2){background-position:center 30%!important}
      .bertha-time-row:nth-child(3){background-position:center 62%!important}
      .bertha-time-row:nth-child(n+4){background-position:center 100%!important}
      .bertha-time-row::before{display:none!important}
      .bertha-time-row i{
        ...
        color:#786d78!important;
        background:rgba(255,255,255,.34)!important;
        border-radius:8px!important;
        padding:7px 6px!important;
        text-align:center!important;
      }
      .bertha-time-row.is-commitment i{color:#a06161!important}
      .bertha-time-row b{font-size:12px!important}
      .bertha-time-row span{font-size:12px!important}

      /* verde só chega no fim da página */
      .free-space{
        background:linear-gradient(135deg,rgba(237,226,220,.72),rgba(216,239,226,.80))!important;
      }

      @media(max-width:390px){
        .bertha-time-row{grid-template-columns:72px 82px minmax(0,1fr)!important;gap:7px!important}
        .bertha-time-row i{font-size:8px!important;letter-spacing:.07em!important}
      }


      /* v2.8.166 — MEU DIA: cor no ambiente, conteúdo em creme */
      body:has(#app .day-hero){
        background:
          radial-gradient(ellipse 90% 34% at 20% 7%,rgba(250,205,180,.38),transparent 64%),
          radial-gradient(ellipse 92% 38% at 82% 28%,rgba(238,202,225,.38),transparent 66%),
          radial-gradient(ellipse 96% 42% at 20% 55%,rgba(217,211,243,.34),transparent 68%),
          radial-gradient(ellipse 96% 40% at 86% 82%,rgba(199,231,224,.30),transparent 68%),
          #fff9f1!important;
        background-attachment:fixed!important;
      }
      body:has(#app .day-hero) #app{
        background:transparent!important;
      }

      /* Hero: continua sendo o ponto mais identitário, mas não vira um bloco saturado */
      #app .day-hero{
        background:rgba(255,249,242,.62)!important;
        backdrop-filter:blur(18px) saturate(1.08)!important;
        -webkit-backdrop-filter:blur(18px) saturate(1.08)!important;
        border:1px solid rgba(255,255,255,.62)!important;
        box-shadow:0 10px 28px rgba(78,60,82,.035)!important;
      }
      #app .day-hero:before{
        content:""!important;
        position:absolute!important;inset:0!important;pointer-events:none!important;
        background:linear-gradient(135deg,
          rgba(246,182,145,.20) 0%,
          rgba(237,177,207,.17) 47%,
          rgba(195,188,235,.16) 100%)!important;
      }
      #app .day-hero>*{position:relative!important;z-index:1!important}
      #app .day-hero:after{
        z-index:1!important;color:rgba(71,66,82,.075)!important;
      }

      /* Todos os cartões usam creme translúcido: a paleta aparece por trás deles */
      .home-utility-card,
      .home-scheduled-row,
      #app .bertha-now,
      .bertha-focus,
      ...
      .bertha-empty{
        ...
        border:1px solid rgba(255,255,255,.62)!important;
        box-shadow:
          inset 0 0 0 1px rgba(126,105,126,.045),
          0 6px 18px rgba(73,56,78,.028)!important;
        backdrop-filter:blur(16px) saturate(1.06)!important;
        -webkit-backdrop-filter:blur(16px) saturate(1.06)!important;
      }

      .home-utility-card{background:rgba(255,250,244,.68)!important}
      #app .bertha-now{background:rgba(255,249,243,.74)!important}
      .bertha-focus{background:rgba(255,250,244,.67)!important}
      .bertha-time-row{background:rgba(255,250,244,.62)!important}
      .free-space{background:rgba(255,250,244,.60)!important}

      /* Ícones sem medalhões/círculos */
      .home-utility-icon{
        background:transparent!important;
        border-radius:0!important;
      }
      .home-utility-card:first-child .home-utility-icon{color:#b96f8d!important}
      .home-utility-card:nth-child(2) .home-utility-icon{color:#688ca1!important}

      /* CTA principal mantém o pêssego da marca */
      .bertha-primary{
        background:linear-gradient(135deg,#f2a27d 0%,#eb9275 56%,#df846f 100%)!important;
        box-shadow:0 6px 16px rgba(220,128,103,.13)!important;
      }
      .bertha-secondary,
      .bertha-focus button{
        background:rgba(255,250,246,.70)!important;
        border:1px solid rgba(111,88,117,.08)!important;
        color:#6c5b72!important;
      }

      /* Estrutura diária vem antes da lista flexível e vira uma agenda editorial */
      .home-day-structure{margin-top:4px!important}
      .bertha-timeline{gap:6px!important}
      .bertha-time-row{
        grid-template-columns:78px 92px minmax(0,1fr)!important;
      ...
      .bertha-time-row i{
        ...
        color:#9b7b88!important;
        font-size:8.5px!important;
        letter-spacing:.10em!important;
      }
      .bertha-time-row.is-commitment i{color:#c57967!important}

      /* O que importa hoje: editorial, quase sem decoração */
      .bertha-focus{
        min-height:64px!important;
        padding:12px 14px!important;
        border-radius:17px!important;
      }
      .bertha-focus strong{color:#433b47!important}
      .bertha-focus small{color:#837982!important}
      .focus-dot{display:none!important}

      /* Contadores e pequenos estados: sem novas manchas de cor */
      .day-section .soft-count,
      .home-utility-count{
        background:rgba(255,250,246,.64)!important;
        border:1px solid rgba(107,87,111,.07)!important;
      }

      @media(max-width:390px){
        .bertha-time-row{
          grid-template-columns:67px 82px minmax(0,1fr)!important;
        }
      }


      /* v2.8.167 — MEU DIA: fundo inteiro BERTH.A + cards creme */
      body:has(#app .day-hero){
        background:
          radial-gradient(ellipse 120% 48% at 10% 8%,rgba(249,192,157,.58),transparent 66%),
          radial-gradient(ellipse 115% 48% at 88% 26%,rgba(239,190,217,.56),transparent 68%),
          radial-gradient(ellipse 118% 50% at 14% 56%,rgba(211,201,242,.50),transparent 70%),
          radial-gradient(ellipse 112% 48% at 90% 88%,rgba(191,230,220,.46),transparent 70%),
          linear-gradient(180deg,#fff6ef 0%,#fff8f3 100%)!important;
        background-attachment:fixed!important;
      }
      body:has(#app .day-hero) #app{
        background:transparent!important;
      ...
      /* Hero passa a ser creme, deixando a identidade viver no fundo */
      #app .day-hero{
        ...
        box-shadow:
          inset 0 0 0 1px rgba(113,93,112,.035),
          0 8px 22px rgba(74,58,77,.03)!important;
      }
      #app .day-hero:before{display:none!important}
      #app .day-hero:after{color:rgba(68,63,78,.065)!important}

      /* Cards: creme quente, quase opaco, todos na mesma linguagem */
      .home-utility-card,
      .home-scheduled-row,
      #app .bertha-now,
      .bertha-focus,
      ...
      .home-health-mini,
      .bertha-empty{
        background:rgba(255,249,240,.91)!important;
        background-image:none!important;
        border:1px solid rgba(255,255,255,.74)!important;
        box-shadow:
          inset 0 0 0 1px rgba(115,96,113,.035),
          0 5px 16px rgba(74,58,77,.022)!important;
        backdrop-filter:blur(12px) saturate(1.02)!important;
        -webkit-backdrop-filter:blur(12px) saturate(1.02)!important;
      }
      .home-utility-card{background:rgba(255,249,240,.93)!important}
      #app .bertha-now{background:rgba(255,249,240,.94)!important}
      .bertha-focus{background:rgba(255,249,240,.91)!important}
      .bertha-time-row{background:rgba(255,249,240,.89)!important}
      .free-space{background:rgba(255,249,240,.88)!important}

      /* Ícones: sem bolinhas, só traço da marca */
      .home-utility-icon{
        background:transparent!important;
        border-radius:0!important;
      }
      .home-utility-card:first-child .home-utility-icon{color:#b56f86!important}
      .home-utility-card:nth-child(2) .home-utility-icon{color:#6f8da0!important}

      /* Botões: pêssego como assinatura de ação */
      .bertha-primary,
      #app button.bertha-primary{
        background:linear-gradient(135deg,#f5aa84 0%,#ee9677 54%,#e3876b 100%)!important;
        color:#fff!important;
        border:0!important;
        box-shadow:0 6px 16px rgba(224,136,104,.14)!important;
      }
      .bertha-secondary,
      .bertha-focus button{
        background:rgba(255,247,240,.96)!important;
        border:1px solid rgba(111,88,117,.075)!important;
        color:#6c5b72!important;
        box-shadow:none!important;
      }

      /* leitura do dia: sem cromia própria */
      .bertha-time-row i{
        background:transparent!important;
        border:0!important;
        color:#947985!important;
      }
      .bertha-time-row.is-commitment i{color:#c47764!important}
      .day-section .soft-count,
      .home-utility-count{
        background:rgba(255,248,241,.94)!important;
        border:1px solid rgba(108,86,111,.06)!important;
      }
  ...
  function ensureMeuDiaV168FinalStyles(){
    let s=document.getElementById('bertha-meu-dia-v168-final');
    if(s) return;
    s=document.createElement('style');
    s.id='bertha-meu-dia-v168-final';
    s.textContent=`
      /* v2.8.168 — fechamento visual do Meu Dia
         Horários acima do que importa. Cor no ambiente, cards creme. */

      body:has(#app .day-hero){
        background:
          radial-gradient(ellipse 118% 46% at 12% 6%,rgba(249,190,158,.54),transparent 66%),
          radial-gradient(ellipse 116% 48% at 88% 28%,rgba(239,191,218,.51),transparent 68%),
          radial-gradient(ellipse 116% 48% at 16% 57%,rgba(214,204,242,.45),transparent 70%),
          radial-gradient(ellipse 112% 46% at 88% 89%,rgba(197,231,222,.39),transparent 70%),
          #fff7ef!important;
        background-attachment:fixed!important;
      }
      body:has(#app .day-hero) #app{background:transparent!important}

      #app .day-hero,
      #app .home-utility-card,
      #app .home-scheduled-row,
      #app .bertha-now,
      #app .bertha-focus,
      #app .bertha-time-row,
      #app .free-space,
      #app .home-health-mini,
      #app .bertha-empty{
        background:rgba(255,249,240,.93)!important;
        background-image:none!important;
        border:1px solid rgba(255,255,255,.78)!important;
        box-shadow:
          inset 0 0 0 1px rgba(112,92,112,.035),
          0 5px 16px rgba(66,52,69,.025)!important;
        backdrop-filter:blur(11px) saturate(1.02)!important;
        -webkit-backdrop-filter:blur(11px) saturate(1.02)!important;
      }

      #app .day-hero{background:rgba(255,249,240,.90)!important}
      #app .bertha-now{background:rgba(255,249,240,.95)!important}
      #app .bertha-focus{background:rgba(255,249,240,.92)!important}
      #app .bertha-time-row{background:rgba(255,249,240,.90)!important}

      #app .day-hero:before{display:none!important}
      #app .day-hero:after{color:rgba(61,57,72,.065)!important}

      /* Nenhuma bolinha/medalhão decorativo nos cards */
      #app .focus-dot{display:none!important}
      #app .home-utility-icon{
        background:transparent!important;
        border-radius:0!important;
      }

      /* Seu dia: agenda editorial compacta */
      #app .home-day-structure{margin-top:8px!important}
      #app .bertha-timeline{gap:7px!important}
      #app .bertha-time-row{
        display:grid!important;
        grid-template-columns:78px 94px minmax(0,1fr)!important;
        gap:10px!important;
        align-items:center!important;
        padding:11px 13px!important;
        border-radius:15px!important;
      }
      #app .bertha-time-row:before{display:none!important}
      #app .bertha-time-row i{
        background:transparent!important;
        border:0!important;
        padding:0!important;
        font-style:normal!important;
        ...
        font-weight:800!important;
        letter-spacing:.10em!important;
        color:#987a87!important;
        text-align:left!important;
      }
      #app .bertha-time-row.is-commitment i{color:#c57865!important}
      #app .bertha-time-row b{
        color:#5f5662!important;
        font-size:12px!important;
        font-weight:720!important;
      }
      #app .bertha-time-row span{
        color:#746c76!important;
        font-size:12px!important;
      }

      /* O que importa: cards creme, sem arco-íris */
      #app .bertha-focus{
        display:grid!important;
        grid-template-columns:minmax(0,1fr) auto!important;
        gap:12px!important;
      ...
      #app .bertha-focus strong{color:#413a45!important}
      #app .bertha-focus small{color:#857b84!important}

      /* CTA = pêssego da marca */
      #app .bertha-primary,
      #app button.bertha-primary,
      #app .bertha-now [data-start]{
        background:linear-gradient(135deg,#f6aa84 0%,#ef9577 55%,#e4866b 100%)!important;
        color:#fff!important;
        border:0!important;
        box-shadow:0 6px 16px rgba(224,136,104,.14)!important;
      }
      #app .bertha-secondary,
      #app .bertha-focus button{
        background:rgba(255,249,242,.96)!important;
        border:1px solid rgba(108,88,113,.075)!important;
        color:#6a5a70!important;
        box-shadow:none!important;
      }

      #app .soft-count,
      #app .home-utility-count{
        background:rgba(255,249,242,.94)!important;
        border:1px solid rgba(106,87,110,.06)!important;
      }
      ...
      @media(max-width:390px){
        ...
        #app .bertha-time-row i{font-size:8px!important}
      }
    `;
    document.head.appendChild(s);
  }
  ...
  function renderIdeal(){ const items=read(IDEAL_KEY,[]); return `<section class="hero bertha-ideal-hero"><div class="bertha-ideal-hero-copy"><div class="eyebrow">MEU DIA IDEAL</div><h2>A life predictable enough to live freely.</h2><p>Você idealiza. A BERTH.A concretiza com você.</p></div><div class="bertha-ideal-hero-mark" aria-hidden="true"><svg viewBox="0 0 64 64"><path d="M15 40c5-7 10-11 17-11 6 0 11 3 17 10"/><path d="M21 30c3-3 7-5 11-5 5 0 9 2 12 5"/><path d="M32 18v7"/><path d="M26.5 22.5l3 2.5"/><path d="M37.5 22.5l-3 2.5"/></svg></div></section><button class="primary add-full bertha-ideal-add-main" data-add-ideal>＋ Adicionar ao meu dia ideal</button><div class="list bertha-ideal-list">${items.map(x=>`<article class="card"><div><strong>${esc(x.title)}</strong><span>${({morning:'Manhã',afternoon:'Tarde',night:'Noite',flex:'Quando houver espaço'})[x.period]||'Flexível'} · ${durationText(+x.minutes||30)}${x.notify?' · <svg class="bertha-inline-icon" viewBox="0 0 24 24" aria-hidden="true"><path d="M18 8a6 6 0 0 0-12 0c0 7-3 7-3 9h18c0-2-3-2-3-9M10 21h4" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/></svg>':''}</span></div><button class="more" data-del-ideal="${x.id}">×</button></article>`).join('')||'<div class="bertha-empty">Ainda não há preferências. Comece com algo que você gostaria de viver com mais frequência.</div>'}</div>${renderProgressGoals()}`; }
  function addIdealDialog(){
    ...
    const d=dialogBase('Adicionar ao Meu Dia Ideal',`
      <div class="bertha-ideal-modal-form">
        <label class="bertha-field"><span>Período</span>
          <select data-period>
        ...
        <label class="bertha-field"><span>O que seria ideal para você fazer nesse período?</span>
          <input data-title placeholder="Ex.: Ler um livro" autocomplete="off">
        </label>
        ...
        <div class="bertha-notify-row">
          <div><strong>Me avisar?</strong><small>Ative se quiser guardar uma preferência de lembrete.</small></div>
          <label class="bertha-switch" aria-label="Me avisar"><input data-notify type="checkbox"><span></span></label>
        </div>
        <div class="bertha-notify-when" data-notify-when hidden>
          <span>Quando avisar?</span>
          <div class="bertha-notify-custom">
            <input data-notify-value type="number" min="1" step="1" inputmode="numeric" value="10">
            <select data-notify-unit>
              <option value="minutes">minutos</option>
              <option value="hours">horas</option>
        ...
        <div class="bertha-modal-actions bertha-ideal-modal-actions">
          <button type="button" class="bertha-secondary" data-cancel>Cancelar</button>
          <button type="button" class="bertha-primary" data-save>Salvar</button>
        </div>
      </div>
    `);

    const mode=d.querySelector('[data-ideal-until-mode]'),dateWrap=d.querySelector('[data-ideal-until-date-wrap]');
    const syncUntil=()=>{if(dateWrap)dateWrap.hidden=mode?.value!=='date';}; mode?.addEventListener('change',syncUntil);syncUntil();
    const notify=d.querySelector('[data-notify]'),notifyWhen=d.querySelector('[data-notify-when]');
    const syncNotify=()=>{if(notifyWhen)notifyWhen.hidden=!notify?.checked;};notify?.addEventListener('change',syncNotify);syncNotify();
    d.querySelector('[data-cancel]')?.addEventListener('click',()=>{d.close();d.remove();});

    d.querySelector('[data-save]').onclick=()=>{
      const title=d.querySelector('[data-title]')?.value.trim(); if(!title)return;
      const durationValue=Math.max(1,+d.querySelector('[data-ideal-duration-value]')?.value||30),durationUnit=d.querySelector('[data-ideal-duration-unit]')?.value||'minutes';
      const period=d.querySelector('[data-period]')?.value||initialPeriod,untilMode=mode?.value||'ongoing',untilDate=untilMode==='date'?(d.querySelector('[data-ideal-until-date]')?.value||''):'';
      const notifyEnabled=!!notify?.checked,notifyValue=Math.max(1,+d.querySelector('[data-notify-value]')?.value||10),notifyUnit=d.querySelector('[data-notify-unit]')?.value||'minutes';
      const arr=read(IDEAL_KEY,[]);arr.push({id:`ideal-${Date.now()}`,title,period,sourceZone:requested||(period==='night'?'evening':period),durationValue,durationUnit,minutes:idealDurationMinutes(durationValue,durationUnit),untilMode,untilDate,notify:notifyEnabled,notifyValue:notifyEnabled?notifyValue:null,notifyUnit:notifyEnabled?notifyUnit:null,active:true});
      write(IDEAL_KEY,arr);window.__berthaIdealRequestedZone=null;try{d.close?.();}catch(_){}d.remove();rerender();setTimeout(enhanceIdealScreen,0);
    };
  }
    ...
    return `
      <section class="bertha-tasks" data-task-view="open">
        <section class="bertha-task-hero">
          ...
          <h1>Everything handled. Nothing carried.</h1>
        ...
        <div class="bertha-task-list" data-task-list="open">
          ${open.length ? open.map(x=>`
            ...
            `<div class="bertha-task-empty"><strong>Nada precisa de você agora.</strong><p>Se algo surgir, coloque aqui. Você não precisa lembrar.</p></div>`
          }
        </div>
  ...
  function taskDialog(existing=null){
        ...
        <div class="bertha-task-modal-body-v2">
          ...
          <div class="bertha-notify-box">
            <label class="bertha-toggle-row">
              <div><strong>Me avisar?</strong><span>Receber uma notificação no horário escolhido</span></div>
    ...
    notify.onchange=async()=>{
      notifyWhen.hidden=!notify.checked;
      if(notify.checked && window.berthaRequestPushPermission){
        const result=await window.berthaRequestPushPermission();
        if(!result?.ok){
          notify.checked=false;
          notifyWhen.hidden=true;
          alert('Para receber avisos no iPhone, abra o BERTH.A pela Tela de Início e permita as notificações.');
        }
      }
    };
    ...
    d.querySelector('[data-save]').onclick=async()=>{
      const saveBtn=d.querySelector('[data-save]');
      ...
      if(saveBtn){ saveBtn.dataset.saving='1'; saveBtn.disabled=true; saveBtn.textContent='Salvando…'; }
      const arr=read(TASKS_KEY,[]);
      const item={
        ...(existing||{}),
      ...
      write(TASKS_KEY,arr);
      close(); rerender();
      if(item.notify && window.berthaSchedulePush){
        setTimeout(async()=>{
          try {
            const scheduled=await window.berthaSchedulePush(item);
            if(scheduled?.ok){
              const current=read(TASKS_KEY,[]);
              const saved=current.find(t=>String(t.id)===String(item.id));
              if(saved){ saved.pushNotificationId=scheduled.notificationId||''; saved.pushScheduledFor=scheduled.scheduledFor||''; write(TASKS_KEY,current); }
            } else { alert(scheduled?.message||'A tarefa foi salva, mas o aviso não foi agendado.'); }
          } catch(error){
            console.warn('BERTH.A schedule push',error);
            alert('A tarefa foi salva, mas o aviso não pôde ser agendado agora.');
          }
        },0);
      }
    };
  ...
  function recordDirectTaskCompletion(task={},done=true){
    const e=engine();
    e.history=Array.isArray(e.history)?e.history:[];
    const itemId=`task:${task.id}`;
    if(done){
      // Conclusão direta registra o fato de que a tarefa foi concluída, mas NÃO inventa tempo real.
      // Tempo real só existe quando a atividade passa por Começar agora → Concluir.
      const end=Date.now();
      const planned=Math.max(1,parseMinutes(task.minutes||task.duration||task.estimate||30));
      e.history.unshift({
        itemId,
        learningKey:itemId,
        title:task.title||task.name||'Tarefa',
        source:'Tarefas',
      ...
      // Ao reabrir, desfaz apenas o registro direto mais recente desta tarefa.
      const i=e.history.findIndex(h=>h.itemId===itemId && h.status==='done' && h.completionMode==='direct');
      if(i>=0)e.history.splice(i,1);
    }
    saveEngine(e);
  }
  ...
  function setTaskDone(id,done){
    ...
    if(!x)return;
    const wasDone=taskIsDone(x);
    const now=Date.now();
    x.done=done; x.completed=done; x.status=done?'Concluído':'Aberto'; x.completedAt=done?now:null; x.updatedAt=now;
    write(TASKS_KEY,arr);
    if(done!==wasDone) recordDirectTaskCompletion(x,done);
    rerender();
  }
  ...
  const CAPILLARY_TEMPLATE_KEY='bertha.ritual.capilar.template.v1';
  function capillaryTemplateStore(){ return read(CAPILLARY_TEMPLATE_KEY,{}); }
  function saveCapillaryTemplateStore(v){ write(CAPILLARY_TEMPLATE_KEY,v); }
  ...
  const RITUAL_ICONS={
    ...
    hair:`<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M7 20c3.4-1.5 5.2-4.7 5.2-9.6 0-2.8 1.1-5 3.4-6.4-.1 3.8 1.4 6.3 4.4 7.7-1.2 4.5-4.1 7.3-8.5 8.3"/><path d="M5 19.2c3.2-2 4.7-5.1 4.4-9.3M3.5 16.8c2.3-1.7 3.4-4.1 3.3-7.1"/></svg>`,
    selfcare:`<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M12 3.8c4.1 0 7 3.2 7 7.2 0 4.8-2.7 8.2-7 10-4.3-1.8-7-5.2-7-10 0-4 2.9-7.2 7-7.2Z"/><path d="M8.8 10.3h.1M15.1 10.3h.1M9.3 14.3c1.7 1.2 3.7 1.2 5.4 0"/></svg>`,
    tea:`<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M5 9h11v7a4 4 0 0 1-4 4H9a4 4 0 0 1-4-4V9Z"/><path d="M16 11h2a2.5 2.5 0 0 1 0 5h-2M8 6c0-1 1-1.2 1-2M12 6c0-1 1-1.2 1-2"/></svg>`,
    procedure:`<svg viewBox="0 0 24 24" aria-hidden="true"><path d="m6 18 9.5-9.5M13 6l5 5M15.5 4.5l4 4M5 19l-1 2 2-1M8.5 15.5l-2-2"/></svg>`,
    footspa:`<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M4 14h16v2a5 5 0 0 1-5 5H9a5 5 0 0 1-5-5v-2Z"/><path d="M8 11c0-1.4 1.2-2.6 2.6-2.6M12 11c0-1.8 1.5-3.3 3.3-3.3M7 5c0 1 1 1.2 1 2M11 4c0 1 1 1.2 1 2"/></svg>`,
    scrub:`<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M7 19h10l1-9H6l1 9Z"/><path d="M9 10V8a3 3 0 0 1 6 0v2M9 14h6M10 17h4"/><circle cx="18.5" cy="5.5" r="1"/><circle cx="5" cy="6" r=".8"/></svg>`,
    depilation:`<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M8 4h8v3H8zM10 7h4v13h-4zM8.5 20h7"/></svg>`,
    ...
    massage:`<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M4 17c2.3-2.2 4.4-3.3 6.4-3.3 2.7 0 3.3 1.7 5.1 1.7 1.5 0 2.7-.7 4.5-2.4"/><path d="M5 11c1.5-1.8 3-2.7 4.5-2.7 2.3 0 3.2 1.6 5 1.6 1.4 0 2.7-.6 4.5-2.1M7 20h10"/></svg>`,
    mask:`<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M12 3.5c4.3 0 7.3 3.4 7.3 7.7 0 4.7-2.8 8-7.3 9.3-4.5-1.3-7.3-4.6-7.3-9.3 0-4.3 3-7.7 7.3-7.7Z"/><path d="M8.5 10h2M13.5 10h2M9.2 14.3c1.8.8 3.8.8 5.6 0"/></svg>`,
    drops:`<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M9 4C6.5 7.2 5 9.1 5 11.2A4 4 0 0 0 13 11.2C13 9.1 11.5 7.2 9 4Z"/><path d="M17 8c-1.8 2.3-2.8 3.7-2.8 5.2a2.8 2.8 0 0 0 5.6 0C19.8 11.7 18.8 10.3 17 8Z"/></svg>`,
    bottle:`<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M9 3h6v4l2 2v11H7V9l2-2V3Z"/><path d="M9 7h6M10 13h4"/></svg>`,
    candle:`<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M7 10h10v11H7z"/><path d="M12 10V7M12 7c-2-1.7-1.1-3.2 0-4.5 1.1 1.3 2 2.8 0 4.5Z"/></svg>`,
    moon:`<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M20 15.2A8.3 8.3 0 0 1 8.8 4 8.5 8.5 0 1 0 20 15.2Z"/></svg>`,
    towel:`<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M6 5h12v14H6zM9 5v14M15 5v14"/></svg>`,
  ...
  const RITUAL_ICON_KEYS=['hair','selfcare','tea','procedure','footspa','scrub','depilation','sun','bath','massage','mask','drops','candle','moon','heart','sparkle'];
  ...
  const RITUAL_SCHEDULE_KEY='bertha.ritual.schedule.v1';
  function ritualScheduleStore(){const v=read(RITUAL_SCHEDULE_KEY,{});return v&&typeof v==='object'?v:{}}
  function saveRitualScheduleStore(v){write(RITUAL_SCHEDULE_KEY,v)}
  function ritualScheduleState(id){const all=ritualScheduleStore(),x=all[id]||{};return {shift:Number(x.shift)||0,overrides:x.overrides&&typeof x.overrides==='object'?x.overrides:{}}}
  function addRitualDays(d,n){const x=dayStart(d);x.setDate(x.getDate()+Number(n||0));return x}
  function ritualBaseStart(id){return id==='capilar'?CAPILLARY_START:SELFCARE_START}
  function ritualOccurrenceDate(id,index){const st=ritualScheduleState(id),key=String(index);if(st.overrides[key])return dayStart(new Date(st.overrides[key]+'T00:00:00'));return addRitualDays(ritualBaseStart(id),Number(index)+st.shift)}
  function moveRitualOccurrence(id,index,newISO,scope){
    const all=ritualScheduleStore(),st=ritualScheduleState(id),key=String(index),old=ritualOccurrenceDate(id,index),next=dayStart(new Date(newISO+'T00:00:00'));
    const delta=Math.round((next-old)/86400000);if(!delta)return;
    if(scope==='cycle'){
      st.shift+=delta;
      Object.keys(st.overrides).forEach(k=>{const d=new Date(st.overrides[k]+'T00:00:00');st.overrides[k]=iso(addRitualDays(d,delta))});
  ...
  function capillaryDay(d){
    const target=iso(dayStart(d)),hits=[];for(let i=0;i<capillaryCycleTotal();i++)if(iso(ritualOccurrenceDate('capilar',i))===target)hits.push(i+1);
    if(hits.length)return hits[0];
    const st=ritualScheduleState('capilar');return Math.floor((dayStart(d)-addRitualDays(CAPILLARY_START,st.shift))/86400000)+1;
  }
  function capillaryBaseEvent(d){
  ...
  function capillaryEventForIndex(index){
    const i=Number(index);if(i<0||i>=capillaryCycleTotal())return null;
    const baseDate=addRitualDays(CAPILLARY_START,i),base=capillaryBaseEvent(baseDate);if(!base)return null;
    const n=i+1,saved=capillaryTemplateStore()[String(n)]||{};
    return {...base,...saved,date:ritualOccurrenceDate('capilar',i),type:base.type,cycleDay:n,_occurrence:i,tasks:Array.isArray(saved.tasks)?saved.tasks:(base.tasks||[]),products:Array.isArray(saved.products)?saved.products:undefined,includeInMyDay:saved.includeInMyDay!==false};
  }
  function capillaryEventsOn(d){const target=iso(dayStart(d)),out=[];for(let i=0;i<capillaryCycleTotal();i++){const ev=capillaryEventForIndex(i);if(ev&&iso(ev.date)===target)out.push(ev)}return out}
  function capillaryEvent(d){return capillaryEventsOn(d)[0]||null}
  const SELFCARE_TEMPLATE_KEY='bertha.ritual.autocuidado.template.v1';
  function selfcareTemplateStore(){ return read(SELFCARE_TEMPLATE_KEY,{}); }
  function saveSelfcareTemplateStore(v){ write(SELFCARE_TEMPLATE_KEY,v); }
  function selfcareTemplateItem(day){
    const idx=Math.max(1,Math.min(14,Number(day)||1))-1, base=SELFCARE_CYCLE[idx]||SELFCARE_CYCLE[0], saved=selfcareTemplateStore()[String(idx+1)]||{};
    return {...base,...saved,tasks:Array.isArray(saved.tasks)?saved.tasks:(base.tasks||[]),products:Array.isArray(saved.products)?saved.products:[],includeInMyDay:saved.includeInMyDay!==false};
  }
  function selfcareEventForIndex(index){
    const i=Math.max(0,Number(index)||0),day=((i%14)+14)%14+1,base=selfcareTemplateItem(day);
    return {...base,date:ritualOccurrenceDate('autocuidado',i),cycleDay:day,_occurrence:i,type:'selfcare',period:base.period||'night',subtitle:`Dia ${day} / 14`};
  }
  function selfcareEventsOn(d){
    const target=iso(dayStart(d)),st=ritualScheduleState('autocuidado'),estimate=Math.floor((dayStart(d)-addRitualDays(SELFCARE_START,st.shift))/86400000),keys=new Set();
    for(let i=Math.max(0,estimate-3);i<=estimate+3;i++)keys.add(i);Object.keys(st.overrides).forEach(k=>keys.add(Number(k)));
    return [...keys].filter(i=>i>=0).map(selfcareEventForIndex).filter(ev=>iso(ev.date)===target).sort((a,b)=>a._occurrence-b._occurrence);
  }
  function selfcareEvent(d){return selfcareEventsOn(d)[0]||null}
  function selfcareCurrentCycleStartIndex(){const st=ritualScheduleState('autocuidado'),n=Math.max(0,Math.floor((dayStart(new Date())-addRitualDays(SELFCARE_START,st.shift))/86400000));return Math.floor(n/14)*14}
  ...
  function customEventsOn(d){
    ...
    return ritualEvents().filter(e=>{
      const cycleStart=e.cycleStartedAt||e.startDate||e.date||'';
      if(e.cycleDays&&cycleStart){
        const elapsed=Math.floor((dayStart(d)-new Date(cycleStart+'T00:00:00'))/86400000);
        if(elapsed<0)return false;
        if(!e.restartCycle && elapsed>=Number(e.cycleDays))return false;
      }
  ...
  function ritualSourcesForToday(){
    ...
    capillaryEventsOn(d).forEach(cap=>{if(cap&&cap.includeInMyDay!==false){const s=ritualSetting('capilar',cap.type);out.push({id:`ritual:capilar:${iso(d)}:${cap.type}:${cap._occurrence}`,source:'Ritual Capilar',title:`Ritual Capilar · ${cap.title}`,date:iso(d),minutes:cap.minutes,time:s.time||'',period:cap.period||s.period||'morning',priority:s.priority||'Normal',notify:!!s.notify,notifyOffset:s.notifyOffset||'at-time',kind:'ritual'})}});
    const ss=ritualSetting('autocuidado','selfcare');
    selfcareEventsOn(d).forEach(sc=>{if(!sc.optional&&sc.includeInMyDay!==false)out.push({id:`ritual:autocuidado:${iso(d)}:${sc._occurrence}`,source:'Autocuidado',title:`Autocuidado · ${sc.title}`,date:iso(d),minutes:sc.minutes,time:ss.time||'',period:sc.period||ss.period||'night',priority:ss.priority||'Normal',notify:!!ss.notify,notifyOffset:ss.notifyOffset||'at-time',kind:'ritual'})});
  ...
  const BERTHA_RITUAL_PHRASES=['Your plans have a place. So do you.','Your life, with somewhere to land.','Everything handled. Nothing carried.','Create headroom for life.'];
  function ritualBrandPhrase(id){let n=0;for(const c of String(id||''))n=(n+c.charCodeAt(0))%BERTHA_RITUAL_PHRASES.length;return BERTHA_RITUAL_PHRASES[n];}
  function ritualLines(v){return Array.isArray(v)?v:String(v||'').split('\n').map(x=>x.trim()).filter(Boolean)}
  const SELFCARE_PROCEDURES_KEY='bertha.ritual.autocuidado.procedures.v1';
  function selfcareProcedureStore(){return read(SELFCARE_PROCEDURES_KEY,{})}
  function selfcareProcedureKey(ev){return String(ev?.title||'geral').toLowerCase()}
  function selfcareProcedure(ev){const x=selfcareProcedureStore()[selfcareProcedureKey(ev)]||{};return {...ev,tasks:Array.isArray(x.steps)?x.steps:(ev?.tasks||[])}}
  function saveSelfcareProcedure(ev,steps,products){const all=selfcareProcedureStore();all[selfcareProcedureKey(ev)]={steps};write(SELFCARE_PROCEDURES_KEY,all);const st=ritualProductsStore();st[ritualProductKey('autocuidado',ev)]=products;saveRitualProductsStore(st)}
  function renderRituals(){
    const cards=ritualCatalog().map((r,i)=>`
      <button class="bertha-ritual-card tone-${i%3}" type="button" data-open-ritual="${r.id}">
    ...
    return `<section class="bertha-ritual-page">
      <section class="bertha-ritual-hero"><div class="bertha-task-kicker">RITUAIS</div><h1>Your plans have a place. So do you.</h1><p>Você idealiza. A BERTH.A ritualiza para você.</p><span class="bertha-ritual-hero-mark" aria-hidden="true"><svg viewBox="0 0 48 48"><ellipse cx="24" cy="24" rx="14" ry="7" transform="rotate(38 24 24)"/><ellipse cx="24" cy="24" rx="14" ry="7" transform="rotate(-38 24 24)"/><circle cx="24" cy="24" r="2.2"/></svg></span></section>
  ...
  function ritualDetailData(id,d=new Date()){
    ...
    if(id==='autocuidado') return selfcareProcedure(selfcareEvent(d));
    return null;
  }
  function renderRitualDetail(id){
    const r=ritualCatalog().find(x=>x.id===id)||defaultRituals()[0], ev=ritualDetailData(id,new Date());
    const extras=ritualEvents().filter(x=>x.ritualId===id&&!x.primary), isCustom=id!=='capilar'&&id!=='autocuidado';
    let upcoming='';
    if(!isCustom){for(let i=1;i<=8;i++){const d=new Date();d.setDate(d.getDate()+i);const xs=id==='capilar'?capillaryEventsOn(d):id==='autocuidado'?selfcareEventsOn(d):[];xs.forEach(x=>{const attr=id==='autocuidado'?` type="button" data-edit-selfcare-occurrence="${x._occurrence}"`:id==='capilar'?` type="button" data-edit-capillary-occurrence="${x._occurrence}"`:'';upcoming+=`<button class="bertha-ritual-upcoming"${attr}><span>${d.toLocaleDateString('pt-BR',{day:'2-digit',month:'short'}).replace('.','')}</span><div><strong>${esc(x.title)}</strong><small>${esc(x.subtitle||'')}</small></div><b>›</b></button>`})}}
    const phrase=r.subtitle||ritualBrandPhrase(id), customSteps=ritualLines(r.steps);
    return `<section class="bertha-ritual-page">
      <a class="bertha-back-link" href="#rituais" data-back-rituals>‹ Rituais</a>
      <section class="bertha-ritual-detail-hero tone-${Math.abs(String(id).length)%3}"><span class="bertha-line-icon large">${ritualIcon(r.icon)}</span><div><div class="bertha-task-kicker">${esc(r.title).toUpperCase()}</div><h1>${id==='capilar'?'Seu cronograma, no lugar certo.':esc(phrase)}</h1>${r.subtitle?`<p>${esc(r.subtitle)}</p>`:''}</div></section>
      ${isCustom?`<div class="bertha-section-row"><span>COMO FAZER</span><button type="button" data-edit-custom-ritual="${id}">Editar ritual</button></div><section class="bertha-ritual-today custom-how">${customSteps.length?`<div class="bertha-check-steps">${customSteps.map(t=>`<label><input type="checkbox"><span>${esc(t)}</span></label>`).join('')}</div>`:`<div class="bertha-free-note">Nenhuma etapa cadastrada. Toque em Editar ritual para adicionar.</div>`}</section>`:''}
      ${ev?`<section class="bertha-ritual-today"><div class="bertha-ritual-head"><div><span>HOJE · ${dateLabelLong(new Date())}</span><h2>${esc(ev.title)}</h2><p>${esc(ev.subtitle||'')}</p><small class="bertha-cycle-position">${id==='capilar'?`Dia ${capillaryDay(new Date())} de ${capillaryCycleTotal()}`:`Dia ${ev.cycleDay} de 14`}</small></div><button type="button" ${id==='autocuidado'?'data-edit-selfcare-procedure':'data-config-ritual="'+id+'"'}>${id==='autocuidado'?'Editar cuidado':'Editar'}</button></div>
      ...
      ${!isCustom?`<div class="bertha-section-row"><span>PRÓXIMOS DIAS</span>${id==='capilar'?'<button type="button" data-full-capillary>Ver 45 dias</button>':id==='autocuidado'?'<button type="button" data-full-selfcare>Ver 14 dias</button>':''}</div><div class="bertha-upcoming-list">${upcoming}</div>`:''}
      <div class="bertha-section-row"><span>CUIDADOS & EXTRAS</span><button type="button" data-add-ritual-event="${id}">＋ Adicionar</button></div>
      <div class="bertha-extra-list">${extras.length?extras.map(e=>`<button class="bertha-extra-card" data-edit-ritual-event="${e.id}"><span class="bertha-line-icon">${ritualIcon(e.icon||'sparkle')}</span><span><strong>${esc(e.title)}</strong><small>${esc(ritualRepeatLabel(e))} · ${durationText(e.minutes)}</small></span><b>›</b></button>`).join(''):`<div class="bertha-empty-soft">Você pode adicionar outros cuidados quando fizer sentido.</div>`}</div>
  ...
  function ritualEventDialog(ritualId,eventId=''){
    ...
    d.querySelector('[data-save]').onclick=()=>{
      ...
      const arr=ritualEvents(),i=arr.findIndex(e=>e.id===x.id);if(i>=0)arr[i]=x;else arr.push(x);saveRitualEvents(arr);if(ritualId==='autocuidado'){const st=ritualProductsStore();st[ritualProductKey('autocuidado',x)]=x.products;saveRitualProductsStore(st)}d.close();d.remove();rerender();
    };
  ...
  function selfcareProcedureDialog(){const ev=selfcareEvent(new Date());if(ev)selfcareOccurrenceEditor(ev._occurrence);}
  ...
  function bindRitualDetail(id){
    document.querySelector('[data-back-rituals]')?.addEventListener('click',(e)=>{e.preventDefault(); history.pushState(null,'','#rituais'); rerender(); window.scrollTo(0,0);});
    document.querySelector('[data-edit-selfcare-procedure]')?.addEventListener('click',selfcareProcedureDialog);
    document.querySelector('[data-edit-custom-ritual]')?.addEventListener('click',()=>customRitualDialog(id));
    ...
    document.querySelectorAll('[data-edit-selfcare-occurrence]').forEach(b=>b.onclick=()=>selfcareOccurrenceEditor(b.dataset.editSelfcareOccurrence));
    document.querySelectorAll('[data-edit-capillary-occurrence]').forEach(b=>b.onclick=()=>capillaryOccurrenceEditor(b.dataset.editCapillaryOccurrence));
    ...
    document.querySelectorAll('[data-ritual-buy-name]').forEach(b=>b.onclick=()=>{const added=addRitualProductToShopping(b.dataset.ritualBuyName,b.dataset.ritualBuySource);b.textContent=added?'Na lista':'Já está na lista';b.disabled=true;});
    document.querySelector('[data-delete-ritual]')?.addEventListener('click',()=>deleteRitual(id));
  }

  function isMovementReserve(item){
    return !!item && item.kind==='ideal' && /exerc|moviment/i.test(String(item.title||''));
  }
  function movementPlans(){
    const data=read('minha-vida.exercicios.v1',{plans:[]})||{plans:[]};
    const today=iso(),dow=['Dom','Seg','Ter','Qua','Qui','Sex','Sáb'][new Date().getDay()];
    return (Array.isArray(data.plans)?data.plans:[])
      .filter(p=>p.active!==false && !(p.untilMode==='date'&&p.untilDate&&p.untilDate<today))
      .map(p=>({...p,__today:(p.frequency!=='Dias específicos'||!p.days?.length||p.days.includes(dow))}))
      .sort((a,b)=>Number(b.__today)-Number(a.__today));
  }
  function planMinutes(p){ const dv=Math.max(1,+p.durationValue||parseMinutes(p.target)||30); return p.durationUnit==='hours'?dv*60:dv; }
  function openMovementSelector(reserve){
    const plans=movementPlans();
    const d=dialogBase('O que você quer fazer agora?',`
      <p class="bertha-muted">Você reservou ${durationText(+reserve.minutes||30)} para movimento. Escolha o que combina com você agora.</p>
      <div class="bertha-stack" data-movement-list>
    ...
    d.querySelectorAll('[data-movement-plan]').forEach(b=>b.onclick=()=>{
      ...
      d.close();d.remove();
      startItem({
        id:`exercise:${p.id}`,learningKey:`exercise-plan-v2:${p.id}`,source:'Movimento',title:p.name||'Movimento',
        minutes:reserveMinutes,configuredMinutes:planMinutes(p),period:reserve.period||p.period||'flex',kind:'exercise',
        plannedItemId:reserve.id,plannedName:reserve.title||'Movimento',selectedPlanId:p.id,category:p.type||'Outro'
      });
    });
    d.querySelector('[data-open-exercises]')?.addEventListener('click',()=>{d.close();d.remove();location.hash='#exercicios'});
  }
  function startFromHome(item){ if(isMovementReserve(item)) openMovementSelector(item); else startItem(item); }

  function bindHome(){ document.querySelectorAll('[data-start]').forEach(b=>b.onclick=()=>{const x=sourcesToday().find(i=>i.id===b.dataset.start);if(x)startFromHome(x)}); document.querySelectorAll('[data-postpone]').forEach(b=>b.onclick=()=>{const x=sourcesToday().find(i=>i.id===b.dataset.postpone);if(x)postponeDialog(x)}); document.querySelectorAll('[data-finish-active]').forEach(f=>f.onclick=()=>finishActivePrompt(f.dataset.finishActive)); document.querySelectorAll('[data-edit-completed]').forEach(b=>b.onclick=()=>{const h=(engine().history||[]).find(x=>String(x.endedAt)===String(b.dataset.editCompleted));if(h)editCompletedTimeDialog({...h,id:h.itemId,historyEndedAt:h.endedAt},h.realMinutes)}); document.querySelectorAll('[data-resume-completed]').forEach(b=>b.onclick=()=>resumeCompleted(b.dataset.resumeCompleted)); document.querySelectorAll('[data-edit-day-hours]').forEach(b=>b.onclick=editDayBlocksDialog); window.BerthaShopping?.bindHealthMini?.(); setTimeout(maybePromptFixedCommitmentConflict,180); }
  ...
  function ensureStyles(){
    ...
    .bertha-dialog .bertha-modal{background:#fbf7ef;border-radius:28px;padding:22px}
    .bertha-dialog .bertha-modal-head{align-items:flex-start;gap:14px;margin-bottom:12px}
    .bertha-dialog .bertha-modal-head strong{font-size:21px;line-height:1.18;font-weight:650;color:#3d3744}
    .bertha-dialog .bertha-modal-head button{width:38px;height:38px;flex:0 0 38px;padding:0;border:0!important;outline:0!important;background:transparent!important;color:#8c858e!important;box-shadow:none!important;-webkit-appearance:none;line-height:1}
    .bertha-dialog .bertha-modal-head button:focus,.bertha-dialog .bertha-modal-head button:focus-visible{outline:0!important;box-shadow:none!important}
    ...
    .bertha-exercise-repeat-dialog .bertha-repeat-card{background:linear-gradient(145deg,#fbf8ff 0%,#f8f2ff 52%,#fff6ef 100%)!important;border:1px solid rgba(121,98,151,.12)!important}
    .bertha-exercise-repeat-dialog .eyebrow{color:#775f9d!important}.bertha-exercise-repeat-dialog .primary{background:linear-gradient(135deg,#b79bd0,#e7b7a3)!important;color:#5e4a6b!important}.bertha-exercise-repeat-dialog .secondary{background:rgba(255,255,255,.56)!important;color:#75697b!important}
    ...
    /* v2.8.177 — Menu final + backup visível */
    #berthaMore{width:min(92vw,430px)!important;max-height:calc(100dvh - 34px)!important}
    #berthaMore .bertha-more-modal{max-height:calc(100dvh - 56px)!important;overflow:auto!important;padding:20px 20px 18px!important;scrollbar-width:none}
    #berthaMore .bertha-more-modal::-webkit-scrollbar{display:none}
    #berthaMore .bertha-more-head{align-items:flex-start!important;margin-bottom:14px!important}
    #berthaMore .bertha-more-head>div{min-width:0}
    #berthaMore .bertha-more-head strong{font-size:19px!important;line-height:1.12!important;font-weight:520!important;letter-spacing:-.025em!important;color:#423b47!important}
    #berthaMore .bertha-more-head button{width:38px!important;height:38px!important;flex:0 0 38px!important;border-radius:50%!important;background:rgba(255,255,255,.58)!important;color:#8a8187!important;font-size:25px!important;font-weight:300!important}
    .bertha-more-eyebrow{font-size:10px!important;line-height:1!important;letter-spacing:.19em!important;font-weight:800!important;color:#c77a84!important;margin:2px 0 8px!important}
    #berthaMore .module-links.bertha-module-cards{gap:8px!important}
    #berthaMore .module-links.bertha-module-cards a{min-height:84px!important;padding:9px 5px 8px!important;border-radius:17px!important;box-shadow:0 5px 0 rgba(102,82,106,.04),0 8px 15px rgba(73,56,78,.04)!important}
    #berthaMore .bertha-menu-icon{width:34px!important;height:34px!important;flex-basis:34px!important}
    #berthaMore .bertha-menu-icon svg{width:26px!important;height:26px!important;stroke-width:1.55!important}
    .bertha-more-data{margin-top:18px;padding:15px 2px 2px;border-top:1px solid rgba(112,96,104,.10)}
    .bertha-more-data-copy{display:flex;align-items:baseline;justify-content:space-between;gap:12px;margin:0 2px 10px}
    .bertha-more-data-copy span{font-size:10px;letter-spacing:.17em;font-weight:800;color:#8d8189}
    .bertha-more-data-copy small{font-size:11px;color:#a0959b;font-weight:450}
    .bertha-more-data-actions{display:grid;grid-template-columns:1fr 1fr;gap:8px}
    .bertha-more-data-actions button{appearance:none;border:1px solid rgba(121,103,112,.10);border-radius:15px;padding:12px 9px;font:inherit;font-size:12px;font-weight:720;color:#625861;background:#fffaf3}
    .bertha-more-data-actions button:first-child{background:linear-gradient(135deg,rgba(252,224,210,.82),rgba(246,198,181,.78));color:#7b5147}
    @media(max-width:380px){#berthaMore .module-links.bertha-module-cards a{min-height:78px!important}.bertha-more-data-copy small{display:none}}
    ...
    .bertha-task-hero{
      ...
      border:1px solid rgba(177,132,88,.14);
      border-radius:28px;
      background:
        radial-gradient(circle at 12% 6%,rgba(255,248,213,.64),transparent 46%),
        radial-gradient(circle at 95% 92%,rgba(219,157,115,.28),transparent 46%),
        linear-gradient(135deg,rgba(255,252,234,.96),rgba(246,220,196,.62));
      box-shadow:0 12px 30px rgba(91,69,47,.045);
    }
    .bertha-task-kicker{
      color:#a97b59;
    ...
    .bertha-task-hero h1{
      ...
      font-size:26px!important;line-height:1.10!important;font-weight:510!important;
      letter-spacing:-.035em!important;color:#3f3545!important;
    }
    .bertha-task-hero p{
      margin:0;color:#7d746f;font-size:13px;line-height:1.46;
    }
    .bertha-task-tools{
    ...
    .bertha-task-add{
      ...
      background:linear-gradient(135deg,#f5e5b0,#dfad77 52%,#c98360);color:#fff;
      font:700 14px/1 Inter,sans-serif;box-shadow:0 8px 18px rgba(172,116,74,.14);
    }
    ...
    .bertha-task-tabs button.active{background:#f5e7bd;color:#9a6d49}
    .bertha-task-tabs em{
    ...
    .bertha-task-empty>div{display:none!important}
    ...
    .bertha-task-modal-head{
      ...
      padding:20px 20px 15px;border-bottom:1px solid rgba(151,112,80,.08);
      background:
        radial-gradient(circle at 18% 8%,rgba(255,247,176,.78),transparent 48%),
        linear-gradient(135deg,rgba(255,250,220,.98) 0%,rgba(249,230,189,.94) 52%,rgba(239,197,160,.86) 100%);
    }
    .bertha-task-modal-head span{
      display:block;margin-bottom:5px;color:#9b7458;font:700 11px/1 "Montserrat",Inter,sans-serif;
      letter-spacing:.16em;
    }
    ...
    .bertha-task-modal-head button{
      width:40px;height:40px;border:0;border-radius:50%;background:rgba(255,255,255,.74);
      color:#817770;font-size:27px;line-height:1;
    }
    ...
    .bertha-task-field input:focus,.bertha-task-field select:focus,.bertha-task-field textarea:focus{
      border-color:#d5a06f!important;box-shadow:0 0 0 3px rgba(213,160,111,.14)!important;
    }
    ...
    .bertha-segment button{
      ...
      background:#fbf7ef;color:#776f6b;font:700 11px/1 Inter,sans-serif;
    }
    .bertha-segment button.active{
      background:linear-gradient(135deg,#f7e59f 0%,#edc072 48%,#de9a67 100%);color:#8a5f40;border-color:rgba(176,122,76,.22);
    ...
    .bertha-notify-box{
      ...
      background:linear-gradient(135deg,rgba(255,248,216,.92) 0%,rgba(252,236,198,.92) 52%,rgba(246,224,196,.90) 100%);
    }
    ...
    .bertha-toggle-row strong{font-size:14px;color:#6f5542}
    .bertha-toggle-row span{font-size:11px;color:#907969}
    .bertha-toggle-row input{position:absolute;opacity:0;pointer-events:none}
    .bertha-toggle-row i{
      width:44px;height:26px;border-radius:999px;background:#e3d7c9;position:relative;transition:.18s ease;
    }
    ...
    .bertha-toggle-row input:checked+i{background:#c49a53}
    .bertha-toggle-row input:checked+i::after{transform:translateX(18px)}
    .bertha-notify-when{margin:12px 0 0!important}
    .bertha-notify-note{display:block;color:#9a846f!important;font-size:10.5px!important;line-height:1.35}
    ...
    .bertha-task-cancel{background:#f2eee7;color:#746a64}
    .bertha-task-save{background:linear-gradient(135deg,#f8e287 0%,#ebba67 45%,#dd9056 100%);color:white;min-width:92px}
    ...
    button.bertha-ritual-upcoming{width:100%;font:inherit;color:inherit;text-align:left;cursor:pointer}.bertha-full-selfcare-edit{width:100%;display:grid!important;grid-template-columns:70px minmax(0,1fr) 18px!important;align-items:center;text-align:left;border:0;background:transparent;font:inherit;color:inherit}.bertha-full-selfcare-edit>b{font-size:22px;font-weight:400;color:#9a8e99}
    ...
    .bertha-ritual-card.tone-0,.bertha-ritual-detail-hero.tone-0{background:linear-gradient(135deg,rgba(249,239,190,.70),rgba(229,218,248,.74),rgba(255,250,247,.88))}
    .bertha-ritual-card.tone-1,.bertha-ritual-detail-hero.tone-1{background:linear-gradient(135deg,rgba(218,241,230,.72),rgba(250,220,203,.70),rgba(255,250,247,.88))}
    .bertha-ritual-card.tone-2,.bertha-ritual-detail-hero.tone-2{background:linear-gradient(135deg,rgba(219,232,249,.72),rgba(248,218,232,.70),rgba(255,250,247,.88))}
    .bertha-back-link{border:0;background:transparent;padding:4px 0 8px;font-family:inherit;cursor:pointer}
    .bertha-extra-card [data-ritual-buy-name]{justify-self:end;border:0;border-radius:999px;background:rgba(235,222,244,.82);color:#725a82;padding:8px 10px;font:750 10px/1.1 Inter,-apple-system,BlinkMacSystemFont,"SF Pro Text",sans-serif;white-space:nowrap}

    .bertha-extra-card [data-ritual-buy-name]:disabled{opacity:.65}

    /* v2.8.157 — RITUAIS · revisão visual + Modal System BERTH.A v1
       Funcionalidade preservada. Paleta: blush + lilás sobre creme. */
    .bertha-ritual-page{
      color:#403743;
    }
    .bertha-ritual-hero{
      position:relative;overflow:hidden;
      border-color:rgba(177,135,165,.18)!important;
      background:
        radial-gradient(circle at 12% 8%,rgba(249,210,224,.58),transparent 49%),
        radial-gradient(circle at 92% 92%,rgba(220,207,244,.64),transparent 50%),
        linear-gradient(135deg,rgba(255,249,245,.98) 0%,rgba(249,229,238,.88) 48%,rgba(235,226,249,.90) 100%)!important;
      box-shadow:0 11px 30px rgba(104,78,109,.055)!important;
    }
    .bertha-ritual-hero:after{
      content:"";position:absolute;right:21px;top:20px;width:44px;height:44px;opacity:.64;
      background:
        linear-gradient(currentColor,currentColor) center/1.4px 29px no-repeat,
        linear-gradient(90deg,currentColor,currentColor) center/29px 1.4px no-repeat;
      color:#a18aa8;transform:rotate(45deg);border-radius:50%;pointer-events:none;
    }
    .bertha-ritual-hero h1{
      max-width:300px;
    }
    .bertha-ritual-hero p{
      max-width:300px;color:#827580!important;
    }

    .bertha-ritual-grid{gap:11px!important}
    .bertha-ritual-card{
      border-color:rgba(119,91,116,.085)!important;
      box-shadow:0 6px 18px rgba(91,67,96,.035)!important;
    }
    .bertha-ritual-card.tone-0{
      background:linear-gradient(135deg,rgba(255,249,245,.98),rgba(248,220,232,.48) 52%,rgba(232,223,247,.42) 100%)!important;
    }
    .bertha-ritual-card.tone-1{
      background:linear-gradient(135deg,rgba(255,249,245,.98),rgba(238,226,249,.50) 48%,rgba(249,223,233,.40) 100%)!important;
    }
    .bertha-ritual-card.tone-2{
      background:linear-gradient(135deg,rgba(255,249,245,.98),rgba(247,225,235,.44) 42%,rgba(231,222,247,.52) 100%)!important;
    }
    .bertha-new-ritual{
      background:linear-gradient(135deg,rgba(255,249,245,.98),rgba(246,228,237,.38),rgba(235,226,248,.42))!important;
      border-color:rgba(119,91,116,.085)!important;
    }
    .bertha-line-icon{color:#8d7894!important}

    .bertha-ritual-detail-hero{
      border-color:rgba(177,135,165,.17)!important;
      box-shadow:0 9px 25px rgba(91,67,96,.04)!important;
    }
    .bertha-ritual-detail-hero.tone-0,
    .bertha-ritual-detail-hero.tone-1,
    .bertha-ritual-detail-hero.tone-2{
      background:linear-gradient(135deg,rgba(255,249,245,.98),rgba(247,222,233,.50) 48%,rgba(232,223,248,.52) 100%)!important;
    }
    .bertha-ritual-today{
      background:linear-gradient(135deg,rgba(255,249,245,.97),rgba(248,225,235,.48) 48%,rgba(237,228,249,.54) 100%)!important;
      border-color:rgba(119,91,116,.085)!important;
    }
    .bertha-ritual-head>div>span{color:#956f87!important}
    .bertha-ritual-head button{
      background:linear-gradient(135deg,rgba(247,221,233,.92),rgba(231,220,248,.94))!important;
      color:#745d7a!important;
    }
    .bertha-check-steps input{accent-color:#a97998!important}
    .bertha-back-link{color:#806684!important}
    .bertha-ritual-upcoming>span{color:#956f87!important}
    .bertha-extra-card [data-ritual-buy-name]{
      background:linear-gradient(135deg,rgba(247,222,234,.92),rgba(232,222,247,.94))!important;
      color:#715b78!important;
    }

    /* Todos os modais de Rituais herdam o Modal System v1, mas mantêm identidade blush + lilás. */
    .bertha-ritual-dialog .bertha-task-modal,
    .bertha-dialog:has(.bertha-task-modal-head span:first-child) .bertha-task-modal{
      background:#fbf7f0!important;
    }
    .bertha-ritual-dialog .bertha-task-modal-head{
      background:
        radial-gradient(circle at 12% 5%,rgba(249,211,225,.52),transparent 48%),
        linear-gradient(135deg,rgba(255,249,245,.98),rgba(247,226,236,.88) 52%,rgba(235,225,249,.88))!important;
      border-bottom-color:rgba(126,94,120,.08)!important;
    }
    .bertha-ritual-dialog .bertha-task-modal-head span{color:#956f87!important}
    .bertha-ritual-dialog .bertha-task-modal-head button{
      background:rgba(255,255,255,.72)!important;color:#817880!important;
    }
    .bertha-ritual-dialog .bertha-task-field>span{color:#6f626f!important}
    .bertha-ritual-dialog .bertha-task-field input,
    .bertha-ritual-dialog .bertha-task-field select,
    .bertha-ritual-dialog .bertha-task-field textarea,
    .bertha-ritual-dialog .bertha-duration-input input,
    .bertha-ritual-dialog .bertha-duration-input select{
      font-size:16px!important;
      background:#fffdfa!important;
      border-color:rgba(126,94,120,.12)!important;
    }
    .bertha-ritual-dialog .bertha-task-field input:focus,
    .bertha-ritual-dialog .bertha-task-field select:focus,
    .bertha-ritual-dialog .bertha-task-field textarea:focus,
    .bertha-ritual-dialog .bertha-duration-input input:focus,
    .bertha-ritual-dialog .bertha-duration-input select:focus{
      border-color:#bc8eaa!important;
      box-shadow:0 0 0 3px rgba(188,142,170,.12)!important;
    }
    .bertha-ritual-dialog .bertha-segment button{
      background:#fbf7f4!important;color:#746a75!important;
      border-color:rgba(126,94,120,.10)!important;
    }
    .bertha-ritual-dialog .bertha-segment button.active{
      background:linear-gradient(135deg,#f5d7e4 0%,#e1d3f1 100%)!important;
      color:#765c79!important;border-color:rgba(160,111,143,.18)!important;
    }
    .bertha-ritual-dialog .bertha-notify-box{
      background:linear-gradient(135deg,rgba(251,232,240,.76),rgba(239,229,249,.78))!important;
      border-color:rgba(126,94,120,.09)!important;
    }
    .bertha-ritual-dialog .bertha-toggle-row strong{color:#67596a!important}
    .bertha-ritual-dialog .bertha-toggle-row span{color:#8b7e8c!important}
    .bertha-ritual-dialog .bertha-toggle-row i{background:#d9cfd9!important}
    .bertha-ritual-dialog .bertha-toggle-row input:checked+i{background:#a97798!important}
    .bertha-ritual-dialog .bertha-task-cancel{
      background:#f1ecef!important;color:#756b75!important;
    }
    .bertha-ritual-dialog .bertha-task-save{
      background:linear-gradient(135deg,#e8bfd2 0%,#cdaedc 54%,#a989bd 100%)!important;
      color:#fff!important;
    }
    body:has(.bertha-ritual-dialog[open]){overflow:hidden!important}
    .bertha-cycle-restart{margin:0!important;align-self:end;min-height:48px;display:flex;align-items:center}
    .bertha-cycle-restart .bertha-toggle-row{width:100%}

    /* v2.8.158 — ícones de Rituais mais específicos; hero preservado */
    .bertha-ritual-hero:after{content:none!important}
    .bertha-ritual-hero-mark{position:absolute;right:20px;top:19px;width:46px;height:46px;color:#a6a0a7;opacity:.62;pointer-events:none}
    .bertha-ritual-hero-mark svg{width:100%;height:100%;fill:none;stroke:currentColor;stroke-width:1.2;stroke-linecap:round;stroke-linejoin:round}
    .bertha-ritual-card .bertha-line-icon,.bertha-new-ritual .bertha-line-icon{color:#9b8e99!important;opacity:.92}
    .bertha-icon-picker button{color:#a06f8b!important;background:#fcf9f8!important;border-color:rgba(134,105,124,.11)!important}
    .bertha-icon-picker button.active{color:#9d6686!important;background:linear-gradient(135deg,rgba(248,220,233,.88),rgba(234,223,248,.82))!important;border-color:rgba(164,108,143,.22)!important}
    .bertha-icon-picker svg{width:25px!important;height:25px!important;stroke-width:1.45!important}
    /* v2.8.162 — modal de Rituais imóvel no iPhone:
       viewport estático e rolagem apenas no corpo interno */
    .bertha-ritual-dialog[open]{
    ...
    .bertha-ritual-dialog[open] .bertha-task-modal{
      position:relative!important;
      inset:auto!important;
      width:min(720px,calc(100vw - 24px))!important;
      max-width:calc(100vw - 24px)!important;
      height:min(760px,calc(100svh - 24px))!important;
    ...
    .bertha-extra-card:has([data-ritual-buy-name]){grid-template-columns:40px minmax(0,1fr);position:relative;padding-bottom:50px}
    .bertha-extra-card [data-ritual-buy-name]{position:absolute;right:12px;bottom:10px}
    .custom-how{background:linear-gradient(135deg,rgba(255,250,247,.88),rgba(239,232,249,.64))}
    ...
    .bertha-goals-block{margin-top:24px}.bertha-goals-head span{display:block;font-size:11px;letter-spacing:.18em;color:#8b7f8d;font-weight:800}.bertha-goals-head strong{display:block;font-size:18px;margin-top:5px}.bertha-goals-note{color:#817783;line-height:1.45;margin:8px 0 12px}.bertha-goals-grid{display:grid;grid-template-columns:1fr 1fr;gap:9px}.bertha-goal-card{border:1px solid rgba(82,69,85,.10);background:rgba(255,255,255,.72);border-radius:17px;padding:13px;text-align:left;color:#4f4352;min-height:72px}.bertha-goal-card span,.bertha-goal-card strong{display:block}.bertha-goal-card span{font-weight:800}.bertha-goal-card strong{font-size:12px;color:#806b82;margin-top:5px;font-weight:700;line-height:1.35}.bertha-goal-fields{display:grid;gap:12px}.bertha-goal-fields .bertha-field{display:grid;gap:7px}.bertha-goal-fields .bertha-field span{font-size:13px;font-weight:750;color:#4a4050}.bertha-goal-fields input{width:100%;box-sizing:border-box}
    ...
    /* v2.8.141 — Meu Dia Ideal: hero + cartões azul/blush + modal creme */
    .bertha-ideal-hero{
      position:relative;overflow:hidden;display:grid;grid-template-columns:minmax(0,1fr) 66px;gap:14px;align-items:start;
      padding:24px 22px 22px!important;border-radius:29px!important;
      border:1px solid rgba(152,165,189,.14)!important;
      background:
        linear-gradient(135deg, rgba(246,213,223,.80) 0%, rgba(252,246,239,.96) 27%, rgba(230,241,255,.96) 100%),
        linear-gradient(135deg, rgba(255,255,255,0) 38%, rgba(255,244,192,.12) 42%, rgba(255,245,192,.34) 45.5%, rgba(255,247,205,.60) 47.8%, rgba(255,255,255,.99) 49.4%, rgba(255,255,255,1) 50%, rgba(255,255,255,.99) 50.6%, rgba(255,247,205,.58) 52.2%, rgba(255,245,192,.32) 54.5%, rgba(255,244,192,.10) 58%, rgba(255,255,255,0) 62%),
        radial-gradient(ellipse 23% 140% at 50% 49%, rgba(255,255,255,1) 0%, rgba(255,252,234,.98) 13%, rgba(255,247,201,.82) 24%, rgba(255,242,186,.48) 34%, rgba(255,239,181,.20) 44%, rgba(255,239,181,0) 57%)!important;
      box-shadow:0 12px 34px rgba(87,87,111,.05)!important;
    }
    .bertha-ideal-hero:before{
      content:"";position:absolute;inset:auto auto -52px -36px;width:180px;height:180px;border-radius:50%;
      background:radial-gradient(circle, rgba(247,209,220,.18), rgba(247,209,220,0) 66%);pointer-events:none;
    }
    .bertha-ideal-hero:after{
      content:"";position:absolute;inset:-38px -36px auto auto;width:220px;height:220px;border-radius:50%;
      background:radial-gradient(circle, rgba(209,231,248,.22), rgba(209,231,248,0) 68%);pointer-events:none;
    }
    .bertha-ideal-hero .eyebrow{font-size:11px!important;letter-spacing:.19em!important;font-weight:800!important;color:#847b8f!important;text-transform:uppercase!important;margin:0 0 7px!important;font-family:inherit!important;line-height:1.1!important}
    .bertha-ideal-hero h2{margin:0 0 10px!important;font-size:28px!important;line-height:1.04!important;letter-spacing:-.034em!important;font-weight:500!important;color:#2f344d!important;max-width:292px}
    .bertha-ideal-hero p{margin:0!important;max-width:282px;font-size:13.5px!important;line-height:1.42!important;color:#6b6e7f!important;font-weight:400!important;opacity:.95}
    .bertha-ideal-hero-copy,.bertha-ideal-hero-mark{position:relative;z-index:1}
    .bertha-ideal-hero-mark{display:grid;place-items:start end;color:#c7c4cb;opacity:.94;padding-top:4px}
    .bertha-ideal-hero-mark svg{width:42px;height:42px;fill:none;stroke:currentColor;stroke-width:1.45;stroke-linecap:round;stroke-linejoin:round}
    .bertha-ideal-add-main,
    [data-route="ideal"] .primary.add-full,
    .bertha-ideal-page .primary.add-full{
      background:linear-gradient(135deg, rgba(223,160,183,.99) 0%, rgba(245,224,200,.98) 44%, rgba(210,227,244,.99) 100%)!important;
      color:#fff!important;border:1px solid rgba(193,149,171,.24)!important;box-shadow:0 10px 24px rgba(196,170,198,.10)!important
    }
    .bertha-ideal-zone{border:1px solid rgba(82,69,85,.08);border-radius:22px;padding:15px 14px 13px;position:relative;overflow:hidden}
    .bertha-ideal-zone:before{content:"";position:absolute;inset:auto auto -44px -22px;width:140px;height:140px;border-radius:50%;background:radial-gradient(circle, rgba(255,255,255,.36), rgba(255,255,255,0) 66%);pointer-events:none}
    .bertha-ideal-zone:after{content:"";position:absolute;right:-28px;top:-28px;width:140px;height:140px;border-radius:50%;background:radial-gradient(circle, rgba(255,248,220,.28), rgba(255,248,220,0) 65%);pointer-events:none}
    .bertha-ideal-zone.morning{background:linear-gradient(135deg,rgba(251,247,239,.97),rgba(248,237,240,.50) 40%,rgba(255,249,227,.44) 100%)}
    .bertha-ideal-zone.afternoon{background:linear-gradient(135deg,rgba(251,247,241,.97),rgba(248,236,240,.44) 36%,rgba(237,246,252,.28) 72%,rgba(255,249,227,.38) 100%)}
    .bertha-ideal-zone.evening{background:linear-gradient(135deg,rgba(244,249,253,.97),rgba(227,238,250,.50) 54%,rgba(251,249,240,.40) 100%)}
    .bertha-ideal-zone.flex{background:linear-gradient(135deg,rgba(251,247,240,.97),rgba(238,246,252,.36) 64%,rgba(255,248,223,.36) 100%)}
    .bertha-ideal-zone.free{background:linear-gradient(135deg,rgba(251,247,240,.97),rgba(236,245,252,.40) 60%,rgba(255,248,223,.34) 100%)}
    .bertha-ideal-zone-head,.bertha-ideal-zone-items{position:relative;z-index:1}
    .bertha-ideal-zone-icon{width:24px;height:24px;border-radius:0;background:transparent;display:grid;place-items:center;color:#7c879e;flex:0 0 auto}
    .bertha-ideal-zone-icon svg{width:18px;height:18px;fill:none;stroke:currentColor;stroke-width:1.7;stroke-linecap:round;stroke-linejoin:round}
    .bertha-ideal-zone-head span{font-size:10px;letter-spacing:.14em;font-weight:800;color:#6f6978}
    .bertha-ideal-zone-head small{display:block;margin-top:2px;font-size:10px;color:#8f8893;letter-spacing:0;font-weight:500}
    .bertha-ideal-map-card{display:grid;grid-template-columns:minmax(0,1fr) 34px;align-items:center;gap:10px;padding:12px 10px 12px 14px;border-radius:16px;background:rgba(252,250,245,.82);border:1px solid rgba(86,73,89,.05)}
    .bertha-ideal-remove{width:32px;height:32px;border:0;border-radius:50%;background:rgba(255,255,255,.76);display:grid;place-items:center;color:#a8a0aa;font-size:20px;line-height:1;padding:0;margin:0}
    .bertha-ideal-add-zone{width:100%;border:0;border-radius:13px;background:linear-gradient(135deg,rgba(253,249,244,.92),rgba(248,245,239,.86));padding:9px;margin-top:9px;color:#746f7e;font-size:10px;font-weight:750;box-shadow:inset 0 0 0 1px rgba(86,73,89,.045)}
    .bertha-goals-head span{color:#8f8390}
    .bertha-goals-note{color:#7f7480}
    .bertha-goal-card{border:1px solid rgba(82,69,85,.07);background:linear-gradient(135deg,rgba(252,249,244,.96),rgba(248,240,243,.10) 35%,rgba(236,245,252,.10) 70%,rgba(255,249,226,.08) 100%);border-radius:17px;padding:13px;text-align:left;color:#4f4352;min-height:72px}
    .bertha-goal-card strong{color:#7c7181}
    .bertha-goals-grid{gap:12px}
    .bertha-goal-card{display:grid;grid-template-columns:24px minmax(0,1fr);align-items:center;gap:11px;padding:14px 14px;min-height:82px;border-color:rgba(82,69,85,.08);background:linear-gradient(135deg,rgba(252,249,244,.98),rgba(249,244,246,.10) 35%,rgba(239,246,252,.10) 70%,rgba(255,250,232,.08) 100%);box-shadow:0 6px 18px rgba(63,53,76,.022)}
    .bertha-goal-card span,.bertha-goal-card strong{display:block}
    .bertha-goal-icon{width:24px;height:24px;display:grid;place-items:center;color:#726a83;opacity:.86}
    .bertha-goal-icon svg{width:22px;height:22px;fill:none;stroke:currentColor;stroke-width:1.55;stroke-linecap:round;stroke-linejoin:round}
    .bertha-goal-text{min-width:0;display:block}
    .bertha-goal-text > span{font-weight:745;color:#352f43;font-size:15px;line-height:1.18}
    .bertha-goal-text strong{font-size:12px;color:#807583;margin-top:6px;font-weight:680;line-height:1.3}
    .bertha-goal-card.goal-movimento{background:linear-gradient(135deg,rgba(252,248,244,.98),rgba(242,231,248,.22) 38%,rgba(251,238,229,.18) 100%)}
    .bertha-goal-card.goal-movimento .bertha-goal-icon{color:#7c748e}
    .bertha-goal-card.goal-alimentacao{background:linear-gradient(135deg,rgba(251,249,244,.98),rgba(232,243,252,.28) 36%,rgba(255,248,219,.26) 100%)}
    .bertha-goal-card.goal-alimentacao .bertha-goal-icon{color:#7a8498}
    .bertha-goal-card.goal-autocuidado{background:linear-gradient(135deg,rgba(252,249,245,.98),rgba(249,233,239,.26) 46%,rgba(255,248,233,.16) 100%)}
    .bertha-goal-card.goal-autocuidado .bertha-goal-icon{color:#876f86}
    .bertha-goal-card.goal-estudos{background:linear-gradient(135deg,rgba(251,249,245,.98),rgba(233,244,252,.28) 42%,rgba(250,239,232,.20) 100%)}
    .bertha-goal-card.goal-estudos .bertha-goal-icon{color:#6f7d95}
    .bertha-goal-card.goal-projetos{background:linear-gradient(135deg,rgba(251,249,245,.98),rgba(227,239,251,.22) 34%,rgba(241,235,249,.24) 100%)}
    .bertha-goal-card.goal-projetos .bertha-goal-icon{color:#7d748f}
    .bertha-goal-card.goal-trabalho{background:linear-gradient(135deg,rgba(251,249,245,.98),rgba(240,233,248,.24) 34%,rgba(229,243,240,.22) 100%)}
    .bertha-goal-card.goal-trabalho .bertha-goal-icon{color:#7a758d}
    .bertha-goal-card.goal-casa{background:linear-gradient(135deg,rgba(251,249,245,.98),rgba(249,233,239,.22) 30%,rgba(234,242,237,.24) 100%)}
    .bertha-goal-card.goal-casa .bertha-goal-icon{color:#7b758a}
    .bertha-goal-card.goal-rituais{background:linear-gradient(135deg,rgba(251,249,245,.98),rgba(251,245,225,.18) 28%,rgba(241,233,248,.24) 100%)}
    .bertha-goal-card.goal-rituais .bertha-goal-icon{color:#83718c}
    .bertha-task-modal,.bertha-dialog,.bertha-modal,[role="dialog"] .bertha-task-modal{
      background:#fbf7f0!important;border:1px solid rgba(140,126,145,.10)!important;box-shadow:0 20px 52px rgba(47,37,58,.14)!important
    }
    .bertha-task-modal-body,.bertha-dialog-body,.bertha-modal-body{background:#fbf7f0!important}
    .bertha-task-modal .bertha-field input,
    .bertha-task-modal .bertha-field select,
    .bertha-task-modal .bertha-field textarea,
    .bertha-task-modal-body input,
    .bertha-task-modal-body select,
    .bertha-task-modal-body textarea,
    .bertha-goal-fields input{background:#fffdfa!important;border-color:rgba(140,126,145,.14)!important;color:#3e3d4b!important}
    .bertha-task-modal .bertha-check input{accent-color:#7fa9cf!important}
    .bertha-task-modal .bertha-primary,
    .bertha-task-modal-body .bertha-primary,
    .bertha-dialog .bertha-primary{
      background:linear-gradient(135deg, rgba(223,160,183,.99) 0%, rgba(245,224,200,.98) 44%, rgba(210,227,244,.99) 100%)!important;
      color:#fff!important;border:1px solid rgba(193,149,171,.24)!important;box-shadow:none!important
    }
    ...
    .bertha-dialog:has(.bertha-ideal-modal-form) .bertha-modal-head{background:#fbf7ef!important;border-bottom:1px solid rgba(120,108,126,.08)!important;padding-bottom:14px!important;margin-bottom:14px!important}
    .bertha-dialog:has(.bertha-ideal-modal-form) .bertha-modal-head strong{font-size:20px!important;font-weight:620!important;color:#3b3544!important}
    .bertha-dialog:has(.bertha-ideal-modal-form) .bertha-modal-head button{background:transparent!important;border:0!important;color:#8c858e!important;box-shadow:none!important}
    .bertha-ideal-modal-form{display:grid;gap:14px}
    .bertha-ideal-modal-form .bertha-field,.bertha-ideal-until label,.bertha-notify-when{display:grid;gap:7px}
    .bertha-ideal-modal-form .bertha-field>span,.bertha-ideal-until span,.bertha-notify-when>span{font-size:13px;font-weight:720;color:#554d5b}
    .bertha-ideal-modal-form input,.bertha-ideal-modal-form select{font-size:16px!important;min-height:46px!important;background:#fffdfa!important;border:1px solid rgba(122,108,128,.13)!important;border-radius:14px!important;color:#403946!important;box-shadow:none!important}
    .bertha-notify-row{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:14px;align-items:center;padding:13px 14px;border:1px solid rgba(122,108,128,.09);border-radius:17px;background:rgba(255,253,249,.72)}
    .bertha-notify-row strong,.bertha-notify-row small{display:block}.bertha-notify-row strong{font-size:13px;color:#4a424f}.bertha-notify-row small{font-size:10px;line-height:1.35;color:#8d858e;margin-top:3px}
    .bertha-switch{position:relative;width:46px;height:28px;display:block!important}.bertha-switch input{position:absolute!important;opacity:0!important;pointer-events:none!important}.bertha-switch span{position:absolute;inset:0;border-radius:999px;background:#e6e1df;transition:.18s ease;box-shadow:inset 0 0 0 1px rgba(92,80,98,.08)}.bertha-switch span:after{content:"";position:absolute;width:22px;height:22px;left:3px;top:3px;border-radius:50%;background:#fff;box-shadow:0 2px 7px rgba(50,42,52,.16);transition:.18s ease}.bertha-switch input:checked+span{background:linear-gradient(135deg,#e2a9bc,#d6e6f5)}.bertha-switch input:checked+span:after{transform:translateX(18px)}
    .bertha-notify-custom{display:grid;grid-template-columns:74px minmax(0,1fr) auto;gap:8px;align-items:center}.bertha-notify-custom em{font-style:normal;font-size:12px;color:#7d7580}
    .bertha-ideal-modal-actions{display:flex;justify-content:flex-end;gap:9px;padding-top:4px}.bertha-ideal-modal-actions button{min-width:92px}
    [data-notify-when][hidden],[data-ideal-until-date-wrap][hidden]{display:none!important}
    .bertha-task-modal .x,
    .bertha-task-modal .bertha-close,
    .bertha-dialog .x,
    .bertha-dialog .bertha-close{color:#8e8792!important}
    @media(max-width:480px){
      .bertha-ideal-hero{grid-template-columns:minmax(0,1fr) 48px;padding:22px 18px 20px!important}
      .bertha-ideal-hero h2{font-size:26px!important;max-width:none;font-weight:510!important}
      .bertha-ideal-hero p{font-size:13px!important;max-width:none}
  ...
  const icon = (name) => {
    const p={
      ...
      award:'<path d="M8 4h8v3a4 4 0 0 1-8 0V4z"/><path d="M8 6H5v1a4 4 0 0 0 4 4M16 6h3v1a4 4 0 0 1-4 4M12 11v5M8 21h8M9 16h6v5H9z"/>',
  ...
  function nav(){
    ...
    n.innerHTML=`<a class="nav-item" data-route="meu-dia" href="#meu-dia" aria-label="Início"><span class="bertha-nav-mark">B•A</span><span>Início</span></a><a class="nav-item" data-route="planos" href="#planos" aria-label="Planos">${icon('tasks')}<span>Planos</span></a><a class="nav-item" data-route="rituais" href="#rituais">${icon('ritual')}<span>Rituais</span></a><button class="nav-item" type="button" data-bertha-more>${icon('more')}<span>Mais</span></button>`;
  ...
  function openMore(){
    ...
    const modules=[
      ['dia-ideal','heart','Meu Dia Ideal','Sua rotina de referência'],
      ['trabalho','briefcase','Trabalho','Frentes e tarefas'],
      ['estudos','study','Estudos','Mapas e sessões'],
      ['casa','home','Casa','Rotina e organização'],
      ['exercicios','move','Exercícios','Movimento e constância'],
      ...
      ['premiacoes','award','Premiações','Conquistas, Kids e reconhecimentos']
    ];
  ...
  function rerender(){
    ...
    if(route==='financeiro' && typeof originalRender==='function'){ originalRender(); nav(); return; }
    if(typeof originalRender==='function') originalRender(); nav();
  }
  window.__BERTHA_FINAL_RITUAL_ROUTER__ = true;
...
/* v2.8.185 — Meu Dia · lembretes financeiros compactos e ícone linear íntegro */
(function(){
  if(document.getElementById('bertha-fin-home-v185'))return;
  const st=document.createElement('style');st.id='bertha-fin-home-v185';st.textContent=`
    .home-fin-reminders{margin:12px 0!important}.home-fin-reminders .section-head{margin-bottom:7px!important}.home-fin-reminder-row{min-height:0!important;padding:9px 11px!important;grid-template-columns:24px minmax(0,1fr) 16px!important;border-radius:16px!important;background:rgba(255,253,249,.78)!important;border:1px solid rgba(104,91,125,.09)!important;box-shadow:0 4px 14px rgba(73,56,78,.025)!important}.home-fin-reminder-row+.home-fin-reminder-row{margin-top:6px!important}.home-fin-reminder-row strong{font-size:13px!important}.home-fin-reminder-row small{font-size:10.5px!important;line-height:1.25!important;margin-top:1px!important}.home-fin-reminder-icon{width:20px!important;height:20px!important;display:grid!important;place-items:center!important;color:#75669a!important;background:transparent!important;border:0!important}.home-fin-reminder-icon:before,.home-fin-reminder-icon:after{content:none!important;display:none!important}.home-fin-reminder-icon svg{width:19px!important;height:19px!important;fill:none!important;stroke:currentColor!important;stroke-width:1.45!important;stroke-linecap:round!important;stroke-linejoin:round!important}.home-fin-reminder-icon.is-late{color:#956875!important}
  `;document.head.appendChild(st);
})();


/* v2.8.203 — Rituais · estabilidade de scroll/layout no iPhone
   Escopo cirúrgico: sem alterar estética, conteúdo ou lógica do módulo. */
(function(){
  if(document.getElementById('ritual-scroll-stability-v203')) return;
  const st=document.createElement('style');
  st.id='ritual-scroll-stability-v203';
  st.textContent=`
    body:has(#app .bertha-ritual-page){
      overflow-x:hidden!important;
      min-height:100svh!important;
      overscroll-behavior-y:none;
      -webkit-text-size-adjust:100%;
    }
    #app:has(.bertha-ritual-page){
      min-height:100svh!important;
      overflow:visible!important;
      overflow-anchor:none!important;
    }
    #app .bertha-ritual-page{
      box-sizing:border-box!important;
      width:100%!important;
      min-height:100svh!important;
    ...
    #app .bertha-ritual-page *{box-sizing:border-box}
    body:has(#app .bertha-ritual-page) .bottom-nav{
      position:fixed!important;
      left:0!important;
      right:0!important;
      bottom:0!important;
...
/* v2.8.205 — Ritual Capilar: viewport estável + cards futuros editáveis.
   Usa svh para não recalcular a altura quando a barra do Safari aparece/some. */
(function(){
  if(document.getElementById('capillary-stability-v205')) return;
  const st=document.createElement('style'); st.id='capillary-stability-v205';
  st.textContent=`
    body:has(#app .bertha-ritual-page){min-height:100svh!important}
    #app:has(.bertha-ritual-page),#app .bertha-ritual-page{min-height:100svh!important}
    #app .bertha-ritual-page{transform:none!important;-webkit-transform:none!important;contain:layout style;}
    #app .bertha-upcoming-list{align-content:start!important}
    #app button.bertha-ritual-upcoming{appearance:none;-webkit-appearance:none;width:100%;margin:0;text-align:left;color:inherit;font:inherit;cursor:pointer;grid-template-columns:58px minmax(0,1fr) 18px}
    #app button.bertha-ritual-upcoming>b{font-size:22px;font-weight:400;color:#9a8f9b;justify-self:end}
  `;
  document.head.appendChild(st);
})();


/* v2.8.209 — Modal de conclusão contextual
   A paleta pertence ao CONTEXTO onde a atividade foi concluída, não ao tipo da atividade.
   Meu Dia -> pêssego/coral. Trabalho -> lilás + areia. Estudos -> azul + pêssego. */
(function(){
  if(document.getElementById('bertha-repeat-context-v209')) return;
  const st=document.createElement('style');
  st.id='bertha-repeat-context-v209';
  st.textContent=`
    .bertha-repeat-dialog .bertha-repeat-card{
      background:#fbf7ef!important;
      border-color:rgba(116,103,121,.10)!important;
    }
    .bertha-repeat-dialog .bertha-repeat-x{
      background:transparent!important;
      color:#8c858e!important;
      border:0!important;
      box-shadow:none!important;
    }

    /* Meu Dia: mesma família dos CTAs Começar agora / Começar */
    .bertha-meudia-repeat-dialog .bertha-repeat-actions .primary{
      background:linear-gradient(135deg,#f6aa84 0%,#ef9577 55%,#e4866b 100%)!important;
      color:#fff!important;
      border:0!important;
      box-shadow:0 6px 16px rgba(224,136,104,.14)!important;
    }
    .bertha-meudia-repeat-dialog .bertha-repeat-actions .secondary{
      background:rgba(255,249,242,.96)!important;
      color:#6a5a70!important;
      border:1px solid rgba(108,88,113,.075)!important;
      box-shadow:none!important;
    ...
    /* Trabalho: mesma família dos botões do módulo Trabalho */
    .bertha-work-repeat-dialog .bertha-repeat-actions .primary{
      background:linear-gradient(120deg,#e8def8 0%,#f4e8cf 100%)!important;
      color:#5e5378!important;
      border:1px solid rgba(126,117,140,.08)!important;
      box-shadow:none!important;
    }
    .bertha-work-repeat-dialog .bertha-repeat-actions .secondary{
      background:#ede8f7!important;
      color:#6f6590!important;
      border:1px solid rgba(126,117,140,.08)!important;
      box-shadow:none!important;
    }

    /* Estudos: azul + pêssego, somente quando concluído DENTRO de Estudos */
    .bertha-study-repeat-dialog .bertha-repeat-actions .primary{
      background:linear-gradient(120deg,#cfe6f5 0%,#f8cfba 100%)!important;
      color:#5f5965!important;
      border:1px solid rgba(91,125,158,.10)!important;
      box-shadow:none!important;
    }
    .bertha-study-repeat-dialog .bertha-repeat-actions .secondary{
      background:linear-gradient(120deg,rgba(222,238,248,.92),rgba(252,230,218,.92))!important;
      color:#5f5965!important;
      border:1px solid rgba(91,125,158,.08)!important;
      box-shadow:none!important;
    }
  `;
  document.head.appendChild(st);
...
/* BERTH.A v2.8.215 — Menu Mais: creme + ícones com degradê integral.
   Alteração estética isolada. Rotas, dados, backup/restauração e homologação preservados. */
(function installMoreV215Styles(){
  if(document.getElementById('bertha-more-v215-style'))return;
  const st=document.createElement('style');st.id='bertha-more-v215-style';st.textContent=`
    #berthaMore{width:min(94vw,440px)!important;max-height:calc(100dvh - 22px)!important}
    #berthaMore::backdrop{background:rgba(56,48,59,.28)!important;backdrop-filter:blur(7px)!important;-webkit-backdrop-filter:blur(7px)!important}
    #berthaMore .bertha-more-v215{
      box-sizing:border-box!important;
      padding:22px 18px 18px!important;
      border-radius:30px!important;
      background:#fbf6ee!important;
      border:1px solid rgba(112,92,101,.10)!important;
      box-shadow:0 26px 72px rgba(60,48,64,.16)!important;
      overflow:auto!important;
      max-height:calc(100dvh - 42px)!important;
    }
    #berthaMore .bertha-more-head{display:flex!important;align-items:flex-start!important;justify-content:space-between!important;gap:14px!important;margin-bottom:18px!important}
    #berthaMore .bertha-more-brand{min-width:0!important;padding:1px 0 0 3px!important}
    #berthaMore .bertha-more-title{
      margin:0 0 7px!important;
      font-family:Didot,"Bodoni 72","Bodoni 72 Smallcaps",BodoniMT,Georgia,"Times New Roman",serif!important;
      font-size:48px!important;line-height:.94!important;font-weight:400!important;letter-spacing:-.045em!important;color:#383444!important;
    }
    #berthaMore .bertha-more-tagline{
      font-family:Montserrat,"Avenir Next",Inter,-apple-system,BlinkMacSystemFont,"SF Pro Text",sans-serif!important;
      font-size:9.5px!important;line-height:1.25!important;font-weight:560!important;letter-spacing:.22em!important;color:#5f5968!important;text-transform:uppercase!important;
      white-space:nowrap!important;
    }
    #berthaMore .bertha-more-promise{
      margin-top:7px!important;
      font-family:Montserrat,"Avenir Next",Inter,-apple-system,BlinkMacSystemFont,"SF Pro Text",sans-serif!important;
      font-size:12.5px!important;line-height:1.35!important;font-weight:350!important;letter-spacing:-.005em!important;color:#948b91!important;
    }
    #berthaMore .bertha-more-head button{
      width:38px!important;height:38px!important;flex:0 0 38px!important;margin-top:0!important;
      border-radius:50%!important;background:rgba(255,255,255,.54)!important;border:1px solid rgba(105,91,98,.10)!important;
      color:#837b81!important;font-size:25px!important;font-weight:300!important;box-shadow:none!important;outline:none!important;
    }
    #berthaMore .bertha-module-cards-v215{
      display:grid!important;grid-template-columns:repeat(3,minmax(0,1fr))!important;gap:9px!important;
    }
    #berthaMore .bertha-module-cards-v215 a{
      min-height:101px!important;padding:12px 7px 11px!important;box-sizing:border-box!important;
      ...
      border-radius:20px!important;background:rgba(255,252,247,.83)!important;color:#494451!important;
      border:1px solid rgba(111,93,101,.095)!important;
      box-shadow:0 8px 22px rgba(71,57,73,.045),inset 0 1px 0 rgba(255,255,255,.86)!important;
      text-align:center!important;text-decoration:none!important;
      font-family:Montserrat,Inter,-apple-system,BlinkMacSystemFont,"SF Pro Text",sans-serif!important;font-size:11.5px!important;line-height:1.18!important;font-weight:620!important;
    }
    #berthaMore .bertha-module-cards-v215 a:active{transform:scale(.975)!important;box-shadow:0 4px 12px rgba(71,57,73,.04)!important}
    #berthaMore .bertha-menu-icon-v215{
      width:48px!important;height:48px!important;flex:0 0 48px!important;border-radius:16px!important;display:grid!important;place-items:center!important;
      color:#fff!important;
      background:
        radial-gradient(circle at 16% 12%,rgba(255,248,207,.94) 0 8%,transparent 31%),
        radial-gradient(circle at 20% 78%,rgba(255,171,168,.92) 0 9%,transparent 38%),
        radial-gradient(circle at 53% 26%,rgba(239,156,222,.90) 0 8%,transparent 38%),
        radial-gradient(circle at 84% 18%,rgba(161,160,247,.91) 0 11%,transparent 41%),
        radial-gradient(circle at 90% 75%,rgba(143,229,205,.91) 0 13%,transparent 43%),
        linear-gradient(135deg,#ffdca8 0%,#f5b6c8 28%,#d9aff1 50%,#9dc9f6 73%,#aeeacb 100%)!important;
      box-shadow:inset 0 1px 0 rgba(255,255,255,.70),0 5px 14px rgba(111,85,126,.10)!important;
    }
    #berthaMore .bertha-menu-icon-v215 svg{width:27px!important;height:27px!important;stroke:currentColor!important;stroke-width:1.65!important;filter:drop-shadow(0 1px 1px rgba(75,59,82,.09))}
    #berthaMore .bertha-module-cards-v215 a>span:last-child{color:#4a4551!important;font-weight:620!important}
    #berthaMore .bertha-more-data-v215{margin-top:18px!important;padding:15px 1px 1px!important;border-top:1px solid rgba(112,96,104,.095)!important}
    #berthaMore .bertha-more-data-copy{display:block!important;margin:0 3px 10px!important}
    #berthaMore .bertha-more-data-copy span{display:block!important;color:#857981!important;font-size:9.5px!important;letter-spacing:.17em!important;font-weight:800!important}
    #berthaMore .bertha-more-data-copy small{display:block!important;margin-top:5px!important;color:#a0969b!important;font-size:10.5px!important;line-height:1.35!important;font-weight:420!important}
    #berthaMore .bertha-more-data-actions{display:grid!important;grid-template-columns:1fr 1fr!important;gap:8px!important}
    #berthaMore .bertha-more-data-actions button{
      min-height:42px!important;border-radius:14px!important;border:1px solid rgba(121,103,112,.10)!important;
      padding:10px 9px!important;background:rgba(255,252,247,.92)!important;color:#655b64!important;box-shadow:none!important;
      font-family:Montserrat,Inter,-apple-system,sans-serif!important;font-size:11px!important;font-weight:660!important;
    }
    #berthaMore .bertha-more-data-actions button:first-child{
      background:linear-gradient(135deg,rgba(255,223,195,.80),rgba(247,200,205,.72),rgba(221,203,240,.68))!important;color:#715963!important;
    }
    #berthaMore .bertha-more-data-actions [data-hml-reset]{grid-column:1/-1!important;background:#fffaf3!important;color:#8b7379!important;border:1px dashed rgba(126,104,116,.18)!important}
    @media(max-width:380px){
      #berthaMore .bertha-more-title{font-size:43px!important}
      #berthaMore .bertha-more-tagline{font-size:8.4px!important;letter-spacing:.17em!important}
      #berthaMore .bertha-more-promise{font-size:11.5px!important}
      #berthaMore .bertha-module-cards-v215{gap:7px!important}
      #berthaMore .bertha-module-cards-v215 a{min-height:92px!important;padding:9px 4px!important;font-size:10.5px!important}
      #berthaMore .bertha-menu-icon-v215{width:43px!important;height:43px!important;flex-basis:43px!important;border-radius:14px!important}
      #berthaMore .bertha-menu-icon-v215 svg{width:24px!important;height:24px!important}
    }
  `;document.head.appendChild(st);
})();


/* BERTH.A v2.8.216 — refinamento Menu Mais: tipografia oficial + microtextos. */
(function installMoreV216Styles(){
  if(document.getElementById('bertha-more-v216-style'))return;
  const st=document.createElement('style');st.id='bertha-more-v216-style';st.textContent=`
    #berthaMore .bertha-more-v216 .bertha-more-title{
      font-family:"Montserrat","Avenir Next",Inter,-apple-system,BlinkMacSystemFont,"SF Pro Display",sans-serif!important;
      font-size:31px!important;line-height:1.08!important;font-weight:500!important;letter-spacing:-.035em!important;color:#3f3545!important;
      margin:0 0 8px!important;
    }
    #berthaMore .bertha-module-cards-v216{
      display:grid!important;grid-template-columns:repeat(3,minmax(0,1fr))!important;gap:9px!important;
    }
    #berthaMore .bertha-module-cards-v216 a{
      min-height:128px!important;padding:11px 6px 10px!important;box-sizing:border-box!important;
      ...
      box-shadow:0 8px 22px rgba(71,57,73,.045),inset 0 1px 0 rgba(255,255,255,.86)!important;
      text-align:center!important;text-decoration:none!important;
      font-family:Montserrat,Inter,-apple-system,BlinkMacSystemFont,"SF Pro Text",sans-serif!important;
    }
    #berthaMore .bertha-menu-icon-v216{
      width:48px!important;height:48px!important;flex:0 0 48px!important;border-radius:16px!important;display:grid!important;place-items:center!important;
      color:#fff!important;
      background:
        radial-gradient(circle at 16% 12%,rgba(255,248,207,.94) 0 8%,transparent 31%),
    ...
    #berthaMore .bertha-menu-icon-v216 svg{width:27px!important;height:27px!important;stroke:currentColor!important;stroke-width:1.65!important;filter:drop-shadow(0 1px 1px rgba(75,59,82,.09))}
    #berthaMore .bertha-menu-copy{display:flex!important;flex-direction:column!important;align-items:center!important;gap:3px!important;min-width:0!important;width:100%!important}
    #berthaMore .bertha-menu-copy strong{display:block!important;color:#494451!important;font-size:11.2px!important;line-height:1.15!important;font-weight:650!important;letter-spacing:-.015em!important}
    #berthaMore .bertha-menu-copy small{display:block!important;color:#958d92!important;font-size:8.9px!important;line-height:1.23!important;font-weight:420!important;letter-spacing:-.005em!important;max-width:92%!important}
    #berthaMore .bertha-more-data-v216{margin-top:18px!important;padding:15px 1px 1px!important;border-top:1px solid rgba(112,96,104,.095)!important}
    @media(max-width:380px){
      #berthaMore .bertha-more-v216 .bertha-more-title{font-size:29px!important}
      #berthaMore .bertha-module-cards-v216{gap:7px!important}
      #berthaMore .bertha-module-cards-v216 a{min-height:120px!important;padding:9px 4px!important}
      #berthaMore .bertha-menu-icon-v216{width:43px!important;height:43px!important;flex-basis:43px!important;border-radius:14px!important}
      #berthaMore .bertha-menu-icon-v216 svg{width:24px!important;height:24px!important}
      #berthaMore .bertha-menu-copy strong{font-size:10.4px!important}
      #berthaMore .bertha-menu-copy small{font-size:8.2px!important}
    }
  `;document.head.appendChild(st);
})();


/* BERTH.A RC35 — Rituais: Modal System nos dialogs reais do módulo. */
(function installRitualModalRC35(){
  if(document.getElementById('bertha-ritual-modal-rc35'))return;
  const st=document.createElement('style');st.id='bertha-ritual-modal-rc35';st.textContent=`
    .bertha-ritual-dialog .bertha-task-modal{background:radial-gradient(circle at 10% 4%,rgba(248,216,230,.42),transparent 40%),radial-gradient(circle at 94% 96%,rgba(226,216,246,.46),transparent 42%),linear-gradient(145deg,#fffaf6 0%,#fbf4f5 52%,#f5f0fa 100%)!important;border:1px solid rgba(132,103,128,.10)!important;box-shadow:0 22px 64px rgba(78,58,84,.16)!important}
    .bertha-ritual-dialog .bertha-task-modal-head{background:transparent!important;border-bottom:1px solid rgba(126,94,120,.075)!important}
    .bertha-ritual-dialog .bertha-task-modal-head button[data-close]{border:0!important;outline:0!important;box-shadow:none!important;background:rgba(255,255,255,.52)!important;color:#817880!important}
    .bertha-ritual-dialog .bertha-task-modal-body{background:transparent!important}
    .bertha-ritual-dialog .bertha-task-modal-actions{background:linear-gradient(180deg,rgba(251,247,246,.72),rgba(249,243,248,.96))!important;border-top:1px solid rgba(126,94,120,.07)!important}
    .bertha-ritual-dialog .bertha-task-save{background:linear-gradient(135deg,#e6bfd2 0%,#d5bee8 52%,#bca8da 100%)!important;color:#fff!important;border:0!important;box-shadow:none!important}
...
;(()=>{try{const id='bertha-rc80-exercise-final-hotfix';document.getElementById(id)?.remove();const st=document.createElement('style');st.id=id;st.textContent=`
html body dialog.exercise-dialog[open]{border:0!important;outline:0!important;background:transparent!important;box-shadow:none!important}
html body dialog.exercise-dialog[open]>.bertha-modal,html body dialog.exercise-dialog[open]>.study-v10-modal{border:0!important;outline:0!important;box-shadow:inset 0 0 0 1px rgba(145,120,158,.10),0 22px 54px rgba(55,43,67,.13)!important}
html body dialog.exercise-dialog .study-v10-actions,html body dialog.exercise-dialog .modal-actions{position:static!important;top:auto!important;right:auto!important;bottom:auto!important;left:auto!important;inset:auto!important;transform:none!important;z-index:auto!important;margin:22px 0 0!important;padding:0!important;background:transparent!important;border:0!important;box-shadow:none!important;backdrop-filter:none!important;-webkit-backdrop-filter:none!important}
html body dialog.exercise-dialog.exercise-plan-dialog .exercise-plan-icon-section,html body dialog.exercise-dialog.exercise-plan-dialog .exercise-icon-picker{overflow:visible!important;max-height:none!important;height:auto!important}
html body dialog.exercise-dialog input[type=checkbox]{-webkit-appearance:none!important;appearance:none!important;width:20px!important;height:20px!important;min-width:20px!important;min-height:20px!important;margin:0!important;border:1px solid rgba(135,111,151,.18)!important;border-radius:6px!important;background:rgba(255,255,255,.72)!important;box-shadow:none!important;display:inline-grid!important;place-content:center!important}
html body dialog.exercise-dialog input[type=checkbox]:checked{background:linear-gradient(135deg,#d9c4ea 0%,#efc9cb 100%)!important;border-color:rgba(135,111,151,.20)!important}
html body dialog.exercise-dialog input[type=checkbox]:checked::after{content:'✓';font-size:13px;line-height:1;color:#fff;font-weight:500}
html body dialog.exercise-dialog :is(.study-v10-field>span,.exercise-cycle-label,label,strong,b){font-weight:500!important}
...
;(function(){if(document.getElementById('rc119-postpone-modal'))return;const st=document.createElement('style');st.id='rc119-postpone-modal';st.textContent=`
/* RC119 — Meu Dia: modal Adiar alinhado ao Modal System BERTH.A */
html body dialog.bertha-postpone-dialog[open]>.bertha-modal{background:linear-gradient(145deg,#fffaf4 0%,#fbf7f0 64%,#f3f8f4 100%)!important;border-color:rgba(148,123,128,.10)!important;padding:20px!important}
html body dialog.bertha-postpone-dialog[open] .bertha-modal-head strong{font-size:18px!important;line-height:1.22!important;font-weight:500!important;color:#3f3942!important}
html body dialog.bertha-postpone-dialog[open] .bertha-muted{font-size:14px!important;font-weight:400!important;color:#77707a!important;margin:10px 0 13px!important}
html body dialog.bertha-postpone-dialog[open] .bertha-choice-grid{display:grid!important;grid-template-columns:repeat(2,minmax(0,1fr))!important;gap:9px!important}
html body dialog.bertha-postpone-dialog[open] .bertha-choice-grid button{min-height:44px!important;border-radius:15px!important;padding:8px 10px!important;font-size:14px!important;font-weight:500!important;background:linear-gradient(135deg,rgba(242,229,235,.92),rgba(232,239,233,.94))!important;color:#655c66!important;border:1px solid rgba(120,103,125,.07)!important}
...
;(function(){
  if(document.getElementById('rc131-sat-final-visual-last')) return;
  const st=document.createElement('style');
  st.id='rc131-sat-final-visual-last';
  st.textContent=`

...
window.BERTHA_BUILD="2.8.186-financeiro-final-hml";
...
(() => {
  ...
  toggleFinPrivacyNative=function(){
    ...
    else if(location.hash==='#progresso') renderProgressOverview();
  };
  ...
  function v186PeriodRows(periods){
    return (periods.length?periods:[{id:v186Uid(),label:'',min:0,max:0,contributions:[]}]).map(p=>`<div class="fin-v186-period-edit" data-period-id="${escapeHtml(p.id||v186Uid())}"><input class="v186-period-label" maxlength="40" value="${escapeHtml(p.label||'')}" placeholder="Ex.: Outubro"><input class="v186-period-min" type="number" min="0" step="0.01" value="${Number(p.min||0)||''}" placeholder="mínimo"><input class="v186-period-max" type="number" min="0" step="0.01" value="${Number(p.max||0)||''}" placeholder="máximo"><button type="button" class="v186-remove-period" aria-label="Remover período">×</button></div>`).join('');
  }

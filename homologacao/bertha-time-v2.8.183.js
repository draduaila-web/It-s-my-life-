  ...
  function renderFinanceRemindersHome(){
    try{
      ...
      const html=rows.slice(0,4).map(o=>{
        const [yy,mm,dd]=o.due.split('-');
        return `<a class="home-fin-reminder-row" href="#financeiro"><span class="home-fin-reminder-icon ${o.days<0?'is-late':'is-clock'}" aria-hidden="true"></span><span><strong>${esc(o.name)}</strong><small>${esc(o.label)} · ${dd}/${mm}</small></span><b>›</b></a>`;
      }).join('');

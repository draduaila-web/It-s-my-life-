...
(() => {
  ...
  function sourcesToday(){
    ...
    if(ex && Array.isArray(ex.plans)) ex.plans.filter(p=>p.active!==false && (!(p.untilMode==='date'&&p.untilDate&&p.untilDate<today)) && (p.frequency!=='Dias específicos'||!p.days?.length||p.days.includes(dow))).forEach(p=>{ const dv=Math.max(1,+p.durationValue||parseMinutes(p.target)), mins=p.durationUnit==='hours'?dv*60:dv; const periods={morning:['05:16','07:35'],afternoon:['12:00','18:00'],evening:['18:00','22:00'],flex:['','']}; const w=periods[p.period||'flex']||periods.flex; push({id:`exercise:${p.id}`,learningKey:`exercise-plan-v2:${p.id}`,source:'Movimento',title:p.name||'Movimento',date:today,minutes:mins,configuredMinutes:mins,time:p.time||'',period:p.period||'flex',windowStart:p.time?'':w[0],windowEnd:p.time?'':w[1],notify:!!p.notify,notifyOffset:p.notifyWhen||'',kind:'window'}); });

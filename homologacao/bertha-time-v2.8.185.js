...
(() => {
  ...
  function renderFinanceRemindersHome(){
    try{
      ...
      bills.forEach(b=>{
        if(!b||b.active===false||isBec(b))return;
        ...
        if(rec==='monthly'){
          ...
          if(!(b.dueDate||'').startsWith(month))return; due=b.dueDate;
        }
        const paid=!!(b.payments||{})[month]?.paid; if(paid)return;
        const [dy,dm,dd]=due.split('-').map(Number), dueDate=new Date(dy,dm-1,dd);
        const days=Math.round((dueDate-today)/86400000), lead=Number(b.reminderDays??3);
        if(days>lead)return;
        const label=days<0?`Atrasada · ${Math.abs(days)}d`:days===0?'Vence hoje':days===1?'Vence amanhã':`Vence em ${days}d`;
        rows.push({name:b.name||'Conta prevista',due,days,label});
      });
      if(!rows.length)return '';
      rows.sort((a,b)=>a.due.localeCompare(b.due));
      const html=rows.slice(0,4).map(o=>{
        const [yy,mm,dd]=o.due.split('-'),bill=bills.find(x=>(x.name||'Conta prevista')===o.name)||{};
        let detail=`${o.label} · ${dd}/${mm}`;
        if(o.days===0){const first=bill.reminderTime||'09:00',again=bill.repeatReminderTime||'21:30',repeat=bill.repeatIfPending!==false,hhmm=`${String(now.getHours()).padStart(2,'0')}:${String(now.getMinutes()).padStart(2,'0')}`;if(hhmm<first)detail=`Vence hoje · aviso ${first}`;else if(repeat&&hhmm<again)detail=`Vence hoje · próximo aviso ${again}`;}
        return `<a class="home-fin-reminder-row" href="#financeiro"><span class="home-fin-reminder-icon ${o.days<0?'is-late':'is-clock'}" aria-hidden="true"></span><span><strong>${esc(o.name)}</strong><small>${esc(detail)}</small></span><b>›</b></a>`;
      }).join('');

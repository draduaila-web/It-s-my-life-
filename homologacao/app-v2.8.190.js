...
window.BERTHA_BUILD="2.8.190-financeiro-route-goals-privacy-hml";
...
(() => {
  ...
  toggleFinPrivacyNative = function(){
    window.berthaHmlStorage.setItem(FIN_PRIVACY_KEY, finPrivacyHidden()?'visible':'hidden');
    const route=(location.hash||'').replace('#','');
    if(route==='financeiro') renderFinanceiro();
    else if(route==='progresso') renderProgressOverview();
  };
  ...
  document.addEventListener('click',e=>{
    ...
    if(add){ e.preventDefault(); e.stopImmediatePropagation(); openGoalModal189(); return; }
    const privacy=e.target.closest&&e.target.closest('#toggleFinPrivacy');
    if(privacy){ e.preventDefault(); e.stopImmediatePropagation(); toggleFinPrivacyNative(); return; }
  },true);

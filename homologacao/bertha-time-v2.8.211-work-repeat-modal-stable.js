...
(() => {
  ...
  function enqueueRepeat(item={}){
    ...
    const fresh={...item,repeatBaseId:base,
      ...
      repeatedFromTime:item.time||'',time:'',
  ...
  function sourcesToday(){
    ...
    [['minha-vida.trabalho.bec.v1','BEC','bec'],['minha-vida.trabalho.tiktok.v1','TikTok','tiktok']].forEach(([key,label,workKind])=>{
      read(key,[]).forEach(x=>{
        ...
        push({id:`${key}:${x.id}`,learningKey:`${key}:${x.id}`,workKind,workTaskId:x.id,source:`Trabalho · ${label}`,title:x.title||'Tarefa',date:x.date||today,minutes:+x.minutes||parseMinutes(x.duration),configuredMinutes:+x.minutes||parseMinutes(x.duration),time:x.time||'',period:({manha:'morning',tarde:'afternoon',noite:'night',flexivel:'flex'})[String(x.period||'flex').toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'')]||'flex',priority:x.priority||'Normal',kind:'work'});
      });
    });
    read('minha-vida.trabalho.crefito.v1',[]).forEach(x=>{
      if(x.status==='Concluído'||x.status==='Aguardando'||x.status==='Pausado'||(x.date&&x.date>today)) return;
      push({id:`crefito:${x.id}`,learningKey:`crefito:${x.id}`,workKind:'crefito',workTaskId:x.id,source:'Trabalho · CREFITO-11',title:x.title||'Trabalho',date:x.date||today,minutes:+x.minutes||parseMinutes(x.duration),configuredMinutes:+x.minutes||parseMinutes(x.duration),time:x.time||'',period:({manha:'morning',tarde:'afternoon',noite:'night',flexivel:'flex'})[String(x.period||'flex').toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'')]||'flex',priority:x.priority||'Normal',kind:'work'});
    });

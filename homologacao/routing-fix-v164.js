...
/* ============================================================
   BERTH.A RC164 — rota única + hero unificado
   ============================================================ */
(function(){
  ...
  function ensureStyle(){
    ...
    st.textContent=`
      ...
      /* CTA oficial — referência visual: Meu Dia Ideal */
      body[data-bertha-route="satelites"] .s156-main,
      body[data-bertha-route="convites"] .sat-primary,
      body[data-bertha-route="convites"] .primary,
      body[data-bertha-route="premiacoes"] .primary,
      body[data-bertha-route="premiacoes"] .r147-action.primary{
        background:linear-gradient(90deg,
          rgba(232,150,182,.92) 0%,
          rgba(244,190,182,.86) 30%,
          rgba(247,221,183,.84) 52%,
          rgba(210,222,238,.90) 78%,
          rgba(178,211,226,.92) 100%)!important;
        color:#fff!important;
        border:1px solid rgba(164,145,155,.10)!important;
        box-shadow:0 8px 22px rgba(125,91,109,.06)!important;
  ...
  function renderRoute(route){
    ...
    if(route==='convites'){
      const out=window.renderConvitesV162?.() ?? window.renderConvites?.();
      ensureStyle();
      return out;
    }
    if(route==='premiacoes'){
      if(typeof window.renderRewardsV164==='function') return window.renderRewardsV164();
      if(typeof window.renderRewardsV163==='function') return window.renderRewardsV163();
      if(typeof window.renderRewardsRC152==='function') return window.renderRewardsRC152();
      return window.renderUniversalRewards?.();
    }
  }

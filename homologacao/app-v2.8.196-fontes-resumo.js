...
(()=>{
  ...
  function fgCard(g){
    ...
    const periods=(g.periods||[]).map(p=>{const real=fgPeriodTotal(p);return `<div class="fg-period-row"><div><strong>${escapeHtml(p.label)}</strong><small>${(p.contributions||[]).length} aporte${(p.contributions||[]).length===1?'':'s'}</small></div><div class="fg-period-values"><b>R$ ${money(real)}</b><span>planejado R$ ${money(p.min)}–${money(p.max)}</span></div><button type="button" class="fg-plus" data-fg-add="${g.id}" data-fg-period="${p.id}" aria-label="Adicionar aporte">＋</button></div>`}).join('');
...
(()=>{
  ...
  function saveSources196(d){
    ...
    if(d.financialSources.length)d.income=d.financialSources.filter(s=>s.active!==false).reduce((sum,s)=>sum+Number(s.monthlyEntry||0),0);
    saveFin(d);
  }
  ...
  function financeFlowCompact196(){
    const d=loadFin(),month=finCurrentMonth(),tx=monthTx196(d,month),t=totals196(d,month);
    return `<div class="fin-flow-v196"><div class="section-title">FLUXO DO MÊS</div><section class="card fin-flow-shell196"><div class="panel-head"><div><span class="eyebrow">${escapeHtml(finMonthLabel(month).toUpperCase())}</span><h3>Como o dinheiro circulou</h3></div></div>${summaryMetrics196(t)}<div class="fin-flow-detail196"><div class="fin-flow-mini196"><span class="eyebrow">COMO FOI PAGO</span>${audit196(tx,'paymentMethod')}</div><div class="fin-flow-mini196"><span class="eyebrow">PARA ONDE FOI</span>${audit196(tx,'category')}</div></div></section></div>`;
  }

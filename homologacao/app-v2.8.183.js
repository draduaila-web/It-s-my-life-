...
function renderFinanceRemindersMeuDia(){
 ...
 const rows=items.map(o=>`<a class="home-fin-reminder-row" href="#financeiro"><span class="home-fin-reminder-icon ${o.days<0?'is-late':'is-clock'}" aria-hidden="true"></span><span><strong>${escapeHtml(o.bill.name||'Conta prevista')}</strong><small>${escapeHtml(o.status.label)} · ${formatDate(o.due)}</small></span><b>›</b></a>`).join('');
...
function ensureSoftMeuDiaStyles(){
  ...
  style.textContent=`
    ...
    .home-fin-reminders{background:radial-gradient(circle at 92% 12%,rgba(135,177,218,.08),transparent 32%),linear-gradient(135deg,rgba(255,251,244,.98),rgba(248,246,252,.94))!important;border:1px solid rgba(112,92,156,.08)!important;border-radius:24px!important;padding:16px!important;box-shadow:0 8px 24px rgba(76,58,82,.035)!important;}
    ...
    .home-fin-reminder-icon{display:block!important;position:relative!important;width:24px!important;height:24px!important;color:#8067a6!important;background:transparent!important;border-radius:0!important;font-weight:400!important}.home-fin-reminder-icon.is-clock:before{content:"";position:absolute;inset:3px;border:1.5px solid currentColor;border-radius:50%;opacity:.78}.home-fin-reminder-icon.is-clock:after{content:"";position:absolute;left:11px;top:7px;width:1.5px;height:6px;background:currentColor;transform-origin:50% 5px;transform:rotate(-20deg);border-radius:2px;box-shadow:3px 4px 0 -0.25px currentColor;opacity:.78}.home-fin-reminder-icon.is-late:before{content:"!";position:absolute;inset:3px;border:1.5px solid currentColor;border-radius:50%;display:grid;place-items:center;font-size:12px;line-height:1;font-weight:700;opacity:.82}
...
/* v2.8.183 — Meu Dia · lembrete financeiro alinhado à identidade + ícone de atenção corrigido */

...
window.BERTHA_BUILD="2.8.191-financeiro-route-goals-privacy-hml";
...
function render() {
  ...
  else if (route === "financeiro") { const fn=(typeof window.renderFinanceiro==='function'?window.renderFinanceiro:renderFinanceiro); fn(); }
...
function loadEx(){try{const raw=JSON.parse(window.berthaHmlStorage.getItem(EX_KEY));if(raw){return {...EX_BASE,...raw,plans:(Array.isArray(raw.plans)?raw.plans:EX_BASE.plans).map(normalizeExPlan),sessions:Array.isArray(raw.sessions)?raw.sessions:[]};}}catch{}return JSON.parse(JSON.stringify(EX_BASE));}
...
function exerciseRecordProgress({name,minutes,date,plannedName=null,substituted=false,note='Registrado manualmente'}){
 try{
  const key='bertha.time-engine.v1',e=JSON.parse(window.berthaHmlStorage.getItem(key)||'{"active":null,"history":[],"snoozed":{}}');
  ...
  window.berthaHmlStorage.setItem(key,JSON.stringify(e));
 }catch{}
}
...
function loadFood(){try{const d=JSON.parse(window.berthaHmlStorage.getItem(FOOD_KEY));if(d)return {...FOOD_BASE,...d,mealLog:d.mealLog||{}};}catch{}return JSON.parse(JSON.stringify(FOOD_BASE));}
function saveFood(d){window.berthaHmlStorage.setItem(FOOD_KEY,JSON.stringify(d));}
...
/* BERTH.A Homologação v2.8.186 — fechamento Financeiro
   - privacidade revela sem refresh
   - "Já pago" e pontualidade derivados do histórico realizado
   - exclusão de previsão preserva passado sem derrubar totais
   - metas financeiras nomeadas, criáveis/editáveis e com aportes reais
   - metas visíveis no Meu Progresso
   - lembretes compactos com ícone simples
*/
(() => {
  const V186_GOALS_KEY = 'financialGoals';
  const _loadFin185 = loadFin;
  const _saveFin185 = saveFin;

  function v186Uid(){ return uid(); }
  function v186Clone(x){ return JSON.parse(JSON.stringify(x)); }
  function v186LegacyGoalToPeriod(g){
    return {
      id:g.id||v186Uid(), label:String(g.month||'Etapa'), min:Number(g.min||0), max:Number(g.max||0),
      contributions:Array.isArray(g.contributions)?g.contributions.map(c=>({...c,id:c.id||v186Uid(),value:Number(c.value||0)})):(Number(g.saved||0)>0?[{id:v186Uid(),value:Number(g.saved||0),date:todayISO(),note:'Valor migrado'}]:[])
    };
  }
  function v186NormalizeGoal(goal){
    const g={...goal,id:goal.id||v186Uid(),name:String(goal.name||'Meta financeira'),description:String(goal.description||''),archived:!!goal.archived,target:Number(goal.target||0)};
    g.periods=Array.isArray(goal.periods)?goal.periods.map(p=>({
      ...p,id:p.id||v186Uid(),label:String(p.label||'Etapa'),min:Number(p.min||0),max:Number(p.max||0),
      contributions:Array.isArray(p.contributions)?p.contributions.map(c=>({...c,id:c.id||v186Uid(),value:Number(c.value||0)})):[]
    })):[];
    return g;
  }
  function v186EnsureGoals(d){
    if(!Array.isArray(d[V186_GOALS_KEY])){
      d[V186_GOALS_KEY]=(Array.isArray(d.goals)&&d.goals.length)?[{id:v186Uid(),name:'Fundo Carro',description:'',target:0,archived:false,periods:d.goals.map(v186LegacyGoalToPeriod)}]:[];
    }
    d[V186_GOALS_KEY]=d[V186_GOALS_KEY].map(v186NormalizeGoal);
    return d;
  }
  loadFin=function(){ return v186EnsureGoals(_loadFin185()); };
  saveFin=function(d){
    d=v186EnsureGoals(d||{});
    /* Compatibilidade: o primeiro conjunto de períodos continua disponível no campo legado,
       mas a fonte oficial a partir daqui é financialGoals. */
    const first=d[V186_GOALS_KEY].find(g=>!g.archived)||d[V186_GOALS_KEY][0];
    d.goals=first?first.periods.map(p=>({id:p.id,month:p.label,min:p.min,max:p.max,contributions:v186Clone(p.contributions),saved:p.contributions.reduce((s,c)=>s+Number(c.value||0),0)})):[];
    _saveFin185(d);
  };

  function v186MonthTx(d,month){ return finMonthTransactions(d,month); }
  function v186PaymentTx(d,month){
    return v186MonthTx(d,month).filter(t=>t.plannedBillId || /^Conta prevista\b/i.test(String(t.note||'')));
  }
  function v186PaidTotal(d,month){ return v186PaymentTx(d,month).reduce((s,t)=>s+Number(t.value||0),0); }
  function v186PaymentStats(d,month){
    const pays=v186PaymentTx(d,month), known=pays.filter(t=>t.daysLate!=null || t.dueDate);
    const late=known.filter(t=>Number(t.daysLate||0)>0), ontime=known.filter(t=>Number(t.daysLate||0)<=0);
    const interest=pays.reduce((s,t)=>s+Number(t.interest||0),0), fine=pays.reduce((s,t)=>s+Number(t.fine||0),0), other=pays.reduce((s,t)=>s+Number(t.otherCharges||0),0), discount=pays.reduce((s,t)=>s+Number(t.discount||0),0);
    return {pays,late,ontime,interest,fine,other,discount,loss:interest+fine};
  }
  function v186GoalPeriodTotal(p){ return (p.contributions||[]).reduce((s,c)=>s+Number(c.value||0),0); }
  function v186GoalTotal(g){ return (g.periods||[]).reduce((s,p)=>s+v186GoalPeriodTotal(p),0); }
  function v186GoalPlan(g){ return (g.periods||[]).reduce((a,p)=>({min:a.min+Number(p.min||0),max:a.max+Number(p.max||0)}),{min:0,max:0}); }
  function v186GoalState(total,min,max){ if(total<min)return 'abaixo da faixa'; if(max>0&&total>max)return 'acima da faixa'; return 'na faixa'; }

  /* Privacidade: alternar sempre re-renderiza, evitando depender de textos mascarados no DOM. */
  applyFinPrivacyNative=function(){
  ...
  toggleFinPrivacyNative=function(){
    window.berthaHmlStorage.setItem(FIN_PRIVACY_KEY,finPrivacyHidden()?'visible':'hidden');
    if((location.hash||'').replace('#','').startsWith('financeiro')){
      renderFinanceiro();
      requestAnimationFrame(()=>applyFinPrivacyNative());
    }else if((location.hash||'').replace('#','').startsWith('progresso')) renderProgressOverview();
  };

  /* Ícone simples e inteiro: sino para lembrete, alerta circular para atraso. */
  const _renderFinanceRemindersMeuDia185 = renderFinanceRemindersMeuDia;
  renderFinanceRemindersMeuDia=function(){
    const items=finHomeReminders();if(!items.length)return '';
    const bell=`<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M8 16.5h8"></path><path d="M9 16.5V11a3 3 0 0 1 6 0v5.5"></path><path d="M7.5 16.5h9"></path><path d="M10.5 19h3"></path></svg>`;
    const late=`<svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="12" cy="12" r="8"></circle><path d="M12 7.5v5.2"></path><path d="M12 16.5h.01"></path></svg>`;
    const rows=items.map(o=>`<a class="home-fin-reminder-row" href="#financeiro"><span class="home-fin-reminder-icon ${o.days<0?'is-late':'is-clock'}" aria-hidden="true">${o.days<0?late:bell}</span><span><strong>${escapeHtml(o.bill.name||'Conta prevista')}</strong><small>${escapeHtml(finReminderMomentLabel(o))}</small></span><b>›</b></a>`).join('');
    return `<section class="day-section home-fin-reminders"><div class="section-head"><h2>Contas para lembrar</h2><a href="#financeiro">ver financeiro</a></div>${rows}</section>`;
  };

  function v186GoalCard(g,gi){
    const total=v186GoalTotal(g),plan=v186GoalPlan(g),state=v186GoalState(total,plan.min,plan.max);
    const periods=(g.periods||[]).map((p,pi)=>{
      const real=v186GoalPeriodTotal(p);
      return `<div class="fin-v186-period"><div><strong>${escapeHtml(p.label)}</strong><small>Real R$ ${money(real)} · planejado R$ ${money(p.min)}–${money(p.max)}</small></div><div class="fin-v186-period-side"><b>R$ ${money(real)}</b><button type="button" class="secondary tiny" data-v186-contrib="${gi}:${pi}">＋ aporte</button></div></div>`;
    }).join('')||`<div class="empty compact"><strong>Nenhum período criado.</strong><span>Edite a meta para adicionar etapas ou meses.</span></div>`;
    return `<section class="card fin-v186-goal"><div class="panel-head"><div><span class="eyebrow">PLANEJADO × REAL</span><h3>${escapeHtml(g.name)}</h3>${g.description?`<small>${escapeHtml(g.description)}</small>`:''}</div><button type="button" class="secondary tiny" data-v186-edit-goal="${gi}">Editar meta</button></div><div class="fin-v186-goal-summary"><span><small>Realizado acumulado</small><b>R$ ${money(total)}</b></span><span><small>Planejado acumulado</small><b>R$ ${money(plan.min)}–${money(plan.max)}</b></span><span class="pill">${escapeHtml(state)}</span></div><div class="fin-v186-periods">${periods}</div></section>`;
  }

  function v186PeriodRows(periods){
    return (periods.length?periods:[{id:v186Uid(),label:'',min:0,max:0,contributions:[]}]).map(p=>`<div class="fin-v186-period-edit" data-period-id="${escapeHtml(p.id||v186Uid())}"><input class="v186-period-label" maxlength="40" value="${escapeHtml(p.label||'')}" placeholder="Nome do período (ex.: Outubro)"><input class="v186-period-min" type="number" min="0" step="0.01" value="${Number(p.min||0)||''}" placeholder="mínimo"><input class="v186-period-max" type="number" min="0" step="0.01" value="${Number(p.max||0)||''}" placeholder="máximo"><button type="button" class="v186-remove-period" aria-label="Remover período">×</button></div>`).join('');
  }
  function v186OpenGoalModal(index=null){
    const d=loadFin(),existing=Number.isInteger(index)?d[V186_GOALS_KEY][index]:null,g=existing?v186Clone(existing):{id:v186Uid(),name:'',description:'',target:0,archived:false,periods:[]};
    const dlg=document.createElement('dialog');dlg.className='fin-planned-dialog fin-unified-dialog fin-goal-dialog';
    dlg.innerHTML=`<form method="dialog" class="modal-card" id="v186GoalForm"><div class="modal-head"><div><div class="eyebrow">META FINANCEIRA</div><h2>${existing?'Editar meta':'Nova meta financeira'}</h2></div><button type="button" class="icon-btn fin-modal-x" data-fin-close aria-label="Fechar">×</button></div><label>Nome da meta<input id="v186GoalName" required maxlength="70" value="${escapeHtml(g.name||'')}" placeholder="Ex.: Reserva de emergência"></label><label>Objetivo / descrição <span class="optional">opcional</span><input id="v186GoalDesc" maxlength="120" value="${escapeHtml(g.description||'')}" placeholder="Para que esta meta existe?"></label><label>Valor-alvo total <span class="optional">opcional</span><input id="v186GoalTarget" type="number" min="0" step="0.01" inputmode="decimal" value="${Number(g.target||0)||''}" placeholder="0,00"></label><div class="fin-contribution-head"><strong>Planejamento por mês / etapa</strong><button type="button" class="secondary tiny" id="v186AddPeriod">＋ Adicionar período</button></div><div id="v186Periods" class="fin-v186-period-edit-list">${v186PeriodRows(g.periods||[])}</div>${existing?'<button type="button" class="fin-delete-bill fin-danger-soft" id="v186DeleteGoal">Excluir meta</button>':''}<div class="modal-actions"><div class="grow"></div><button type="button" class="secondary" id="v186CancelGoal">Cancelar</button><button class="primary">Salvar meta</button></div></form>`;
    document.body.appendChild(dlg);dlg.showModal();const close=bindFinanceModalClose(dlg);dlg.querySelector('#v186CancelGoal').onclick=e=>{e.preventDefault();close()};
    const list=dlg.querySelector('#v186Periods');
    const bind=()=>list.querySelectorAll('.v186-remove-period').forEach(b=>b.onclick=()=>{b.closest('.fin-v186-period-edit').remove()});bind();
    dlg.querySelector('#v186AddPeriod').onclick=()=>{const wrap=document.createElement('div');wrap.innerHTML=v186PeriodRows([{id:v186Uid(),label:'',min:0,max:0,contributions:[]}]);list.appendChild(wrap.firstElementChild);bind()};
    if(existing)dlg.querySelector('#v186DeleteGoal').onclick=()=>{if(!confirm(`Excluir a meta “${g.name}”? Os aportes desta meta também serão removidos.`))return;d[V186_GOALS_KEY]=d[V186_GOALS_KEY].filter((_,i)=>i!==index);saveFin(d);dlg.close();dlg.remove();renderFinanceiro()};
    dlg.querySelector('#v186GoalForm').onsubmit=e=>{e.preventDefault();const oldById=new Map((g.periods||[]).map(p=>[p.id,p]));const periods=[...list.querySelectorAll('.fin-v186-period-edit')].map(row=>{const id=row.dataset.periodId||v186Uid(),old=oldById.get(id);return {id,label:row.querySelector('.v186-period-label').value.trim()||'Etapa',min:Number(row.querySelector('.v186-period-min').value||0),max:Number(row.querySelector('.v186-period-max').value||0),contributions:v186Clone(old?.contributions||[])};});const obj={...g,id:g.id||v186Uid(),name:dlg.querySelector('#v186GoalName').value.trim(),description:dlg.querySelector('#v186GoalDesc').value.trim(),target:Number(dlg.querySelector('#v186GoalTarget').value||0),periods,updatedAt:Date.now()};d[V186_GOALS_KEY]=existing?d[V186_GOALS_KEY].map((v,i)=>i===index?obj:v):[...d[V186_GOALS_KEY],obj];saveFin(d);dlg.close();dlg.remove();renderFinanceiro()};
  }
  function v186OpenContribution(gi,pi){
    const d=loadFin(),g=d[V186_GOALS_KEY]?.[gi],p=g?.periods?.[pi];if(!p)return;
    const dlg=document.createElement('dialog');dlg.className='fin-planned-dialog fin-unified-dialog fin-goal-dialog';
    const rows=(p.contributions?.length?p.contributions:[{id:v186Uid(),value:'',date:todayISO(),note:''}]).map(c=>`<div class="fin-contribution-row" data-contribution="${escapeHtml(c.id||v186Uid())}"><input class="gcValue" type="number" min="0" step="0.01" inputmode="decimal" value="${c.value?Number(c.value):''}" placeholder="0,00"><input class="gcDate" type="date" value="${escapeHtml(c.date||todayISO())}"><input class="gcNote" maxlength="70" value="${escapeHtml(c.note||'')}" placeholder="Observação"><button type="button" class="gcRemove" aria-label="Remover">×</button></div>`).join('');
    dlg.innerHTML=`<form method="dialog" class="modal-card" id="v186ContributionForm"><div class="modal-head"><div><div class="eyebrow">${escapeHtml(g.name)}</div><h2>${escapeHtml(p.label)}</h2></div><button type="button" class="icon-btn fin-modal-x" data-fin-close aria-label="Fechar">×</button></div><div class="fin-goal-plan"><span>Planejado</span><strong>R$ ${money(p.min)}–${money(p.max)}</strong></div><div class="fin-contribution-head"><strong>Aportes reais</strong><button type="button" class="secondary tiny" id="v186AddContribution">＋ Adicionar aporte</button></div><div id="v186ContributionRows" class="fin-contribution-list">${rows}</div><div class="fin-goal-live-total"><span>Realizado neste período</span><strong id="v186ContributionTotal">R$ 0,00</strong></div><div class="modal-actions"><div class="grow"></div><button type="button" class="secondary" id="v186CancelContribution">Cancelar</button><button class="primary">Salvar aportes</button></div></form>`;
    document.body.appendChild(dlg);dlg.showModal();const close=bindFinanceModalClose(dlg);dlg.querySelector('#v186CancelContribution').onclick=e=>{e.preventDefault();close()};const list=dlg.querySelector('#v186ContributionRows'),total=dlg.querySelector('#v186ContributionTotal');
    const recalc=()=>{total.textContent=`R$ ${money([...list.querySelectorAll('.gcValue')].reduce((s,x)=>s+Number(x.value||0),0))}`};
    const bind=()=>list.querySelectorAll('.fin-contribution-row').forEach(row=>{row.querySelector('.gcRemove').onclick=()=>{row.remove();recalc()};row.querySelector('.gcValue').oninput=recalc});bind();recalc();
    dlg.querySelector('#v186AddContribution').onclick=()=>{const row=document.createElement('div');row.className='fin-contribution-row';row.dataset.contribution=v186Uid();row.innerHTML=`<input class="gcValue" type="number" min="0" step="0.01" inputmode="decimal" placeholder="0,00"><input class="gcDate" type="date" value="${todayISO()}"><input class="gcNote" maxlength="70" placeholder="Observação"><button type="button" class="gcRemove" aria-label="Remover">×</button>`;list.appendChild(row);bind();recalc()};
    dlg.querySelector('#v186ContributionForm').onsubmit=e=>{e.preventDefault();p.contributions=[...list.querySelectorAll('.fin-contribution-row')].map(row=>({id:row.dataset.contribution||v186Uid(),value:Number(row.querySelector('.gcValue').value||0),date:row.querySelector('.gcDate').value||todayISO(),note:row.querySelector('.gcNote').value.trim()})).filter(x=>x.value>0);saveFin(d);dlg.close();dlg.remove();renderFinanceiro()};
  }

  /* Histórico de pagamento é a fonte dos totais realizados. */
  const _finTransactionHtml185=finTransactionHtml;
  finTransactionHtml=function(x){
    const meta=[x.date?formatDate(x.date):'',escapeHtml(x.category||'Variável'),x.paymentMethod?escapeHtml(x.paymentMethod):'',x.paymentSource?escapeHtml(x.paymentSource):''].filter(Boolean).join(' · ');
    const isPayment=!!x.plannedBillId || /^Conta prevista\b/i.test(String(x.note||''));
    return `<article class="card finance-row"><div><strong>${escapeHtml(x.name)}</strong><span>${meta}</span>${Number(x.interest||0)+Number(x.fine||0)>0?`<small>Juros + multa: R$ ${money(Number(x.interest||0)+Number(x.fine||0))}</small>`:''}</div><b>R$ ${money(x.value)}</b>${isPayment?'':`<button class="mini-delete" data-fin-delete="${x.id}" aria-label="Excluir">×</button>`}</article>`;
  };

  renderFinanceiro=function(){
    const d=loadFin(),month=finCurrentMonth(),fixed=finFixedTotal(d),variable=finMonthTotal(d,month),remaining=Number(d.income||0)-fixed-variable,catsMap=finCategoryTotals(d,month),monthBills=finPlannedMonth(d,month),nextMonth=finNextMonthKey(month),nextBills=finPlannedMonth(d,nextMonth);
    const openPlanned=monthBills.filter(x=>!x.paid).reduce((s,x)=>s+x.expected,0),paidPlanned=v186PaidTotal(d,month),afterKnown=remaining-openPlanned;
    const catHtml=Object.entries(catsMap).sort((a,b)=>b[1]-a[1]).slice(0,6).map(([k,v])=>`<div class="finance-cat"><span>${escapeHtml(k)}</span><strong>R$ ${money(v)}</strong></div>`).join('')||`<div class="empty compact"><strong>Nenhum gasto variável registrado.</strong><span>Registre apenas o que realmente precisar acompanhar.</span></div>`;
    const alerts=monthBills.filter(o=>!o.paid&&o.bill.reminderEnabled!==false&&finDaysTo(o.due)<=Number(o.bill.reminderDays??0)),stats=v186PaymentStats(d,month),goals=d[V186_GOALS_KEY].filter(g=>!g.archived),goalTotal=goals.reduce((s,g)=>s+v186GoalTotal(g),0);
    app.innerHTML=`<section class="finance-hero"><div><span class="finance-hero-kicker">FINANCEIRO</span><h2>Know what’s ahead.<br>Make room for life.</h2><p>Você registra o essencial.<br>A BERTH.A clareia o horizonte.</p></div><span class="finance-hero-icon finance-hero-symbol" aria-hidden="true"><svg viewBox="0 0 40 40" focusable="false"><path d="M7 22.5c4.4-4.2 8.7-6.3 13-6.3s8.6 2.1 13 6.3"/><path d="M9.5 27.5h21"/><circle cx="20" cy="16.2" r="1.35"/></svg></span></section>
    <section class="finance-summary card"><div class="finance-main"><span class="eyebrow">RENDA MENSAL</span><strong>R$ ${money(d.income)}</strong><button class="text-btn" id="editIncome">editar</button><button class="text-btn" id="toggleFinPrivacy" type="button">Ocultar valores</button></div><div class="finance-metrics"><div><span>Base</span><b>R$ ${money(fixed)}</b></div><div><span>Realizado · ${escapeHtml(finMonthLabel(month))}</span><b>R$ ${money(variable)}</b></div><div><span>Disponível conhecido</span><b>R$ ${money(remaining)}</b></div></div><div class="fin-forecast-strip"><span>Depois das contas previstas ainda pendentes</span><strong>R$ ${money(afterKnown)}</strong></div></section>
    ${alerts.length?`<section class="fin-due-alert"><strong>${financeAlertSvg()}${alerts.length===1?'1 conta pede atenção':`${alerts.length} contas pedem atenção`}</strong><span>${alerts.slice(0,3).map(o=>`${escapeHtml(o.bill.name)} · ${escapeHtml(finPlannedStatus(o).label)}`).join('<br>')}</span></section>`:''}
    <div class="section-title">CONTAS PREVISTAS · ${escapeHtml(finMonthLabel(month).toUpperCase())}</div><section class="card planned-bills-panel"><div class="panel-head"><div><span class="eyebrow">PREVISTO → REALIZADO</span><h3>Vencimentos do mês</h3></div><button class="primary compact-btn" id="addPlannedBill">＋ Nova conta prevista</button></div><div class="planned-mini-metrics"><div><span>A vencer</span><b>R$ ${money(openPlanned)}</b></div><div><span>Já pago</span><b>R$ ${money(paidPlanned)}</b></div></div><div class="planned-bills-list">${monthBills.length?monthBills.map(finPlannedBillHtml).join(''):`<div class="empty compact"><strong>Nenhuma conta prevista neste mês.</strong><span>Cadastre vencimentos para a BERTH.A lembrar o que vem pela frente.</span></div>`}</div></section>
    <div class="section-title">PRÓXIMOS VENCIMENTOS · ${escapeHtml(finNextMonthLabel(month).toUpperCase())}</div><section class="card fin-next-panel"><div class="fin-next-list">${nextBills.length?nextBills.map(finNextOccurrenceHtml).join(''):`<div class="empty compact"><strong>Nada previsto para o próximo mês.</strong><span>Contas recorrentes e vencimentos futuros aparecem aqui.</span></div>`}</div></section>
    <div class="section-title">ORÇAMENTO BASE</div><div class="list">${finPersonal(d.fixed).map(finFixedHtml).join('')||`<div class="empty compact"><strong>Seu orçamento base está vazio.</strong><span>Adicione apenas compromissos que realmente fazem parte da sua estrutura mensal.</span></div>`}</div><button class="add-full secondary" id="addFixed">＋ Adicionar item ao orçamento</button>
    <div class="section-title">GASTOS DO MÊS</div><section class="card"><div class="panel-head"><div><span class="eyebrow">${escapeHtml(finMonthLabel(month))}</span><h3>O que saiu de verdade</h3></div><button class="primary compact-btn" id="addTransaction">＋ Registrar</button></div><div class="list inner-list">${v186MonthTx(d,month).slice().reverse().slice(0,20).map(finTransactionHtml).join('')||`<div class="empty compact"><strong>Nenhum gasto registrado.</strong><span>O registro é opcional — use quando ajudar a enxergar seu mês.</span></div>`}</div></section>
    <div class="section-title">PONTUALIDADE</div><section class="card fin-punctuality"><div><span>Pagas no prazo</span><b>${stats.ontime.length}</b></div><div><span>Pagas em atraso</span><b>${stats.late.length}</b></div><div><span>Juros + multas</span><b>R$ ${money(stats.loss)}</b></div></section>
    <div class="section-title">POR CATEGORIA</div><section class="card finance-cats">${catHtml}</section>
    <div class="section-title">METAS FINANCEIRAS</div><div class="fin-v186-goals-head"><span>${goals.length?`${goals.length} meta${goals.length===1?'':'s'} ativa${goals.length===1?'':'s'}`:'Nenhuma meta cadastrada'}</span><button type="button" class="secondary compact-btn" id="v186AddGoal">＋ Nova meta</button></div><div class="fin-v186-goals">${goals.map((g,i)=>v186GoalCard(g,d[V186_GOALS_KEY].indexOf(g))).join('')||`<div class="card empty compact"><strong>Crie metas quando elas fizerem sentido.</strong><span>O planejado é referência; o realizado mostra o que aconteceu de verdade.</span></div>`}</div>
    <div class="section-title finance-covered-title">DESPESAS COBERTAS</div><div class="finance-covered-intro">Compromissos que hoje não saem da sua renda.</div><div class="list finance-covered-list">${finPersonal(d.excluded).map((x,i)=>`<div class="card excluded-card"><button type="button" class="covered-edit" data-fin-covered-index="${i}" aria-label="Editar despesa coberta"><div><strong>${escapeHtml(x.name)}</strong><span>${x.value?`R$ ${money(x.value)} · `:''}${x.category?`${escapeHtml(x.category)} · `:''}${escapeHtml(x.reason||'')}</span></div><span class="pill">${escapeHtml(x.payer||'Outra fonte')}</span></button></div>`).join('')||`<div class="covered-empty">Nenhuma despesa coberta cadastrada.</div>`}</div><button class="add-full secondary covered-add" id="addCovered">＋ Adicionar despesa coberta</button>`;
    ensureFinanceStyles();ensureFinance185Styles();v186Styles();
    document.getElementById('editIncome').onclick=()=>openFinModal('income');document.getElementById('toggleFinPrivacy').onclick=toggleFinPrivacyNative;document.getElementById('addPlannedBill').onclick=()=>openPlannedBillModal();document.getElementById('addFixed').onclick=()=>openFixedBudgetModal();document.getElementById('addTransaction').onclick=()=>openFinModal('transaction');document.getElementById('addCovered').onclick=()=>openCoveredExpenseModal();document.querySelectorAll('[data-fin-covered-index]').forEach(b=>b.onclick=()=>openCoveredExpenseModal(Number(b.dataset.finCoveredIndex)));
    document.querySelectorAll('[data-fin-edit-bill]').forEach(b=>b.onclick=()=>openPlannedBillModal(b.dataset.finEditBill));document.querySelectorAll('[data-fin-pay]').forEach(b=>b.onclick=()=>openPlannedPayModal(b.dataset.finPay,b.dataset.month));document.querySelectorAll('[data-fin-unpay]').forEach(b=>b.onclick=()=>undoPlannedPayment(b.dataset.finUnpay,b.dataset.month));document.querySelectorAll('[data-fin-edit-fixed]').forEach(b=>b.onclick=()=>openFixedBudgetModal(b.dataset.finEditFixed));
    document.querySelectorAll('[data-fin-delete]').forEach(b=>b.onclick=()=>{const x=loadFin();x.transactions=x.transactions.filter(t=>t.id!==b.dataset.finDelete);saveFin(x);renderFinanceiro()});document.getElementById('v186AddGoal').onclick=()=>v186OpenGoalModal();document.querySelectorAll('[data-v186-edit-goal]').forEach(b=>b.onclick=()=>v186OpenGoalModal(Number(b.dataset.v186EditGoal)));document.querySelectorAll('[data-v186-contrib]').forEach(b=>b.onclick=()=>{const [gi,pi]=b.dataset.v186Contrib.split(':').map(Number);v186OpenContribution(gi,pi)});applyFinPrivacyNative();
  };

  progressFinanceRow=function(){
    const d=loadFin(),month=finCurrentMonth(),fixed=finFixedTotal(d),variable=finMonthTotal(d,month),bills=finPlannedMonth(d,month),open=bills.filter(x=>!x.paid),covered=finPersonal(d.excluded).reduce((s,x)=>s+Number(x.value||0),0),remaining=Number(d.income||0)-fixed-variable,stats=v186PaymentStats(d,month),paidTotal=v186PaidTotal(d,month),goals=d[V186_GOALS_KEY].filter(g=>!g.archived),hidden=finPrivacyHidden();
    const cash=v=>hidden?'R$ ••••••':`R$ ${money(v)}`;
    const goalHtml=goals.length?`<div class="progress-fin-goals"><div class="progress-fin-goals-title">Metas financeiras</div>${goals.map(g=>{const total=v186GoalTotal(g),plan=v186GoalPlan(g);return `<div class="progress-fin-goal"><span><strong>${escapeHtml(g.name)}</strong><small>Planejado acumulado ${cash(plan.min)}–${cash(plan.max)}</small></span><b>${cash(total)}</b></div>`}).join('')}</div>`:'';
    return `<div class="progress-fin-wrap"><details class="progress-category"><summary class="progress-category-head"><strong>Financeiro</strong><b>${cash(remaining)} disponíveis</b></summary><small>${v186PaymentTx(d,month).length} pagamento${v186PaymentTx(d,month).length===1?'':'s'} de contas · realizado ${cash(variable)}</small><div class="progress-detail"><div><span>Orçamento base</span><span>${cash(fixed)}</span></div><div><span>Gastos registrados no mês</span><span>${cash(variable)}</span></div><div><span>Já pago em contas</span><span>${cash(paidTotal)}</span></div><div><span>Contas previstas pendentes</span><span>${open.length} · ${cash(open.reduce((s,x)=>s+x.expected,0))}</span></div><div><span>Pagas no prazo</span><span>${stats.ontime.length}</span></div><div><span>Pagas em atraso</span><span>${stats.late.length}</span></div><div><span>Juros e multas no mês</span><span>${cash(stats.loss)}</span></div><div><span>Dinheiro perdido por atraso</span><span>${cash(stats.loss)}</span></div><div><span>Despesas cobertas hoje</span><span>${cash(covered)}</span></div></div></details>${goalHtml}</div>`;
  };

  function v186Styles(){
    if(document.getElementById('finance-v186-styles'))return;const st=document.createElement('style');st.id='finance-v186-styles';st.textContent=`
      .fin-v186-goals-head{display:flex;justify-content:space-between;align-items:center;gap:12px;margin:0 2px 10px;color:#857a87;font-size:12px}.fin-v186-goals{display:grid;gap:12px}.fin-v186-goal{padding:16px}.fin-v186-goal .panel-head small{display:block;margin-top:4px;color:#8a808b;font-size:11px}.fin-v186-goal-summary{display:grid;grid-template-columns:1fr 1fr auto;gap:9px;align-items:center;margin:12px 0;padding:11px;border-radius:16px;background:linear-gradient(120deg,rgba(238,230,248,.66),rgba(229,241,246,.72))}.fin-v186-goal-summary span small,.fin-v186-goal-summary span b{display:block}.fin-v186-goal-summary small{font-size:10px;color:#827785}.fin-v186-goal-summary b{margin-top:3px;font-size:13px}.fin-v186-periods{display:grid;gap:0}.fin-v186-period{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:10px;align-items:center;padding:12px 0;border-top:1px solid rgba(92,72,104,.08)}.fin-v186-period:first-child{border-top:0}.fin-v186-period strong,.fin-v186-period small{display:block}.fin-v186-period small{margin-top:3px;color:#897f8b;font-size:11px}.fin-v186-period-side{display:flex;align-items:center;gap:8px}.fin-v186-period-side b{font-size:13px;white-space:nowrap}.fin-v186-period-edit-list{display:grid;gap:8px}.fin-v186-period-edit{display:grid;grid-template-columns:1.3fr .8fr .8fr 34px;gap:7px;align-items:center}.fin-v186-period-edit input{min-height:44px!important;padding:9px 10px!important}.v186-remove-period{width:34px;height:34px;border-radius:50%;border:1px solid rgba(151,93,103,.16);background:transparent;color:#95666e;font-size:18px}.progress-fin-goals{margin-top:10px;display:grid;gap:8px}.progress-fin-goals-title{font-size:11px;font-weight:800;letter-spacing:.12em;text-transform:uppercase;color:#9a7282}.progress-fin-goal{display:flex;justify-content:space-between;gap:12px;align-items:center;padding:12px 14px;border:1px solid rgba(92,72,104,.08);border-radius:18px;background:rgba(255,253,249,.76)}.progress-fin-goal strong,.progress-fin-goal small{display:block}.progress-fin-goal small{margin-top:3px;color:#8b818c;font-size:10px}.progress-fin-goal b{white-space:nowrap;font-size:13px}.home-fin-reminder-icon svg{stroke-width:1.6!important}
      @media(max-width:560px){.fin-v186-goal-summary{grid-template-columns:1fr 1fr}.fin-v186-goal-summary>.pill{grid-column:1/3;justify-self:start}.fin-v186-period{grid-template-columns:1fr}.fin-v186-period-side{justify-content:space-between}.fin-v186-period-edit{grid-template-columns:1fr 1fr 34px}.fin-v186-period-edit .v186-period-label{grid-column:1/3}}
    `;document.head.appendChild(st);
  }

  window.__berthaFinance186={v186OpenGoalModal,v186OpenContribution};
  window.renderFinanceiro=renderFinanceiro;
  window.toggleFinPrivacyNative=toggleFinPrivacyNative;
  if(location.hash==='#financeiro')renderFinanceiro();
  if(location.hash==='#progresso')renderProgressOverview();
})();

/* v2.8.188 deployment bridge: guarantees the visible finance controls use the current handlers. */
document.addEventListener('click',function(e){
  const privacy=e.target.closest&&e.target.closest('#toggleFinPrivacy');
  if(privacy){
    ...
    requestAnimationFrame(()=>{ try{ applyFinPrivacyNative(); }catch(_){} });
    return;
  }
  const addGoal=e.target.closest&&e.target.closest('#addGoal,#v186AddGoal');
  if(addGoal && window.__berthaFinance186?.v186OpenGoalModal){ e.preventDefault(); e.stopImmediatePropagation(); window.__berthaFinance186.v186OpenGoalModal(); return; }
},true);

/* BERTH.A Homologação v2.8.189 — bloqueadores finais Financeiro
   - privacidade instantânea sem reload
   - metas como entidades independentes e persistentes
   - modal claro: Nome da meta -> períodos -> aportes
   - cards de metas no Meu Progresso
*/
(() => {
  const GOALS_KEY = 'financialGoals';
  const MONTH_NAMES = ['Janeiro','Fevereiro','Março','Abril','Maio','Junho','Julho','Agosto','Setembro','Outubro','Novembro','Dezembro'];
  const monthNowLabel = () => MONTH_NAMES[new Date().getMonth()];
  const clone = x => JSON.parse(JSON.stringify(x));
  const goalUid = () => (typeof uid==='function'?uid():('g-'+Date.now()+'-'+Math.random().toString(16).slice(2)));
  const periodTotal = p => (p.contributions||[]).reduce((s,c)=>s+Number(c.value||0),0);
  const goalTotal = g => (g.periods||[]).reduce((s,p)=>s+periodTotal(p),0);
  const goalPlan = g => (g.periods||[]).reduce((a,p)=>({min:a.min+Number(p.min||0),max:a.max+Number(p.max||0)}),{min:0,max:0});
  const goalState = (real,min,max) => real<min?'abaixo da faixa':(max>0&&real>max?'acima da faixa':'na faixa');
  const esc = v => escapeHtml(String(v??''));

  function normalizeGoals(d){
    if(!Array.isArray(d[GOALS_KEY])) d[GOALS_KEY]=[];
    // Migra legado somente se ainda não houver metas modernas.
    if(!d[GOALS_KEY].length && Array.isArray(d.goals) && d.goals.length){
      d[GOALS_KEY]=[{id:goalUid(),name:'Fundo Carro',description:'',target:0,archived:false,periods:d.goals.map(x=>({id:x.id||goalUid(),label:String(x.month||'Etapa'),min:Number(x.min||0),max:Number(x.max||0),contributions:Array.isArray(x.contributions)?x.contributions.map(c=>({...c,id:c.id||goalUid(),value:Number(c.value||0)})):(Number(x.saved||0)>0?[{id:goalUid(),value:Number(x.saved),date:todayISO(),note:'Valor migrado'}]:[])}))}];
    }
    d[GOALS_KEY]=d[GOALS_KEY].map(g=>({
      ...g,id:g.id||goalUid(),name:String(g.name||'Meta financeira'),description:String(g.description||''),target:Number(g.target||0),archived:!!g.archived,
      periods:Array.isArray(g.periods)?g.periods.map(p=>({...p,id:p.id||goalUid(),label:String(p.label||'Etapa'),min:Number(p.min||0),max:Number(p.max||0),contributions:Array.isArray(p.contributions)?p.contributions.map(c=>({...c,id:c.id||goalUid(),value:Number(c.value||0)})):[]})):[]
    }));
    return d;
  }

  const baseLoadFin = loadFin;
  const baseSaveFin = saveFin;
  loadFin = function(){ return normalizeGoals(baseLoadFin()); };
  saveFin = function(d){ d=normalizeGoals(d||{}); baseSaveFin(d); };

  function cash(v){ return finPrivacyHidden()?'R$ ••••••':`R$ ${money(Number(v||0))}`; }
  function syncPrivacyButton(){
    const b=document.getElementById('toggleFinPrivacy');
    if(b) b.innerHTML=finEyeIcon(!finPrivacyHidden())+(finPrivacyHidden()?'Mostrar valores':'Ocultar valores');
  }
  toggleFinPrivacyNative = function(){
    window.berthaHmlStorage.setItem(FIN_PRIVACY_KEY, finPrivacyHidden()?'visible':'hidden');
    const route=(location.hash||'').replace('#','');
    if(route==='financeiro') { const fn=(typeof window.renderFinanceiro==='function'?window.renderFinanceiro:renderFinanceiro); fn(); }
    else if(route==='progresso') renderProgressOverview();
  };

  function periodEditorRows(periods){
    const rows=(periods&&periods.length)?periods:[{id:goalUid(),label:monthNowLabel(),min:0,max:0,contributions:[]}];
    return rows.map(p=>`<div class="fin189-period-edit" data-period-id="${esc(p.id||goalUid())}">
      <label>Período<input class="fin189-period-label" maxlength="40" value="${esc(p.label||'')}" placeholder="Ex.: Setembro"></label>
      <div class="form-grid"><label>Planejado mínimo<input class="fin189-period-min" type="number" min="0" step="0.01" inputmode="decimal" value="${Number(p.min||0)||''}" placeholder="0,00"></label><label>Planejado máximo<input class="fin189-period-max" type="number" min="0" step="0.01" inputmode="decimal" value="${Number(p.max||0)||''}" placeholder="0,00"></label></div>
      <button type="button" class="fin189-remove-period" aria-label="Remover período">Remover período</button>
    </div>`).join('');
  }

  function openGoalModal189(index=null){
    const d=loadFin();
    const existing=Number.isInteger(index)?d[GOALS_KEY][index]:null;
    const g=existing?clone(existing):{id:goalUid(),name:'',description:'',target:0,archived:false,periods:[]};
    const dlg=document.createElement('dialog'); dlg.className='fin-planned-dialog fin-unified-dialog fin189-goal-dialog';
    dlg.innerHTML=`<form method="dialog" class="modal-card" id="fin189GoalForm">
      <div class="modal-head"><div><div class="eyebrow">META FINANCEIRA</div><h2>${existing?'Editar meta':'Nova meta financeira'}</h2></div><button type="button" class="icon-btn fin-modal-x" data-fin-close aria-label="Fechar">×</button></div>
      <label>Nome da meta<input id="fin189GoalName" required maxlength="70" value="${esc(g.name)}" placeholder="Ex.: Reserva de emergência"></label>
      <label>Objetivo / descrição <span class="optional">opcional</span><input id="fin189GoalDesc" maxlength="120" value="${esc(g.description)}" placeholder="Ex.: criar uma reserva de 6 meses"></label>
      <label>Valor-alvo total <span class="optional">opcional</span><input id="fin189GoalTarget" type="number" min="0" step="0.01" inputmode="decimal" value="${Number(g.target||0)||''}" placeholder="0,00"></label>
      <div class="fin189-plan-head"><div><strong>Planejamento por período</strong><small>Defina a referência. O realizado será registrado nos aportes.</small></div><button type="button" class="secondary tiny" id="fin189AddPeriod">＋ período</button></div>
      <div id="fin189Periods" class="fin189-period-list">${periodEditorRows(g.periods)}</div>
      ${existing?'<button type="button" class="fin-danger-soft" id="fin189DeleteGoal">Excluir meta</button>':''}
      <div class="modal-actions"><div class="grow"></div><button type="button" class="secondary" id="fin189CancelGoal">Cancelar</button><button class="primary">Salvar meta</button></div>
    </form>`;
    document.body.appendChild(dlg); dlg.showModal();
    const close=bindFinanceModalClose(dlg);
    dlg.querySelector('#fin189CancelGoal').onclick=e=>{e.preventDefault();close()};
    const list=dlg.querySelector('#fin189Periods');
    const bindRemove=()=>list.querySelectorAll('.fin189-remove-period').forEach(b=>b.onclick=()=>{ if(list.children.length>1) b.closest('.fin189-period-edit').remove(); });
    bindRemove();
    dlg.querySelector('#fin189AddPeriod').onclick=()=>{ const wrap=document.createElement('div'); wrap.innerHTML=periodEditorRows([{id:goalUid(),label:'',min:0,max:0,contributions:[]}]); list.appendChild(wrap.firstElementChild); bindRemove(); };
    if(existing) dlg.querySelector('#fin189DeleteGoal').onclick=()=>{ if(!confirm(`Excluir a meta “${g.name}”?`))return; d[GOALS_KEY]=d[GOALS_KEY].filter((_,i)=>i!==index); saveFin(d); dlg.close(); dlg.remove(); renderFinanceiro(); };
    dlg.querySelector('#fin189GoalForm').onsubmit=e=>{
      e.preventDefault();
      const name=dlg.querySelector('#fin189GoalName').value.trim(); if(!name)return;
      const oldById=new Map((g.periods||[]).map(p=>[p.id,p]));
      const periods=[...list.querySelectorAll('.fin189-period-edit')].map(row=>{ const id=row.dataset.periodId||goalUid(),old=oldById.get(id); return {id,label:row.querySelector('.fin189-period-label').value.trim()||'Etapa',min:Number(row.querySelector('.fin189-period-min').value||0),max:Number(row.querySelector('.fin189-period-max').value||0),contributions:clone(old?.contributions||[])}; });
      const obj={...g,id:g.id||goalUid(),name,description:dlg.querySelector('#fin189GoalDesc').value.trim(),target:Number(dlg.querySelector('#fin189GoalTarget').value||0),periods,updatedAt:Date.now()};
      if(existing) d[GOALS_KEY]=d[GOALS_KEY].map((v,i)=>i===index?obj:v); else d[GOALS_KEY]=[...d[GOALS_KEY],obj];
      saveFin(d); dlg.close(); dlg.remove(); renderFinanceiro();
    };
  }

  function openContribution189(gi,pi){
    const d=loadFin(),g=d[GOALS_KEY]?.[gi],p=g?.periods?.[pi]; if(!p)return;
    const dlg=document.createElement('dialog');dlg.className='fin-planned-dialog fin-unified-dialog fin189-goal-dialog';
    const rows=(p.contributions?.length?p.contributions:[{id:goalUid(),value:'',date:todayISO(),note:''}]).map(c=>`<div class="fin-contribution-row" data-cid="${esc(c.id||goalUid())}"><input class="cval" type="number" min="0" step="0.01" inputmode="decimal" value="${c.value?Number(c.value):''}" placeholder="0,00"><input class="cdate" type="date" value="${esc(c.date||todayISO())}"><input class="cnote" maxlength="70" value="${esc(c.note||'')}" placeholder="Observação"><button type="button" class="cremove" aria-label="Remover">×</button></div>`).join('');
    dlg.innerHTML=`<form method="dialog" class="modal-card" id="fin189ContributionForm"><div class="modal-head"><div><div class="eyebrow">${esc(g.name)}</div><h2>${esc(p.label)}</h2></div><button type="button" class="icon-btn fin-modal-x" data-fin-close aria-label="Fechar">×</button></div><div class="fin-goal-plan"><span>Planejado</span><strong>${cash(p.min)}–${cash(p.max)}</strong></div><div class="fin-contribution-head"><strong>Aportes reais</strong><button type="button" class="secondary tiny" id="fin189AddContribution">＋ aporte</button></div><div id="fin189ContributionRows" class="fin-contribution-list">${rows}</div><div class="fin-goal-live-total"><span>Realizado neste período</span><strong id="fin189ContributionTotal">${cash(periodTotal(p))}</strong></div><div class="modal-actions"><div class="grow"></div><button type="button" class="secondary" id="fin189CancelContribution">Cancelar</button><button class="primary">Salvar aportes</button></div></form>`;
    document.body.appendChild(dlg);dlg.showModal();const close=bindFinanceModalClose(dlg);dlg.querySelector('#fin189CancelContribution').onclick=e=>{e.preventDefault();close()};
    const list=dlg.querySelector('#fin189ContributionRows'),total=dlg.querySelector('#fin189ContributionTotal');
    const recalc=()=>{ const v=[...list.querySelectorAll('.cval')].reduce((s,x)=>s+Number(x.value||0),0); total.textContent=finPrivacyHidden()?'R$ ••••••':`R$ ${money(v)}`; };
    const bind=()=>list.querySelectorAll('.fin-contribution-row').forEach(row=>{row.querySelector('.cremove').onclick=()=>{row.remove();recalc()};row.querySelector('.cval').oninput=recalc}); bind();
    dlg.querySelector('#fin189AddContribution').onclick=()=>{const row=document.createElement('div');row.className='fin-contribution-row';row.dataset.cid=goalUid();row.innerHTML=`<input class="cval" type="number" min="0" step="0.01" inputmode="decimal" placeholder="0,00"><input class="cdate" type="date" value="${todayISO()}"><input class="cnote" maxlength="70" placeholder="Observação"><button type="button" class="cremove">×</button>`;list.appendChild(row);bind();recalc()};
    dlg.querySelector('#fin189ContributionForm').onsubmit=e=>{e.preventDefault();p.contributions=[...list.querySelectorAll('.fin-contribution-row')].map(row=>({id:row.dataset.cid||goalUid(),value:Number(row.querySelector('.cval').value||0),date:row.querySelector('.cdate').value||todayISO(),note:row.querySelector('.cnote').value.trim()})).filter(x=>x.value>0);saveFin(d);dlg.close();dlg.remove();renderFinanceiro()};
  }

  function goalCard189(g,gi){
    const total=goalTotal(g),plan=goalPlan(g),state=goalState(total,plan.min,plan.max);
    return `<section class="card fin189-goal-card"><div class="panel-head"><div><span class="eyebrow">PLANEJADO × REAL</span><h3>${esc(g.name)}</h3>${g.description?`<small>${esc(g.description)}</small>`:''}</div><button type="button" class="secondary tiny" data-fin189-edit-goal="${gi}">Editar</button></div><div class="fin189-goal-summary"><div><small>Realizado acumulado</small><strong>${cash(total)}</strong></div><div><small>Planejado acumulado</small><strong>${cash(plan.min)}–${cash(plan.max)}</strong></div><span class="pill">${esc(state)}</span></div><div class="fin189-periods">${(g.periods||[]).map((p,pi)=>`<div class="fin189-period"><div><strong>${esc(p.label)}</strong><small>${(p.contributions||[]).length} aporte${(p.contributions||[]).length===1?'':'s'}</small></div><div class="fin189-period-values"><span>${cash(periodTotal(p))} / ${cash(p.min)}–${cash(p.max)}</span><button type="button" class="secondary tiny" data-fin189-contrib="${gi}:${pi}">＋ aporte</button></div></div>`).join('')||'<div class="empty compact">Nenhum período.</div>'}</div></section>`;
  }

  // Re-render final: usa os blocos maduros existentes, trocando apenas Metas e privacidade.
  const renderBase189 = renderFinanceiro;
  renderFinanceiro = function(){
    renderBase189();
    // Substitui a seção de metas antiga/legada por entidades independentes.
    const d=loadFin(),goals=d[GOALS_KEY].filter(g=>!g.archived);
    const sectionTitle=[...document.querySelectorAll('.section-title')].find(x=>x.textContent.trim()==='METAS FINANCEIRAS');
    if(sectionTitle){
      let n=sectionTitle.nextElementSibling;
      while(n && !n.classList.contains('section-title') && !n.classList.contains('finance-covered-title')){ const next=n.nextElementSibling; n.remove(); n=next; }
      const head=document.createElement('div'); head.className='fin189-goals-head'; head.innerHTML=`<span>${goals.length?`${goals.length} meta${goals.length===1?'':'s'} ativa${goals.length===1?'':'s'}`:'Nenhuma meta cadastrada'}</span><button type="button" class="secondary compact-btn" id="fin189AddGoal">＋ Nova meta</button>`;
      const wrap=document.createElement('div'); wrap.className='fin189-goals'; wrap.innerHTML=goals.map((g,i)=>goalCard189(g,d[GOALS_KEY].indexOf(g))).join('')||'<div class="card empty compact"><strong>Nenhuma meta financeira cadastrada.</strong><span>Crie apenas as metas que fizerem sentido.</span></div>';
      sectionTitle.after(head,wrap);
      head.querySelector('#fin189AddGoal').onclick=()=>openGoalModal189();
      wrap.querySelectorAll('[data-fin189-edit-goal]').forEach(b=>b.onclick=()=>openGoalModal189(Number(b.dataset.fin189EditGoal)));
      wrap.querySelectorAll('[data-fin189-contrib]').forEach(b=>b.onclick=()=>{const [gi,pi]=b.dataset.fin189Contrib.split(':').map(Number);openContribution189(gi,pi)});
    }
    const privacy=document.getElementById('toggleFinPrivacy'); if(privacy) privacy.onclick=toggleFinPrivacyNative;
    syncPrivacyButton();
  };

  const oldProgressFinanceRow = progressFinanceRow;
  progressFinanceRow = function(){
    const d=loadFin(); const goals=d[GOALS_KEY].filter(g=>!g.archived);
    let base=oldProgressFinanceRow();
    // Remove qualquer bloco antigo de metas do HTML retornado.
    base=base.replace(/<div class="progress-fin-goals">[\s\S]*?<\/div><\/div>(?=<\/div>$)/,'');
    const cards=goals.length?`<div class="progress-fin189-goals"><div class="progress-fin189-title">Metas financeiras</div>${goals.map(g=>{const total=goalTotal(g),plan=goalPlan(g);return `<div class="progress-fin189-card"><strong>${esc(g.name)}</strong><div><small>Realizado acumulado</small><b>${cash(total)}</b></div><div><small>Planejado acumulado</small><span>${cash(plan.min)}–${cash(plan.max)}</span></div></div>`}).join('')}</div>`:'';
    return `<div class="progress-fin189-wrap">${base}${cards}</div>`;
  };

  const style=document.createElement('style'); style.id='finance-v189-styles'; style.textContent=`
    .fin189-goals-head{display:flex;justify-content:space-between;align-items:center;gap:12px;margin:0 2px 10px;color:#857a87;font-size:12px}.fin189-goals{display:grid;gap:12px}.fin189-goal-card{padding:16px}.fin189-goal-card .panel-head small{display:block;margin-top:4px;color:#8a808b;font-size:11px}.fin189-goal-summary{display:grid;grid-template-columns:1fr 1fr auto;gap:10px;align-items:center;margin:12px 0;padding:12px;border-radius:16px;background:linear-gradient(120deg,rgba(238,230,248,.66),rgba(229,241,246,.72))}.fin189-goal-summary small,.fin189-goal-summary strong{display:block}.fin189-goal-summary small{font-size:10px;color:#827785}.fin189-goal-summary strong{margin-top:4px}.fin189-period{display:flex;justify-content:space-between;gap:12px;align-items:center;padding:12px 0;border-top:1px solid rgba(92,72,104,.08)}.fin189-period:first-child{border-top:0}.fin189-period small{display:block;color:#8b818c;font-size:10px;margin-top:3px}.fin189-period-values{display:flex;align-items:center;gap:8px;font-size:12px}.fin189-plan-head{display:flex;justify-content:space-between;align-items:flex-end;gap:12px;margin:8px 0}.fin189-plan-head small{display:block;margin-top:3px;color:#8b818c;font-size:10px}.fin189-period-list{display:grid;gap:10px}.fin189-period-edit{padding:12px;border:1px solid rgba(92,72,104,.09);border-radius:16px;background:rgba(255,255,255,.48)}.fin189-period-edit label{margin:0 0 8px}.fin189-remove-period{border:0;background:transparent;color:#9a5f68;font-weight:700;padding:6px 0}.progress-fin189-goals{display:grid;gap:10px;margin-top:12px}.progress-fin189-title{font-size:11px;font-weight:800;letter-spacing:.12em;text-transform:uppercase;color:#9a7282}.progress-fin189-card{padding:14px 16px;border:1px solid rgba(92,72,104,.08);border-radius:18px;background:rgba(255,253,249,.78)}.progress-fin189-card>strong{display:block;font-size:15px;margin-bottom:9px}.progress-fin189-card>div{display:flex;justify-content:space-between;gap:12px;padding-top:7px}.progress-fin189-card small{color:#8b818c}.progress-fin189-card b,.progress-fin189-card span{font-size:13px;text-align:right}.fin189-goal-dialog .modal-card{max-height:min(86vh,780px);overflow:auto}.fin189-goal-dialog input{font-size:16px!important}
    @media(max-width:560px){.fin189-goal-summary{grid-template-columns:1fr 1fr}.fin189-goal-summary>.pill{grid-column:1/3;justify-self:start}.fin189-period{align-items:flex-start;flex-direction:column}.fin189-period-values{width:100%;justify-content:space-between}.fin189-plan-head{align-items:flex-start;flex-direction:column}}
  `; document.head.appendChild(style);

  // Captura forte para impedir que handlers legados abram o modal antigo.
  document.addEventListener('click',e=>{
    const add=e.target.closest&&e.target.closest('#addGoal,#v186AddGoal,#fin189AddGoal');
    if(add){ e.preventDefault(); e.stopImmediatePropagation(); (window.__berthaFinance189?.openGoalModal189||openGoalModal189)(); return; }
    const privacy=e.target.closest&&e.target.closest('#toggleFinPrivacy');
    if(privacy){ e.preventDefault(); e.stopImmediatePropagation(); (window.toggleFinPrivacyNative||toggleFinPrivacyNative)(); return; }
  },true);

  window.__berthaFinance189={openGoalModal189,openContribution189};
  window.renderFinanceiro=renderFinanceiro;
  window.progressFinanceRow=progressFinanceRow;
  window.toggleFinPrivacyNative=toggleFinPrivacyNative;
  if((location.hash||'')==='#financeiro') renderFinanceiro();
  if((location.hash||'')==='#progresso') renderProgressOverview();
})();
...
/* BERTH.A HML v2.8.190 — fechamento Financeiro
   Corrige o roteamento que ainda chamava o renderer legado do Financeiro,
   garante privacidade imediata e migra tentativas de metas legadas não mensais.
*/
(() => {
  const MONTHS_190 = new Set(['janeiro','fevereiro','março','abril','maio','junho','julho','agosto','setembro','outubro','novembro','dezembro']);
  const norm190 = v => String(v||'').trim().toLocaleLowerCase('pt-BR');
  const clone190 = x => JSON.parse(JSON.stringify(x));
  const uid190 = () => (typeof uid==='function'?uid():('g190-'+Date.now()+'-'+Math.random().toString(16).slice(2)));

  // Migração defensiva: se uma tentativa feita pelo modal legado criou uma linha
  // como "Reserva" dentro de d.goals, converte-a em meta independente uma única vez.
  function migrateLegacyNamedGoals190(){
    try{
      const d=loadFin();
      if(!Array.isArray(d.financialGoals)) d.financialGoals=[];
      if(!Array.isArray(d.goals)) return;
      const existingNames=new Set(d.financialGoals.map(g=>norm190(g.name)));
      const monthRows=[], namedRows=[];
      d.goals.forEach(row=>{
        const label=String(row.month||row.label||'').trim();
        if(label && !MONTHS_190.has(norm190(label))) namedRows.push(row); else monthRows.push(row);
      });
      let changed=false;
      for(const row of namedRows){
        const name=String(row.month||row.label||'Meta financeira').trim();
        if(existingNames.has(norm190(name))) continue;
        const contributions=Array.isArray(row.contributions)?clone190(row.contributions):
          (Number(row.saved||0)>0?[{id:uid190(),value:Number(row.saved||0),date:(typeof todayISO==='function'?todayISO():new Date().toISOString().slice(0,10)),note:'Migrado da versão anterior'}]:[]);
        d.financialGoals.push({
          id:uid190(), name, description:'', target:0, archived:false,
          periods:[{id:uid190(),label:(typeof finMonthLabel==='function'&&typeof finCurrentMonth==='function'?finMonthLabel(finCurrentMonth()):'Etapa'),min:Number(row.min||0),max:Number(row.max||0),contributions}]
        });
        existingNames.add(norm190(name)); changed=true;
      }
      if(changed){
        // mantém apenas meses no legado para não reaparecer como etapa do Fundo Carro
        d.goals=monthRows;
        saveFin(d);
      }
    }catch(err){ console.warn('BERTH.A v190 migration',err); }
  }

  migrateLegacyNamedGoals190();

  // O renderer final de v189 é o renderer oficial. O bertha-time carregado depois
  // também passa a apontar para ele através de __berthaCoreRenderFinanceiro.
  const finalFinanceRenderer190 = window.renderFinanceiro || renderFinanceiro;
  window.renderFinanceiro = finalFinanceRenderer190;
  window.__berthaCoreRenderFinanceiro = finalFinanceRenderer190;

  // Privacidade: não depende mais de refresh nem de restauração textual do DOM.
  // Sempre alterna o estado e reconstrói a tela com a fonte de dados intacta.
  function togglePrivacy190(){
    const hidden = finPrivacyHidden();
    window.berthaHmlStorage.setItem(FIN_PRIVACY_KEY, hidden ? 'visible' : 'hidden');
    const route=(location.hash||'').replace('#','');
    if(route==='financeiro') finalFinanceRenderer190();
    else if(route==='progresso' && typeof renderProgressOverview==='function') renderProgressOverview();
  }
  window.toggleFinPrivacyNative = togglePrivacy190;

  // Último listener em captura: usa os controles VISÍVEIS, mesmo se alguma camada
  // antiga tiver deixado onclicks no DOM.
  document.addEventListener('click', e => {
    const privacy=e.target.closest&&e.target.closest('#toggleFinPrivacy');
    if(privacy){
      e.preventDefault(); e.stopImmediatePropagation();
    ...
    const add=e.target.closest&&e.target.closest('#addGoal,#v186AddGoal,#fin189AddGoal');
    if(add && window.__berthaFinance189?.openGoalModal189){
      e.preventDefault(); e.stopImmediatePropagation();
      window.__berthaFinance189.openGoalModal189();
      return;
    }
  }, true);

  // Se a página já estiver no Financeiro após atualização, força o renderer final.
  if((location.hash||'').replace('#','')==='financeiro') finalFinanceRenderer190();
})();


/* BERTH.A HML v2.8.191 — finance hardening
   One public renderer, one privacy toggle, one goals entry point.
*/
(() => {
  const fin191Renderer = window.renderFinanceiro || window.__berthaCoreRenderFinanceiro;
  if (typeof fin191Renderer !== 'function') return;
  window.renderFinanceiro = fin191Renderer;
  window.__berthaCoreRenderFinanceiro = fin191Renderer;

  function rerenderFinance191(){
    const route=(location.hash||'').replace('#','');
    if(route==='financeiro') window.renderFinanceiro();
    else if(route==='progresso' && typeof renderProgressOverview==='function') renderProgressOverview();
  }

  window.toggleFinPrivacyNative = function(){
    const hidden = finPrivacyHidden();
    window.berthaHmlStorage.setItem(FIN_PRIVACY_KEY, hidden ? 'visible' : 'hidden');
    rerenderFinance191();
  };

  const core = window.__berthaCoreRender;
  if (typeof core === 'function' && !window.__berthaCoreRender191Wrapped){
    window.__berthaCoreRender191Wrapped = true;
    window.__berthaCoreRender = function(){
      const route=(location.hash||'#meu-dia').replace('#','')||'meu-dia';
      if(route==='financeiro'){ window.renderFinanceiro(); return; }
      return core();
    };
  }
})();

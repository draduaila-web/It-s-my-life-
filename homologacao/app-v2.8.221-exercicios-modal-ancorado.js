...
function renderSatellites(){
 ...
 app.innerHTML=`<div class="sat-page"><section class="sat-hero"><div class="eyebrow">REDE BERTH.A</div><h2>Seu mundo, compartilhado só onde faz sentido.</h2><p>Você escolhe quem participa e o que cada pessoa pode acessar.</p></section><div class="sat-actions"><button class="primary" id="addSatellite">＋ Adicionar satélite</button>${permission!=='granted'&&permission!=='unsupported'?'<button class="secondary" id="satEnableNotifications">Ativar notificações do Owner</button>':''}</div><div class="sat-cloud-note"><span aria-hidden="true">◎</span><span>Casa é a primeira integração: tarefas podem ser oferecidas a vários satélites e ficam bloqueadas quando alguém assume.</span></div>${events.length?`<section class="sat-card card"><div class="panel-head"><div><h3>Atividade da rede</h3><p class="note">${unread?`${unread} nova${unread===1?'':'s'} para o Owner.`:'Tudo visto.'}</p></div><button type="button" class="secondary" id="satMarkRead">Marcar como visto</button></div><div class="sat-event-list">${events.slice(0,6).map(e=>`<div class="sat-event ${e.read?'':'unread'}"><span>✓</span><div><strong>${escapeHtml(e.memberName)} concluiu ${escapeHtml(e.taskName)}</strong><small>${new Date(e.at).toLocaleString('pt-BR',{day:'2-digit',month:'2-digit',hour:'2-digit',minute:'2-digit'})}</small></div></div>`).join('')}</div></section>`:''}<section class="sat-card card"><div class="panel-head"><div><h3>Minha rede</h3><p class="note">Adultos colaboram. Kids recebem uma experiência gamificada.</p></div><span class="pill">${members.length}</span></div>${members.length?members.map(m=>`<div class="sat-member"><span class="sat-avatar">${m.role==='kids'?'★':'◎'}</span><span><strong>${escapeHtml(m.name)}</strong><small>${[m.permissions?.casa!==false?'Casa':'',m.permissions?.compras?'Compras':'',m.permissions?.planos?'Planos':''].filter(Boolean).join(' · ')||'Sem módulos compartilhados'}</small></span><span><span class="sat-role">${m.role==='kids'?'KIDS':'ADULTO'}</span><div class="sat-actions"><button type="button" class="secondary" data-sat-preview="${m.id}">Ver perfil</button><button type="button" class="secondary" data-sat-edit="${m.id}">Editar</button></div></span></div>`).join(''):`<div class="sat-empty">Adicione o primeiro satélite para começar a compartilhar tarefas com controle de acesso.</div>`}</section></div>`;

...
function ensureSatelliteStyles(){if(document.getElementById('bertha-sat-v2'))return;const s=document.createElement('style');s.id='bertha-sat-v2';s.textContent=`
.sat-page{display:grid;gap:14px;padding-bottom:10px}
.sat-hero{padding:24px;border-radius:30px;background:linear-gradient(135deg,#fbf4ec 0%,#edf3f4 52%,#f3edf7 100%);border:1px solid rgba(113,96,130,.09);box-shadow:0 12px 28px rgba(74,57,69,.05)}
.sat-hero .eyebrow,.sat-card .eyebrow,.sat-mini-card .eyebrow{font-size:10px;letter-spacing:.18em;font-weight:500;color:#84778b}
.sat-hero h2{margin:6px 0 8px;font-size:28px;font-weight:400;color:#3f3944;line-height:1.1}
.sat-hero p{margin:0;color:#766f7a;font-size:13px;line-height:1.5}
...
.sat-actions{display:flex;gap:8px;flex-wrap:wrap}
...
.sat-cloud-note{padding:12px 14px;border-radius:18px;background:linear-gradient(145deg,rgba(237,244,242,.88),rgba(252,248,243,.92));border:1px solid rgba(113,96,130,.07);font-size:11px;color:#6f7773;line-height:1.5}
...
.sat-card.card{padding:16px;border-radius:26px;background:linear-gradient(145deg,rgba(255,255,255,.93),rgba(250,247,244,.98));border:1px solid rgba(113,96,130,.08);box-shadow:0 10px 24px rgba(74,57,69,.04)}
.sat-member{display:grid;grid-template-columns:46px minmax(0,1fr) auto;gap:12px;align-items:center;padding:16px;border-radius:22px;background:linear-gradient(145deg,rgba(255,255,255,.86),rgba(248,246,243,.94));border:1px solid rgba(113,96,130,.08)}
...
.sat-check{display:flex!important;align-items:flex-start!important;gap:12px!important;padding:12px!important;border-radius:18px!important;background:rgba(255,255,255,.82)!important;border:1px solid rgba(113,96,130,.09)!important;cursor:pointer!important;user-select:none!important}
.sat-check input[type=checkbox]{-webkit-appearance:none!important;appearance:none!important;width:20px!important;height:20px!important;min-width:20px!important;min-height:20px!important;border-radius:6px!important;border:1.5px solid rgba(137,121,142,.34)!important;background:#fffdfa!important;display:inline-grid!important;place-content:center!important;box-shadow:none!important;margin:1px 0 0!important;flex:0 0 20px!important}
...
function openSatelliteEditor(id){
 ...
 dlg.className='bertha-dialog casa-dialog sat-modal';
 dlg.innerHTML=`<form class="modal-card casa-modal-card sat-editor-form" id="satForm"><div class="modal-head"><div><div class="eyebrow">SATÉLITES</div><h2>${old?'Editar satélite':'Novo satélite'}</h2></div><button type="button" class="icon-btn casa-modal-x" data-close>×</button></div><div class="sat-editor-fields"><label>Nome<input id="satName" required value="${escapeHtml(old?.name||'')}" placeholder="Ex.: Henrique"></label><label>Tipo<select id="satRole"><option value="adult" ${old?.role==='adult'?'selected':''}>Adulto</option><option value="kids" ${old?.role==='kids'?'selected':''}>Kids</option></select></label></div><div class="sat-permissions"><div class="eyebrow">PERMISSÕES</div><label class="sat-check" data-check-row><input type="checkbox" id="satCasa" ${old?.permissions?.casa!==false?'checked':''}><span><strong>Casa e tarefas compartilhadas</strong><small>Recebe rotinas delegadas, aceita ajuda e pode ter Meu Dia próprio.</small></span></label><label class="sat-check" data-check-row><input type="checkbox" id="satCompras" ${old?.permissions?.compras?'checked':''}><span><strong>Lista de Compras</strong><small>Visualiza a lista compartilhada e pode ajudar nas compras.</small></span></label><label class="sat-check" data-check-row><input type="checkbox" id="satPlanos" ${old?.permissions?.planos?'checked':''}><span><strong>Planos compartilhados</strong><small>Recebe apenas os planos que você autorizar.</small></span></label></div><label class="sat-check sat-check-standalone" data-check-row><input type="checkbox" id="satValidation" ${old?.requiresValidation?'checked':''}><span><strong>Conclusões precisam de validação do Owner</strong><small>Ideal para Kids ou tarefas que exigem confirmação final antes de encerrar.</small></span></label><div class="modal-actions"><div class="grow"></div><button type="button" class="secondary" data-close>Cancelar</button><button class="primary" type="submit">Salvar</button></div></form>`;
 document.body.appendChild(dlg);
 const close=()=>dlg.close();
 dlg.querySelectorAll('[data-close]').forEach(b=>b.onclick=close);
 dlg.onclose=()=>dlg.remove();
 dlg.querySelectorAll('[data-check-row]').forEach(row=>row.addEventListener('click',e=>{
   if(e.target.closest('input,button,a,select,option'))return;
   e.preventDefault();

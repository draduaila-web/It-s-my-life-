...
function render() {
  ...
  else if (route === "convites") renderInvites();
...
function openSatelliteEditor(id){
 ...
 const toggle=(key,title,desc,on=false)=>`<button type="button" class="sat-permission ${on?'is-on':''}" data-perm="${key}" aria-pressed="${on?'true':'false'}"><span class="sat-perm-check" aria-hidden="true">${on?'✓':''}</span><span><strong>${title}</strong><small>${desc}</small></span></button>`;
 dlg.innerHTML=`<form class="modal-card casa-modal-card sat-editor-form" id="satForm"><div class="modal-head"><div><div class="eyebrow">SATÉLITES</div><h2>${old?'Editar satélite':'Novo satélite'}</h2></div><button type="button" class="icon-btn casa-modal-x" data-close>×</button></div><div class="sat-editor-fields"><label>Nome<input id="satName" required value="${escapeHtml(old?.name||'')}" placeholder="Ex.: Henrique"></label><label>Tipo<select id="satRole"><option value="adult" ${old?.role==='adult'?'selected':''}>Adulto</option><option value="kids" ${old?.role==='kids'?'selected':''}>Kids</option></select></label></div><div class="sat-permissions"><div class="eyebrow">PERMISSÕES</div>${toggle('casa','Casa e tarefas compartilhadas','Recebe rotinas delegadas, aceita ajuda e pode ter Meu Dia próprio.',perms.casa!==false)}${toggle('compras','Lista de Compras','Visualiza a lista compartilhada e pode ajudar nas compras.',!!perms.compras)}${toggle('planos','Planos compartilhados','Recebe apenas os planos que você autorizar.',!!perms.planos)}${toggle('convites','Convites','Pode receber convites para almoço, academia, passeios e outros momentos.',!!perms.convites)}${toggle('validation','Validar conclusões com o Owner','A conclusão encerra o período, mas a confirmação final fica com o Owner.',!!old?.requiresValidation)}</div><section class="sat-kids-config" id="satKidsConfig" ${old?.role==='kids'?'':'hidden'}><div class="eyebrow">KIDS · PREMIAÇÃO</div><p>Pontos viram progresso dentro da BERTH.A. A recompensa real continua sendo combinada em casa.</p><div class="sat-kids-grid"><label>Pontos-base por missão<input id="satBasePoints" type="number" min="0" inputmode="numeric" value="${old?.gamification?.basePoints??10}"></label><label>Meta para medalha<input id="satMedalAt" type="number" min="1" inputmode="numeric" value="${old?.gamification?.medalAt??50}"></label><label>Meta para troféu<input id="satTrophyAt" type="number" min="1" inputmode="numeric" value="${old?.gamification?.trophyAt??100}"></label><label>Meta para skin<input id="satSkinAt" type="number" min="1" inputmode="numeric" value="${old?.gamification?.skinAt??150}"></label></div></section><div class="modal-actions"><div class="grow"></div><button type="button" class="secondary" data-close>Cancelar</button><button class="primary" type="submit">Salvar</button></div></form>`;
 document.body.appendChild(dlg);
 const close=()=>dlg.close(); dlg.querySelectorAll('[data-close]').forEach(b=>b.onclick=close); dlg.onclose=()=>dlg.remove();
 dlg.querySelectorAll('[data-perm]').forEach(b=>b.onclick=()=>{const on=b.getAttribute('aria-pressed')!=='true';b.setAttribute('aria-pressed',String(on));b.classList.toggle('is-on',on);b.querySelector('.sat-perm-check').textContent=on?'✓':''});
 const role=dlg.querySelector('#satRole'),kids=dlg.querySelector('#satKidsConfig'); role.onchange=()=>{kids.hidden=role.value!=='kids';if(role.value==='kids'){const v=dlg.querySelector('[data-perm="validation"]');v.setAttribute('aria-pressed','true');v.classList.add('is-on');v.querySelector('.sat-perm-check').textContent='✓'}};
 dlg.querySelector('#satForm').onsubmit=e=>{e.preventDefault();const m=old||{id:satId(),status:'active',createdAt:Date.now()};m.name=dlg.querySelector('#satName').value.trim();m.role=role.value;const on=k=>dlg.querySelector(`[data-perm="${k}"]`)?.getAttribute('aria-pressed')==='true';m.permissions={casa:on('casa'),compras:on('compras'),planos:on('planos'),convites:on('convites')};m.requiresValidation=m.role==='kids'||on('validation');m.gamification=m.role==='kids'?{basePoints:+dlg.querySelector('#satBasePoints').value||10,medalAt:+dlg.querySelector('#satMedalAt').value||50,trophyAt:+dlg.querySelector('#satTrophyAt').value||100,skinAt:+dlg.querySelector('#satSkinAt').value||150}:m.gamification;if(!m.name){dlg.querySelector('#satName').focus();return}if(!old)data.members.push(m);saveSatellites(data);close();renderSatellites()};
 dlg.showModal()
}
function renderSatellites(){
 ...
 const networkIcon=`<svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="12" cy="12" r="2.4"/><circle cx="5.5" cy="6.5" r="2"/><circle cx="18.5" cy="6.5" r="2"/><circle cx="5.5" cy="17.5" r="2"/><circle cx="18.5" cy="17.5" r="2"/><path d="M10.2 10.5 7 7.8m7-1  -1.8 3.7m1.6 3 3.2 2.7m-7-2.7-3.1 2.7"/></svg>`;
 app.innerHTML=`<div class="sat-page"><section class="sat-hero sat-hero-ideal"><div class="sat-hero-copy"><div class="eyebrow">REDE BERTH.A</div><h2>Everything handled.<br>Nothing carried.</h2><p>Você conta com a BERTH.A. A BERTH.A conta com os seus.</p></div><span class="sat-hero-icon">${networkIcon}</span></section><div class="sat-actions sat-main-actions"><button class="primary" id="addSatellite">＋ Adicionar satélite</button>${permission!=='granted'&&permission!=='unsupported'?'<button class="secondary" id="satEnableNotifications">Ativar notificações</button>':''}</div><div class="sat-cloud-note"><span aria-hidden="true">◎</span><span>Você decide quem participa e quais partes da sua vida cada satélite pode acessar.</span></div>${events.length?`<section class="sat-card card"><div class="panel-head"><div><h3>Atividade da rede</h3><p class="note">${unread?`${unread} nova${unread===1?'':'s'} para o Owner.`:'Tudo visto.'}</p></div><button type="button" class="secondary" id="satMarkRead">Marcar como visto</button></div><div class="sat-event-list">${events.slice(0,6).map(e=>`<div class="sat-event ${e.read?'':'unread'}"><span>✓</span><div><strong>${escapeHtml(e.memberName)} concluiu ${escapeHtml(e.taskName)}</strong><small>${new Date(e.at).toLocaleString('pt-BR',{day:'2-digit',month:'2-digit',hour:'2-digit',minute:'2-digit'})}</small></div></div>`).join('')}</div></section>`:''}<section class="sat-card card"><div class="panel-head"><div><h3>Minha rede</h3><p class="note">Pessoas que participam dos recortes que você compartilha.</p></div><span class="pill">${members.length}</span></div>${members.length?members.map(m=>`<div class="sat-member"><span class="sat-avatar">${m.role==='kids'?'★':'◎'}</span><span><strong>${escapeHtml(m.name)}</strong><small>${[m.permissions?.casa?'Casa':'',m.permissions?.compras?'Compras':'',m.permissions?.planos?'Planos':'',m.permissions?.convites?'Convites':''].filter(Boolean).join(' · ')||'Sem módulos compartilhados'}</small></span><span><span class="sat-role">${m.role==='kids'?'KIDS':'ADULTO'}</span><div class="sat-actions"><button type="button" class="secondary" data-sat-preview="${m.id}">Ver perfil</button><button type="button" class="secondary" data-sat-edit="${m.id}">Editar</button></div></span></div>`).join(''):`<div class="sat-empty">Adicione o primeiro satélite para começar a construir sua rede.</div>`}</section></div>`;
...
const SAT_INVITES_KEY='bertha_satellite_invites_v1';
function loadInvites(){try{return JSON.parse(window.berthaHmlStorage.getItem(SAT_INVITES_KEY)||'[]')}catch{return[]}}
function saveInvites(x){window.berthaHmlStorage.setItem(SAT_INVITES_KEY,JSON.stringify(x))}
function openInviteEditor(){ensureSatelliteStyles();ensureSatelliteIdentityV4();const members=loadSatellites().members.filter(m=>m.status!=='removed'&&m.permissions?.convites),d=document.createElement('dialog');d.className='bertha-dialog sat-modal sat-ideal-modal';d.innerHTML=`<form class="modal-card sat-editor-form" id="inviteForm"><div class="modal-head"><div><div class="eyebrow">CONVITES</div><h2>Novo convite</h2></div><button type="button" class="icon-btn casa-modal-x" data-close>×</button></div><div class="sat-editor-fields"><label>Convite<input id="inviteTitle" required placeholder="Ex.: Almoço, academia, cinema"></label><label>Quando<input id="inviteWhen" type="datetime-local"></label></div><div class="sat-permissions"><div class="eyebrow">CONVIDAR</div>${members.length?members.map(m=>`<button type="button" class="sat-permission" data-invite-member="${m.id}" aria-pressed="false"><span class="sat-perm-check"></span><span><strong>${escapeHtml(m.name)}</strong><small>${m.role==='kids'?'Kids':'Adulto'}</small></span></button>`).join(''):'<div class="sat-empty-mini">Nenhum satélite tem permissão para receber convites.</div>'}</div><div class="modal-actions"><div class="grow"></div><button type="button" class="secondary" data-close>Cancelar</button><button class="primary" type="submit">Enviar convite</button></div></form>`;document.body.appendChild(d);d.querySelectorAll('[data-close]').forEach(b=>b.onclick=()=>d.close());d.onclose=()=>d.remove();d.querySelectorAll('[data-invite-member]').forEach(b=>b.onclick=()=>{const on=b.getAttribute('aria-pressed')!=='true';b.setAttribute('aria-pressed',String(on));b.classList.toggle('is-on',on);b.querySelector('.sat-perm-check').textContent=on?'✓':''});d.querySelector('#inviteForm').onsubmit=e=>{e.preventDefault();const title=d.querySelector('#inviteTitle').value.trim(),targets=[...d.querySelectorAll('[data-invite-member][aria-pressed="true"]')].map(b=>b.dataset.inviteMember);if(!title||!targets.length)return;const all=loadInvites();all.unshift({id:satId(),title,when:d.querySelector('#inviteWhen').value,targets,responses:{},createdAt:Date.now()});saveInvites(all);d.close();renderInvites()};d.showModal()}
function renderInvites(){ensureSatelliteStyles();ensureSatelliteIdentityV4();const all=loadInvites(),members=loadSatellites().members;app.innerHTML=`<div class="sat-page"><section class="sat-hero sat-hero-ideal invite-hero"><div class="sat-hero-copy"><div class="eyebrow">CONVITES</div><h2>Life is better<br>when shared.</h2><p>Chame os seus para os momentos que também fazem parte da vida.</p></div><span class="sat-hero-icon">♡</span></section><div class="sat-actions sat-main-actions"><button class="primary" id="newInvite">＋ Novo convite</button></div><section class="sat-card card"><div class="panel-head"><div><h3>Seus convites</h3><p class="note">Participar é sempre uma escolha: Vou, Talvez ou Não vou.</p></div><span class="pill">${all.length}</span></div>${all.length?all.map(i=>`<div class="sat-member invite-card"><span class="sat-avatar">♡</span><span><strong>${escapeHtml(i.title)}</strong><small>${i.when?new Date(i.when).toLocaleString('pt-BR',{day:'2-digit',month:'2-digit',hour:'2-digit',minute:'2-digit'}):'Sem horário definido'} · ${i.targets.map(id=>members.find(m=>String(m.id)===String(id))?.name).filter(Boolean).join(', ')}</small></span></div>`).join(''):'<div class="sat-empty">Nenhum convite enviado ainda.</div>'}</section></div>`;document.querySelector('#newInvite').onclick=openInviteEditor}

...
(()=>{
  ...
  const wishCard=w=>`<article class="wish-card ${w.status==='done'?'is-done':''}"><div class="wish-card-top"><div><span class="eyebrow">${escapeHtml(w.category||'DESEJO')}</span><h3>${escapeHtml(w.name||'Desejo')}</h3></div><button type="button" class="fg-edit" data-wish-edit="${w.id}">Editar</button></div><div class="wish-tags"><span>${wishStatusLabel(w.status)}</span><span>Prioridade ${wishPriorityLabel(w.priority).toLowerCase()}</span>${w.deadline?`<span>Até ${formatDate(w.deadline)}</span>`:''}</div>${Number(w.value||0)>0?`<strong class="wish-value">R$ ${money(Number(w.value))}</strong>`:''}${w.note?`<p>${escapeHtml(w.note)}</p>`:''}<div class="wish-actions">${w.link?`<a class="secondary tiny wish-link" href="${escapeHtml(w.link)}" target="_blank" rel="noopener">Abrir link ↗</a>`:''}${w.goalId?`<span class="wish-goal-chip">Vinculado a uma meta</span>`:''}</div></article>`;
  ...
  function openWishGoal(){
    ...
    dlg.querySelector('#wishGoalForm').onsubmit=e=>{e.preventDefault();const ids=checks.filter(c=>c.checked).map(c=>c.value);if(!ids.length){alert('Selecione pelo menos um desejo.');return;}let goalId=mode.value;if(goalId==='new'){const name=dlg.querySelector('#wishGoalName').value.trim();if(!name){dlg.querySelector('#wishGoalName').focus();return;}const total=all.filter(w=>ids.includes(w.id)).reduce((s,w)=>s+Number(w.value||0),0);goalId=uid();goals.push({id:goalId,name,description:'Meta criada a partir da Lista de Desejos',periods:[{id:uid(),label:'Objetivo',min:total,max:total,contributions:[]}],createdAt:Date.now(),updatedAt:Date.now()});goalSave(goals);}const raw=wishLoad();wishSave(raw.map(w=>ids.includes(w.id)?{...w,goalId,status:'goal',updatedAt:Date.now()}:w));close();renderFinanceiro();};
  }

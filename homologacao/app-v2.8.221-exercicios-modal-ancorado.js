...
function openExerciseWeekPlan(){
 const d=loadEx(),w=exWeeklyPlan(d),dlg=document.createElement('dialog');dlg.className='bertha-dialog exercise-dialog';
...
function openExercisePlan(id=null){
 ...
 <div class="exercise-cycle-section exercise-plan-icon-section"><span class="exercise-cycle-label">Ícone do treino</span><p class="exercise-cycle-help">Escolha um símbolo para identificar rapidamente o treino. Automático usa o tipo ou o nome como referência.</p><div class="exercise-icon-tools"><input id="exerciseIconSearch" type="search" placeholder="Buscar ícone…" aria-label="Buscar ícone do treino"><div class="exercise-icon-categories">${[['all','Todos'],['cardio','Cardio'],['strength','Força'],['body','Corpo'],['mind','Bem-estar']].map(([k,l])=>`<button type="button" class="exercise-icon-filter ${k==='all'?'selected':''}" data-icon-filter="${k}">${l}</button>`).join('')}</div></div><div class="exercise-icon-picker">${[['','Automático','all'],['walk','Caminhada','cardio'],['run','Corrida','cardio'],['treadmill','Esteira','cardio'],['bike','Bicicleta','cardio'],['stairs','Escada','cardio'],['elliptical','Elíptico','cardio'],['rowing','Remo','cardio'],['rope','Pular corda','cardio'],['dance','Dança','cardio'],['swim','Natação','cardio'],['boxing','Boxe','cardio'],['hiit','HIIT','cardio'],['weights','Halteres','strength'],['strength','Força','strength'],['upper','Superior','strength'],['lower','Inferior','strength'],['legs','Pernas','strength'],['glutes','Glúteos','strength'],['arms','Braços','strength'],['back','Costas','strength'],['chest','Peito','strength'],['core','Core','strength'],['fullbody','Corpo inteiro','body'],['functional','Funcional','body'],['stretch','Alongamento','body'],['mobility','Mobilidade','body'],['balance','Equilíbrio','body'],['yoga','Yoga','mind'],['pilates','Pilates','mind'],['meditation','Meditação','mind'],['breathing','Respiração','mind'],['wellness','Bem-estar','mind']].map(([k,l,c])=>`<button type="button" class="exercise-icon-choice ${(p.icon||'')===k?'selected':''}" data-plan-icon="${k}" data-icon-category="${c}" data-icon-name="${l.toLowerCase()}" aria-label="${l}"><span>${k?exerciseIconSvg(k):exerciseTypeIcon(p.type)}</span><small>${l}</small></button>`).join('')}</div><div class="exercise-icon-empty" hidden>Nenhum ícone encontrado.</div><input type="hidden" id="eIcon" value="${escapeHtml(p.icon||'')}"></div>
 ...
 const syncAutoPlanIcon=()=>{const auto=dlg.querySelector('[data-plan-icon=""] span');if(auto)auto.innerHTML=exerciseTypeIcon(dlg.querySelector('#eType').value)};dlg.querySelector('#eType').addEventListener('change',syncAutoPlanIcon);syncAutoPlanIcon();dlg.querySelectorAll('[data-plan-icon]').forEach(b=>b.addEventListener('click',()=>{dlg.querySelectorAll('[data-plan-icon]').forEach(x=>x.classList.remove('selected'));b.classList.add('selected');dlg.querySelector('#eIcon').value=b.dataset.planIcon||''}));const iconSearch=dlg.querySelector('#exerciseIconSearch'),iconFilters=[...dlg.querySelectorAll('[data-icon-filter]')];let activeIconFilter='all';const filterExerciseIcons=()=>{const q=(iconSearch?.value||'').trim().toLowerCase();let visible=0;dlg.querySelectorAll('[data-plan-icon]').forEach(b=>{const cat=b.dataset.iconCategory||'all',name=b.dataset.iconName||'';const show=(activeIconFilter==='all'||cat===activeIconFilter||cat==='all')&&(!q||name.includes(q));b.hidden=!show;if(show)visible++});const empty=dlg.querySelector('.exercise-icon-empty');if(empty)empty.hidden=visible>0};iconSearch?.addEventListener('input',filterExerciseIcons);iconFilters.forEach(b=>b.addEventListener('click',()=>{activeIconFilter=b.dataset.iconFilter||'all';iconFilters.forEach(x=>x.classList.toggle('selected',x===b));filterExerciseIcons()}));

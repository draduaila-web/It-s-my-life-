...
// BERTH.A v2.8.221 RC95 — Alimentação: lancheira do Henrique + cozinha inteligente
...
function loadFood(){try{const d=JSON.parse(window.berthaHmlStorage.getItem(FOOD_KEY));if(d)return {...FOOD_BASE,...d,breakfasts:Array.isArray(d.breakfasts)?d.breakfasts:FOOD_BASE.breakfasts,lunchboxes:Array.isArray(d.lunchboxes)?d.lunchboxes:FOOD_BASE.lunchboxes,dinners:Array.isArray(d.dinners)?d.dinners:FOOD_BASE.dinners,mealLog:d.mealLog||{},cookingHistory:Array.isArray(d.cookingHistory)?d.cookingHistory:[],lastCookedRecipeIds:Array.isArray(d.lastCookedRecipeIds)?d.lastCookedRecipeIds:[]};}catch{}const base=JSON.parse(JSON.stringify(FOOD_BASE));base.cookingHistory=[];base.lastCookedRecipeIds=[];return base;}
function saveFood(d){window.berthaHmlStorage.setItem(FOOD_KEY,JSON.stringify(d));syncFoodPrepIntelligence(d);}
...
function foodPlanItem(day,type,d=loadFood()){
 ...
 if(type==='lunchbox'){const x=(d.lunchboxes||[]).find(v=>v[0]===day);const name=x?.[1]||'Lancheira a definir';return {name,recipe:recipeForMealName(name)}}

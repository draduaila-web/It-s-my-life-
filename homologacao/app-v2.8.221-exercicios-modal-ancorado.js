...
function studyMapDialog(id,onDone){
 ...
 document.body.appendChild(dlg);bindStudyIconPicker(dlg,map.icon||'auto');const close=()=>dlg.close();dlg.querySelector('.study-v10-x').onclick=close;dlg.querySelector('[data-map-cancel]').onclick=close;
...
function contentDialog(id){
 const items=loadStudyContents(),e=items.find(x=>x.id===id),dlg=document.createElement("dialog");dlg.className="study-v10-dialog study-module-dialog";dlg.innerHTML=`<div class="study-v10-modal"><div class="study-v10-head"><div><div class="eyebrow">ESTUDOS</div><h2>${e?"Editar conteúdo":"Adicionar conteúdo"}</h2><p>MBA, Tarot, áudio, aula ou outro material.</p></div><button class="study-v10-x">×</button></div>${field("Nome",`<input data-title value="${escapeHtml(e?.title||"")}" placeholder="Ex.: MBA · Aula 04">`)}${field("Frente de estudo",`<input data-group value="${escapeHtml(e?.group||"")}" placeholder="Ex.: MBA, Tarot, TCDF">`)}${field("Link para abrir o conteúdo",`<input data-url value="${escapeHtml(e?.url||"")}" placeholder="https://…">`)}${field("Ícone do estudo",studyIconPicker(e?.icon||"auto"))}${field("Duração planejada",`<div class="study-v10-duration"><input data-dur type="number" min="1" value="${e?.durationValue||e?.minutes||60}"><select data-unit><option value="minutes">minutos</option><option value="hours" ${e?.durationUnit==="hours"?"selected":""}>horas</option></select></div>`)}${field("Observação",`<textarea data-note rows="3">${escapeHtml(e?.note||"")}</textarea>`)}<div class="study-v10-actions">${e?'<button class="danger" data-delete>Excluir</button>':""}<button class="secondary" data-cancel>Cancelar</button><button class="primary" data-save>Salvar</button></div></div>`;document.body.appendChild(dlg);bindStudyIconPicker(dlg,e?.icon||"auto");dlg.querySelector(".study-v10-x").onclick=()=>dlg.close();dlg.querySelector("[data-cancel]").onclick=()=>dlg.close();dlg.querySelector("[data-delete]")?.addEventListener("click",()=>{if(!confirm(`Excluir “${e.title}”?`))return;saveStudyContents(loadStudyContents().filter(x=>x.id!==e.id));dlg.close();setTimeout(renderEstudos,0)});dlg.querySelector("[data-save]").onclick=()=>{const title=dlg.querySelector("[data-title]").value.trim();if(!title)return;const v=Math.max(1,+dlg.querySelector("[data-dur]").value||60),u=dlg.querySelector("[data-unit]").value,obj={id:e?.id||`sc-${Date.now()}`,title,group:dlg.querySelector("[data-group]").value.trim(),url:dlg.querySelector("[data-url]").value.trim(),durationValue:v,durationUnit:u,minutes:u==="hours"?v*60:v,note:dlg.querySelector("[data-note]").value.trim(),icon:dlg.dataset.studyIcon||"auto"};saveStudyContents(e?items.map(x=>x.id===e.id?obj:x):[...items,obj]);dlg.close();renderEstudos()};dlg.onclose=()=>dlg.remove();dlg.showModal()
}

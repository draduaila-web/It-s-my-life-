...
`;document.head.appendChild(s)}
function satelliteAvatarIcon(key='person'){
 const icons={
 ...
 return `<svg viewBox="0 0 24 24" aria-hidden="true" fill="none" stroke="currentColor" stroke-width="1.55" stroke-linecap="round" stroke-linejoin="round">${icons[key]||icons.person}</svg>`
}
const SATELLITE_AVATARS=[['person','Pessoa'],['people','Rede'],['star','Estrela'],['heart','Coração'],['bolt','Energia'],['home','Casa'],['book','Livro'],['sport','Esporte'],['pet','Pet']];
...
function openSatelliteEditor(id){
 ...
 const syncKids=()=>{const isKids=dlg.querySelector('#satRole').value==='kids';dlg.querySelector('#satKidsBlock').hidden=!isKids;if(isKids){dlg.querySelector('#satValidation').checked=true;const r=dlg.querySelector('#satValidation').closest('.sat-permission');r?.classList.add('is-checked');r?.setAttribute('aria-checked','true');if(!old&&dlg.querySelector('#satAvatarKey').value==='person'){const star=dlg.querySelector('[data-avatar="star"]');star?.click()}}};dlg.querySelector('#satRole').onchange=syncKids;syncKids();

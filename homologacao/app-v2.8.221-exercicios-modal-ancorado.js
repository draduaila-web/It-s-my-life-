...
function openExercisePlan(id=null){
 ...
 <div class="exercise-cycle-section exercise-plan-icon-section"><span class="exercise-cycle-label">Ícone do treino</span><p class="exercise-cycle-help">Escolha o símbolo que identifica este treino. Se não escolher, a BERTH.A usa o ícone do tipo automaticamente.</p><div class="exercise-icon-picker">${[['','Automático'],['walk','Caminhada'],['run','Corrida'],['bike','Bicicleta'],['stairs','Escada'],['boxing','Boxe'],['stretch','Alongamento'],['meditation','Meditação'],['upper','Superior'],['lower','Inferior'],['weights','Pesos'],['core','Core']].map(([k,l])=>`<button type="button" class="exercise-icon-choice ${(p.icon||'')===k?'selected':''}" data-plan-icon="${k}" aria-label="${l}"><span>${k?exerciseIconSvg(k):exerciseTypeIcon(p.type)}</span><small>${l}</small></button>`).join('')}</div><input type="hidden" id="eIcon" value="${escapeHtml(p.icon||'')}"></div>

...
function openExercisePlan(id=null){
 ...
 <div class="exercise-cycle-section exercise-plan-icon-section"><span class="exercise-cycle-label">Ícone do treino</span><p class="exercise-cycle-help">Escolha o símbolo que identifica este treino. Se não escolher, a BERTH.A usa o ícone do tipo automaticamente.</p><div class="exercise-icon-picker">${[['','Automático'],['strength','Força'],['cardio','Cardio'],['mobility','Mobilidade'],['walk','Caminhada'],['wellness','Bem-estar']].map(([k,l])=>`<button type="button" class="exercise-icon-choice ${(p.icon||'')===k?'selected':''}" data-plan-icon="${k}" aria-label="${l}"><span>${k?exerciseIconSvg(k):exerciseTypeIcon(p.type)}</span><small>${l}</small></button>`).join('')}</div><input type="hidden" id="eIcon" value="${escapeHtml(p.icon||'')}"></div>
...
function getMealsForOffset(offset=0){if(offset===0)return getTodayMeals();const d=loadFood(),key=foodISOForOffset(offset),day=foodDayNameForOffset(offset),prevDay=foodDayNameForOffset(offset-1),breakfast=d.breakfasts.find(x=>x[0]===day)||null,dinner=d.dinners.find(x=>x[0]===day)||d.dinners[0],prev=d.dinners.find(x=>x[0]===prevDay),lunchName=prev?`Marmita: ${String(prev[1]||'').split(' + ')[0]}`:'Marmita / almoço';return [
 {id:'breakfast',label:'Café da manhã',name:breakfast?.[1]||'Café da manhã',recipe:recipeForMealName(breakfast?.[1]||'')},
 {id:'lunch',label:'Almoço',name:lunchName,recipe:recipeForMealName(lunchName)},
 {id:'dinner',label:'Jantar',name:dinner?.[1]||'Jantar',recipe:recipeForMealName(dinner?.[1]||'')}
].map(m=>({...m,log:(d.mealLog[key]?.meals||{})[m.id]||{}}));}
...
function renderReceitas(){
 const d=loadRec();
 app.innerHTML=`<section class="recipe-hero"><div><span class="eyebrow">RECEITAS</span><h2>Know what’s next.</h2><p>Você planeja. A BERTH.A organiza o seu dia.</p></div><span class="recipe-hero-icon">${recipeHeroIcon()}</span></section>${renderRecipeToday()}${renderUpcomingRecipes()}<div class="recipe-tools"><input id="recipeSearch" placeholder="Buscar receita ou ingrediente…"><button type="button" id="addFoodRecipe">＋ Nova receita</button></div><div class="recipe-library-head"><div><span class="eyebrow">BIBLIOTECA</span><h3>Minhas receitas</h3></div><button type="button" id="toggleAllRecipes">Ver todas</button></div><div class="recipe-category-row" id="recipeCategoryRow" hidden>${['Todas',...RECIPE_CATEGORIES].map((c,i)=>`<button type="button" data-recipe-cat="${i?escapeHtml(c):''}" class="${i===0?'active':''}">${escapeHtml(c)}</button>`).join('')}</div><div class="list" id="recipeList"></div><div class="card freezer-rule recipe-freezer"><span class="eyebrow">FREEZER</span><p>Identifique cada preparo com <strong>nome · data · nº de porções · finalização</strong>.</p></div>`;

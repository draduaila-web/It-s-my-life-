...
/* RC138 — Premiações: explicações, ícones editáveis e configuração Kids mais clara */
;(function(){
  if(document.getElementById('rc138-rewards-refine'))return;
  const st=document.createElement('style');st.id='rc138-rewards-refine';st.textContent=`
  .rewards-explainer{padding:14px 15px;border-radius:18px;background:rgba(255,253,249,.78);border:1px solid rgba(126,115,132,.08);color:#756d78;font-size:12px;line-height:1.5}
  .rewards-explainer strong{display:block;margin-bottom:4px;color:#4b4550;font-weight:450;font-size:13px}
  .reward-icon-picker{display:flex;flex-wrap:wrap;gap:7px;margin-top:6px}
  .reward-icon-picker button{-webkit-appearance:none;appearance:none;width:38px;height:38px;border-radius:12px;border:1px solid rgba(126,115,132,.11);background:#fffdfa;color:#777e91;font-size:18px;font-weight:400;display:grid;place-items:center;padding:0}
  .reward-icon-picker button.active{border-color:rgba(216,157,183,.34);box-shadow:inset 0 0 0 1px rgba(216,157,183,.10);background:linear-gradient(135deg,rgba(249,221,230,.48),rgba(255,243,223,.52),rgba(220,235,250,.56))}
  .reward-icon-hidden{position:absolute!important;opacity:0!important;pointer-events:none!important;width:1px!important;height:1px!important}
  .kid-reward-extra{display:grid;grid-template-columns:1fr 1fr;gap:9px;margin-top:9px}
  .kid-reward-extra[hidden]{display:none!important}.kid-reward-extra label{display:grid;gap:5px;font-size:10.5px;font-weight:400;color:#817984}.kid-reward-extra input,.kid-reward-extra select{min-height:42px;border-radius:13px;border:1px solid rgba(126,115,132,.13);background:#fff;padding:9px 10px;font-size:16px;font-weight:400;color:#49434d}
  .recognition-preview{align-items:center!important}.recognition-preview>div{display:grid!important;gap:3px!important;min-width:0!important}.recognition-preview strong{display:block!important;font-weight:420!important;color:#47414b!important;line-height:1.25!important}.recognition-preview small{display:block!important;font-weight:400!important;color:#8a818b!important;line-height:1.35!important;margin:0!important}
  .rewards-pane[data-reward-pane="helpers"] .panel-head h3,.rewards-pane[data-reward-pane="helpers"] .reward-config-head strong,.rewards-pane[data-reward-pane="helpers"] .reward-config-card label{font-weight:400!important}
  @media(max-width:520px){.kid-reward-extra{grid-template-columns:1fr}}
  `;document.head.appendChild(st);
})();

function rewardIconPickerHTML(value,kind){
  const sets={owner:['✦','◇','○','△','☆','♡','⌁','☼'],kids:['◇','○','☆','♕','✦','♡','☼','✿'],helper:['✿','♡','☆','✦','☼','◇','❀','⌁']};
  const arr=sets[kind]||sets.owner, val=value||arr[0];
  return `<input class="reward-icon-hidden" data-icon-value data-or-icon data-rp-icon value="${escapeHtml(val)}"><div class="reward-icon-picker">${arr.map(x=>`<button type="button" data-icon-choice="${escapeHtml(x)}" class="${x===val?'active':''}">${escapeHtml(x)}</button>`).join('')}</div>`;
}

function enhanceRewardsRC138(){
  const ownerPane=document.querySelector('[data-reward-pane="owner"]');
  const kidsPane=document.querySelector('[data-reward-pane="kids"]');
  const helperPane=document.querySelector('[data-reward-pane="helpers"]');
  if(ownerPane&&!ownerPane.querySelector('.rewards-explainer')){
    ownerPane.insertAdjacentHTML('afterbegin','<div class="rewards-explainer"><strong>Como funcionam as conquistas do Owner</strong>Você escolhe o que a BERTH.A deve reconhecer. O critério é o gatilho; o valor/meta define quando ele acontece; a recorrência define se a conquista pode voltar. Ex.: Progresso em 100% + mensal = um reconhecimento a cada mês em que uma dimensão chegar a 100%.</div>');
  }
  if(kidsPane&&!kidsPane.querySelector('.rewards-explainer')){
    kidsPane.insertAdjacentHTML('afterbegin','<div class="rewards-explainer"><strong>Como configurar as premiações Kids</strong>Os pontos vêm das missões. Aqui você define o que será desbloqueado quando a criança atingir cada marco. Cápsulas podem revelar uma surpresa, skin, acessório ou personagem; medalhas e troféus registram conquistas; skins e personagens mudam apenas o universo visual Kids.</div>');
  }
  if(helperPane&&!helperPane.querySelector('.rewards-explainer')){
    helperPane.insertAdjacentHTML('afterbegin','<div class="rewards-explainer"><strong>Reconhecimentos para Helpers</strong>São gestos de agradecimento, sem pontuação ou ranking. Você escolhe o símbolo, o nome e a mensagem que poderá enviar para cada satélite adulto.</div>');
  }

  document.querySelectorAll('[data-owner-reward]').forEach(row=>{
    const inp=row.querySelector('[data-or-icon]'); if(!inp||row.querySelector('.reward-icon-picker'))return;
    const label=inp.closest('label'); const val=inp.value||'✦';
    label.innerHTML='Ícone'+rewardIconPickerHTML(val,'owner');
  });
  document.querySelectorAll('[data-rec-preset]').forEach(row=>{
    const inp=row.querySelector('[data-rp-icon]'); if(!inp||row.querySelector('.reward-icon-picker'))return;
    const label=inp.closest('label'); const val=inp.value||'✿';
    label.innerHTML='Símbolo'+rewardIconPickerHTML(val,'helper');
  });
  document.querySelectorAll('[data-kid-reward]').forEach(row=>{
    const section=row.closest('[data-rewards-kid]'), mid=section?.dataset.rewardsKid, key=row.dataset.kidReward;
    const saved=mid?kidsRewardsFor(mid)[key]:null;
    if(!row.querySelector('.reward-icon-picker')){
      const head=row.querySelector('.reward-config-head');
      const current=saved?.icon||head?.querySelector('.reward-config-icon')?.textContent?.trim()||'◇';
      head?.insertAdjacentHTML('afterend',`<label style="display:grid;gap:5px;font-size:10.5px;color:#817984">Ícone${rewardIconPickerHTML(current,'kids')}</label>`);
      const display=head?.querySelector('.reward-config-icon'); if(display)display.textContent=current;
    }
    if(!row.querySelector('.kid-reward-extra')){
      row.insertAdjacentHTML('beforeend',`<div class="kid-reward-extra"><label>Ao desbloquear<select data-kr-unlock><option value="badge">Registrar conquista</option><option value="surprise">Abrir surpresa</option><option value="skin">Liberar skin/tema</option><option value="accessory">Liberar acessório</option><option value="companion">Liberar personagem</option></select></label><label>Item liberado<input data-kr-item placeholder="Ex.: Skin Céu, Coroa dourada, Luma"></label></div>`);
    }
    const unlock=row.querySelector('[data-kr-unlock]'), item=row.querySelector('[data-kr-item]');
    if(unlock&&saved?.unlock){unlock.value=saved.unlock;unlock.dataset.saved=saved.unlock;}
    if(item&&saved?.item)item.value=saved.item;
    const type=row.querySelector('[data-kr-type]'); const extra=row.querySelector('.kid-reward-extra');
    const sync=()=>{if(!type||!extra)return;extra.hidden=!['egg','skin','companion'].includes(type.value);const sel=row.querySelector('[data-kr-unlock]');if(type.value==='egg'&&sel&&!sel.dataset.saved)sel.value='surprise';if(type.value==='skin'&&sel&&!sel.dataset.saved)sel.value='skin';if(type.value==='companion'&&sel&&!sel.dataset.saved)sel.value='companion';};
    type?.addEventListener('change',sync);sync();
  });

  document.querySelectorAll('.reward-icon-picker button').forEach(btn=>btn.onclick=()=>{const picker=btn.closest('.reward-icon-picker'),wrap=picker.parentElement,hidden=wrap.querySelector('[data-icon-value]');picker.querySelectorAll('button').forEach(x=>x.classList.toggle('active',x===btn));if(hidden)hidden.value=btn.dataset.iconChoice;const card=btn.closest('.reward-config-card');const display=card?.querySelector('.reward-config-icon');if(display)display.textContent=btn.dataset.iconChoice;});
}

const _renderUniversalRewardsRC138=renderUniversalRewards;
renderUniversalRewards=function(){_renderUniversalRewardsRC138();enhanceRewardsRC138();
  // Replace save handlers so icon picker values are persisted.
  const ownerBtn=document.querySelector('#saveOwnerRewards');
  if(ownerBtn){ownerBtn.onclick=()=>{const current=loadOwnerRewards();const rows=[...document.querySelectorAll('[data-owner-reward]')];const vals=rows.map((row,i)=>({...current[i],name:row.querySelector('[data-or-name]').value.trim()||current[i].name,icon:row.querySelector('[data-icon-value]')?.value||current[i].icon||'◇',criterion:row.querySelector('[data-or-criterion]').value,value:Math.max(1,+row.querySelector('[data-or-value]').value||1),recurrence:row.querySelector('[data-or-rec]').value,visual:row.querySelector('[data-or-visual]').value.trim(),description:row.querySelector('[data-or-desc]').value.trim(),active:row.querySelector('[data-or-active]').checked}));saveOwnerRewards(vals);ownerBtn.textContent='Salvo';setTimeout(()=>ownerBtn.textContent='Salvar conquistas',900)}}
  const helperBtn=document.querySelector('#saveRecognitionPresets');
  if(helperBtn){helperBtn.onclick=()=>{const presets=loadRecognitionPresets();const vals=[...document.querySelectorAll('[data-rec-preset]')].map((row,i)=>({...presets[i],icon:row.querySelector('[data-icon-value]')?.value||presets[i].icon||'✿',name:row.querySelector('[data-rp-name]').value.trim()||presets[i].name,message:row.querySelector('[data-rp-message]').value.trim(),active:row.querySelector('[data-rp-active]').checked}));saveRecognitionPresets(vals);helperBtn.textContent='Salvo';setTimeout(()=>helperBtn.textContent='Salvar reconhecimentos',900)}}
  document.querySelectorAll('[data-rewards-kid]').forEach(section=>{const btn=section.querySelector('.saveKidRewards');if(!btn)return;btn.onclick=()=>{const all=loadKidsRewards(),mid=section.dataset.rewardsKid,current=kidsRewardsFor(mid);section.querySelectorAll('[data-kid-reward]').forEach(row=>{const key=row.dataset.kidReward;current[key]={...current[key],name:row.querySelector('[data-kr-name]').value.trim()||current[key].name,points:Math.max(1,+row.querySelector('[data-kr-points]').value||1),type:row.querySelector('[data-kr-type]').value,recurrence:row.querySelector('[data-kr-rec]').value,visual:row.querySelector('[data-kr-visual]').value.trim(),description:row.querySelector('[data-kr-desc]').value.trim(),icon:row.querySelector('[data-icon-value]')?.value||current[key].icon||'◇',unlock:row.querySelector('[data-kr-unlock]')?.value||current[key].unlock||'',item:row.querySelector('[data-kr-item]')?.value.trim()||current[key].item||'',active:row.querySelector('[data-kr-active]').checked}});all[mid]=current;saveKidsRewards(all);btn.textContent='Salvo';setTimeout(()=>btn.textContent='Salvar premiações',900)}});
}
renderKidsRewards=renderUniversalRewards;

...
(() => {
  ...
  function renderRitualDetail(id){
    ...
    let upcoming='';
    if(!isCustom){for(let i=1;i<=8;i++){const d=new Date();d.setDate(d.getDate()+i);const x=ritualDetailData(id,d);if(x)upcoming+=`<div class="bertha-ritual-upcoming"><span>${d.toLocaleDateString('pt-BR',{day:'2-digit',month:'short'}).replace('.','')}</span><div><strong>${esc(x.title)}</strong><small>${esc(x.subtitle||'')}</small></div></div>`;}}
  ...
  function selfcareProcedureDialog(){
    const ev=ritualDetailData('autocuidado',new Date());if(!ev)return;
    const products=ritualProducts('autocuidado',ev,defaultRituals()[1]);
    const d=document.createElement('dialog');d.className='bertha-dialog bertha-task-dialog bertha-ritual-dialog';
    d.innerHTML=`<form class="bertha-task-modal" method="dialog"><div class="bertha-task-modal-head"><div><span>AUTOCUIDADO</span><h2>${esc(ev.title)}</h2></div><button type="button" data-close>×</button></div><div class="bertha-task-modal-body"><label class="bertha-task-field"><span>Como fazer <small>uma instrução por linha</small></span><textarea data-steps rows="7">${esc((ev.tasks||[]).join('\n'))}</textarea></label><label class="bertha-task-field"><span>Produtos <small>um por linha</small></span><textarea data-products rows="7" placeholder="Ex.: Nome do produto">${esc(products.join('\n'))}</textarea></label><div class="bertha-schedule-note">Os produtos ficam ligados especificamente a este cuidado e podem ser enviados para a Lista de Compras.</div></div><div class="bertha-task-modal-actions"><span></span><div><button class="bertha-task-cancel" type="button" data-close>Cancelar</button><button class="bertha-task-save" type="button" data-save>Salvar</button></div></div></form>`;
    document.body.appendChild(d);d.querySelectorAll('[data-close]').forEach(b=>b.onclick=()=>{d.close();d.remove()});d.querySelector('[data-save]').onclick=()=>{saveSelfcareProcedure(ev,ritualLines(d.querySelector('[data-steps]').value),ritualLines(d.querySelector('[data-products]').value));d.close();d.remove();rerender()};d.showModal();
  }

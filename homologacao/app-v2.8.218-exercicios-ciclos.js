...
function renderExercicios(){
 ...
 <div class="list exercise-plan-list">${d.plans.map(p=>{const c=d.cycles.find(x=>String(x.id)===String(p.cycleId));return `<div class="card exercise-plan" data-plan="${p.id}"><div class="exercise-plan-main"><span class="exercise-icon">${exerciseTypeIcon(p.type)}</span><div class="exercise-plan-copy"><strong>${escapeHtml(p.name)}</strong><span>${escapeHtml(p.type)} · ${exMinutes(p)} min · ${escapeHtml(exPeriodText(p))}</span>${p.frequency==='X vezes por semana'?`<small>${exWeeklyDone(p,d)}/${p.weeklyTarget} nesta semana${c?` · ${escapeHtml(c.name)}`:''}</small>`:p.days.length?`<small>${p.days.join(' · ')}${c?` · ${escapeHtml(c.name)}`:''}</small>`:`<small>${escapeHtml(p.frequency)}${c?` · ${escapeHtml(c.name)}`:''}</small>`}${p.link?`<a class="exercise-external-link" href="${escapeHtml(p.link)}" target="_blank" rel="noopener" data-ex-link="${p.id}">Abrir treino ↗</a>`:''}</div></div><div class="exercise-plan-actions"><span class="pill">${p.active?'Ativo':'Pausado'}</span>${p.active?`<button class="secondary" type="button" data-start-plan="${p.id}">Iniciar agora</button>`:''}</div></div>`}).join('')}</div>

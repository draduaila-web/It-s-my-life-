...
function normalizeExCycle(c={}){return {id:c.id||uid(),name:String(c.name||'Novo ciclo'),weeks:Math.max(1,+c.weeks||4),startDate:c.startDate||new Date().toISOString().slice(0,10),icon:c.icon||'cycle',note:c.note||'',active:c.active!==false};}
...
function exerciseTypeIcon(type='Outro'){const t=String(type).toLowerCase();if(t.includes('cardio')||t.includes('caminh'))return `<svg viewBox="0 0 32 32"><path d="M4 17h5l2-5 4 11 3-7 2 1h8"/><path d="M7 8c2-3 7-3 9 1 2-4 8-4 10 0 3 6-4 12-10 16C10 21 3 15 7 8z"/></svg>`;if(t.includes('força')||t.includes('forca'))return `<svg viewBox="0 0 32 32"><path d="M5 12v8M9 9v14M23 9v14M27 12v8M9 16h14"/></svg>`;if(t.includes('yoga')||t.includes('mobil')||t.includes('pilates'))return `<svg viewBox="0 0 32 32"><circle cx="16" cy="7" r="2.5"/><path d="M16 10v8m0-4-6 4m6-4 6 4m-6 0-5 7m5-7 5 7"/></svg>`;return `<svg viewBox="0 0 32 32"><path d="M6 22c4-8 7-12 10-12s5 4 10 12"/><path d="M9 22h14"/></svg>`}
function exerciseCycleIcon(){return `<svg viewBox="0 0 32 32"><path d="M25 11a10 10 0 0 0-17-2l-2 3"/><path d="M6 7v5h5"/><path d="M7 21a10 10 0 0 0 17 2l2-3"/><path d="M26 25v-5h-5"/></svg>`}
...
function renderExercicios(){
 ...
 <div class="exercise-cycle-list">${d.cycles.filter(c=>c.active!==false).map(c=>{const pg=exerciseCycleProgress(c,d),count=d.plans.filter(p=>String(p.cycleId)===String(c.id)).length;return `<div class="card exercise-cycle" data-cycle="${c.id}"><div class="exercise-cycle-head"><span class="exercise-icon">${exerciseCycleIcon()}</span><div class="exercise-cycle-copy"><strong>${escapeHtml(c.name)}</strong><span>Semana ${pg.week} de ${c.weeks} · ${count} treino${count===1?'':'s'}</span><small>${pg.done}${pg.target?`/${pg.target}`:''} realizações no ciclo</small></div></div><div class="exercise-cycle-progress"><i style="width:${pg.pct}%"></i></div></div>`}).join('')||`<div class="empty compact"><strong>Nenhum ciclo criado.</strong><span>Agrupe treinos em ciclos de 4, 6, 8 semanas ou no período que fizer sentido.</span></div>`}</div>
...
function openExerciseCycle(id=null){const d=loadEx(),current=id?d.cycles.find(x=>String(x.id)===String(id)):null,c=normalizeExCycle(current||{}),dlg=document.createElement('dialog');dlg.className='bertha-dialog exercise-dialog';dlg.innerHTML=`<form class="bertha-modal study-v10-modal" id="exCycleForm"><div class="bertha-modal-head"><div><div class="eyebrow">EXERCÍCIOS · CICLO</div><h2>${current?'Editar ciclo':'Novo ciclo'}</h2><p>Organize um período de treino sem transformar o dia em uma agenda rígida.</p></div><button type="button" data-close>×</button></div>${field('Nome do ciclo',`<input id="cName" required maxlength="70" value="${escapeHtml(current?c.name:'')}" placeholder="Ex.: Fortalecimento 8 semanas">`)}<div class="form-grid">${field('Duração',`<input id="cWeeks" type="number" min="1" max="52" value="${c.weeks}">`)}${field('Início',`<input id="cStart" type="date" value="${escapeHtml(c.startDate)}">`)}</div>${field('Observação',`<textarea id="cNote" rows="3" placeholder="Objetivo, progressão ou orientação geral">${escapeHtml(c.note||'')}</textarea>`)}${field('Status',`<select id="cActive"><option value="1" ${c.active?'selected':''}>Ativo</option><option value="0" ${!c.active?'selected':''}>Pausado</option></select>`)}<div class="study-v10-actions">${current?'<button type="button" class="danger" id="deleteExCycle">Excluir</button>':''}<button type="button" class="secondary" data-close>Cancelar</button><button class="primary" type="submit">Salvar</button></div></form>`;document.body.appendChild(dlg);const close=()=>dlg.close();dlg.querySelectorAll('[data-close]').forEach(b=>b.onclick=close);dlg.onclose=()=>dlg.remove();dlg.querySelector('#deleteExCycle')?.addEventListener('click',()=>{if(!confirm(`Excluir o ciclo “${c.name}”? Os treinos serão mantidos fora do ciclo.`))return;d.cycles=d.cycles.filter(x=>String(x.id)!==String(c.id));d.plans=d.plans.map(p=>String(p.cycleId)===String(c.id)?{...p,cycleId:''}:p);saveEx(d);close();renderExercicios()});dlg.querySelector('#exCycleForm').onsubmit=e=>{e.preventDefault();const obj={...c,name:dlg.querySelector('#cName').value.trim(),weeks:Math.max(1,+dlg.querySelector('#cWeeks').value||4),startDate:dlg.querySelector('#cStart').value,note:dlg.querySelector('#cNote').value.trim(),active:dlg.querySelector('#cActive').value==='1'};d.cycles=current?d.cycles.map(x=>String(x.id)===String(c.id)?obj:x):[...d.cycles,{...obj,id:uid()}];saveEx(d);close();renderExercicios();};dlg.showModal();}

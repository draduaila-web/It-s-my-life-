...
(() => {
  ...
  function ritualProducts(id,ev,r){const st=ritualProductsStore(),key=ritualProductKey(id,ev);if(id==='autocuidado'&&Array.isArray(ev?.products)&&ev.products.length)return ev.products;if(Array.isArray(st[key]))return st[key];if(id==='capilar')return capillaryProductsFromEvent(ev);if(id!=='autocuidado'&&Array.isArray(r?.products))return r.products;return[];}
  ...
  function renderRitualDetail(id){
    ...
    let upcoming='';
    if(!isCustom){for(let i=1;i<=8;i++){const d=new Date();d.setDate(d.getDate()+i);const x=ritualDetailData(id,d);if(x){const tag=id==='autocuidado'?'button':'div',attr=id==='autocuidado'?` type="button" data-edit-selfcare-day="${x.cycleDay}"`:'';upcoming+=`<${tag} class="bertha-ritual-upcoming"${attr}><span>${d.toLocaleDateString('pt-BR',{day:'2-digit',month:'short'}).replace('.','')}</span><div><strong>${esc(x.title)}</strong><small>${esc(x.subtitle||'')}</small></div></${tag}>`;}}}

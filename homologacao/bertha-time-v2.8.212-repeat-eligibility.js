...
(() => {
  ...
  function capillaryEvent(d){
    const base=capillaryBaseEvent(d); if(!base)return null;
    const n=capillaryDay(dayStart(d)), saved=capillaryTemplateStore()[String(n)]||{};
    return {...base,...saved,date:base.date,type:base.type,tasks:Array.isArray(saved.tasks)?saved.tasks:(base.tasks||[]),products:Array.isArray(saved.products)?saved.products:undefined,includeInMyDay:saved.includeInMyDay!==false};
  }
  ...
  function selfcareEvent(d){
    const diff=Math.floor((dayStart(d)-SELFCARE_START)/86400000), idx=((diff%14)+14)%14, base=selfcareTemplateItem(idx+1);
    return {...base,date:dayStart(d),cycleDay:idx+1,type:'selfcare',period:base.period||'night',subtitle:`Dia ${idx+1} / 14`};
  }
  ...
  function ritualSourcesForToday(){
    ...
    if(cap && cap.includeInMyDay!==false){
      const s=ritualSetting('capilar',cap.type);
      out.push({id:`ritual:capilar:${iso(d)}:${cap.type}`,source:'Ritual Capilar',title:`Ritual Capilar · ${cap.title}`,date:iso(d),minutes:cap.minutes,time:s.time||'',period:cap.period||s.period||'morning',priority:s.priority||'Normal',notify:!!s.notify,notifyOffset:s.notifyOffset||'at-time',kind:'ritual'});
    }
    const sc=selfcareEvent(d), ss=ritualSetting('autocuidado','selfcare');
    if(!sc.optional && sc.includeInMyDay!==false) out.push({id:`ritual:autocuidado:${iso(d)}`,source:'Autocuidado',title:`Autocuidado · ${sc.title}`,date:iso(d),minutes:sc.minutes,time:ss.time||'',period:sc.period||ss.period||'night',priority:ss.priority||'Normal',notify:!!ss.notify,notifyOffset:ss.notifyOffset||'at-time',kind:'ritual'});
  ...
  function renderRitualDetail(id){
    ...
    let upcoming='';
    if(!isCustom){for(let i=1;i<=8;i++){const d=new Date();d.setDate(d.getDate()+i);const x=ritualDetailData(id,d);if(x){const editable=id==='autocuidado'||id==='capilar',tag=editable?'button':'div',attr=id==='autocuidado'?` type="button" data-edit-selfcare-day="${x.cycleDay}"`:id==='capilar'?` type="button" data-edit-capillary-day="${capillaryDay(d)}"`:'';upcoming+=`<${tag} class="bertha-ritual-upcoming"${attr}><span>${d.toLocaleDateString('pt-BR',{day:'2-digit',month:'short'}).replace('.','')}</span><div><strong>${esc(x.title)}</strong><small>${esc(x.subtitle||'')}</small></div>${editable?'<b>›</b>':''}</${tag}>`;}}}
  ...
  function selfcareDayEditor(day){
    const n=Math.max(1,Math.min(14,Number(day)||1)), x=selfcareTemplateItem(n), products=ritualProducts('autocuidado',{...x,cycleDay:n},defaultRituals()[1]);
    const d=document.createElement('dialog');d.className='bertha-dialog bertha-task-dialog bertha-ritual-dialog';
    d.innerHTML=`<form class="bertha-task-modal" method="dialog"><div class="bertha-task-modal-head"><div><span>AUTOCUIDADO · DIA ${n}/14</span><h2>Editar cuidado</h2></div><button type="button" data-close>×</button></div><div class="bertha-task-modal-body">
      ...
      <div class="bertha-schedule-note">Este ajuste altera o Dia ${n} do ciclo de 14 dias e será repetido nos próximos ciclos. A ordem do ciclo permanece preservada.</div>
    </div><div class="bertha-task-modal-actions"><span></span><div><button class="bertha-task-cancel" type="button" data-close>Cancelar</button><button class="bertha-task-save" type="button" data-save>Salvar</button></div></div></form>`;
    document.body.appendChild(d);d.querySelectorAll('[data-close]').forEach(b=>b.onclick=()=>{d.close();d.remove()});d.querySelectorAll('[data-period]').forEach(b=>b.onclick=()=>d.querySelectorAll('[data-period]').forEach(z=>z.classList.toggle('active',z===b)));
    d.querySelector('[data-save]').onclick=()=>{const all=selfcareTemplateStore(), vals=ritualLines(d.querySelector('[data-products]').value);all[String(n)]={title:d.querySelector('[data-title]').value.trim()||x.title,minutes:Math.max(0,+d.querySelector('[data-minutes]').value||0),period:d.querySelector('[data-period].active')?.dataset.period||'night',tasks:ritualLines(d.querySelector('[data-steps]').value),products:vals,includeInMyDay:d.querySelector('[data-myday]').checked,optional:!!x.optional};saveSelfcareTemplateStore(all);const ev=selfcareEvent(new Date(SELFCARE_START.getTime()+(n-1)*86400000)),st=ritualProductsStore();st[ritualProductKey('autocuidado',ev)]=vals;saveRitualProductsStore(st);d.close();d.remove();rerender();};d.showModal();
  }

  function capillaryDayEditor(day){
    const total=capillaryCycleTotal(), n=Math.max(1,Math.min(total,Number(day)||1));
    const date=new Date(CAPILLARY_START); date.setDate(CAPILLARY_START.getDate()+n-1);
    const x=capillaryEvent(date); if(!x)return;
    const products=ritualProducts('capilar',x,defaultRituals()[0]);
    const d=document.createElement('dialog');d.className='bertha-dialog bertha-task-dialog bertha-ritual-dialog';
    d.innerHTML=`<form class="bertha-task-modal" method="dialog"><div class="bertha-task-modal-head"><div><span>RITUAL CAPILAR · DIA ${n}/${total}</span><h2>Editar cuidado</h2></div><button type="button" data-close>×</button></div><div class="bertha-task-modal-body">
      <label class="bertha-task-field"><span>Nome</span><input data-title value="${esc(x.title||'')}"></label>
  ...
  function fullSelfcareSchedule(){
    ...
    for(let i=0;i<14;i++){
      ...
      if(!ev)continue;
      rows+=`<button type="button" class="bertha-full-schedule-row bertha-full-selfcare-edit" data-selfcare-cycle-day="${i+1}"><span>${date.toLocaleDateString('pt-BR',{day:'2-digit',month:'short'}).replace('.','')}</span><div><strong>${esc(ev.title)}</strong><small>Dia ${i+1} de 14${ev.optional?' · Livre':` · ${durationText(ev.minutes)} · ${taskLabelPeriod(ev.period||ritualSetting('autocuidado',ev.type).period||'night')}`}${ev.includeInMyDay===false?' · fora do Meu Dia':''}</small></div><b>›</b></button>`;
    }
    d.querySelector('.bertha-modal-content').innerHTML=`<div class="bertha-full-schedule">${rows}</div>`;
    d.querySelectorAll('[data-selfcare-cycle-day]').forEach(b=>b.onclick=()=>{d.close();d.remove();selfcareDayEditor(b.dataset.selfcareCycleDay)});
  }


  function fullCapillarySchedule(){
    const d=modalShell('Cronograma capilar','45 dias'); let rows='';
    for(let i=0;i<45;i++){const date=new Date(CAPILLARY_START);date.setDate(CAPILLARY_START.getDate()+i);const e=capillaryEvent(date);if(!e)continue;rows+=`<button type="button" class="bertha-full-schedule-row bertha-full-capillary-edit" data-capillary-cycle-day="${i+1}"><span>${date.toLocaleDateString('pt-BR',{day:'2-digit',month:'short'}).replace('.','')}</span><div><strong>${esc(e.title)}</strong><small>Dia ${i+1} de 45 · ${durationText(e.minutes)} · ${taskLabelPeriod(e.period||ritualSetting('capilar',e.type).period||'morning')}${e.includeInMyDay===false?' · fora do Meu Dia':''}</small></div><b>›</b></button>`;}
    d.querySelector('.bertha-modal-content').innerHTML=`<div class="bertha-full-schedule">${rows}</div>`;
    d.querySelectorAll('[data-capillary-cycle-day]').forEach(b=>b.onclick=()=>{d.close();d.remove();capillaryDayEditor(b.dataset.capillaryCycleDay)});
  }
  function selfcareProcedureDialog(){ const ev=selfcareEvent(new Date()); selfcareDayEditor(ev.cycleDay); }
  ...
  function bindRitualDetail(id){
    ...
    document.querySelectorAll('[data-edit-selfcare-day]').forEach(b=>b.onclick=()=>selfcareDayEditor(b.dataset.editSelfcareDay));
    document.querySelectorAll('[data-edit-capillary-day]').forEach(b=>b.onclick=()=>capillaryDayEditor(b.dataset.editCapillaryDay));

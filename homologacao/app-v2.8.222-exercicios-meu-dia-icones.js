...
function exerciseSuggestionForMeuDia(){
 ...
 return `<section class="day-section home-exercise-suggestion"><div class="section-head"><h2>Movimento de hoje</h2><a href="#exercicios">ver exercícios</a></div><a class="home-exercise-card" href="#exercicios"><span class="home-exercise-icon">${exercisePlanIcon(p)}</span><span class="home-exercise-copy"><small>${kicker}</small><strong>${escapeHtml(p.name)} · ${exMinutes(p)} min</strong><em>${escapeHtml(why)} · ${escapeHtml(exPeriodText(p))}</em></span><b>›</b></a></section>`;
}
...
/* v2.8.222 — iPhone: trava a página atrás do modal de Exercícios e ancora o diálogo ao viewport visual */
...
function normalizeExCycle(c={}){return {id:c.id||uid(),name:String(c.name||'Novo ciclo'),weeks:Math.max(1,+c.weeks||4),startDate:c.startDate||new Date().toISOString().slice(0,10),icon:c.icon||'cycle',goal:c.goal||'Fortalecimento',progression:c.progression||'',note:c.note||'',active:c.active!==false};}
...
function renderExercicios(){
 ...
 <div class="exercise-cycle-list">${d.cycles.filter(c=>c.active!==false).map(c=>{const pg=exerciseCycleProgress(c,d),count=d.plans.filter(p=>String(p.cycleId)===String(c.id)).length;return `<div class="card exercise-cycle" data-cycle="${c.id}"><div class="exercise-cycle-head"><span class="exercise-icon">${exerciseCycleIcon(c.icon)}</span><div class="exercise-cycle-copy"><strong>${escapeHtml(c.name)}</strong><span>${escapeHtml(c.goal||'Ciclo de treino')} · Semana ${pg.week} de ${c.weeks}</span><small>${count} treino${count===1?'':'s'} · ${pg.done}${pg.target?`/${pg.target}`:''} realizações</small></div></div><div class="exercise-cycle-progress"><i style="width:${pg.pct}%"></i></div></div>`}).join('')||`<div class="empty compact"><strong>Nenhum ciclo criado.</strong><span>Agrupe treinos em ciclos de 4, 6, 8 semanas ou no período que fizer sentido.</span></div>`}</div>

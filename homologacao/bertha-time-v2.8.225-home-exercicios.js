...
(() => {
  ...
  function renderExerciseHome225(){
    try{
      const ex=read('minha-vida.exercicios.v1',{plans:[],cycles:[],sessions:[]})||{};
      const plans=Array.isArray(ex.plans)?ex.plans:[];
      const cycles=Array.isArray(ex.cycles)?ex.cycles:[];
      const sessions=Array.isArray(ex.sessions)?ex.sessions:[];
      const today=iso();
      const doneToday=new Set(sessions.filter(x=>x&&x.date===today).map(x=>String(x.planId||'')));
      const start=new Date(); start.setHours(0,0,0,0); start.setDate(start.getDate()-((start.getDay()+6)%7));
      const weekStart=iso(start);
      const weeklyDone=p=>sessions.filter(x=>x&&String(x.planId||'')===String(p.id)&&x.date>=weekStart&&x.date<=today).length;
      const dow=['Dom','Seg','Ter','Qua','Qui','Sex','Sáb'][new Date().getDay()];
      const due=p=>{
        if(!p||p.active===false||doneToday.has(String(p.id)))return false;
        if(p.untilMode==='date'&&p.untilDate&&p.untilDate<today)return false;
        if(p.frequency==='Dias específicos')return !(p.days||[]).length||(p.days||[]).includes(dow);
        if(p.frequency==='X vezes por semana')return weeklyDone(p)<Math.max(1,+p.weeklyTarget||1);
        return true;
      };
      const active=plans.filter(due);
      if(!active.length)return '';
      const now=new Date();
      const activeCycles=cycles.filter(c=>{
        if(!c||c.active===false)return false;
        const st=new Date((c.startDate||today)+'T12:00:00');
        const en=new Date(st); en.setDate(en.getDate()+Math.max(1,+c.weeks||4)*7);
        return now>=st&&now<en;
      });
      let p=null,c=null;
      for(const cyc of activeCycles){
        const linked=active.filter(x=>String(x.cycleId||'')===String(cyc.id));
        if(!linked.length)continue;
        linked.sort((a,b)=>{
          const at=Math.max(1,+a.weeklyTarget||1),bt=Math.max(1,+b.weeklyTarget||1);
          return (weeklyDone(a)/at)-(weeklyDone(b)/bt);
        });
        p=linked[0]; c=cyc; break;
      }
      if(!p)p=active[0];
      if(!c&&p.cycleId)c=cycles.find(x=>String(x.id)===String(p.cycleId)&&x.active!==false)||null;
      const mins=Math.max(1,Math.round((+p.durationValue||30)*(p.durationUnit==='hours'?60:1)));
      let kicker='MOVIMENTO SUGERIDO';
      if(c){
        const st=new Date((c.startDate||today)+'T12:00:00');
        const elapsed=Math.max(0,Math.floor((now-st)/604800000));
        kicker=`CICLO · ${esc(c.name||'Treino')} · semana ${Math.min(Math.max(1,+c.weeks||4),elapsed+1)}/${Math.max(1,+c.weeks||4)}`;
      }
      const why=p.frequency==='X vezes por semana'?`${weeklyDone(p)}/${Math.max(1,+p.weeklyTarget||1)} feitos nesta semana`:p.frequency==='Dias específicos'?'Programado para hoje':'Cabe no seu dia quando fizer sentido';
      const period=({morning:'Manhã',afternoon:'Tarde',evening:'Noite',flex:'Flexível'})[p.period]||'Flexível';
      const activityIcon=(()=>{
        const key=String(p.icon||'sport');
        const paths={
          glutes:'<path d="M8 8c2 2 2 6 0 8M16 8c-2 2-2 6 0 8M8 12h8"/>',
          upper:'<path d="M5 10h3l2-2 2 4 2-4 2 2h3M8 16h8"/>',
          abs:'<path d="M9 5c-1 3-1 11 0 14M15 5c1 3 1 11 0 14M9 9h6M9 15h6"/>',
          treadmill:'<path d="M6 18h12l2-8H8M8 6h5M11 6l2 4"/><circle cx="8" cy="19" r="1"/><circle cx="18" cy="19" r="1"/>',
          stairs:'<path d="M4 18h5v-4h5v-4h6"/>',
          bike:'<circle cx="7" cy="17" r="4"/><circle cx="18" cy="17" r="4"/><path d="M7 17l4-8 4 8M10 9h5M12 13h6"/>',
          boxing:'<path d="M8 15c-2-1-3-3-2-5l2-4 4 1 2 5-2 5zM14 13l3-3 2 2-2 5-4 1"/>',
          walk:'<circle cx="12" cy="5" r="2"/><path d="M11 8l-2 5 3 2 2 5M9 13l-4 5M12 10l5 2"/>',
          meditation:'<circle cx="12" cy="5" r="2"/><path d="M12 8v5M7 20c1-4 3-6 5-6s4 2 5 6M5 20h14"/>',
          stretch:'<circle cx="12" cy="5" r="2"/><path d="M12 8v6M12 10l-5 3M12 10l5-3M12 14l-4 6M12 14l5 4"/>',
          swim:'<path d="M4 15c2 2 4 2 6 0 2 2 4 2 6 0 2 2 4 2 6 0M9 11l3-3 4 3"/>',
          dance:'<circle cx="12" cy="5" r="2"/><path d="M11 8l-2 6 4 3M9 14l-5 2M13 17l4 4M11 8l5 3"/>',
          sport:'<circle cx="12" cy="12" r="8"/><path d="M8 6l3 4-2 4-4 1M16 6l-3 4 2 4 4 1"/>'
        };
        return `<svg viewBox="0 0 24 24" aria-hidden="true">${paths[key]||paths.sport}</svg>`;
      })();
      return `<section class="day-section home-exercise-suggestion"><div class="section-head"><h2>Movimento de hoje</h2><a href="#exercicios">ver exercícios</a></div><a class="home-exercise-card" href="#exercicios"><span class="home-exercise-icon">${activityIcon}</span><span class="home-exercise-copy"><small>${kicker}</small><strong>${esc(p.name||'Treino')} · ${mins} min</strong><em>${esc(why)} · ${esc(period)}</em></span><b>›</b></a></section>`;
    }catch(_){return ''}
  }

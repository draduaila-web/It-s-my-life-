...
function setTodayMealConsumption(mealId,status,title,source){const {d,key}=getTodayMealState(mealId);d.mealLog[key]=d.mealLog[key]||{};d.mealLog[key].meals=d.mealLog[key].meals||{};d.mealLog[key].meals[mealId]={...(d.mealLog[key].meals[mealId]||{}),consumption:status,consumedAt:new Date().toISOString()};saveFood(d);if(status==='consumed')recordFoodProgress(mealId,title,source);if(state.route==='receitas')renderReceitas();else if(state.route==='alimentacao')renderAlimentacao();}
function renderTodayFoodCard(){const meals=getTodayMeals();return `<div class="card today-food-card"><div class="panel-head"><div><span class="eyebrow">COMIDA DE HOJE · ${escapeHtml(foodDayName().toUpperCase())}</span><h3>Seu dia já começa decidido.</h3></div></div>${meals.map(m=>{const selected=getRecipeById(m.log.selectedRecipeId)||m.recipe,name=m.log.otherText||selected?.name||m.name,done=m.log.consumption==='consumed';return `<div class="food-row"><span>${escapeHtml(m.label)}</span><strong>${escapeHtml(name)}</strong><div style="display:flex;gap:7px;flex-wrap:wrap;margin-top:7px"><button type="button" class="secondary" data-food-done="${m.id}">${done?'✓ Fiz esta':'Fiz esta'}</button><button type="button" class="secondary" data-food-other="${m.id}">Fiz outra</button>${selected?`<button type="button" class="secondary" data-food-view="${m.id}">Ver receita</button>`:''}</div></div>`}).join('')}</div>`;}
function bindTodayFoodActions(){document.querySelectorAll('[data-food-done]').forEach(b=>b.onclick=()=>{const s=getTodayMealState(b.dataset.foodDone),title=s.log.otherText||s.selected?.name||s.meal.name;setTodayMealConsumption(b.dataset.foodDone,'consumed',title,s.log.source||'planned')});document.querySelectorAll('[data-food-other]').forEach(b=>b.onclick=()=>openChooseRecipe(b.dataset.foodOther));document.querySelectorAll('[data-food-view]').forEach(b=>b.onclick=()=>{const s=getTodayMealState(b.dataset.foodView);if(s.selected)openFoodRecipe(s.selected.id,false,b.dataset.foodView)});}
function renderAlimentacao(){
 const d=loadFood();
 app.innerHTML=`<section class="hero"><h2>🍽️ Alimentação</h2><p>Comer bem com menos decisões — com liberdade para mudar de ideia sem perder o planejamento.</p></section>${renderTodayFoodCard()}<div class="food-principle card"><span class="eyebrow">SISTEMA DA CASA</span><strong>Planejado é sugestão, não obrigação.</strong><p>Escolha o que faz sentido hoje. A BERTH.A guarda o planejado e o realizado para aprender com o consumo real.</p></div><div class="section-title">SEMANA 1 · CARDÁPIO</div><div class="list"><div class="card"><div class="panel-head"><h3>Café da manhã</h3><span class="pill">7 dias</span></div>${d.breakfasts.map(x=>`<div class="food-row"><span>${x[0]}</span><strong>${escapeHtml(x[1])}</strong></div>`).join("")}</div><div class="card"><div class="panel-head"><h3>Jantar + marmita</h3><span class="pill">3 pessoas</span></div>${d.dinners.map(x=>{const r=recipeForMealName(x[1]);return `<div class="food-row"><span>${x[0]}</span><strong>${escapeHtml(x[1])}</strong>${x[2]?`<small>marmita → ${x[2]}</small>`:""}${r?`<button type="button" class="secondary" style="margin-top:6px" data-food-recipe="${r.id}">Ver receita</button>`:""}</div>`}).join("")}</div></div><div class="section-title">ROTINA DE PREPARO</div><div class="card food-checklist"><label><input type="checkbox" id="foodCook" ${d.cookingDone?"checked":""}><span><strong>Cozinha quinzenal</strong><small>Produzir bases, porcionar, etiquetar e congelar.</small></span></label></div>`;
...
function renderRecipeToday(){const meals=getTodayMeals();return `<section class="recipe-today card"><div class="recipe-today-head"><div><span class="eyebrow">HOJE · ${escapeHtml(foodDayName().toUpperCase())}</span><h3>O que entra no seu dia</h3></div><a href="#meu-dia">Meu Dia →</a></div><div class="recipe-meals">${meals.map(m=>{const selected=getRecipeById(m.log.selectedRecipeId)||m.recipe,name=m.log.otherText||selected?.name||m.name,done=m.log.consumption==='consumed';return `<div class="recipe-meal ${done?'done':''}"><span class="recipe-meal-icon">${recipeMealIcon()}</span><div class="recipe-meal-copy"><small>${escapeHtml(m.label)}</small><strong>${escapeHtml(name)}</strong></div><div class="recipe-meal-actions"><button type="button" data-food-done="${m.id}">${done?'✓ Feito':'Fiz esta'}</button><button type="button" data-food-other="${m.id}">Fiz outra</button>${selected?`<button type="button" class="recipe-view" data-food-view="${m.id}" aria-label="Ver receita">›</button>`:''}</div></div>`}).join('')}</div></section>`}

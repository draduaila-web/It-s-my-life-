/* BERTH.A — Meu Dia v2 / Motor de Tempo v2.8.65 — Home preservada + referências por área específicas
...
(() => {
  ...
  function sourcesToday(){
    ...
    // Casa — itens do dia e microtarefas entram no Meu Dia quando cabem, sem despejar a casa inteira na Home.
    ...
    if(casa && Array.isArray(casa.areas)){
      ...
      casa.areas.forEach(a=>(a.tasks||[]).forEach(t=>{
        const freq=String(t.freq||'').toLowerCase(),last=casaDay(t.completedAt),mins=Math.max(5,+casaDur[t.id]||15);
        const daily=freq.includes('diário')||freq.includes('diaria')||freq.includes('diária');
        const doneNow=daily?last===today:!!t.done;
        if(doneNow)return;
        const micro=mins<=20;
        if(daily||micro)routines.push({a,t,mins,daily,micro});
      }));
      routines.sort((x,y)=>(y.daily-x.daily)||(x.mins-y.mins)).slice(0,5).forEach(({a,t,mins})=>push({id:`casa:routine:${t.id}`,learningKey:`casa:routine:${t.id}`,source:`Casa · ${a.title||'Rotina'}`,title:t.name||'Rotina da casa',date:today,minutes:mins,configuredMinutes:mins,time:'',period:'flex',priority:'normal',kind:'home-routine',homeWindowHint:casaTimes[t.id]||t.when||''}));
    }

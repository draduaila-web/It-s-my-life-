...
(() => {
  ...
  function renderHomeScheduled(items){
    const timed=items.filter(x=>x.time).sort((a,b)=>(a.time||'99:99').localeCompare(b.time||'99:99')).slice(0,4);
  ...
  function renderHomeTimeline(){
    const m=minsNow(), w=new Date().getDay(); const status=(a,b,prot=false)=>prot?'—':m>=b?'✓':m>=a?'●':'○';
    ...
    return rows.map((r,i)=>`<div class="bertha-time-row ${r[5]?'is-commitment':''}"><i>${status(r[0],r[1],r[4])}</i><b>${r[2]}</b><span>${esc(r[3])}</span></div>`).join('');
  }
  function ensureMeuDiaV164Styles(){
    if(document.getElementById('bertha-meu-dia-v164'))return;
    const st=document.createElement('style');st.id='bertha-meu-dia-v164';st.textContent=`
      /* MEU DIA v164 — fluxo cromático contínuo da marca */

...
function renderExercicios(){
 ...
 <div class="list">${d.plans.map(p=>`<div class="card exercise-plan" data-plan="${p.id}"><div><strong>${escapeHtml(p.name)}</strong><span>${escapeHtml(p.type)} · ${exMinutes(p)} min · ${escapeHtml(exPeriodText(p))}</span>${p.days.length?`<small>${p.days.join(' · ')}</small>`:`<small>${escapeHtml(p.frequency)}</small>`}</div><span class="pill">${p.active?'Ativo':'Pausado'}</span></div>`).join('')}</div>
 ...
 const pickNow=sessionStorage.getItem('bertha.exercise.pickNow')==='1';
 if(pickNow){
   const marker=document.createElement('div');marker.className='card';marker.style.marginBottom='14px';marker.innerHTML='<span class="eyebrow">ESCOLHA PARA AGORA</span><p style="margin:8px 0 0">Toque no treino que você quer fazer agora.</p>';
   const title=document.querySelector('.section-title');if(title)title.parentNode.insertBefore(marker,title);
 }
 document.querySelector('#addPlan').onclick=()=>openExercisePlan();
 document.querySelectorAll('[data-plan]').forEach(el=>el.onclick=()=>{
   const plan=d.plans.find(p=>String(p.id)===String(el.dataset.plan));
   if(pickNow&&plan){
     sessionStorage.removeItem('bertha.exercise.pickNow');
     startViaBertha({id:`exercise:${plan.id}`,learningKey:`exercise-plan-v2:${plan.id}`,source:'Exercícios',title:plan.name,minutes:exMinutes(plan),configuredMinutes:exMinutes(plan),period:plan.period,kind:'exercise'});
     setTimeout(renderExercicios,80);
     return;
   }
   openExercisePlan(el.dataset.plan);
 });

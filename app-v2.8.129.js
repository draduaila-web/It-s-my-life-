...
/* BERTH.A Casa v2.8.129 — corrige escala dos ícones no modal Como Fazer */
...
function renderCasaCore(){
 ...
 const scheduleHtml=`<div class="card casa-schedule-card"><p class="note">Esses horários são referências para encaixe no dia — não uma agenda rígida. Para mudar um horário, edite a própria rotina.</p><div class="casa-schedule">${scheduled.map(t=>`<div class="casa-schedule-row"><span class="casa-time">${escapeHtml(casaTime(t.id))}</span><strong>${escapeHtml(t.name)}</strong><button type="button" class="casa-schedule-edit" data-casa-edit="${escapeHtml(String(t.id))}">Editar</button></div>`).join("")}</div></div>`;

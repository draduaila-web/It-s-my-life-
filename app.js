...
const FIN_PRIV_KEY="minha-vida.financeiro.privado.v1";
const FIN_BASE={
  income: 19172.96,
  expenses: [
...
const FIN_CATEGORIES=["Casa","Alimentação","Transporte","Henrique","Pets","Saúde","Autocuidado","Compras","Lazer","Educação","Trabalho/BEC","Dívidas","Assinaturas","Outros"];
...
function loadFin(){try{const d=JSON.parse(localStorage.getItem(FIN_KEY));if(d)return {...FIN_BASE,...d,expenses:Array.isArray(d.expenses)?d.expenses:[],transactions:Array.isArray(d.transactions)?d.transactions:[]};}catch{}return JSON.parse(JSON.stringify(FIN_BASE));}
function saveFin(d){localStorage.setItem(FIN_KEY,JSON.stringify(d));}
function finPrivate(){return localStorage.getItem(FIN_PRIV_KEY)==="1";}
function toggleFinPrivacy(){localStorage.setItem(FIN_PRIV_KEY,finPrivate()?"0":"1");renderFinanceiro();}
...
function finMonthTransactions(d){const ym=todayISO().slice(0,7);return d.transactions.filter(x=>(x.date||"").slice(0,7)===ym);}
function finTransactionTotal(d){return finMonthTransactions(d).reduce((s,x)=>s+Number(x.value||0),0);}
function finCategoryTotals(d){const out={};finMonthTransactions(d).forEach(x=>{const c=x.category||"Outros";out[c]=(out[c]||0)+Number(x.value||0)});return Object.entries(out).sort((a,b)=>b[1]-a[1]);}
function finHidden(v){return finPrivate()?"R$ ••••••":`R$ ${money(v)}`;}
function renderFinanceiro(){
 const d=loadFin(),known=finKnownTotal(d),txTotal=finTransactionTotal(d),balance=d.income-known-txTotal;
 const min=d.goals.reduce((s,x)=>s+Number(x.min||0),0),max=d.goals.reduce((s,x)=>s+Number(x.max||0),0);
 const cats=finCategoryTotals(d), monthTx=finMonthTransactions(d);
 ...
 <div class="money-hero card"><div class="panel-head"><div><span class="eyebrow">RENDA MENSAL</span><strong>${finHidden(d.income)}</strong></div><button class="secondary" id="finPrivacy">${finPrivate()?"👁 Mostrar valores":"🙈 Ocultar valores"}</button></div><div class="money-grid"><div><span>Orçamento-base conhecido</span><b>${finHidden(known)}</b></div><div><span>Gastos lançados no mês</span><b>${finHidden(txTotal)}</b></div><div><span>Saldo conhecido</span><b>${finHidden(balance)}</b></div></div></div>
 <div class="section-title">LANÇAR GASTO</div><div class="card"><p class="note">Registre aqui o que acontecer ao longo do dia. Cada lançamento fica separado do orçamento-base e alimenta os dados que a Inteligência poderá analisar depois.</p><button class="primary" id="addTransaction">＋ Registrar gasto</button></div>
 <div class="section-title">GASTOS DE ${new Date().toLocaleDateString("pt-BR",{month:"long"}).toUpperCase()}</div><div class="list">${monthTx.slice().reverse().map(transactionHtml).join("")||`<div class="empty compact"><strong>Nenhum gasto lançado.</strong><span>Comece pelo próximo gasto do dia.</span></div>`}</div>
 <div class="section-title">LEITURA RÁPIDA</div><div class="card"><div class="panel-head"><div><span class="eyebrow">POR CATEGORIA</span><h3>Onde o dinheiro está indo</h3></div><span class="pill">${monthTx.length} lançamentos</span></div>${cats.length?`<div class="goal-list">${cats.slice(0,6).map(([c,v])=>`<div class="goal-row"><span>${escapeHtml(c)}</span><strong>${finHidden(v)}</strong></div>`).join("")}</div>`:`<p class="note">Os dados aparecem aqui conforme você registrar seus gastos.</p>`}</div>
 <div class="section-title">ORÇAMENTO BASE</div><div class="list">${d.expenses.map(finExpenseHtml).join("")}</div><button class="add-full secondary" id="addExpense">＋ Adicionar despesa ao orçamento</button>
 <div class="section-title">FUNDO CARRO</div><div class="card goal-card"><div class="panel-head"><div><span class="eyebrow">SETEMBRO → DEZEMBRO</span><h3>Meta acumulada</h3></div><span class="pill today">${finPrivate()?"••••":`R$ ${money(min)}–${money(max)}`}</span></div><div class="goal-list">${d.goals.map((g,i)=>`<div class="goal-row"><span>${escapeHtml(g.month)}</span><strong>${finPrivate()?"••••":`R$ ${money(g.min)}–${money(g.max)}`}</strong><button class="goal-toggle ${g.status==="Concluído"?"done":""}" data-goal="${i}">${g.status==="Concluído"?"✓":"○"}</button></div>`).join("")}</div></div>
 <div class="section-title">FORA DO SEU ORÇAMENTO</div><div class="list">${d.excluded.map(x=>`<div class="card excluded-card"><div><strong>${escapeHtml(x.name)}</strong><span>${x.value?`${finHidden(x.value)} · `:""}${escapeHtml(x.reason)}</span></div><span class="pill">${escapeHtml(x.payer)}</span></div>`).join("")}</div>`;
 document.querySelector("#finPrivacy").onclick=toggleFinPrivacy;
...
function finExpenseHtml(x){return `<article class="card finance-row"><div><strong>${escapeHtml(x.name)}</strong><span>${escapeHtml(x.category)} · ${escapeHtml(x.status)}</span></div><b>${finHidden(x.value)}</b></article>`;}
function transactionHtml(x){return `<article class="card finance-row"><div><strong>${escapeHtml(x.name)}</strong><span>${x.date?formatDate(x.date):""} · ${escapeHtml(x.category||"Variável")} · ${escapeHtml(x.payment||"Pagamento não informado")} · ${escapeHtml(x.account||"")}${x.behavior?` · ${escapeHtml(x.behavior)}`:""}</span></div><b>${finHidden(x.value)}</b></article>`;}
function openFinModal(type){
 const d=loadFin(),dlg=document.createElement("dialog");
 const cats=FIN_CATEGORIES.map(x=>`<option>${x}</option>`).join(""), pays=FIN_PAYMENT.map(x=>`<option>${x}</option>`).join(""), accs=FIN_ACCOUNTS.map(x=>`<option>${x}</option>`).join(""), beh=FIN_BEHAVIOR.map(x=>`<option>${x}</option>`).join("");
 dlg.innerHTML=`<form method="dialog" class="modal-card" id="finForm"><div class="modal-head"><div><div class="eyebrow">💰 FINANCEIRO</div><h2>${type==="expense"?"Adicionar ao orçamento":"Registrar gasto"}</h2></div><button class="icon-btn" value="cancel">×</button></div>
 <label>Descrição<input id="fName" required maxlength="100" placeholder="Ex.: mercado, gasolina, farmácia..."></label>
 <div class="form-grid"><label>Valor<input id="fValue" required type="number" min="0" step="0.01" inputmode="decimal"></label><label>Data<input id="fDate" type="date" value="${todayISO()}"></label></div>
 <div class="form-grid"><label>Categoria<select id="fCategory">${type==="expense"?`<option>Casa</option><option>Dívidas</option><option>Transporte</option><option>Animais</option><option>Cartão</option>`:cats}</select></label><label>${type==="expense"?"Responsável":"Como pagou"}<select id="fPayment">${type==="expense"?`<option>Usuária</option><option>Mãe</option><option>Empregador</option>`:pays}</select></label></div>
 ${type==="transaction"?`<label>Qual conta / cartão<select id="fAccount">${accs}</select></label><div class="form-grid"><label>Tipo de gasto<select id="fBehavior">${beh}</select></label><label>Parcelado?<select id="fInstallment"><option value="0">Não</option><option value="2">2x</option><option value="3">3x</option><option value="4">4x</option><option value="5">5x</option><option value="6">6x</option><option value="10">10x</option><option value="12">12x</option></select></label></div><label>Observação <input id="fNote" maxlength="160" placeholder="Opcional"></label>`:""}
 ...
 dlg.querySelector("#finForm").addEventListener("submit",e=>{e.preventDefault();const v=id=>dlg.querySelector(id)?.value??"";const obj={id:uid(),name:v("#fName").trim(),value:+v("#fValue")||0,category:v("#fCategory"),date:v("#fDate")||todayISO(),updatedAt:Date.now()};if(type==="expense"){obj.payer=v("#fPayment");obj.status="manual";d.expenses.push(obj)}else{obj.payment=v("#fPayment");obj.account=v("#fAccount");obj.behavior=v("#fBehavior");obj.installments=Number(v("#fInstallment"))||0;obj.note=v("#fNote").trim();d.transactions.push(obj)}saveFin(d);dlg.close();dlg.remove();renderFinanceiro()});
}

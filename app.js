...
function openFinModal(type){
 ...
 dlg.innerHTML=`<form method="dialog" class="modal-card" id="finForm"><div class="modal-head"><div><div class="eyebrow">💰 FINANCEIRO</div><h2>${title}</h2></div><button type="button" class="icon-btn" id="closeFinX" aria-label="Fechar">×</button></div>
 ...
 document.body.appendChild(dlg);dlg.showModal();
 const closeFin=()=>{if(dlg.open) dlg.close();dlg.remove()};
 dlg.querySelector("#closeFinX").onclick=closeFin;
 dlg.querySelector("#cancelFin").onclick=closeFin;

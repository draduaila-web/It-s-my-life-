...
function loadCasaRecipes(){
 try{const x=JSON.parse(localStorage.getItem(CASA_RECIPES_KEY)||"null");if(Array.isArray(x))return x;}catch{}
 return JSON.parse(JSON.stringify(CASA_RECIPES));
}
...
function openCasaRecipeEditor(id){
 ...
 o.innerHTML=`<div class="mv-how casa-editor-card"><button type="button" class="mv-how-x" data-close>×</button><div class="eyebrow">CASA · RECEITAS</div><h2>${r?"Editar receita":"Nova receita"}</h2><form class="casa-editor-form" id="casaRecipeForm">
 <label>Nome<input id="crTitle" required value="${escapeHtml(base.title)}"></label>
 <div class="form-grid"><label>Tempo<input id="crTime" value="${escapeHtml(base.time||"")}"></label><label>Rendimento<input id="crYield" value="${escapeHtml(base.yieldText||"")}"></label></div>
 <label>Ingredientes <small>um por linha, no formato: ingrediente | quantidade</small><textarea id="crIngredients" rows="7">${escapeHtml((base.ingredients||[]).map(x=>`${x[0]} | ${x[1]||""}`).join("\n"))}</textarea></label>
 <label>Utensílios / materiais <small>um por linha</small><textarea id="crMaterials" rows="4">${escapeHtml((base.materials||[]).join("\n"))}</textarea></label>
 <label>Preparo <small>um passo por linha</small><textarea id="crSteps" rows="7">${escapeHtml((base.steps||[]).join("\n"))}</textarea></label>
 <label>Observação / dica<textarea id="crTip" rows="3">${escapeHtml(base.tip||"")}</textarea></label>
 <div class="modal-actions">${r?'<button type="button" class="secondary" id="deleteCasaRecipe">Excluir</button>':'<button type="button" class="secondary" data-close>Cancelar</button>'}<button class="primary" type="submit">Salvar</button></div></form></div>`;
 document.body.appendChild(o);const close=()=>o.remove();o.querySelectorAll('[data-close]').forEach(b=>b.onclick=close);o.addEventListener('click',e=>{if(e.target===o)close()});
 o.querySelector('#deleteCasaRecipe')?.addEventListener('click',()=>{if(!confirm(`Excluir “${base.title}”?`))return;saveCasaRecipes(recipes.filter(x=>String(x.id)!==String(base.id)));close();renderCasa()});
 o.querySelector('#casaRecipeForm').addEventListener('submit',e=>{e.preventDefault();const ing=casaHowList(o.querySelector('#crIngredients').value).map(line=>{const [a,...rest]=line.split('|');return[(a||'').trim(),rest.join('|').trim()]});const obj={...base,title:o.querySelector('#crTitle').value.trim(),time:o.querySelector('#crTime').value.trim(),yieldText:o.querySelector('#crYield').value.trim(),ingredients:ing,materials:casaHowList(o.querySelector('#crMaterials').value),steps:casaHowList(o.querySelector('#crSteps').value),tip:o.querySelector('#crTip').value.trim()};saveCasaRecipes(r?recipes.map(x=>String(x.id)===String(base.id)?obj:x):[...recipes,obj]);close();renderCasa()});
}
...
function renderCasa(){
 ...
 const d=loadCasa();
 d.maintenance=(d.maintenance||[]).map(m=>({status:"a_fazer",area:"Casa geral",type:"Reparo / conserto",durationValue:30,durationUnit:"minutes",priority:"normal",frequency:"Única",responsible:"Eu",notify:false,period:"flex",...m}));
...
function openCasaMaintenance(id){
 ...
 const areas=["Casa geral",...d.areas.map(a=>a.title),"Piscina","Jardim","Garagem","Edícula","Área externa"];
 const o=document.createElement("div");o.className="mv-how-overlay casa-editor-overlay";
 o.innerHTML=`<div class="mv-how casa-editor-card"><button type="button" class="mv-how-x" data-close>×</button><div class="eyebrow">CASA · MANUTENÇÃO</div><h2>${current?"Editar manutenção":"Nova manutenção"}</h2><form id="casaMaintForm" class="casa-editor-form">
 <label>O que precisa ser feito?<input id="mName" required maxlength="100" value="${escapeHtml(m.name)}"></label>
 <div class="form-grid"><label>Área / ambiente<select id="mArea">${areas.map(x=>`<option ${x===m.area?'selected':''}>${escapeHtml(x)}</option>`).join('')}</select></label><label>Tipo<select id="mType">${["Preventiva","Reparo / conserto","Melhoria","Limpeza técnica","Compra / instalação"].map(x=>`<option ${x===m.type?'selected':''}>${x}</option>`).join('')}</select></label></div>
 <div class="form-grid"><label>Quando<input id="mDate" type="date" value="${escapeHtml(m.date||'')}"></label><label>Período / janela<select id="mPeriod"><option value="flex" ${m.period==='flex'?'selected':''}>Flexível</option><option value="morning" ${m.period==='morning'?'selected':''}>Manhã</option><option value="afternoon" ${m.period==='afternoon'?'selected':''}>Tarde</option><option value="evening" ${m.period==='evening'?'selected':''}>Noite</option><option value="fixed" ${m.period==='fixed'?'selected':''}>Horário fixo</option></select></label></div>
 <label>Horário opcional<input id="mTime" type="time" value="${escapeHtml(m.time||'')}"></label>
 <label>Duração estimada<div class="form-grid"><input id="mDur" type="number" min="1" value="${+m.durationValue||30}"><select id="mDurUnit"><option value="minutes" ${m.durationUnit==='minutes'?'selected':''}>minutos</option><option value="hours" ${m.durationUnit==='hours'?'selected':''}>horas</option></select></div></label>
 <div class="form-grid"><label>Prioridade<select id="mPriority"><option value="baixa" ${m.priority==='baixa'?'selected':''}>Baixa</option><option value="normal" ${m.priority==='normal'?'selected':''}>Normal</option><option value="alta" ${m.priority==='alta'?'selected':''}>Alta</option><option value="urgente" ${m.priority==='urgente'?'selected':''}>Urgente</option></select></label><label>Frequência<select id="mFreq">${["Única","Diária","Semanal","Quinzenal","Mensal","Conforme necessário"].map(x=>`<option ${x===m.frequency?'selected':''}>${x}</option>`).join('')}</select></label></div>
 <label>Responsável<select id="mResp">${["Eu","Henrique","Thiago","Prestador / profissional"].map(x=>`<option ${x===m.responsible?'selected':''}>${x}</option>`).join('')}</select></label>
 <label style="display:flex;grid-template-columns:auto 1fr;align-items:center;gap:10px"><input id="mNotify" type="checkbox" ${m.notify?'checked':''} style="width:auto"> <span>Me avisar?</span></label>
 <label id="mNotifyWrap" style="${m.notify?'':'display:none'}">Quando avisar?<select id="mNotifyWhen">${["No horário da tarefa","10 min antes","30 min antes","1 hora antes","No início do período","Em um horário escolhido"].map(x=>`<option ${x===m.notifyWhen?'selected':''}>${x}</option>`).join('')}</select></label>
 <label>Observação<textarea id="mNote" rows="3">${escapeHtml(m.note||'')}</textarea></label>
 <div class="modal-actions">${current?'<button type="button" class="secondary" id="deleteMaint">Excluir</button>':'<button type="button" class="secondary" data-close>Cancelar</button>'}<button class="primary" type="submit">Salvar</button></div></form></div>`;
 document.body.appendChild(o);const close=()=>o.remove();o.querySelectorAll('[data-close]').forEach(b=>b.onclick=close);o.addEventListener('click',e=>{if(e.target===o)close()});o.querySelector('#mNotify').onchange=e=>o.querySelector('#mNotifyWrap').style.display=e.target.checked?'':'none';
 o.querySelector('#deleteMaint')?.addEventListener('click',()=>{if(!confirm(`Excluir “${m.name}”?`))return;d.maintenance=d.maintenance.filter(x=>String(x.id)!==String(m.id));saveCasa(d);close();renderCasa()});
 o.querySelector('#casaMaintForm').addEventListener('submit',e=>{e.preventDefault();const obj={...m,name:o.querySelector('#mName').value.trim(),area:o.querySelector('#mArea').value,type:o.querySelector('#mType').value,date:o.querySelector('#mDate').value,period:o.querySelector('#mPeriod').value,time:o.querySelector('#mTime').value,durationValue:Math.max(1,+o.querySelector('#mDur').value||30),durationUnit:o.querySelector('#mDurUnit').value,priority:o.querySelector('#mPriority').value,frequency:o.querySelector('#mFreq').value,responsible:o.querySelector('#mResp').value,notify:o.querySelector('#mNotify').checked,notifyWhen:o.querySelector('#mNotifyWhen').value,note:o.querySelector('#mNote').value.trim(),status:m.status||'a_fazer',updatedAt:Date.now()};d.maintenance=current?d.maintenance.map(x=>String(x.id)===String(m.id)?obj:x):[...d.maintenance,{...obj,createdAt:Date.now()}];saveCasa(d);close();renderCasa()});
}

...
(() => {
  ...
  function sourcesToday(){
    ...
    const nextMapId=Array.from({length:260},(_,i)=>i+1).find(id=>!mapStatus[String(id)]?.domino && !mapStatus[id]?.domino) || null;
    if(nextMapId){
      push({id:`study:tcdf:${nextMapId}`,learningKey:`study:tcdf:${nextMapId}`,source:'Estudos',title:`TCDF · Mapa ${String(nextMapId).padStart(3,'0')}`,date:today,minutes:30,configuredMinutes:30,period:'flex',priority:'normal',kind:'study'});
    }

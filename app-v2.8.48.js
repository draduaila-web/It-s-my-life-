...
function openIdeaModal(id=null){
  ...
  d.innerHTML=`<form method="dialog" id="ideaForm" class="modal-card">
    <div class="modal-head"><div><div class="eyebrow">${p?'EDITAR':'CRIAÇÃO & IDEIAS'}</div><h2>${p?'Editar registro':'Nova ideia'}</h2></div><button class="icon-btn" value="cancel" aria-label="Fechar">×</button></div>
  ...
  function drawConditional(){
    ...
    conditional.innerHTML=`
      ...
      <label>Blocos que funcionam para você <span class="muted">(em minutos)</span><input id="projectBlocks" value="${raw}" placeholder="Ex.: 120, 240, 360"><small>Ex.: 120, 240, 360 = sessões de 2h, 4h ou 6h. A BERTA escolhe o bloco que couber na janela.</small></label>
      ...
      <div id="projectTrackOptions" class="card" style="padding:12px;margin:0 0 12px">
        ...
        <label>Dias sem avanço<input id="projectInactiveDays" type="number" min="1" max="60" value="${+p?.inactiveDays||7}"></label>
      </div>`;
    const tr=conditional.querySelector('#projectTrack'),opts=conditional.querySelector('#projectTrackOptions'); if(tr)tr.onchange=()=>opts.style.display=tr.checked?'block':'none'; if(tr)tr.onchange();
  }
  d.querySelector('#ideaType').onchange=drawConditional;drawConditional();
  ...
  d.querySelector('#ideaForm').addEventListener('submit',e=>{
    ...
    if(t==='projeto'){
      ...
      const blocks=[...new Set(String(d.querySelector('#projectBlocks').value||'').split(/[,; ]+/).map(Number).filter(n=>n>0&&n<=1440))].sort((a,b)=>a-b);

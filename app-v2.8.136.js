...
window.BERTHA_BUILD="2.8.136-estudos-mapas-crud";
...
function estudoSugestao(min=30){const d=loadMapasStatus(),maps=studyMapsAll(),rev=maps.filter(x=>nextReviewFor(d[x.id])),due=maps.filter(x=>!d[x.id]?.domino);if(rev.length)return{title:"Revisão primeiro",text:"Há revisão prevista antes de abrir conteúdo novo.",maps:rev.slice(0,1)};return{title:min<=30?`Janela de ${min} min`:"Próximo conteúdo",text:"Escolha um mapa que caiba no tempo disponível.",maps:due.slice(0,1)}}

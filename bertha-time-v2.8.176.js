...
(() => {
  ...
  function renderFinanceRemindersHome(){
    try{
      const raw=JSON.parse(localStorage.getItem('minha-vida.financeiro.v2')||'null');
      ...
      const html=rows.slice(0,4).map(o=>{
        ...
        return `<a class="home-fin-reminder-row" href="#financeiro"><span class="home-fin-reminder-icon">${o.days<0?'!':'⏰'}</span><span><strong>${esc(o.name)}</strong><small>${esc(o.label)} · ${dd}/${mm}</small></span><b>›</b></a>`;
      }).join('');
  ...
  function ensureStyles(){
    ...
    /* v2.8.176 — Menu final + backup visível */

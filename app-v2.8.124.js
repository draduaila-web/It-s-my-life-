...
// BERTH.A app-v2.8.123 · Casa sanfona refinada
...
function renderCasaCore(){
 ...
 const routinesHtml=`<div class="casa-routine-browser"><div class="casa-routine-search"><span>${casaLineIcon('routines')}</span><input id="casaRoutineSearch" type="search" inputmode="search" placeholder="Buscar rotina ou área…" autocomplete="off"></div><div class="casa-routine-groups">${d.areas.map(a=>`<details class="casa-routine-group" data-routine-area="${escapeHtml(String(a.title).toLowerCase())}"><summary><span>${casaAreaIcon(a.title)}<strong>${escapeHtml(a.title)}</strong></span><span class="casa-routine-count">${a.tasks.length}</span></summary><div class="casa-routine-group-body">${a.tasks.length?a.tasks.map(t=>routineTaskHtml(a,t)).join(""):`<p class="note">Sem rotina cadastrada. Mantemos espaço para incluir apenas o que realmente for necessário.</p>`}</div></details>`).join("")}</div><div class="empty compact casa-routine-empty" hidden><strong>Nenhuma rotina encontrada.</strong><span>Tente outro termo.</span></div></div>`
 ...
 const routineSearch=document.querySelector('#casaRoutineSearch');
 if(routineSearch){routineSearch.addEventListener('input',()=>{const q=routineSearch.value.trim().toLowerCase();let total=0;document.querySelectorAll('.casa-routine-group').forEach(group=>{let matches=0;group.querySelectorAll('.casa-routine-row').forEach(row=>{const ok=!q||String(row.dataset.routineSearch||'').includes(q);row.hidden=!ok;if(ok)matches++;});group.hidden=matches===0;if(q&&matches)group.open=true;total+=matches;});const empty=document.querySelector('.casa-routine-empty');if(empty)empty.hidden=total!==0;});}

/* BERTH.A — Meu Dia v2 / Motor de Tempo v2.8.24 — base 2.8.17 + seletor de Movimento + limpeza segura
...
(() => {
  ...
  function sourcesToday(){
    ...
    // Meu Dia Ideal pode conter duplicatas históricas (mesmo título criado em versões diferentes).
    // Na Home mostramos apenas uma ocorrência por título normalizado, sem apagar dados do usuário.
    const seenIdealTitles=new Set();
    idealItems
      .filter(x=>x.active!==false)
      .filter(x=>!(x.untilMode==='date' && x.untilDate && x.untilDate<today))
      .filter(x=>{ const k=normalizedLearningText(x.title||''); if(!k)return true; if(seenIdealTitles.has(k))return false; seenIdealTitles.add(k); return true; })
  ...
  // Limpa somente uma reserva genérica de Exercícios/Movimento deixada em andamento por testes antigos.
  // Não toca em treino específico nem em qualquer outra atividade ativa.
  (function clearLegacyStaleMovementReserve(){
    const e=engine(),a=e.active;
    const elapsed=a?.startedAt ? (Date.now()-a.startedAt)/60000 : 0;
    if(a && String(a.id||'').startsWith('ideal:') && /exerc|moviment/i.test(String(a.title||'')) && elapsed>180){

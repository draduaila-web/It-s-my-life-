// BERTH.A v2.8.45 — mantém Financeiro nativo + modal organizado + lembretes na Home
...
function bindIdeaCards(){document.querySelectorAll('[data-idea-id]').forEach(card=>card.onclick=e=>{if(e.target.closest('.idea-more'))e.stopPropagation();openIdeaModal(card.dataset.ideaId)})}

function openIdeaModal(id=null){
  const p=id?loadIdeias().find(x=>x.id===id):null, type=p?.type||'ideia';
  const d=document.createElement('dialog');d.id='ideaDialog';
  ...
  d.querySelector('#ideaForm').addEventListener('submit',e=>{
    ...
    let data={title:d.querySelector('#ideaTitle').value.trim(),type:t,note:d.querySelector('#ideaNote').value.trim()}; if(!data.title)return;
    if(t==='plano'){

// BERTH.A v2.8.50 — Criação: modal padrão + campo de link
...
function renderIdeias(){
  ...
  app.innerHTML=`
    <section class="hero"><h2>Guarde sem se obrigar.</h2><p>Uma ideia pode só existir. Quando ganhar forma, a BERTA ajuda a transformar em plano ou projeto — e a continuar sem abrir mil frentes.</p></section>
...
function ideaCardHtml(x){
  ...
  if(x.type==='projeto'){
    ...
    extra=`<div class="meta" style="margin-top:7px;gap:6px;flex-wrap:wrap">${total?`<span class="pill">${pct}% · faltam ${minsLabel(rem)}</span>`:''}${x.projectSuggestWindow?'<span class="pill">☁️ BERTA pode sugerir</span>':''}${x.nextAction?`<span class="pill">Próximo: ${escapeHtml(x.nextAction)}</span>`:''}</div>`;
...
function ideaEmptyHtml(){return `<div class="empty"><div class="symbol">✦</div><strong>Esse espaço está leve.</strong><span>Registre uma ideia quando ela aparecer. Ela não precisa virar tarefa.</span></div>`}
...
function openIdeaModal(id=null){
  ...
  const d=document.createElement('dialog');d.id='ideaDialog';d.className='fin-planned-dialog idea-dialog';
  // Criação usa o padrão de modal diretamente, sem depender de CSS carregado por outro módulo.
  [['border','0'],['outline','0'],['padding','0'],['margin','auto'],['background','transparent'],['width','min(92vw, 540px)'],['max-width','540px'],['max-height','calc(100dvh - 28px)'],['overflow','visible'],['box-shadow','none']].forEach(([k,v])=>d.style.setProperty(k,v,'important'));
  d.innerHTML=`<form method="dialog" id="ideaForm" class="modal-card">
    <div class="modal-head"><div><div class="eyebrow">${p?'EDITAR':'CRIAÇÃO & IDEIAS'}</div><h2 id="ideaModalTitle">${p?'Editar registro':'Nova ideia'}</h2></div><button class="icon-btn" value="cancel" aria-label="Fechar">×</button></div>
    ...
    <label>Tipo<select id="ideaType"><option value="ideia" ${type==='ideia'?'selected':''}>Ideia</option><option value="projeto" ${type==='projeto'?'selected':''}>Projeto</option><option value="plano" ${type==='plano'?'selected':''}>Plano</option></select></label>
  ...
  const ideaCard=d.querySelector('#ideaForm');
  if(ideaCard){
    [['box-sizing','border-box'],['width','100%'],['max-width','none'],['max-height','calc(100dvh - 28px)'],['overflow','auto'],['margin','0'],['padding','20px'],['border','0'],['border-radius','26px'],['background','#fffdfb'],['box-shadow','0 22px 64px rgba(58,44,63,.20)']].forEach(([k,v])=>ideaCard.style.setProperty(k,v,'important'));
  }
document.body.appendChild(d);d.showModal();
  const conditional=d.querySelector('#ideaConditional');
  function drawConditional(){
    const t=d.querySelector('#ideaType').value;
    if(t==='ideia'){conditional.innerHTML=`<div class="card" style="padding:12px;margin:0 0 12px"><small>✦ Ideias não ocupam tempo no Meu Dia. Guarde primeiro; decida depois.</small></div>`;return}
    if(data.link&&!ideaSafeLink(data.link)){d.querySelector('#ideaLink').setCustomValidity('Use um link começando com http:// ou https://');d.querySelector('#ideaLink').reportValidity();return}else d.querySelector('#ideaLink').setCustomValidity('');
    if(t==='plano'){
    ...
    const blocks=projectBlocks(p||{}); const raw=blocks.length?blocks.map(n=>Number((n/60).toFixed(2))).join(', '):'2, 4, 6';
    conditional.innerHTML=`
      ...
      <div class="form-grid"><label>Tempo total estimado<input id="projectTotal" type="number" min="1" value="${p?.totalDurationValue||4}"></label><label>Unidade<select id="projectTotalUnit"><option value="hours" ${p?.totalDurationUnit!=='minutes'?'selected':''}>horas</option><option value="minutes" ${p?.totalDurationUnit==='minutes'?'selected':''}>minutos</option></select></label></div>
      <label>Blocos que funcionam para você <span class="muted">(em horas)</span><input id="projectBlocks" inputmode="decimal" value="${raw}" placeholder="Ex.: 2, 4, 6"><small>Ex.: 2, 4, 6 = sessões de 2h, 4h ou 6h. A BERTA escolhe o bloco que couber na janela.</small></label>
      <label>Bloco preferido<select id="projectPreferred"><option value="60" ${+p?.preferredSession===60?'selected':''}>1 hora</option><option value="120" ${!p?.preferredSession||+p?.preferredSession===120?'selected':''}>2 horas</option><option value="240" ${+p?.preferredSession===240?'selected':''}>4 horas</option><option value="360" ${+p?.preferredSession===360?'selected':''}>6 horas</option></select></label>
      ...
      <label style="display:flex;gap:10px;align-items:flex-start"><input id="projectTrack" type="checkbox" ${p?.trackProject!==false?'checked':''} style="width:auto;margin-top:3px"><span><strong>Quero que a BERTA acompanhe este projeto</strong><small style="display:block">Ela pode lembrar de continuidade e mostrar em que etapa você está.</small></span></label>
      <div id="projectTrackOptions" class="card" style="padding:12px;margin:0 0 12px">
        ...
        <label id="projectInactiveDaysWrap">Dias sem avanço<input id="projectInactiveDays" type="number" min="1" max="60" value="${+p?.inactiveDays||7}"></label>
      </div>`;
  ...
  function syncIdeaModalTitle(){if(p){ideaModalTitle.textContent='Editar registro';return}const t=ideaTypeSelect.value;ideaModalTitle.textContent=t==='projeto'?'Novo projeto':t==='plano'?'Novo plano':'Nova ideia'}
  ideaTypeSelect.onchange=()=>{drawConditional();syncIdeaModalTitle()};drawConditional();syncIdeaModalTitle();
  ...
  d.querySelector('#ideaForm').addEventListener('submit',e=>{
    ...
    if(t==='projeto'){
      const val=Math.max(1,+d.querySelector('#projectTotal').value||1),unit=d.querySelector('#projectTotalUnit').value,total=unit==='hours'?val*60:val;
      const blocks=[...new Set(String(d.querySelector('#projectBlocks').value||'').split(/[,; ]+/).map(s=>Number(String(s).replace(',','.'))).filter(h=>h>0&&h<=24).map(h=>Math.round(h*60)))].sort((a,b)=>a-b);
      const track=d.querySelector('#projectTrack').checked;
      Object.assign(data,{outcome:d.querySelector('#projectOutcome').value.trim(),totalDurationValue:val,totalDurationUnit:unit,totalMinutes:total,progressMinutes:+p?.progressMinutes||0,sessionMinutes:blocks.length?blocks:[120],preferredSession:+d.querySelector('#projectPreferred').value||120,nextAction:d.querySelector('#projectNext').value.trim(),priority:d.querySelector('#projectPriority').value,dueDate:d.querySelector('#projectDue').value,trackProject:track,projectSuggestWindow:track&&d.querySelector('#projectWindow').checked,projectContinuePrompt:track&&d.querySelector('#projectContinue').checked,projectInactiveReminder:track&&d.querySelector('#projectInactive').checked,inactiveDays:Math.max(1,+d.querySelector('#projectInactiveDays').value||7),status:p?.status||'ativo',lastWorkedAt:p?.lastWorkedAt||null});
    }
    ...
    saveIdeias(items);d.close();d.remove();renderIdeias();
  });
...
function renderFinanceiro(){
 ...
 ${alerts.length?`<section class="fin-due-alert"><strong>⏰ ${alerts.length===1?"1 conta pede atenção":`${alerts.length} contas pedem atenção`}</strong><span>${alerts.slice(0,3).map(o=>`${escapeHtml(o.bill.name)} · ${escapeHtml(finPlannedStatus(o).label)}`).join("<br>")}</span></section>`:""}
 ...
 <div class="section-title">FORA DO SEU ORÇAMENTO</div><div class="list">${finPersonal(d.excluded).map(x=>`<div class="card excluded-card"><div><strong>${escapeHtml(x.name)}</strong><span>${x.value?`R$ ${money(x.value)} · `:""}${escapeHtml(x.reason)}</span></div><span class="pill">${escapeHtml(x.payer)}</span></div>`).join("")}</div>`;
 ensureFinanceStyles();
 document.getElementById("editIncome").onclick=()=>openFinModal("income");document.getElementById("toggleFinPrivacy").onclick=toggleFinPrivacyNative;document.getElementById("addPlannedBill").onclick=()=>openPlannedBillModal();document.getElementById("addFixed").onclick=()=>openFinModal("fixed");document.getElementById("addTransaction").onclick=()=>openFinModal("transaction");
...
function openPlannedBillModal(id){
 ...
 const dlg=document.createElement("dialog");dlg.className="fin-planned-dialog";dlg.innerHTML=`<form method="dialog" class="modal-card" id="finPlanForm"><div class="modal-head"><div><div class="eyebrow">💰 CONTA PREVISTA</div><h2>${existing?"Editar conta prevista":"Nova conta prevista"}</h2></div><button class="icon-btn" value="cancel">×</button></div>
 ...
 document.body.appendChild(dlg);dlg.showModal();dlg.querySelector("#cancelFinPlan").onclick=()=>{dlg.close();dlg.remove()};
...
function renderExercicios(){
 ...
 ${focus?`<div class="exercise-focus card"><span class="eyebrow">HOJE</span><strong>${escapeHtml(focus.name)} · ${exMinutes(focus)} min</strong><p>${escapeHtml(exPeriodText(focus))}${focus.time?` · ${escapeHtml(focus.time)}`:''}. ${escapeHtml(focus.note||'Movimento possível, sem rigidez.')}</p><div class="study-v10-actions">${active&&String(active.id)===`exercise:${focus.id}`?`<button class="primary" id="finishExercise">Concluir treino</button>`:`<button class="primary" id="startExercise">Começar treino</button>`}<button class="secondary" id="doneExercise">Já fiz</button></div></div>`:''}
 <div class="section-title">ROTINA DE MOVIMENTO</div>
 <div class="list">${d.plans.map(p=>`<div class="card exercise-plan" data-plan="${p.id}"><div><strong>${escapeHtml(p.name)}</strong><span>${escapeHtml(p.type)} · ${exMinutes(p)} min · ${escapeHtml(exPeriodText(p))}</span>${p.days.length?`<small>${p.days.join(' · ')}</small>`:`<small>${escapeHtml(p.frequency)}</small>`}</div><div class="exercise-plan-actions"><span class="pill">${p.active?'Ativo':'Pausado'}</span>${p.active?`<button class="secondary" type="button" data-start-plan="${p.id}">Iniciar agora</button>`:''}</div></div>`).join('')}</div>
 ...
 document.querySelectorAll('[data-plan]').forEach(el=>el.onclick=()=>openExercisePlan(el.dataset.plan));
 ...
 document.querySelector('#finishExercise')?.addEventListener('click',()=>{const a=window.BerthaTimeEngine?.active?.();let mins=exMinutes(focus);if(a?.startedAt)mins=Math.max(1,Math.round((Date.now()-a.startedAt)/60000));window.BerthaTimeEngine?.finish?.();addExerciseSession(focus.name,`${mins} min`,today,'Tempo real')});
 document.querySelector('#doneExercise')?.addEventListener('click',()=>addExerciseSession(focus.name,`${exMinutes(focus)} min`,today,'Registrado manualmente'));
...
function addExerciseSession(name,duration,date,note=''){const d=loadEx();if(d.sessions.some(s=>s.date===date&&s.name===name)){alert('Esse treino já foi registrado hoje.');return;}d.sessions.push({id:uid(),name,duration,date,note,createdAt:Date.now()});saveEx(d);renderExercicios();}
...
function getTodayMealState(){const d=loadFood(),key=foodTodayISO(),planned=plannedDinnerToday(d),plannedRecipe=recipeForMealName(planned?.[1]||"");const log=d.mealLog[key]||{};return {d,key,planned,plannedRecipe,log,selected:getRecipeById(log.selectedRecipeId)||plannedRecipe};}
function saveTodayMealChoice(recipeId,source="planned"){const {d,key,plannedRecipe}=getTodayMealState();d.mealLog[key]={...(d.mealLog[key]||{}),plannedRecipeId:plannedRecipe?.id||null,selectedRecipeId:recipeId||null,source,selectedAt:new Date().toISOString()};saveFood(d);}
function setTodayMealConsumption(status){const {d,key}=getTodayMealState();d.mealLog[key]={...(d.mealLog[key]||{}),consumption:status,consumedAt:new Date().toISOString()};saveFood(d);renderAlimentacao();}
function renderTodayFoodCard(){const {planned,plannedRecipe,log,selected}=getTodayMealState();const changed=log.selectedRecipeId&&plannedRecipe&&log.selectedRecipeId!==plannedRecipe.id;const status=log.consumption||"";return `<div class="card today-food-card"><div class="panel-head"><div><span class="eyebrow">COMIDA DE HOJE · ${escapeHtml(foodDayName().toUpperCase())}</span><h3>${escapeHtml(selected?.name||planned?.[1]||"Jantar de hoje")}</h3></div>${changed?`<span class="pill">substituído</span>`:`<span class="pill">planejado</span>`}</div>${changed?`<p class="note">Planejado: ${escapeHtml(planned?.[1]||"")}</p>`:""}<div style="display:flex;gap:8px;flex-wrap:wrap;margin-top:12px"><button type="button" class="primary" id="doTodayRecipe">Fazer esta receita</button><button type="button" class="secondary" id="chooseOtherRecipe">Fazer outra</button>${selected?`<button type="button" class="secondary" id="viewTodayRecipe">Ver receita</button>`:""}</div>${log.selectedRecipeId?`<div style="display:flex;gap:8px;flex-wrap:wrap;margin-top:10px"><button type="button" class="secondary" id="foodConsumed">${status==="consumed"?"✓ Consumido":"Marcar consumido"}</button><button type="button" class="secondary" id="foodNotConsumed">${status==="not-consumed"?"✓ Não consumimos":"Não consumimos"}</button></div>`:""}</div>`;}
function renderAlimentacao(){
 const d=loadFood();
 app.innerHTML=`<section class="hero"><h2>🍽️ Alimentação</h2><p>Comer bem com menos decisões — com liberdade para mudar de ideia sem perder o planejamento.</p></section>
 ${renderTodayFoodCard()}
 <div class="food-principle card"><span class="eyebrow">SISTEMA DA CASA</span><strong>Planejado é sugestão, não obrigação.</strong><p>Escolha o que faz sentido hoje. A BERTH.A guarda o planejado e o realizado para aprender com o consumo real.</p></div>
 ...
 <div class="card"><div class="panel-head"><h3>🌙 Jantar + marmita</h3><span class="pill">3 pessoas</span></div>${d.dinners.map(x=>{const r=recipeForMealName(x[1]);return `<div class="food-row"><span>${x[0]}</span><strong>${escapeHtml(x[1])}</strong>${x[2]?`<small>marmita → ${x[2]}</small>`:""}${r?`<button type="button" class="secondary" style="margin-top:6px" data-food-recipe="${r.id}">Ver receita</button>`:""}</div>`}).join("")}</div></div>
 <div class="section-title">ROTINA DE PREPARO</div>
 <div class="card food-checklist"><label><input type="checkbox" id="foodCook" ${d.cookingDone?"checked":""}><span><strong>Cozinha quinzenal</strong><small>Produzir bases, porcionar, etiquetar e congelar.</small></span></label><div class="food-rule"><b>Compra mensal</b><span>Carnes, arroz, feijão, grãos, massas, flocão, tapioca, Rap10, biscoitos, azeite, manteiga, café e itens de boa validade.</span></div><div class="food-rule"><b>Compra semanal</b><span>Frutas, verduras, folhas, pão, iogurtes, cottage, frios e demais perecíveis.</span></div></div>
 <div class="section-title">AMANHÃ</div><div class="card tomorrow-food"><span class="eyebrow">ANTES DE DORMIR</span><strong>Preparar alimentação de amanhã</strong><p>Deixar encaminhados café da manhã, lancheira e marmita. De manhã, apenas finalizar o que for necessário.</p></div>
 <div class="section-title">LISTA DE COMPRAS</div><div class="card" id="monthlyShoppingCard">${renderMonthlyShopping()}</div>
 ...
 document.querySelector("#doTodayRecipe")?.addEventListener("click",()=>{const s=getTodayMealState(),r=s.selected||s.plannedRecipe;if(r){saveTodayMealChoice(r.id,s.log.selectedRecipeId?"chosen":"planned");openFoodRecipe(r.id,true);}else openChooseRecipe();});
 document.querySelector("#chooseOtherRecipe")?.addEventListener("click",openChooseRecipe);
 document.querySelector("#viewTodayRecipe")?.addEventListener("click",()=>{const r=getTodayMealState().selected;if(r)openFoodRecipe(r.id,false);});
 document.querySelector("#foodConsumed")?.addEventListener("click",()=>setTodayMealConsumption("consumed"));
 document.querySelector("#foodNotConsumed")?.addEventListener("click",()=>setTodayMealConsumption("not-consumed"));
 document.querySelectorAll("[data-food-recipe]").forEach(b=>b.onclick=()=>openFoodRecipe(b.dataset.foodRecipe,false));
 bindMonthlyShopping();
}
function renderMonthlyShopping(){const items=loadSharedShopping().filter(x=>(x.cycle||"monthly")==="monthly");return items.length?`<div class="casa-mini-shopping">${items.map(x=>`<div class="casa-mini-shop-row ${x.done?'done':''}"><label><input type="checkbox" data-month-shop-done="${x.id}" ${x.done?'checked':''}><span>${escapeHtml(x.name)}</span></label><small>${escapeHtml(x.category||x.source||'Compras')}</small><button type="button" class="more" data-month-shop-del="${x.id}">×</button></div>`).join("")}</div>`:`<div class="empty compact"><strong>Nenhum item pendente.</strong><span>Itens enviados pelas Receitas, Casa e outros módulos aparecem aqui.</span></div>`}
...
function loadRec(){try{const d=JSON.parse(localStorage.getItem(REC_KEY));return {...(d||{}),favorites:Array.isArray(d?.favorites)?d.favorites:[],customRecipes:Array.isArray(d?.customRecipes)?d.customRecipes:[]};}catch{return {favorites:[],customRecipes:[]};}}
...
function addSharedShoppingItem(name,source,category="Outros"){
 ...
 if(!items.some(x=>String(x.name||"").trim().toLowerCase()===key&&!x.done)){
   items.push({id:uid(),name,source:source||"BERTH.A",category:category||"Outros",cycle:"monthly",createdAt:Date.now(),done:false});
...
function openFoodRecipe(id,doing=false){const r=getRecipeById(id);if(!r)return;const old=document.querySelector("#foodRecipeDialog");if(old)old.remove();const dlg=document.createElement("dialog");dlg.id="foodRecipeDialog";dlg.className="study-v10-dialog";dlg.innerHTML=`<div class="study-v10-modal"><div class="study-v10-head"><div><div class="eyebrow">${escapeHtml(recipeCategory(r))}</div><h2>${escapeHtml(r.name)}</h2><p>${escapeHtml(r.yield||"Rendimento não informado")} · ${escapeHtml(r.prep||"Preparo livre")}</p></div><button type="button" class="study-v10-x" data-close>×</button></div><div class="card"><h3>Ingredientes</h3><ul>${(r.ingredients||[]).map(x=>`<li>${escapeHtml(x)}</li>`).join("")||"<li>Sem ingredientes cadastrados.</li>"}</ul><h3>Modo de preparo</h3><ol>${(r.steps||[]).map(x=>`<li>${escapeHtml(x)}</li>`).join("")||"<li>Sem modo de preparo cadastrado.</li>"}</ol><p><strong>Finalização:</strong> ${escapeHtml(r.finish||"—")}</p>${r.notes?`<p><strong>Observações:</strong> ${escapeHtml(r.notes)}</p>`:""}</div><div style="display:flex;gap:8px;flex-wrap:wrap"><button type="button" class="primary" id="chooseThisFood">${doing?"✓ Fazer esta receita":"Fazer esta receita"}</button><button type="button" class="secondary" id="chooseOtherFromRecipe">Fazer outra</button><button type="button" class="secondary" id="recipeVariations">Buscar variações</button><button type="button" class="secondary" id="recipeIngredients">Mesmos ingredientes</button><button type="button" class="secondary" id="recipeToShopping">Adicionar ingredientes à lista</button>${isCustomRecipe(r.id)?`<button type="button" class="secondary" id="editThisRecipe">Editar receita</button>`:""}</div></div>`;document.body.appendChild(dlg);dlg.querySelector("[data-close]").onclick=()=>dlg.close();dlg.addEventListener("close",()=>dlg.remove());dlg.querySelector("#chooseThisFood").onclick=()=>{saveTodayMealChoice(r.id,"chosen");dlg.close();if(state.route==="alimentacao")renderAlimentacao();};dlg.querySelector("#chooseOtherFromRecipe").onclick=()=>{dlg.close();openChooseRecipe();};dlg.querySelector("#recipeVariations").onclick=()=>window.open(recipeSearchUrl(r,"variations"),"_blank","noopener");dlg.querySelector("#recipeIngredients").onclick=()=>window.open(recipeSearchUrl(r,"ingredients"),"_blank","noopener");dlg.querySelector("#recipeToShopping").onclick=()=>openRecipeShoppingDialog(r);dlg.querySelector("#editThisRecipe")?.addEventListener("click",()=>{dlg.close();openRecipeForm(r.id);});dlg.showModal();}
function openChooseRecipe(){const old=document.querySelector("#chooseFoodDialog");if(old)old.remove();const dlg=document.createElement("dialog");dlg.id="chooseFoodDialog";dlg.className="study-v10-dialog";dlg.innerHTML=`<div class="study-v10-modal"><div class="study-v10-head"><div><div class="eyebrow">FLEXIBILIDADE</div><h2>O que você quer fazer hoje?</h2><p>Escolha uma receita cadastrada ou procure uma ideia nova.</p></div><button type="button" class="study-v10-x" data-close>×</button></div><input id="chooseFoodSearch" placeholder="Buscar nas minhas receitas…"><div class="list" id="chooseFoodList" style="max-height:55vh;overflow:auto;margin-top:10px"></div><button type="button" class="secondary" id="chooseFoodWeb" style="margin-top:10px">Buscar outra ideia na internet</button></div>`;document.body.appendChild(dlg);const draw=()=>{const q=(dlg.querySelector("#chooseFoodSearch").value||"").toLowerCase();dlg.querySelector("#chooseFoodList").innerHTML=allRecipes().filter(r=>!q||r.name.toLowerCase().includes(q)||recipeCategory(r).toLowerCase().includes(q)||(r.ingredients||[]).join(" ").toLowerCase().includes(q)).map(r=>`<button type="button" class="card" data-choose-food="${r.id}" style="width:100%;text-align:left"><span class="eyebrow">${escapeHtml(recipeCategory(r))}</span><strong style="display:block">${escapeHtml(r.name)}</strong><small>${escapeHtml(r.yield||"Rendimento não informado")} · ${escapeHtml(r.finish||"")}</small></button>`).join("");dlg.querySelectorAll("[data-choose-food]").forEach(b=>b.onclick=()=>{const id=b.dataset.chooseFood;saveTodayMealChoice(id,"alternative");dlg.close();openFoodRecipe(id,true);});};dlg.querySelector("[data-close]").onclick=()=>dlg.close();dlg.addEventListener("close",()=>dlg.remove());dlg.querySelector("#chooseFoodSearch").oninput=draw;dlg.querySelector("#chooseFoodWeb").onclick=()=>window.open(`https://www.google.com/search?q=${encodeURIComponent("receitas fáceis jantar família ingredientes simples")}`,"_blank","noopener");draw();dlg.showModal();}
...
function openRecipeForm(recipeId=null){
 ...
 dlg.innerHTML=`<form class="study-v10-modal" id="recipeForm"><div class="study-v10-head"><div><div class="eyebrow">RECEITAS</div><h2>${existing?"Editar receita":"Nova receita"}</h2><p>Cadastre uma vez e ela já fica disponível em “Fazer outra”.</p></div><button type="button" class="study-v10-x" data-close>×</button></div>
 <label>Nome da receita<input id="newRecipeName" required maxlength="120" value="${escapeHtml(existing?.name||"")}" placeholder="Ex.: Escondidinho de carne"></label>
 <label>Categoria<select id="newRecipeCategory" required>${RECIPE_CATEGORIES.map(c=>`<option value="${escapeHtml(c)}" ${recipeCategory(existing||{})===c?"selected":""}>${escapeHtml(c)}</option>`).join("")}</select></label>
 <div class="form-grid"><label>Rendimento<input id="newRecipeYield" value="${escapeHtml(existing?.yield||"")}" placeholder="Ex.: 4 porções"></label><label>Preparo / tempo<input id="newRecipePrep" value="${escapeHtml(existing?.prep||"")}" placeholder="Ex.: 40 min"></label></div>
 <label>Ingredientes <span class="muted">(um por linha, já com quantidade)</span><textarea id="newRecipeIngredients" rows="7" required placeholder="500 g de carne moída\n1 cebola\n2 dentes de alho">${escapeHtml((existing?.ingredients||[]).join("\n"))}</textarea></label>
 <label>Modo de preparo <span class="muted">(um passo por linha)</span><textarea id="newRecipeSteps" rows="7" required placeholder="Refogue a cebola e o alho.\nAcrescente a carne e cozinhe.">${escapeHtml((existing?.steps||[]).join("\n"))}</textarea></label>
 <label>Finalização<input id="newRecipeFinish" value="${escapeHtml(existing?.finish||"")}" placeholder="Ex.: Servir com salada"></label>
 <label>Observações <span class="muted">(opcional)</span><textarea id="newRecipeNotes" rows="3" placeholder="Congelamento, substituições, preferências…">${escapeHtml(existing?.notes||"")}</textarea></label>
 <div class="modal-actions">${existing?`<button type="button" class="secondary" id="deleteRecipeBtn">Excluir</button>`:""}<div class="grow"></div><button type="button" class="secondary" data-close>Cancelar</button><button type="submit" class="primary">Salvar receita</button></div></form>`;
...
function renderReceitas(){
 const d=loadRec();
 app.innerHTML=`<section class="hero"><h2>📖 Receitas</h2><p>Seu livro de receitas — organizado para crescer sem virar uma lista infinita.</p></section><button type="button" class="primary" id="addFoodRecipe" style="width:100%;margin-bottom:12px">＋ Nova receita</button><div class="recipe-principle card"><span class="eyebrow">FLEXÍVEL</span><strong>Use o que já funciona ou procure outra ideia.</strong><p>Você pode fazer a receita, trocar por outra, buscar variações ou encontrar opções com os mesmos ingredientes.</p></div><div class="recipe-filters"><input id="recipeSearch" placeholder="Buscar por receita ou ingrediente…"><select id="recipeCat"><option value="">Todas as categorias</option>${RECIPE_CATEGORIES.map(c=>`<option value="${escapeHtml(c)}">${escapeHtml(c)}</option>`).join("")}</select></div><div class="list" id="recipeList"></div><div class="card freezer-rule"><span class="eyebrow">❄️ FREEZER</span><p>Identificar cada preparo com <strong>NOME • DATA • Nº DE PORÇÕES • FINALIZAÇÃO</strong>.</p></div>`;
 const update=()=>{const q=(document.querySelector("#recipeSearch")?.value||"").toLowerCase(),c=document.querySelector("#recipeCat")?.value||"";const list=allRecipes().filter(r=>(!q||r.name.toLowerCase().includes(q)||(r.ingredients||[]).join(" ").toLowerCase().includes(q))&&(!c||recipeCategory(r)===c));document.querySelector("#recipeList").innerHTML=recipeCards(list,loadRec());bindRecipeCards(update);};
 document.querySelector("#addFoodRecipe").onclick=()=>openRecipeForm();document.querySelector("#recipeSearch").oninput=update;document.querySelector("#recipeCat").onchange=update;update();
}

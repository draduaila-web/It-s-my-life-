...
function renderCasaCore(){
 ...
 const routineAreaSelect=document.querySelector('#casaRoutineAreaSelect');
 if(routineAreaSelect){routineAreaSelect.addEventListener('change',()=>{const id=routineAreaSelect.value;const groups=[...document.querySelectorAll('.casa-routine-group')];groups.forEach(group=>group.open=false);if(!id)return;const target=groups.find(group=>String(group.dataset.routineId)===String(id));if(target){target.open=true;requestAnimationFrame(()=>target.scrollIntoView({behavior:'smooth',block:'start'}));}});}

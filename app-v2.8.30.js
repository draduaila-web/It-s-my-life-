...
function recipeForMealName(name=""){const q=name.toLowerCase();const aliases=[
 ...
 ]; const hit=aliases.find(([a])=>q.includes(a));return hit?RECIPES.find(r=>r.id===hit[1]):null;}
function getTodayMealState(){const d=loadFood(),key=foodTodayISO(),planned=plannedDinnerToday(d),plannedRecipe=recipeForMealName(planned?.[1]||"");const log=d.mealLog[key]||{};return {d,key,planned,plannedRecipe,log,selected:RECIPES.find(r=>r.id===log.selectedRecipeId)||plannedRecipe};}
...
function renderAlimentacao(){
 ...
 <div class="section-title">COMPRAS DO MÊS</div><div class="card" id="monthlyShoppingCard">${renderMonthlyShopping()}</div>
...
function openFoodRecipe(id,doing=false){const r=RECIPES.find(x=>x.id===id);if(!r)return;const old=document.querySelector("#foodRecipeDialog");if(old)old.remove();const dlg=document.createElement("dialog");dlg.id="foodRecipeDialog";dlg.className="study-v10-dialog";dlg.innerHTML=`<div class="study-v10-modal"><div class="study-v10-head"><div><div class="eyebrow">${escapeHtml(r.cat)}</div><h2>${escapeHtml(r.name)}</h2><p>${escapeHtml(r.yield)} · ${escapeHtml(r.prep)}</p></div><button type="button" class="study-v10-x" data-close>×</button></div><div class="card"><h3>Ingredientes</h3><ul>${(r.ingredients||[]).map(x=>`<li>${escapeHtml(x)}</li>`).join("")}</ul><h3>Modo de preparo</h3><ol>${(r.steps||[]).map(x=>`<li>${escapeHtml(x)}</li>`).join("")}</ol><p><strong>Finalização:</strong> ${escapeHtml(r.finish||"")}</p></div><div style="display:flex;gap:8px;flex-wrap:wrap"><button type="button" class="primary" id="chooseThisFood">${doing?"✓ Fazer esta receita":"Fazer esta receita"}</button><button type="button" class="secondary" id="chooseOtherFromRecipe">Fazer outra</button><button type="button" class="secondary" id="recipeVariations">Buscar variações</button><button type="button" class="secondary" id="recipeIngredients">Mesmos ingredientes</button><button type="button" class="secondary" id="recipeToShopping">Adicionar às compras</button></div></div>`;document.body.appendChild(dlg);dlg.querySelector("[data-close]").onclick=()=>dlg.close();dlg.addEventListener("close",()=>dlg.remove());dlg.querySelector("#chooseThisFood").onclick=()=>{saveTodayMealChoice(r.id,"chosen");dlg.close();if(state.route==="alimentacao")renderAlimentacao();};dlg.querySelector("#chooseOtherFromRecipe").onclick=()=>{dlg.close();openChooseRecipe();};dlg.querySelector("#recipeVariations").onclick=()=>window.open(recipeSearchUrl(r,"variations"),"_blank","noopener");dlg.querySelector("#recipeIngredients").onclick=()=>window.open(recipeSearchUrl(r,"ingredients"),"_blank","noopener");dlg.querySelector("#recipeToShopping").onclick=()=>{(r.ingredients||[]).forEach(x=>{const name=x.replace(/^\d+[\d\s½¼¾⅓⅔.,/-]*\s*(kg|g|ml|l|xícara|xícaras|colher|colheres|unidade|unidades)?\s*(de\s+)?/i,"").trim();if(name)addCasaShopping(name,"Receita · "+r.name)});dlg.querySelector("#recipeToShopping").textContent="✓ Adicionado";};dlg.showModal();}
function openChooseRecipe(){const old=document.querySelector("#chooseFoodDialog");if(old)old.remove();const dlg=document.createElement("dialog");dlg.id="chooseFoodDialog";dlg.className="study-v10-dialog";dlg.innerHTML=`<div class="study-v10-modal"><div class="study-v10-head"><div><div class="eyebrow">FLEXIBILIDADE</div><h2>O que você quer fazer hoje?</h2><p>Escolha uma receita cadastrada ou procure uma ideia nova.</p></div><button type="button" class="study-v10-x" data-close>×</button></div><input id="chooseFoodSearch" placeholder="Buscar nas minhas receitas…"><div class="list" id="chooseFoodList" style="max-height:55vh;overflow:auto;margin-top:10px"></div><button type="button" class="secondary" id="chooseFoodWeb" style="margin-top:10px">Buscar outra ideia na internet</button></div>`;document.body.appendChild(dlg);const draw=()=>{const q=(dlg.querySelector("#chooseFoodSearch").value||"").toLowerCase();dlg.querySelector("#chooseFoodList").innerHTML=RECIPES.filter(r=>!q||r.name.toLowerCase().includes(q)||r.cat.toLowerCase().includes(q)).map(r=>`<button type="button" class="card" data-choose-food="${r.id}" style="width:100%;text-align:left"><span class="eyebrow">${escapeHtml(r.cat)}</span><strong style="display:block">${escapeHtml(r.name)}</strong><small>${escapeHtml(r.yield)} · ${escapeHtml(r.finish)}</small></button>`).join("");dlg.querySelectorAll("[data-choose-food]").forEach(b=>b.onclick=()=>{const id=b.dataset.chooseFood;saveTodayMealChoice(id,"alternative");dlg.close();openFoodRecipe(id,true);});};dlg.querySelector("[data-close]").onclick=()=>dlg.close();dlg.addEventListener("close",()=>dlg.remove());dlg.querySelector("#chooseFoodSearch").oninput=draw;dlg.querySelector("#chooseFoodWeb").onclick=()=>window.open(`https://www.google.com/search?q=${encodeURIComponent("receitas fáceis jantar família ingredientes simples")}`,"_blank","noopener");draw();dlg.showModal();}
function renderReceitas(){
 const d=loadRec();
 app.innerHTML=`<section class="hero"><h2>📖 Receitas</h2><p>Seu livro de receitas — com preparo completo e busca de novas ideias quando quiser variar.</p></section><div class="recipe-principle card"><span class="eyebrow">FLEXÍVEL</span><strong>Use o que já funciona ou procure outra ideia.</strong><p>Você pode fazer a receita, buscar variações dela ou encontrar opções com os mesmos ingredientes.</p></div><div class="recipe-filters"><input id="recipeSearch" placeholder="Buscar receita…"><select id="recipeCat"><option value="">Todas</option>${[...new Set(RECIPES.map(r=>r.cat))].map(c=>`<option>${c}</option>`).join("")}</select></div><div class="list" id="recipeList"></div><div class="card freezer-rule"><span class="eyebrow">❄️ FREEZER</span><p>Identificar cada preparo com <strong>NOME • DATA • Nº DE PORÇÕES • FINALIZAÇÃO</strong>.</p></div>`;
 const update=()=>{const q=(document.querySelector("#recipeSearch")?.value||"").toLowerCase(),c=document.querySelector("#recipeCat")?.value||"";const list=RECIPES.filter(r=>(!q||r.name.toLowerCase().includes(q)||(r.ingredients||[]).join(" ").toLowerCase().includes(q))&&(!c||r.cat===c));document.querySelector("#recipeList").innerHTML=recipeCards(list,loadRec());bindRecipeCards(update);};
 document.querySelector("#recipeSearch").oninput=update;document.querySelector("#recipeCat").onchange=update;update();
}
function bindRecipeCards(refresh){document.querySelectorAll("[data-open-recipe]").forEach(b=>b.onclick=()=>openFoodRecipe(b.dataset.openRecipe,false));document.querySelectorAll("[data-fav]").forEach(b=>b.onclick=e=>{e.stopPropagation();const x=loadRec(),id=b.dataset.fav;x.favorites=x.favorites.includes(id)?x.favorites.filter(v=>v!==id):[...x.favorites,id];saveRec(x);refresh();});}
function recipeCards(list,d){return list.map(r=>`<article class="card recipe-card"><div class="recipe-main"><div><span class="eyebrow">${escapeHtml(r.cat)}</span><h3>${escapeHtml(r.name)}</h3><span>${escapeHtml(r.yield)} · ${escapeHtml(r.prep)}</span></div><button class="favorite ${d.favorites.includes(r.id)?"active":""}" data-fav="${r.id}">${d.favorites.includes(r.id)?"♥":"♡"}</button></div><p><strong>Finalização:</strong> ${escapeHtml(r.finish)}</p><button type="button" class="secondary" data-open-recipe="${r.id}">Ver receita</button></article>`).join("")||`<div class="empty compact"><strong>Nenhuma receita encontrada.</strong></div>`;}

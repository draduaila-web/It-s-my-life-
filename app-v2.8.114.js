...
window.BERTHA_BUILD="2.8.110-receitas-desfazer-x-neutro";
...
function recordFoodProgress(mealId,title,source){try{const data=JSON.parse(localStorage.getItem('bertha.time-engine.v1')||'{}');data.history=Array.isArray(data.history)?data.history:[];const key=`food:${foodTodayISO()}:${mealId}`;data.history=data.history.filter(h=>h.itemId!==key);data.history.push({id:`${key}:${Date.now()}`,itemId:key,source:'Alimentação',category:'Alimentação',title:title||'Refeição',status:'done',startedAt:Date.now(),endedAt:Date.now(),realMinutes:0,learningKey:`meal:${mealId}`,mealSource:source||'planned'});localStorage.setItem('bertha.time-engine.v1',JSON.stringify(data));}catch(e){console.warn('BERTH.A alimentação/progresso',e)}}
function refreshFoodRoute(){if(state.route==='receitas')renderReceitas();else if(state.route==='alimentacao')renderAlimentacao();else if(state.route==='meu-dia')renderMeuDia();else if(state.route==='progresso')renderProgress();}
function setTodayMealConsumption(mealId,status,title,source){const {d,key}=getTodayMealState(mealId);d.mealLog[key]=d.mealLog[key]||{};d.mealLog[key].meals=d.mealLog[key].meals||{};d.mealLog[key].meals[mealId]={...(d.mealLog[key].meals[mealId]||{}),consumption:status,consumedAt:new Date().toISOString()};saveFood(d);if(status==='consumed')recordFoodProgress(mealId,title,source);refreshFoodRoute();}
function undoTodayMeal(mealId){const d=loadFood(),key=foodTodayISO();if(d.mealLog?.[key]?.meals?.[mealId]){delete d.mealLog[key].meals[mealId];if(!Object.keys(d.mealLog[key].meals).length)delete d.mealLog[key];saveFood(d)}try{const data=JSON.parse(localStorage.getItem('bertha.time-engine.v1')||'{}');data.history=Array.isArray(data.history)?data.history:[];const progressKey=`food:${key}:${mealId}`;data.history=data.history.filter(h=>h.itemId!==progressKey);localStorage.setItem('bertha.time-engine.v1',JSON.stringify(data));}catch(e){console.warn('BERTH.A alimentação/desfazer',e)}refreshFoodRoute();}
function ensureFoodModuleStyles(){
 if(document.getElementById('bertha-food-v114-styles'))return;
 const st=document.createElement('style');st.id='bertha-food-v114-styles';st.textContent=`
 .food-v114{--food-blue:#6f98bd;--food-blue-deep:#547ca2;--food-blue-soft:#eaf3fb;--food-butter:#fff4d7;--food-peach:#f9eadf;--food-ink:#35303d;--food-muted:#8b838e}
 .food-v114-hero{position:relative;overflow:hidden;margin:2px 0 18px;padding:24px 26px 25px;border:1px solid rgba(113,137,164,.18);border-radius:26px;background:radial-gradient(circle at 18% 16%,rgba(199,225,247,.72),transparent 42%),radial-gradient(circle at 83% 80%,rgba(255,234,197,.68),transparent 38%),linear-gradient(135deg,rgba(242,248,252,.94),rgba(255,250,242,.94));box-shadow:0 14px 36px rgba(79,93,110,.055)}
 .food-v114-hero .eyebrow{display:block;margin-bottom:13px;color:#537fa6;letter-spacing:.18em;font-weight:800}
 .food-v114-hero h2{max-width:82%;margin:0;color:#343441;font-size:clamp(31px,8vw,42px);line-height:1.02;letter-spacing:-.035em}
 .food-v114-hero p{max-width:82%;margin:13px 0 0;color:#7f7b84;font-size:17px;line-height:1.42}
 .food-v114-abstract{position:absolute;right:28px;top:28px;width:27px;height:27px;border:1.5px solid rgba(109,113,122,.38);transform:rotate(45deg);border-radius:1px}
 .food-v114-today{margin:0 0 18px;padding:20px 22px;border:1px solid rgba(109,136,164,.14);border-radius:25px;background:linear-gradient(135deg,rgba(255,255,255,.68),rgba(246,250,254,.82));box-shadow:0 12px 30px rgba(72,89,106,.045)}
 .food-v114-today-head{display:flex;justify-content:space-between;gap:12px;align-items:flex-start;padding-bottom:14px;border-bottom:1px solid rgba(80,103,126,.09)}
 .food-v114-today-head h3{margin:4px 0 0;font-size:24px;color:#3c3742;letter-spacing:-.02em}.food-v114-today-head .eyebrow{color:#657f99}
 .food-v114-today-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:0}
 .food-v114-meal{min-width:0;padding:16px 14px 4px}.food-v114-meal+ .food-v114-meal{border-left:1px solid rgba(80,103,126,.09)}
 .food-v114-meal-label{display:flex;align-items:center;gap:7px;color:#7d858d;font-size:11px;font-weight:800;letter-spacing:.08em;text-transform:uppercase}
 .food-v114-meal-icon{width:22px;height:22px;display:grid;place-items:center;color:var(--food-blue-deep)}
 .food-v114-meal-icon svg{width:20px;height:20px;fill:none;stroke:currentColor;stroke-width:1.7;stroke-linecap:round;stroke-linejoin:round}
 .food-v114-meal strong{display:block;margin-top:9px;color:#34323b;font-size:14px;line-height:1.28;min-height:36px}
 .food-v114-actions{display:flex;gap:6px;align-items:center;flex-wrap:wrap;margin-top:11px}
 .food-v114-actions button{min-height:34px!important;padding:7px 10px!important;border-radius:999px!important;border:1px solid rgba(93,132,166,.22)!important;background:rgba(235,244,251,.58)!important;color:#557b9d!important;font-size:11px!important;font-weight:760!important;box-shadow:none!important}
 .food-v114-actions button:hover{background:rgba(226,239,249,.8)!important}.food-v114-actions .food-view-btn{width:34px;padding:0!important;display:grid;place-items:center}
 .food-v114-actions .food-done-state{background:linear-gradient(135deg,rgba(223,239,250,.86),rgba(255,244,215,.74))!important}
 .food-v114-week{margin:0 0 18px}.food-v114-section-kicker{display:flex;align-items:center;justify-content:space-between;gap:12px;margin:0 3px 9px}.food-v114-section-kicker .eyebrow{color:#6f7f90}
 .food-v114-card{border:1px solid rgba(104,125,146,.13)!important;border-radius:24px!important;background:rgba(255,255,255,.72)!important;box-shadow:0 11px 26px rgba(72,89,106,.04)!important}
 .food-v114-breakfast{padding:17px 18px!important}.food-v114-breakfast-head{display:flex;justify-content:space-between;align-items:center;margin-bottom:12px}.food-v114-breakfast-head h3{margin:0;font-size:19px}.food-v114-pill{border-radius:999px;padding:6px 10px;background:rgba(255,239,211,.78);color:#8b7862;font-size:11px;font-weight:800}
 .food-v114-days{display:flex;gap:7px;overflow-x:auto;padding:2px 0 4px;scrollbar-width:none}.food-v114-days::-webkit-scrollbar{display:none}.food-v114-day{flex:0 0 92px;padding:11px 9px;border:1px solid rgba(98,126,154,.1);border-radius:15px;background:linear-gradient(145deg,rgba(250,244,237,.78),rgba(255,255,255,.74));text-align:center}.food-v114-day.today{background:linear-gradient(145deg,rgba(220,237,250,.9),rgba(245,249,252,.86));border-color:rgba(91,135,174,.17)}.food-v114-day b{display:block;color:#65798d;font-size:10px;letter-spacing:.08em;text-transform:uppercase}.food-v114-day span{display:block;margin-top:6px;color:#514c55;font-size:11px;line-height:1.25}
 .food-v114-dinner{padding:17px 18px!important;margin-top:10px}.food-v114-dinner-head{display:flex;align-items:center;justify-content:space-between;gap:10px;padding-bottom:8px}.food-v114-dinner-head h3{margin:0;font-size:20px}.food-v114-dinner-row{display:grid;grid-template-columns:50px minmax(0,1fr) auto;gap:10px;align-items:center;padding:11px 0;border-top:1px solid rgba(84,105,126,.08)}.food-v114-dinner-row:first-of-type{border-top:0}.food-v114-daytag{display:inline-flex;justify-content:center;padding:6px 7px;border-radius:999px;background:var(--food-blue-soft);color:#5d84a7;font-size:10px;font-weight:850}.food-v114-dinner-row strong{font-size:13px;line-height:1.25;color:#3f3a43}.food-v114-dinner-row small{display:block;margin-top:3px;color:#9a929b}.food-v114-recipe-btn{width:34px;height:34px;border:1px solid rgba(93,132,166,.18)!important;border-radius:12px!important;background:rgba(239,246,251,.64)!important;color:#5c83a5!important;padding:0!important;display:grid!important;place-items:center!important;box-shadow:none!important}
 .food-v114-routine-title{margin:20px 3px 9px;color:#74707b;letter-spacing:.16em}.food-v114-routine{padding:18px!important}.food-v114-routine label{display:grid;grid-template-columns:auto 1fr auto;gap:13px;align-items:center}.food-v114-routine input{width:24px;height:24px;accent-color:#6d95b8}.food-v114-routine strong,.food-v114-routine small{display:block}.food-v114-routine strong{font-size:17px}.food-v114-routine small{margin-top:3px;color:#8d858f;line-height:1.35}.food-v114-chevron{font-size:25px;color:#8395a4}
 @media(max-width:520px){.food-v114-hero{padding:22px 22px 23px}.food-v114-hero h2,.food-v114-hero p{max-width:84%}.food-v114-abstract{right:24px;top:27px;width:23px;height:23px}.food-v114-today{padding:18px 16px}.food-v114-today-grid{grid-template-columns:1fr}.food-v114-meal{padding:13px 2px}.food-v114-meal+.food-v114-meal{border-left:0;border-top:1px solid rgba(80,103,126,.08)}.food-v114-meal strong{min-height:0;font-size:15px}.food-v114-actions{margin-top:9px}.food-v114-dinner-row{grid-template-columns:44px minmax(0,1fr) auto}}
 `;document.head.appendChild(st)
}
...
function renderTodayFoodCard(){const meals=getTodayMeals();return `<section class="food-v114-today"><div class="food-v114-today-head"><div><span class="eyebrow">COMIDA DE HOJE · ${escapeHtml(foodDayName().toUpperCase())}</span><h3>Seu dia já começa decidido.</h3></div></div><div class="food-v114-today-grid">${meals.map(m=>{const selected=getRecipeById(m.log.selectedRecipeId)||m.recipe,name=m.log.otherText||selected?.name||m.name,done=m.log.consumption==='consumed';return `<div class="food-v114-meal"><div class="food-v114-meal-label"><span class="food-v114-meal-icon">${foodMealLineIcon(m.id)}</span>${escapeHtml(m.label)}</div><strong>${escapeHtml(name)}</strong><div class="food-v114-actions">${done?`<button type="button" class="food-done-state" data-food-undo="${m.id}">Desfazer</button><button type="button" data-food-other="${m.id}">Alterar</button>`:`<button type="button" data-food-done="${m.id}">Fiz esta</button><button type="button" data-food-other="${m.id}">Fiz outra</button>`}${selected?`<button type="button" class="food-view-btn" aria-label="Ver receita" data-food-view="${m.id}">›</button>`:''}</div></div>`}).join('')}</div></section>`;}
function bindTodayFoodActions(){document.querySelectorAll('[data-food-done]').forEach(b=>b.onclick=()=>{const s=getTodayMealState(b.dataset.foodDone),title=s.log.otherText||s.selected?.name||s.meal.name;setTodayMealConsumption(b.dataset.foodDone,'consumed',title,s.log.source||'planned')});document.querySelectorAll('[data-food-undo]').forEach(b=>b.onclick=()=>undoTodayMeal(b.dataset.foodUndo));document.querySelectorAll('[data-food-other]').forEach(b=>b.onclick=()=>openChooseRecipe(b.dataset.foodOther));document.querySelectorAll('[data-food-view]').forEach(b=>b.onclick=()=>{const s=getTodayMealState(b.dataset.foodView);if(s.selected)openFoodRecipe(s.selected.id,false,b.dataset.foodView)});}
...
function renderAlimentacao(){
 ...
 app.innerHTML=`<div class="food-v114"><section class="food-v114-hero"><span class="eyebrow">ALIMENTAÇÃO</span><h2>Know what’s next.</h2><p>Você escolhe. A BERTH.A organiza seu dia.</p><span class="food-v114-abstract" aria-hidden="true"></span></section>${renderTodayFoodCard()}<section class="food-v114-week"><div class="food-v114-section-kicker"><span class="eyebrow">SEMANA ${escapeHtml(String(d.week||1))} · CARDÁPIO</span></div><div class="card food-v114-card food-v114-breakfast"><div class="food-v114-breakfast-head"><h3>Café da manhã</h3><span class="food-v114-pill">7 dias</span></div><div class="food-v114-days">${d.breakfasts.map(x=>`<div class="food-v114-day ${x[0]===today?'today':''}"><b>${dayAbbr[x[0]]||escapeHtml(x[0].slice(0,3))}</b><span>${escapeHtml(x[1])}</span></div>`).join('')}</div></div><div class="card food-v114-card food-v114-dinner"><div class="food-v114-dinner-head"><h3>Jantar + marmita</h3><span class="food-v114-pill">3 pessoas</span></div>${d.dinners.map(x=>{const r=recipeForMealName(x[1]);return `<div class="food-v114-dinner-row"><span class="food-v114-daytag">${dayAbbr[x[0]]||escapeHtml(x[0].slice(0,3))}</span><div><strong>${escapeHtml(x[1])}</strong>${x[2]?`<small>marmita → ${escapeHtml(x[2])}</small>`:''}</div>${r?`<button type="button" class="food-v114-recipe-btn" aria-label="Ver receita" data-food-recipe="${r.id}">›</button>`:'<span></span>'}</div>`}).join('')}</div></section><div class="section-title food-v114-routine-title">ROTINA DE PREPARO</div><div class="card food-v114-card food-v114-routine"><label><input type="checkbox" id="foodCook" ${d.cookingDone?'checked':''}><span><strong>Cozinha quinzenal</strong><small>Produzir bases, porcionar, etiquetar e congelar.</small></span><span class="food-v114-chevron">›</span></label></div></div>`;
 document.querySelector('#foodCook').onchange=e=>{const x=loadFood();x.cookingDone=e.target.checked;saveFood(x)};bindTodayFoodActions();document.querySelectorAll('[data-food-recipe]').forEach(b=>b.onclick=()=>openFoodRecipe(b.dataset.foodRecipe,false));
}

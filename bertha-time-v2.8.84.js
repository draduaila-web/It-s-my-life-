      ...
      if(item.notify && window.berthaSchedulePush){
        try {
          const scheduled=await Promise.race([
            ...
            new Promise(resolve=>setTimeout(()=>resolve({ok:false,message:'Diagnóstico v84 — o serviço de push não respondeu em 8 segundos (TIMEOUT). A tarefa foi salva normalmente. Envie este aviso.'}),8000))
          ]);
          if(!scheduled?.ok){ alert(scheduled?.message||'A tarefa foi salva, mas não consegui agendar o aviso.'); }
        } catch(error){
          console.warn('BERTH.A schedule push',error);
          alert('A tarefa foi salva, mas não consegui agendar o aviso.');
        }
      }
      close(); rerender();
    };

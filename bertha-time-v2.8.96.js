...
(() => {
  ...
  function ensureHomeUtilityStyles(){
    ...
    const st=document.createElement('style');st.id='bertha-home-utility-styles';st.textContent=`
      ...
      .home-food-mini{margin:12px 0 16px;padding:14px 16px;border:1px solid rgba(164,105,126,.10);border-radius:20px;background:linear-gradient(120deg,rgba(255,250,245,.92),rgba(250,225,218,.18),rgba(247,220,230,.15));box-shadow:0 5px 16px rgba(91,65,75,.035)}
      .home-food-mini .section-head{margin-bottom:8px}.home-food-mini .section-head h2{font-size:17px;margin:2px 0 0}.home-food-mini .section-head a{font-size:12px;color:#9b6076;text-decoration:none}.home-food-mini-kicker{font-size:10px;letter-spacing:.18em;font-weight:800;color:#9b6076}.home-food-mini-row{display:grid;grid-template-columns:18px minmax(0,1fr);gap:8px;align-items:start;padding:7px 0;border-top:1px solid rgba(115,91,101,.07)}.home-food-mini-row:first-of-type{border-top:0}.home-food-mini-mark{color:#a4637b;font-size:13px;line-height:1.4}.home-food-mini-row small,.home-food-mini-row strong{display:block}.home-food-mini-row small{font-size:10.5px;color:#91868c}.home-food-mini-row strong{font-size:13px;color:#4d454b;margin-top:1px;font-weight:650}
  ...
  function renderFoodHomeMini(){
    try{
      ...
      const rows=meals.map(m=>{const selected=(typeof getRecipeById==='function'&&getRecipeById(m.log?.selectedRecipeId))||m.recipe;const name=m.log?.otherText||selected?.name||m.name;const done=m.log?.consumption==='consumed';return `<div class="home-food-mini-row"><span class="home-food-mini-mark">${done?'✓':'—'}</span><span><small>${esc(m.label)}</small><strong>${esc(name)}</strong></span></div>`}).join('');
      return `<section class="home-food-mini"><div class="section-head"><div><span class="home-food-mini-kicker">COZINHA DE HOJE</span><h2>Refeições</h2></div><a href="#receitas">ver receitas</a></div>${rows}</section>`;
    }catch(_){return ''}
  }

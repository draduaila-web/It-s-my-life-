...
function finReminderMomentLabel(o){
 const b=o.bill||{}, now=new Date(), today=todayISO();
 if(o.days!==0)return `${o.status.label} · ${formatDate(o.due)}`;
 const first=b.reminderTime||'09:00', again=b.repeatReminderTime||'21:30', repeat=b.repeatIfPending!==false;
 const hhmm=`${String(now.getHours()).padStart(2,'0')}:${String(now.getMinutes()).padStart(2,'0')}`;
 if(hhmm<first)return `Vence hoje · aviso ${first}`;
 if(repeat && hhmm<again)return `Vence hoje · próximo aviso ${again}`;
 return `Vence hoje · ${formatDate(o.due)}`;
}
function renderFinanceRemindersMeuDia(){
 const items=finHomeReminders(); if(!items.length)return '';
 const rows=items.map(o=>`<a class="home-fin-reminder-row" href="#financeiro"><span class="home-fin-reminder-icon ${o.days<0?'is-late':'is-clock'}" aria-hidden="true"></span><span><strong>${escapeHtml(o.bill.name||'Conta prevista')}</strong><small>${escapeHtml(finReminderMomentLabel(o))}</small></span><b>›</b></a>`).join('');
...
function ensureSoftMeuDiaStyles(){
  ...
  style.textContent=`
    ...
    .home-fin-reminders{background:radial-gradient(circle at 92% 12%,rgba(135,177,218,.07),transparent 32%),linear-gradient(135deg,rgba(255,251,244,.98),rgba(248,246,252,.94))!important;border:1px solid rgba(112,92,156,.08)!important;border-radius:22px!important;padding:12px 14px!important;box-shadow:0 7px 20px rgba(76,58,82,.03)!important;}
    ...
    .home-fin-reminder-icon{display:block!important;position:relative!important;width:18px!important;height:18px!important;color:#765f99!important;background:transparent!important;border-radius:0!important}.home-fin-reminder-icon.is-clock:before{content:"";position:absolute;inset:1.5px;border:1.45px solid currentColor;border-radius:50%;opacity:.8}.home-fin-reminder-icon.is-clock:after{content:"";position:absolute;left:8.25px;top:4.5px;width:1.35px;height:5px;background:currentColor;border-radius:2px;transform-origin:50% 4px;transform:rotate(-18deg);opacity:.8}.home-fin-reminder-icon.is-late:before{content:"!";position:absolute;inset:1.5px;border:1.45px solid currentColor;border-radius:50%;display:grid;place-items:center;font-size:10px;line-height:1;font-weight:700;opacity:.82}
...
const FIN_BASE={
 ...
 transactions:[],
 plannedBills:[],
 paymentMethods:["Pix","Boleto","Débito","Crédito","Transferência","Dinheiro","Débito automático","Outro"],
 paymentSources:[]
};
...
function loadFin(){
 try{
  ...
  if(raw){
   const migrated={...FIN_BASE,...raw,fixed:Array.isArray(raw.fixed)?raw.fixed:[],excluded:Array.isArray(raw.excluded)?raw.excluded:[],goals:Array.isArray(raw.goals)?raw.goals:[],transactions:Array.isArray(raw.transactions)?raw.transactions:[],plannedBills:Array.isArray(raw.plannedBills)?raw.plannedBills:[],paymentMethods:Array.isArray(raw.paymentMethods)&&raw.paymentMethods.length?raw.paymentMethods:FIN_BASE.paymentMethods.slice(),paymentSources:Array.isArray(raw.paymentSources)?raw.paymentSources:[]};
   if(!migrated.goalTitle && migrated.goals.length) migrated.goalTitle="Fundo Carro";
   return migrated;
  }
 }catch{}
 try{
...
/* v2.8.185 — Financeiro consolidado: passado auditável · presente claro · futuro previsível */
(function(){
 const style=document.createElement('style');style.id='bertha-fin-v185';style.textContent=`
 .finance-row.fin-editable{display:grid;grid-template-columns:minmax(0,1fr) auto auto;gap:10px;align-items:center}.fin-row-edit{border:0;background:transparent;color:#6f6380;font-size:18px;padding:6px 4px}
 .fin-delete-bill{background:#fffaf7!important;color:#8a6268!important;border:1px solid rgba(138,98,104,.18)!important;box-shadow:none!important}
 .fin-upcoming-panel{padding:16px}.fin-upcoming-row{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:12px;align-items:center;padding:12px 0;border-top:1px solid rgba(92,72,104,.08)}.fin-upcoming-row:first-child{border-top:0}.fin-upcoming-row span,.fin-upcoming-row small{display:block}.fin-upcoming-row small{margin-top:3px;color:#8b818d}.fin-upcoming-row b{white-space:nowrap}
 .fin-payment-breakdown{display:grid;grid-template-columns:1fr 1fr;gap:10px}.fin-payment-total{margin:10px 0 2px;padding:12px 14px;border-radius:16px;background:linear-gradient(120deg,rgba(238,232,248,.75),rgba(234,244,246,.72));display:flex;justify-content:space-between;align-items:center}.fin-payment-total b{font-size:18px}.fin-payment-meta{display:grid;grid-template-columns:1fr 1fr;gap:10px}.fin-payment-hint{font-size:11px;color:#8a808c;line-height:1.4;margin:4px 0 12px}
 .goal-row-v185{display:grid;grid-template-columns:minmax(0,1fr) auto auto;gap:10px;align-items:center;padding:12px 0;border-top:1px solid rgba(92,72,104,.08)}.goal-row-v185:first-child{border-top:0}.goal-row-v185 small{display:block;color:#8b818d;margin-top:3px}.goal-row-v185 button{border:0;border-radius:999px;padding:8px 11px;background:linear-gradient(125deg,#e7d8f2,#dbeaf4);color:#67596f;font-weight:800}.goal-history{margin-top:12px;display:grid;gap:7px}.goal-history-row{display:grid;grid-template-columns:1fr auto auto;gap:8px;align-items:center;padding:9px 10px;border-radius:13px;background:rgba(255,255,255,.56)}.goal-history-row button{border:0;background:transparent;color:#966d72;font-size:18px}
 .home-fin-reminders{margin-top:12px!important;margin-bottom:12px!important}.home-fin-reminders .section-head{margin-bottom:6px!important}.home-fin-reminder-row{min-height:0!important;padding:9px 12px!important;border-radius:16px!important;grid-template-columns:24px minmax(0,1fr) auto!important}.home-fin-reminder-svg{width:19px;height:19px;stroke:#7b7190;fill:none;stroke-width:1.55;stroke-linecap:round;stroke-linejoin:round}.home-fin-reminder-row small{font-size:11px!important;margin-top:1px!important}.home-fin-reminder-row strong{font-size:13px!important}
 .planned-bill-row.paid .planned-bill-copy>span{color:#7f7681}.planned-bill-row.late .planned-bill-copy>span{color:#786d79}
 .fin-status.paid{background:rgba(203,228,214,.72)!important;color:#557462!important}.fin-status.late{background:rgba(235,221,232,.72)!important;color:#775c72!important}
 @media(max-width:430px){.fin-payment-breakdown,.fin-payment-meta{grid-template-columns:1fr}.finance-row.fin-editable{grid-template-columns:minmax(0,1fr) auto auto}.goal-row-v185{grid-template-columns:minmax(0,1fr) auto}.goal-row-v185>button{grid-column:2;grid-row:1/3}.fin-upcoming-row{grid-template-columns:1fr auto}}
 `;document.head.appendChild(style);
})();

function finMonthOffset(month,offset){const [y,m]=month.split('-').map(Number),d=new Date(y,m-1+offset,1);return `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}`}
function finParseMoney(v){if(typeof v==='number')return v;let x=String(v||'').trim().replace(/R\$\s*/g,'').replace(/\./g,'').replace(',','.').replace(/[^0-9.-]/g,'');return Number(x)||0}
function finMoneyField(n){return money(Number(n||0))}
function finGoalActual(g){if(Array.isArray(g?.contributions)&&g.contributions.length)return g.contributions.reduce((s,x)=>s+Number(x.value||0),0);return Number(g?.saved||0)}
function finGoalContributions(g){if(Array.isArray(g.contributions))return g.contributions;if(Number(g.saved||0)>0)return [{id:'legacy-'+Date.now(),value:Number(g.saved),date:'',note:'Valor anterior'}];return []}
function finPaymentPenalty(p){return Number(p?.interest||0)+Number(p?.fine||0)+Number(p?.otherCharges||0)}
function finPaymentTotal(p){if(p?.total!=null)return Number(p.total)||0;return Number(p?.value||0)}
function finPaymentDaysLate(due,paidDate){if(!due||!paidDate)return 0;const a=new Date(due+'T12:00:00'),b=new Date(paidDate+'T12:00:00');return Math.max(0,Math.round((b-a)/86400000))}
function finAttentionState(o){
 if(!o||o.paid||o.bill?.reminderEnabled===false)return false;
 const days=finDaysTo(o.due),lead=Number(o.bill?.reminderDays??0);if(days<0)return true;if(days>lead)return false;
 const now=new Date(),hhmm=`${String(now.getHours()).padStart(2,'0')}:${String(now.getMinutes()).padStart(2,'0')}`;
 if(days===lead && hhmm<(o.bill.reminderTime||'09:00'))return false;
 return true;
}
function finReminderLine(o){
 const b=o.bill||{},days=finDaysTo(o.due),status=finPlannedStatus(o);
 if(o.paid){const pd=o.payment?.date||'';return `${escapeHtml(b.category||'Outros')} · ${pd?`paga em ${formatDate(pd)}`:formatDate(o.due)}${(b.recurrence||'once')==='monthly'?' · mensal':''}`}
 if(days<0){const repeat=b.repeatIfPending!==false?` · próximo aviso ${b.repeatReminderTime||'21:30'}`:'';return `${escapeHtml(b.category||'Outros')} · ${status.label}${b.reminderEnabled===false?' · sem aviso':repeat}`}
 const rem=Number(b.reminderDays??0),copy=b.reminderEnabled===false?'sem aviso':`${rem===0?'no dia':`${rem}d antes`} · ${b.reminderTime||'09:00'}${b.repeatIfPending===false?'':` · repetir ${b.repeatReminderTime||'21:30'} se pendente`}`;
 return `${escapeHtml(b.category||'Outros')} · ${formatDate(o.due)}${(b.recurrence||'once')==='monthly'?' · mensal':''} · ${escapeHtml(copy)}`
}
finFixedHtml=function(x){return `<article class="card finance-row fin-editable"><div><strong>${escapeHtml(x.name)}</strong><span>${escapeHtml(x.category||'Outros')}${x.kind==='teto'?' · teto':' · fixo'}</span></div><b>R$ ${money(x.value)}</b><button type="button" class="fin-row-edit" data-fin-edit-fixed="${x.id}" aria-label="Editar">⌕</button></article>`}
finPlannedBillHtml=function(o){const st=finPlannedStatus(o),b=o.bill,total=o.paid?finPaymentTotal(o.payment):o.expected;return `<article class="card planned-bill-row ${st.cls}"><div class="planned-bill-copy"><div class="planned-bill-top"><strong>${escapeHtml(b.name||'Conta prevista')}</strong><span class="fin-status ${st.cls}">${escapeHtml(st.label)}</span></div><span>${finReminderLine(o)}</span>${b.note?`<small>${escapeHtml(b.note)}</small>`:''}</div><div class="planned-bill-side"><b>R$ ${money(total)}</b><div class="planned-bill-actions">${o.paid?`<button type="button" class="secondary tiny" data-fin-unpay="${b.id}" data-month="${o.month}">Desfazer</button>`:`<button type="button" class="primary tiny" data-fin-pay="${b.id}" data-month="${o.month}">Marcar paga</button>`}<button type="button" class="secondary tiny" data-fin-edit-bill="${b.id}">Editar</button></div></div></article>`}

function openFinModal(type,id=null){
 const d=loadFin(),existing=type==='fixed'&&id?(d.fixed||[]).find(x=>x.id===id):null,dlg=document.createElement('dialog');dlg.className='fin-planned-dialog fin-unified-dialog';
 const title=type==='income'?'Ajustar renda mensal':type==='fixed'?(existing?'Editar item do orçamento':'Adicionar ao orçamento'):'Registrar gasto';
 ...
 dlg.innerHTML=`<form method="dialog" class="modal-card" id="finForm"><div class="modal-head"><div><div class="eyebrow">FINANCEIRO</div><h2>${title}</h2></div><button type="button" class="icon-btn fin-modal-x" data-fin-close>×</button></div>${type==='income'?`<label>Renda mensal<input id="fValue" required type="number" min="0" step="0.01" value="${d.income}"></label>`:`<label>Descrição<input id="fName" required maxlength="100" value="${escapeHtml(existing?.name||'')}"></label><div class="form-grid"><label>Valor<input id="fValue" required type="number" min="0" step="0.01" value="${existing?.value??''}"></label><label>Categoria<select id="fCategory">${cats.map(c=>`<option ${c===(existing?.category||'Casa')?'selected':''}>${c}</option>`).join('')}</select></label></div>${type==='transaction'?`<label>Data<input id="fDate" type="date" value="${todayISO()}"></label>`:`<label>Tipo<select id="fKind"><option value="fixo" ${existing?.kind!=='teto'?'selected':''}>Fixo</option><option value="teto" ${existing?.kind==='teto'?'selected':''}>Teto</option></select></label>`}`}${existing?`<button type="button" class="fin-delete-bill" id="deleteFixed">Excluir item do orçamento</button>`:''}<div class="modal-actions"><div class="grow"></div><button type="button" class="secondary" id="cancelFin">Cancelar</button><button class="primary">Salvar</button></div></form>`;
 document.body.appendChild(dlg);dlg.showModal();const close=bindFinanceModalClose(dlg);dlg.querySelector('#cancelFin').onclick=e=>{e.preventDefault();close()};
 dlg.querySelector('#deleteFixed')?.addEventListener('click',()=>{if(!confirm(`Excluir “${existing?.name||'este item'}”?`))return;d.fixed=(d.fixed||[]).filter(x=>x.id!==existing.id);saveFin(d);close();renderFinanceiro()});
 dlg.querySelector('#finForm').onsubmit=e=>{e.preventDefault();if(type==='income')d.income=+dlg.querySelector('#fValue').value||0;else{const obj={...(existing||{}),id:existing?.id||uid(),name:dlg.querySelector('#fName').value.trim(),value:+dlg.querySelector('#fValue').value||0,category:dlg.querySelector('#fCategory').value,updatedAt:Date.now()};if(type==='transaction'){obj.date=dlg.querySelector('#fDate').value;d.transactions.push(obj)}else{obj.kind=dlg.querySelector('#fKind').value;obj.payer='Usuária';d.fixed=existing?d.fixed.map(x=>x.id===obj.id?obj:x):[...d.fixed,obj]}}saveFin(d);close();renderFinanceiro()}
}

function openPlannedPayModal(id,month){
 const d=loadFin(),b=(d.plannedBills||[]).find(x=>x.id===id);if(!b)return;const o=finPlannedOccurrence(b,month);if(!o)return;const existing=o.payment||{},methods=(d.paymentMethods||FIN_BASE.paymentMethods).filter(Boolean),sources=(d.paymentSources||[]).filter(Boolean);
 const dlg=document.createElement('dialog');dlg.className='fin-planned-dialog fin-pay-dialog fin-unified-dialog';dlg.innerHTML=`<form method="dialog" class="modal-card" id="finPayForm"><div class="modal-head"><div><div class="eyebrow">✓ MARCAR COMO PAGA</div><h2>${escapeHtml(b.name)}</h2></div><button type="button" class="icon-btn fin-modal-x" data-fin-close>×</button></div><p class="note">Previsto: R$ ${money(o.expected)} · vencimento ${formatDate(o.due)}</p><div class="fin-payment-breakdown"><label>Valor principal<input id="pbPrincipal" inputmode="decimal" value="${finMoneyField(existing.principal??o.expected)}"></label><label>Data do pagamento<input id="pbPaidDate" type="date" value="${existing.date||todayISO()}"></label><label>Juros<input id="pbInterest" inputmode="decimal" value="${finMoneyField(existing.interest||0)}"></label><label>Multa<input id="pbFine" inputmode="decimal" value="${finMoneyField(existing.fine||0)}"></label><label>Desconto<input id="pbDiscount" inputmode="decimal" value="${finMoneyField(existing.discount||0)}"></label><label>Outros encargos<input id="pbOther" inputmode="decimal" value="${finMoneyField(existing.otherCharges||0)}"></label></div><div class="fin-payment-total"><span>Total desembolsado</span><b id="pbTotal">R$ ${money(finPaymentTotal(existing)||o.expected)}</b></div><p class="fin-payment-hint">Principal + juros + multa + outros encargos − desconto. A BERTH.A guarda cada parte separadamente para mostrar o custo real de atrasos.</p><div class="fin-payment-meta"><label>Forma de pagamento<input id="pbMethod" list="finMethods" value="${escapeHtml(existing.method||'')}" placeholder="Ex.: Pix"><datalist id="finMethods">${methods.map(x=>`<option value="${escapeHtml(x)}">`).join('')}</datalist></label><label>De onde saiu<input id="pbSource" list="finSources" value="${escapeHtml(existing.source||'')}" placeholder="Ex.: CC BB"><datalist id="finSources">${sources.map(x=>`<option value="${escapeHtml(x)}">`).join('')}</datalist></label></div><div class="modal-actions"><div class="grow"></div><button type="button" class="secondary" id="cancelFinPay">Cancelar</button><button class="primary">Confirmar pagamento</button></div></form>`;
 ...
 const ids=['pbPrincipal','pbInterest','pbFine','pbDiscount','pbOther'];const calc=()=>{const principal=finParseMoney(dlg.querySelector('#pbPrincipal').value),interest=finParseMoney(dlg.querySelector('#pbInterest').value),fine=finParseMoney(dlg.querySelector('#pbFine').value),discount=finParseMoney(dlg.querySelector('#pbDiscount').value),other=finParseMoney(dlg.querySelector('#pbOther').value);dlg.querySelector('#pbTotal').textContent='R$ '+money(Math.max(0,principal+interest+fine+other-discount))};ids.forEach(i=>{const el=dlg.querySelector('#'+i);el.addEventListener('input',calc);el.addEventListener('blur',()=>{el.value=finMoneyField(finParseMoney(el.value));calc()})});calc();
 dlg.querySelector('#finPayForm').onsubmit=e=>{e.preventDefault();const principal=finParseMoney(dlg.querySelector('#pbPrincipal').value),interest=finParseMoney(dlg.querySelector('#pbInterest').value),fine=finParseMoney(dlg.querySelector('#pbFine').value),discount=finParseMoney(dlg.querySelector('#pbDiscount').value),otherCharges=finParseMoney(dlg.querySelector('#pbOther').value),total=Math.max(0,principal+interest+fine+otherCharges-discount),date=dlg.querySelector('#pbPaidDate').value,method=dlg.querySelector('#pbMethod').value.trim(),source=dlg.querySelector('#pbSource').value.trim(),daysLate=finPaymentDaysLate(o.due,date);b.payments=b.payments||{};b.payments[month]={paid:true,value:total,total,principal,interest,fine,discount,otherCharges,date,method,source,daysLate,onTime:daysLate===0,categorySnapshot:b.category||'Outros',dueSnapshot:o.due,at:Date.now()};if(method&&!d.paymentMethods.includes(method))d.paymentMethods.push(method);if(source&&!d.paymentSources.includes(source))d.paymentSources.push(source);const prev=(d.transactions||[]).find(t=>t.plannedBillId===b.id&&t.plannedBillMonth===month),tx={id:prev?.id||uid(),name:b.name,value:total,date,category:b.category||'Outros',plannedBillId:b.id,plannedBillMonth:month,note:'Conta prevista · paga',paymentMethod:method,paymentSource:source,principal,interest,fine,discount,otherCharges,daysLate,onTime:daysLate===0,dueDate:o.due,updatedAt:Date.now()};d.transactions=prev?d.transactions.map(t=>t.id===prev.id?tx:t):[...d.transactions,tx];saveFin(d);if(window.berthaResolveBillReminder){try{window.berthaResolveBillReminder({billId:b.id,month,name:b.name})}catch(_e){}}close();renderFinanceiro()}
}

function openGoalContributionModal(index){
 const d=loadFin(),g=d.goals?.[index];if(!g)return;g.contributions=finGoalContributions(g);g.saved=finGoalActual(g);const dlg=document.createElement('dialog');dlg.className='fin-planned-dialog fin-unified-dialog';
 dlg.innerHTML=`<form class="modal-card" id="goalForm"><div class="modal-head"><div><div class="eyebrow">META FINANCEIRA</div><h2>${escapeHtml(g.month||'Meta')}</h2></div><button type="button" class="icon-btn fin-modal-x" data-fin-close>×</button></div><div class="form-grid"><label>Planejado mínimo<input id="goalMin" type="number" min="0" step="0.01" value="${Number(g.min||0)}"></label><label>Planejado máximo<input id="goalMax" type="number" min="0" step="0.01" value="${Number(g.max||0)}"></label></div><div class="form-grid"><label>Novo aporte<input id="goalValue" inputmode="decimal" placeholder="0,00"></label><label>Data<input id="goalDate" type="date" value="${todayISO()}"></label></div><label>Observação <span class="muted">(opcional)</span><input id="goalNote" maxlength="100" placeholder="Ex.: venda, economia extra…"></label><div class="goal-history">${g.contributions.length?g.contributions.slice().reverse().map(c=>`<div class="goal-history-row"><span>${c.date?formatDate(c.date):'Saldo anterior'}${c.note?` · ${escapeHtml(c.note)}`:''}</span><b>R$ ${money(c.value)}</b><button type="button" data-goal-del="${c.id}">×</button></div>`).join(''):'<div class="empty compact"><strong>Nenhum aporte registrado.</strong></div>'}</div><div class="modal-actions"><div class="grow"></div><button type="button" class="secondary" id="cancelGoal">Cancelar</button><button class="primary">Salvar</button></div></form>`;
 document.body.appendChild(dlg);dlg.showModal();const close=bindFinanceModalClose(dlg);dlg.querySelector('#cancelGoal').onclick=e=>{e.preventDefault();close()};dlg.querySelectorAll('[data-goal-del]').forEach(btn=>btn.onclick=()=>{g.contributions=g.contributions.filter(c=>String(c.id)!==String(btn.dataset.goalDel));g.saved=finGoalActual(g);saveFin(d);close();renderFinanceiro()});
 dlg.querySelector('#goalForm').onsubmit=e=>{e.preventDefault();g.min=+dlg.querySelector('#goalMin').value||0;g.max=+dlg.querySelector('#goalMax').value||0;const value=finParseMoney(dlg.querySelector('#goalValue').value);if(value>0)g.contributions.push({id:uid(),value,date:dlg.querySelector('#goalDate').value,note:dlg.querySelector('#goalNote').value.trim(),at:Date.now()});g.saved=finGoalActual(g);saveFin(d);close();renderFinanceiro()}
}

renderFinanceiro=function(){
 const d=loadFin(),month=finCurrentMonth(),fixed=finFixedTotal(d),variable=finMonthTotal(d,month),remaining=Number(d.income||0)-fixed-variable,cats=finCategoryTotals(d,month),monthBills=finPlannedMonth(d,month),nextMonth=finMonthOffset(month,1),nextBills=finPlannedMonth(d,nextMonth).filter(o=>(o.bill.recurrence||'once')==='monthly');
 const openPlanned=monthBills.filter(x=>!x.paid).reduce((s,x)=>s+x.expected,0),paidPlanned=monthBills.filter(x=>x.paid).reduce((s,x)=>s+finPaymentTotal(x.payment),0),alerts=monthBills.filter(finAttentionState);
 const catHtml=Object.entries(cats).sort((a,b)=>b[1]-a[1]).slice(0,6).map(([k,v])=>`<div class="finance-cat"><span>${escapeHtml(k)}</span><strong>R$ ${money(v)}</strong></div>`).join('')||`<div class="empty compact"><strong>Nenhum gasto variável registrado.</strong><span>Registre apenas o que realmente precisar acompanhar.</span></div>`;
 const goalTotal=(d.goals||[]).reduce((s,g)=>s+finGoalActual(g),0),goalMin=(d.goals||[]).reduce((s,g)=>s+Number(g.min||0),0),goalMax=(d.goals||[]).reduce((s,g)=>s+Number(g.max||0),0),goalTitle=d.goalTitle||'Metas financeiras';
 app.innerHTML=`<section class="finance-hero"><div><span class="finance-hero-kicker">FINANCEIRO</span><h2>Know what’s ahead.<br>Make room for life.</h2><p>Você registra o essencial.<br>A BERTH.A clareia o horizonte.</p></div><span class="finance-hero-icon finance-hero-symbol" aria-hidden="true"><svg viewBox="0 0 40 40"><path d="M7 22.5c4.4-4.2 8.7-6.3 13-6.3s8.6 2.1 13 6.3"/><path d="M9.5 27.5h21"/><circle cx="20" cy="16.2" r="1.35"/></svg></span></section>
 <section class="finance-summary card"><div class="finance-main"><span class="eyebrow">RENDA MENSAL</span><strong>R$ ${money(d.income)}</strong><button class="text-btn" id="editIncome">editar</button><button class="text-btn" id="toggleFinPrivacy">Ocultar valores</button></div><div class="finance-metrics"><div><span>Base</span><b>R$ ${money(fixed)}</b></div><div><span>Realizado · ${escapeHtml(finMonthLabel(month))}</span><b>R$ ${money(variable)}</b></div><div><span>Disponível conhecido</span><b>R$ ${money(remaining)}</b></div></div></section>
 ${alerts.length?`<section class="fin-due-alert"><strong><span class="fin-alert-icon" aria-hidden="true">!</span>${alerts.length===1?'1 conta pede atenção':`${alerts.length} contas pedem atenção`}</strong><span>${alerts.slice(0,3).map(o=>`${escapeHtml(o.bill.name)} · ${escapeHtml(finPlannedStatus(o).label)}`).join('<br>')}</span></section>`:''}
 <div class="section-title">CONTAS PREVISTAS · ${escapeHtml(finMonthLabel(month).toUpperCase())}</div><section class="card planned-bills-panel"><div class="panel-head"><div><span class="eyebrow">PREVISTO → REALIZADO</span><h3>Vencimentos do mês</h3></div><button class="primary compact-btn" id="addPlannedBill">＋ Nova conta prevista</button></div><div class="planned-mini-metrics"><div><span>A vencer</span><b>R$ ${money(openPlanned)}</b></div><div><span>Já pago</span><b>R$ ${money(paidPlanned)}</b></div></div><div class="planned-bills-list">${monthBills.length?monthBills.map(finPlannedBillHtml).join(''):`<div class="empty compact"><strong>Nenhuma conta prevista neste mês.</strong><span>Cadastre vencimentos para a BERTH.A lembrar o que vem pela frente.</span></div>`}</div></section>
 ${nextBills.length?`<div class="section-title">PRÓXIMOS VENCIMENTOS · ${escapeHtml(finMonthLabel(nextMonth).toUpperCase())}</div><section class="card fin-upcoming-panel">${nextBills.map(o=>`<div class="fin-upcoming-row"><div><strong>${escapeHtml(o.bill.name)}</strong><small>${formatDate(o.due)} · ${escapeHtml(o.bill.category||'Outros')} · mensal${o.bill.reminderEnabled===false?' · sem aviso':` · aviso ${o.bill.reminderDays===0?'no dia':`${o.bill.reminderDays}d antes`}`}</small></div><b>R$ ${money(o.expected)}</b></div>`).join('')}</section>`:''}
 <div class="section-title">ORÇAMENTO BASE</div><div class="list">${finPersonal(d.fixed).map(finFixedHtml).join('')||`<div class="empty compact"><strong>Seu orçamento base começa limpo.</strong><span>Adicione apenas os compromissos que fazem sentido para você.</span></div>`}</div><button class="add-full secondary" id="addFixed">＋ Adicionar item ao orçamento</button>
 <div class="section-title">GASTOS DO MÊS</div><section class="card"><div class="panel-head"><div><span class="eyebrow">${escapeHtml(finMonthLabel(month))}</span><h3>O que saiu de verdade</h3></div><button class="primary compact-btn" id="addTransaction">＋ Registrar</button></div><div class="list inner-list">${finMonthTransactions(d,month).slice().reverse().slice(0,20).map(finTransactionHtml).join('')||`<div class="empty compact"><strong>Nenhum gasto registrado.</strong><span>O registro é opcional — use quando ajudar a enxergar seu mês.</span></div>`}</div></section>
 <div class="section-title">POR CATEGORIA</div><section class="card finance-cats">${catHtml}</section>
 <div class="section-title">${escapeHtml(goalTitle.toUpperCase())}</div><section class="card goal-card"><div class="panel-head"><div><span class="eyebrow">PLANEJADO → REAL</span><h3>Evolução da meta</h3></div><span class="pill today">R$ ${money(goalTotal)}</span></div>${(d.goals||[]).length?`<p class="note">Planejado acumulado: R$ ${money(goalMin)}–R$ ${money(goalMax)}. O realizado sempre mostra o que aconteceu de verdade.</p><div class="goal-list">${d.goals.map((g,i)=>{const a=finGoalActual(g);return `<div class="goal-row-v185"><div><strong>${escapeHtml(g.month)}</strong><small>Real R$ ${money(a)} · planejado R$ ${money(g.min)}–${money(g.max)}</small></div><b>R$ ${money(a)}</b><button type="button" data-goal-edit="${i}">＋ aporte</button></div>`}).join('')}</div>`:`<div class="empty compact"><strong>Nenhuma meta cadastrada.</strong><span>A versão limpa não traz metas pessoais pré-carregadas.</span></div>`}</section>
 <div class="section-title finance-covered-title">DESPESAS COBERTAS</div><div class="finance-covered-intro">Compromissos que hoje não saem da sua renda.</div><div class="list finance-covered-list">${finPersonal(d.excluded).map((x,i)=>`<div class="card excluded-card"><button type="button" class="covered-edit" data-fin-covered-index="${i}"><div><strong>${escapeHtml(x.name)}</strong><span>${x.value?`R$ ${money(x.value)} · `:''}${x.category?`${escapeHtml(x.category)} · `:''}${escapeHtml(x.reason||'')}</span></div><span class="pill">${escapeHtml(x.payer||'Outra fonte')}</span></button></div>`).join('')||`<div class="covered-empty">Nenhuma despesa coberta cadastrada.</div>`}</div><button class="add-full secondary covered-add" id="addCovered">＋ Adicionar despesa coberta</button>`;
 ensureFinanceStyles();document.getElementById('editIncome').onclick=()=>openFinModal('income');document.getElementById('toggleFinPrivacy').onclick=toggleFinPrivacyNative;document.getElementById('addPlannedBill').onclick=()=>openPlannedBillModal();document.getElementById('addFixed').onclick=()=>openFinModal('fixed');document.getElementById('addTransaction').onclick=()=>openFinModal('transaction');document.getElementById('addCovered').onclick=()=>openCoveredExpenseModal();document.querySelectorAll('[data-fin-covered-index]').forEach(b=>b.onclick=()=>openCoveredExpenseModal(Number(b.dataset.finCoveredIndex)));document.querySelectorAll('[data-fin-edit-fixed]').forEach(b=>b.onclick=()=>openFinModal('fixed',b.dataset.finEditFixed));document.querySelectorAll('[data-fin-edit-bill]').forEach(b=>b.onclick=()=>openPlannedBillModal(b.dataset.finEditBill));document.querySelectorAll('[data-fin-pay]').forEach(b=>b.onclick=()=>openPlannedPayModal(b.dataset.finPay,b.dataset.month));document.querySelectorAll('[data-fin-unpay]').forEach(b=>b.onclick=()=>undoPlannedPayment(b.dataset.finUnpay,b.dataset.month));document.querySelectorAll('[data-fin-delete]').forEach(b=>b.onclick=()=>{const x=loadFin();x.transactions=x.transactions.filter(t=>t.id!==b.dataset.finDelete);saveFin(x);renderFinanceiro()});document.querySelectorAll('[data-goal-edit]').forEach(b=>b.onclick=()=>openGoalContributionModal(Number(b.dataset.goalEdit)));applyFinPrivacyNative();
}

finHomeReminders=function(){try{const d=loadFin(),months=[finHomeMonthOffset(-1),finHomeMonthOffset(0),finHomeMonthOffset(1)],seen=new Set(),items=[];months.forEach(month=>finPlannedMonth(d,month).forEach(o=>{if(!finAttentionState(o))return;const key=`${o.bill.id}|${o.month}`;if(seen.has(key))return;seen.add(key);items.push({...o,days:finDaysTo(o.due),status:finPlannedStatus(o)})}));return items.sort((a,b)=>a.due.localeCompare(b.due)).slice(0,4)}catch(e){return[]}}
finReminderMomentLabel=function(o){const b=o.bill||{},days=finDaysTo(o.due),now=new Date(),hhmm=`${String(now.getHours()).padStart(2,'0')}:${String(now.getMinutes()).padStart(2,'0')}`,first=b.reminderTime||'09:00',again=b.repeatReminderTime||'21:30';if(days<0)return `${o.status.label}${b.repeatIfPending===false?'':` · próximo aviso ${again}`}`;if(days>0){if(b.repeatIfPending!==false&&hhmm>=first&&hhmm<again)return `${o.status.label} · próximo aviso ${again}`;return `${o.status.label} · ${formatDate(o.due)}`}if(hhmm<first)return `Vence hoje · aviso ${first}`;if(b.repeatIfPending!==false&&hhmm<again)return `Vence hoje · próximo aviso ${again}`;return `Vence hoje · ${formatDate(o.due)}`}
renderFinanceRemindersMeuDia=function(){const items=finHomeReminders();if(!items.length)return '';const icon=`<svg class="home-fin-reminder-svg" viewBox="0 0 24 24" aria-hidden="true"><circle cx="12" cy="12" r="8"/><path d="M12 7.5v5l3.2 1.8"/></svg>`;const rows=items.map(o=>`<a class="home-fin-reminder-row" href="#financeiro"><span>${icon}</span><span><strong>${escapeHtml(o.bill.name||'Conta prevista')}</strong><small>${escapeHtml(finReminderMomentLabel(o))}</small></span><b>›</b></a>`).join('');return `<section class="day-section home-fin-reminders"><div class="section-head"><h2>Contas para lembrar</h2><a href="#financeiro">ver financeiro</a></div>${rows}</section>`}

progressFinanceRow=function(){const d=loadFin(),month=finCurrentMonth(),fixed=finFixedTotal(d),variable=finMonthTotal(d,month),bills=finPlannedMonth(d,month),paid=bills.filter(x=>x.paid),open=bills.filter(x=>!x.paid),covered=finPersonal(d.excluded).reduce((s,x)=>s+Number(x.value||0),0),remaining=Number(d.income||0)-fixed-variable,monthTx=finMonthTransactions(d,month).filter(t=>t.plannedBillId),onTime=monthTx.filter(t=>Number(t.daysLate||0)===0).length,late=monthTx.filter(t=>Number(t.daysLate||0)>0).length,penaltyMonth=monthTx.reduce((s,t)=>s+Number(t.interest||0)+Number(t.fine||0)+Number(t.otherCharges||0),0),year=month.slice(0,4),penaltyYear=finPersonal(d.transactions).filter(t=>(t.date||'').startsWith(year)&&t.plannedBillId).reduce((s,t)=>s+Number(t.interest||0)+Number(t.fine||0)+Number(t.otherCharges||0),0),goalActual=(d.goals||[]).reduce((s,g)=>s+finGoalActual(g),0);return `<details class="progress-category"><summary class="progress-category-head"><strong>Financeiro</strong><b>R$ ${money(remaining)} disponíveis</b></summary><small>${paid.length}/${bills.length} contas previstas pagas · realizado R$ ${money(variable)}</small><div class="progress-detail"><div><span>Orçamento base</span><span>R$ ${money(fixed)}</span></div><div><span>Gastos registrados no mês</span><span>R$ ${money(variable)}</span></div><div><span>Contas pagas no prazo</span><span>${onTime}</span></div><div><span>Contas pagas em atraso</span><span>${late}</span></div><div><span>Dinheiro perdido por juros/multas · mês</span><span>R$ ${money(penaltyMonth)}</span></div><div><span>Dinheiro perdido por juros/multas · ano</span><span>R$ ${money(penaltyYear)}</span></div><div><span>Metas · realizado acumulado</span><span>R$ ${money(goalActual)}</span></div><div><span>Contas previstas pendentes</span><span>${open.length} · R$ ${money(open.reduce((s,x)=>s+x.expected,0))}</span></div><div><span>Despesas cobertas hoje</span><span>R$ ${money(covered)}</span></div></div></details>`}

window.__berthaCoreRenderFinanceiro=renderFinanceiro;

(function(){const st=document.createElement('style');st.textContent='.fin-alert-icon{display:inline-grid!important;place-items:center!important;font-size:10px!important;font-weight:800!important;line-height:1!important}.fin-alert-icon:before,.fin-alert-icon:after{content:none!important}';document.head.appendChild(st)})();

...
(() => {
  ...
  function movementTitle(v){
    const t=String(v||'').normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLowerCase().trim();
    return t==='exercicio'||t==='exercicios'||t==='movimento';
  }
  function clearLegacyGenericMovementActive(){
    const e=engine(),a=e.active;
    if(a && a.source==='Meu Dia Ideal' && movementTitle(a.title) && !a.exercisePlanId){
      e.active=null; saveEngine(e);
    }
  }
  ...
  function sourcesToday(){
    ...
    // Exercícios — o Meu Dia Ideal reserva um espaço de Movimento; a atividade específica é escolhida ao começar.
    const idealItems=read(IDEAL_KEY,[]).filter(x=>x&&x.active!==false).filter(x=>!(x.untilMode==='date'&&x.untilDate&&x.untilDate<today));
    const idealMovement=idealItems.find(x=>movementTitle(x.title));
    const ex=read('minha-vida.exercicios.v1',null); const dow=['Dom','Seg','Ter','Qua','Qui','Sex','Sáb'][new Date().getDay()];
    if(ex && Array.isArray(ex.plans) && ex.plans.some(p=>p&&p.active!==false)){
      const activePlans=ex.plans.filter(p=>p&&p.active!==false&&(!(p.untilMode==='date'&&p.untilDate&&p.untilDate<today)));
      const candidates=activePlans.filter(p=>(p.frequency!=='Dias específicos'||!p.days?.length||p.days.includes(dow)));
      candidates.sort((a,b)=>{const r=p=>p.frequency==='Dias específicos'?0:p.frequency==='X vezes por semana'?1:2;return r(a)-r(b)});
      const p=candidates[0]||activePlans[0];
      if(p){
        const planDv=Math.max(1,+p.durationValue||parseMinutes(p.target)||30), planMins=p.durationUnit==='hours'?planDv*60:planDv;
        const mins=Math.max(1,+idealMovement?.minutes||planMins);
        const period=idealMovement?.period||p.period||'flex', time=idealMovement?.time||p.time||'';
        const periods={morning:['05:16','07:35'],afternoon:['12:00','18:00'],evening:['18:00','22:00'],flex:['','']}; const w=periods[period]||periods.flex;
        push({id:'exercise:movement-slot',learningKey:'exercise:movement-slot',source:idealMovement?'Meu Dia Ideal':'Exercícios',title:'Movimento',subtitle:`Sugestão: ${p.name||'atividade'}`,date:today,minutes:mins,configuredMinutes:mins,time,period,windowStart:time?'':w[0],windowEnd:time?'':w[1],notify:!!(idealMovement?.notify??p.notify),notifyOffset:idealMovement?.notifyOffset||p.notifyWhen||'',kind:'window'});
      }
    }
    // Meu Dia Ideal — sugestões, não obrigações. Movimento é tratado acima como um único espaço escolhível.
    idealItems
      .filter(x=>!movementTitle(x.title))
  ...
  function movementChoiceDialog(){
    ...
    d.classList.add('bertha-movement-dialog');
    const resetX=()=>{const y=window.scrollY||0;requestAnimationFrame(()=>window.scrollTo(0,y));};
    d.addEventListener('close',resetX,{once:true});
    d.querySelectorAll('[data-ex-plan]').forEach(b=>b.onclick=()=>{const p=plans.find(x=>String(x.id)===String(b.dataset.exPlan));d.close();d.remove();resetX();startMovementPlan(p,suggested)});
  }
  function startItem(item){
    if(item && item.source==='Meu Dia Ideal' && movementTitle(item.title) && !item.exercisePlanId){ movementChoiceDialog(); return; }
    const e=engine();
  ...
  function bindHome(){ document.querySelectorAll('[data-start]').forEach(b=>b.onclick=()=>{const x=sourcesToday().find(i=>i.id===b.dataset.start);if(x){if(x.id==='exercise:movement-slot'||(x.source==='Meu Dia Ideal'&&movementTitle(x.title)))movementChoiceDialog();else startItem(x)}}); document.querySelectorAll('[data-postpone]').forEach(b=>b.onclick=()=>{const x=sourcesToday().find(i=>i.id===b.dataset.postpone);if(x)postponeDialog(x)}); const f=document.querySelector('[data-finish-active]'); if(f)f.onclick=finishActive; }
  ...
  window.addEventListener('hashchange',()=>{
    const r=(location.hash||'#meu-dia').slice(1)||'meu-dia';
    if(r==='meu-dia'||r==='pendencias'||r==='rituais'||r.startsWith('ritual-')||r==='dia-ideal') setTimeout(rerender,0);
  });
  ...
  const movementStyle=document.createElement('style');movementStyle.textContent=`
    .bertha-movement-dialog{max-width:calc(100vw - 28px)!important;overflow-x:hidden!important}.bertha-movement-dialog .bertha-modal{box-sizing:border-box!important;width:100%!important;max-width:100%!important;overflow-x:hidden!important;border-radius:28px!important;background:#fffaf7!important;box-shadow:0 24px 70px rgba(69,53,73,.22)!important}
    .bertha-movement-dialog .bertha-modal-head button{border:0!important;outline:0!important;background:rgba(235,222,244,.8)!important;color:#745b82!important;border-radius:999px!important;width:38px!important;height:38px!important;font-size:25px!important;line-height:1!important}
    .bertha-movement-list{display:grid;gap:10px;margin-top:14px}
    .bertha-movement-choice{width:100%;display:flex;align-items:center;justify-content:space-between;gap:12px;text-align:left;padding:14px 15px;border:1px solid rgba(90,70,110,.11);border-radius:18px;background:linear-gradient(135deg,rgba(255,245,248,.96),rgba(246,242,255,.96));color:#453a49!important;box-shadow:none!important}
    .bertha-movement-choice span{display:grid;gap:3px}.bertha-movement-choice strong{color:#453a49!important}.bertha-movement-choice small{font-weight:500;opacity:.68;color:#756a79!important}.bertha-movement-choice b{white-space:nowrap;color:#795d88!important;background:rgba(234,220,244,.78);padding:8px 12px;border-radius:999px}

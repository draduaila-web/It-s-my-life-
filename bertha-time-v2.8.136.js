...
(() => {
  ...
  function sourcesToday(){
    ...
    const mapPool=[...baseMapIds.map(id=>({id,minutes:+mapCrud.overrides?.[String(id)]?.minutes||30,title:''})),...customMaps.map(x=>({id:+x.id,minutes:+x.minutes||30,title:x.topico||''}))].sort((a,b)=>a.id-b.id);
    const nextMap=mapPool.find(x=>!mapStatus[String(x.id)]?.domino && !mapStatus[x.id]?.domino) || null;
    if(nextMap){
      push({id:`study:tcdf:${nextMap.id}`,learningKey:`study:tcdf:${nextMap.id}`,source:'Estudos',title:nextMap.title?`TCDF · Mapa ${String(nextMap.id).padStart(3,'0')} · ${nextMap.title}`:`TCDF · Mapa ${String(nextMap.id).padStart(3,'0')}`,date:today,minutes:Math.max(5,+nextMap.minutes||30),configuredMinutes:Math.max(5,+nextMap.minutes||30),period:'flex',priority:'normal',kind:'study'});
    }
  ...
  function currentSuggestion(){
    ...
    const study=pick('study'); if(study) return {item:study};

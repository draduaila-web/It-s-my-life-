// BERTH.A v2.8.63 — base visual v2.8.61 restaurada; filtro seguro do Meu Dia Ideal
// BERTH.A app-v2.8.13 · Casa schema-safe
window.BERTHA_BUILD="2.8.60-compras-saude-progresso";
...
function openIdeaModal(id=null){
  ...
  d.innerHTML=`<form method="dialog" id="ideaForm" class="bertha-modal study-v10-modal">
    <div class="bertha-modal-head"><div><div class="eyebrow">${p?'EDITAR':'CRIAÇÃO & IDEIAS'}</div><h2 id="ideaModalTitle">${p?'Editar registro':'Nova ideia'}</h2></div><button type="button" data-close aria-label="Fechar">×</button></div>
...
@media(max-width:480px){
 ...
 #ideaDialog .form-grid input,#ideaDialog .form-grid select,#ideaDialog #ideaConditional input,#ideaDialog #ideaConditional select{display:block!important;width:100%!important;max-width:100%!important;min-width:0!important;margin-top:6px!important}
...
function renderProgressOverview(){ensureShoppingStyles();const items=purchasedThisMonth(),groups=progressGroup(items),total=items.reduce((s,x)=>s+shopMoney(x.actualValue),0);app.innerHTML=`<section class="shop-hero"><div><div class="eyebrow">COMO ESTOU CAMINHANDO?</div><h2>Meu Progresso</h2><p>Começamos pelo que já pode ser medido com confiança: suas compras reais.</p></div>${shopSvg('trend')}</section><div class="shop-summary"><div class="card"><span>Itens comprados no mês</span><b>${items.length}</b></div><div class="card"><span>Valor registrado</span><b style="font-size:18px">${shopMoneyBR(total)}</b></div></div><section class="card"><div class="shop-section-head"><h3>Compras do mês</h3><span>${new Date().toLocaleDateString('pt-BR',{month:'long',year:'numeric'})}</span></div>${groups.length?groups.map(([cat,arr])=>{const subtotal=arr.reduce((s,x)=>s+shopMoney(x.actualValue),0);const consolidated=new Map();arr.forEach(x=>{const k=String(x.name).trim().toLowerCase();if(!consolidated.has(k))consolidated.set(k,{name:x.name,qty:0,unit:x.unit||'',value:0,count:0});const z=consolidated.get(k);const q=Number(String(x.qty||'').replace(',','.'));if(Number.isFinite(q)&&q)z.qty+=q;z.value+=shopMoney(x.actualValue);z.count++});return `<details class="progress-category"><summary class="progress-category-head"><strong>${escapeHtml(cat)}</strong><b>${shopMoneyBR(subtotal)}</b></summary><small>${arr.length} item${arr.length===1?'':'s'} comprado${arr.length===1?'':'s'}</small><div class="progress-detail">${[...consolidated.values()].map(z=>`<div><span>${escapeHtml(z.name)}</span><span>${z.qty?`${String(z.qty).replace('.',',')} ${escapeHtml(z.unit)}`:`${z.count} compra${z.count===1?'':'s'}`} · ${shopMoneyBR(z.value)}</span></div>`).join('')}</div></details>`}).join(''):'<div class="empty compact"><strong>Ainda não há dados neste mês.</strong><span>Marque itens como comprados e, quando quiser, registre o valor pago.</span></div>'}</section><section class="card" style="margin-top:12px"><strong>O próximo passo</strong><p class="note">À medida que Casa, Autocuidado, Saúde e outros módulos enviarem itens para a lista, este mesmo quadro passa a mostrar a visão completa sem criar painéis paralelos.</p></section>`}

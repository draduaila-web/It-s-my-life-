...
window.BERTHA_BUILD="2.8.134-estudos-azul-pessego-integrado";
...
function openMaps(focus){
 ...
 dlg.querySelector(".study-v10-x").onclick=()=>dlg.close();input.oninput=draw;dlg.querySelectorAll("[data-filter]").forEach(b=>b.onclick=()=>{filter=b.dataset.filter;dlg.querySelectorAll("[data-filter]").forEach(x=>x.classList.toggle("active",x===b));draw()});dlg.onclose=()=>dlg.remove();draw();dlg.showModal()
}

...
function renderCasaCore(){
 ...
 if(routineAreaSelect){routineAreaSelect.addEventListener('change',()=>{const id=routineAreaSelect.value;if(!id)return;const groups=[...document.querySelectorAll('.casa-routine-group')];groups.forEach(group=>group.open=false);const target=groups.find(group=>String(group.dataset.routineId)===String(id));if(target){target.open=true;requestAnimationFrame(()=>target.scrollIntoView({behavior:'smooth',block:'start'}));}});}

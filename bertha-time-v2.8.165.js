...
(() => {
  ...
  function ensureMeuDiaV165Styles(){
    if(document.getElementById('bertha-meu-dia-v165'))return;
    const st=document.createElement('style');st.id='bertha-meu-dia-v165';st.textContent=`
      /* MEU DIA v165 — fluxo cromático contínuo da marca */
  ...
  function renderHome(){ ensureHomeUtilityStyles(); ensureMeuDiaV165Styles(); const d=new Date(), greet=d.getHours()<12?'Bom dia':d.getHours()<18?'Boa tarde':'Boa noite'; const today=new Intl.DateTimeFormat('pt-BR',{weekday:'long',day:'numeric',month:'long'}).format(d); const raw=sourcesToday().filter(x=>!doneToday(x.id)); const scheduled=raw.filter(x=>x.time); const smartCasa=candidates().filter(x=>x.kind==='home-routine'||x.kind==='home-maintenance'); const nonCasa=raw.filter(x=>x.kind!=='home-routine'&&x.kind!=='home-maintenance'&&!x.time); const current=currentSuggestion(); const currentId=current?.item?.id||current?.active?.id||''; const flexPool=nonCasa.filter(x=>x.id!==currentId); const tasks=(smartCasa.length?[...flexPool.slice(0,3),smartCasa[0]]:flexPool.slice(0,4)); const free=current.free;
    ...
    return `<section class="day-hero"><div class="eyebrow">MEU DIA</div><h1>${greet}, Duaila.</h1><p class="day-date">${today}</p></section>${utility}${renderHomeScheduled(scheduled)}${nowCard()}${renderFoodHomeMini()}${financeReminders}<section class="day-section"><div class="section-head"><h2>O que importa hoje</h2><span class="soft-count">${tasks.length}</span></div>${tasks.length?tasks.map(x=>`<div class="focus-row bertha-focus"><span class="focus-dot">•</span><div><strong>${esc(x.title)}</strong><small>${esc(x.source)} · ${durationText(+x.minutes||30)}</small></div><button data-start="${esc(x.id)}">Começar</button></div>`).join(''):`<div class="bertha-empty">Seu essencial está em dia.</div>`}</section>${health}<section class="day-section"><div class="section-head"><h2>Seu dia, sem excesso</h2></div><div class="timeline bertha-timeline">${renderHomeTimeline()}</div></section>${free?`<section class="free-space"><strong>Espaço livre também faz parte do dia.</strong><p>Se nada precisa ser resolvido agora, não resolva.</p></section>`:''}`;
  }

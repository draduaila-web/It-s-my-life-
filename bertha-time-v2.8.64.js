/* BERTH.A — Meu Dia v2 / Motor de Tempo v2.8.64 — Home com Compras, Progresso e Saúde
...
(() => {
  ...
  function progressGoalLabel(g){
    if(!g||!g.target)return 'Definir referência';
    const unit={sessions:'x',minutes:'min',hours:'h',percent:'%',items:'itens'}[g.unit]||g.unit||'';
    const period={week:'/ semana',month:'/ mês',day:'/ dia',deadline:' no prazo'}[g.period]||'';
    return `${String(g.target).replace('.',',')} ${unit} ${period}`.replace(/\s+/g,' ').trim();
  }
  function renderProgressGoals(){const goals=progressGoals();return `<section class="bertha-goals-block"><div class="bertha-goals-head"><div><span>REFERÊNCIAS POR ÁREA</span><strong>O que significa estar perto do seu ideal?</strong></div></div><p class="bertha-goals-note">Estas referências não criam tarefas. Elas servem apenas para a BERTA comparar o esperado com o que realmente aconteceu.</p><div class="bertha-goals-grid">${progressGoalAreas.map(([id,label])=>`<button type="button" class="bertha-goal-card" data-progress-goal="${id}"><span>${label}</span><strong>${esc(progressGoalLabel(goals[id]))}</strong></button>`).join('')}</div></section>`;}
  function openProgressGoalDialog(area){
    const label=(progressGoalAreas.find(x=>x[0]===area)||[])[1]||area, goals=progressGoals(),g=goals[area]||{};
    const d=dialogBase(`Referência · ${label}`,`<p class="note">Defina uma régua simples. Isso não cria compromisso nem tarefa no Meu Dia.</p><label class="bertha-field">Meta<input data-goal-target type="number" min="0" step="0.5" inputmode="decimal" value="${esc(g.target||'')}"></label><label class="bertha-field">Medida<select data-goal-unit><option value="sessions" ${g.unit==='sessions'?'selected':''}>vezes / sessões</option><option value="minutes" ${g.unit==='minutes'?'selected':''}>minutos</option><option value="hours" ${g.unit==='hours'?'selected':''}>horas</option><option value="percent" ${g.unit==='percent'?'selected':''}>percentual de aderência</option><option value="items" ${g.unit==='items'?'selected':''}>itens / entregas</option></select></label><label class="bertha-field">Período<select data-goal-period><option value="week" ${g.period==='week'?'selected':''}>por semana</option><option value="month" ${g.period==='month'?'selected':''}>por mês</option><option value="day" ${g.period==='day'?'selected':''}>por dia</option><option value="deadline" ${g.period==='deadline'?'selected':''}>dentro do prazo</option></select></label><div class="bertha-stack"><button class="bertha-primary" data-goal-save>Salvar referência</button>${g.target?'<button class="bertha-secondary" data-goal-remove>Remover referência</button>':''}</div>`);
    d.querySelector('[data-goal-save]').onclick=()=>{const target=Number(String(d.querySelector('[data-goal-target]').value||'').replace(',','.'));if(!(target>0))return;const all=progressGoals();all[area]={area,label,target,unit:d.querySelector('[data-goal-unit]').value,period:d.querySelector('[data-goal-period]').value,updatedAt:Date.now()};write(PROGRESS_GOALS_KEY,all);d.close();d.remove();rerender();setTimeout(enhanceIdealScreen,0)};
    d.querySelector('[data-goal-remove]')?.addEventListener('click',()=>{const all=progressGoals();delete all[area];write(PROGRESS_GOALS_KEY,all);d.close();d.remove();rerender();setTimeout(enhanceIdealScreen,0)});
  }

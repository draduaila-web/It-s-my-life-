...
(() => {
  ...
  function renderRituals(){
    ...
    return `<section class="bertha-ritual-page">
      <section class="bertha-ritual-hero"><div class="bertha-task-kicker">RITUAIS</div><h1>Space to think. Space to live.</h1><p>Você cuida de você. A BERTH.A sustenta o ritual.</p></section>
  ...
  function customRitualDialog(id){
    ...
    const d=document.createElement('dialog');d.className='bertha-dialog bertha-task-dialog bertha-ritual-dialog';
    d.innerHTML=`<form class="bertha-task-modal" method="dialog"><div class="bertha-task-modal-head"><div><span>RITUAIS</span><h2>Editar ritual</h2></div><button type="button" data-close>×</button></div><div class="bertha-task-modal-body"><label class="bertha-task-field"><span>Nome do ritual</span><input data-title value="${esc(r.title||'')}"></label><div class="bertha-task-field"><span>Ícone</span><div class="bertha-icon-picker">${Object.keys(RITUAL_ICONS).map(k=>`<button type="button" data-icon="${k}" class="${r.icon===k?'active':''}">${ritualIcon(k)}</button>`).join('')}</div></div><label class="bertha-task-field"><span>Descrição <small>opcional</small></span><input data-subtitle value="${esc(r.subtitle||'')}" placeholder="Se ficar vazia, usamos uma frase BERTH.A"></label><label class="bertha-task-field"><span>Etapas <small>uma por linha</small></span><textarea data-steps rows="7">${esc(ritualLines(r.steps).join('\n'))}</textarea></label><label class="bertha-task-field"><span>Produtos <small>um por linha</small></span><textarea data-products rows="7">${esc(ritualProducts(id,null,r).join('\n'))}</textarea></label></div><div class="bertha-task-modal-actions"><span></span><div><button class="bertha-task-cancel" type="button" data-close>Cancelar</button><button class="bertha-task-save" type="button" data-save>Salvar</button></div></div></form>`;

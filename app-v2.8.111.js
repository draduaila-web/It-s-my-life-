...
function renderTodayFoodCard(){const meals=getTodayMeals();return `<div class="card today-food-card"><div class="panel-head"><div><span class="eyebrow">COMIDA DE HOJE · ${escapeHtml(foodDayName().toUpperCase())}</span><h3>Seu dia já começa decidido.</h3></div></div>${meals.map(m=>{const selected=getRecipeById(m.log.selectedRecipeId)||m.recipe,name=m.log.otherText||selected?.name||m.name,done=m.log.consumption==='consumed';return `<div class="food-row"><span>${escapeHtml(m.label)}</span><strong>${escapeHtml(name)}</strong><div style="display:flex;gap:7px;flex-wrap:wrap;margin-top:7px">${done?`<button type="button" class="secondary" data-food-undo="${m.id}">Desfazer</button><button type="button" class="secondary" data-food-other="${m.id}">Alterar</button>`:`<button type="button" class="secondary" data-food-done="${m.id}">Fiz esta</button><button type="button" class="secondary" data-food-other="${m.id}">Fiz outra</button>`}${selected?`<button type="button" class="secondary" data-food-view="${m.id}">Ver receita</button>`:''}</div></div>`}).join('')}</div>`;}
...
function renderFoodMeuDiaMini(){
 ...
 return `<section class="day-section"><div class="section-head"><h2>Cozinha de hoje</h2><a href="#receitas">ver receitas</a></div>${meals.map(m=>{const selected=getRecipeById(m.log.selectedRecipeId)||m.recipe,name=m.log.otherText||selected?.name||m.name,done=m.log.consumption==='consumed';return `<a href="#receitas" class="compact-item" style="text-decoration:none;color:inherit"><strong>${escapeHtml(m.label)} · ${escapeHtml(name)}</strong>${done?'<small>registrado</small>':''}</a>`}).join('')}</section>`;
}
